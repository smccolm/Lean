/-
Adapted from the Lean 4.30 revision of PrimeNumberTheoremAnd/Rectangle.lean.
Copyright belongs to the upstream contributors.
...
lemma RectSubRect {x₀ x₁ x₂ x₃ y₀ y₁ y₂ y₃ : ℝ} (x₀_le_x₁ : x₀ ≤ x₁)
  ...
  have hx₀₃ : (x₀ + y₀ * I : ℂ).re ≤ (x₃ + y₃ * I : ℂ).re := by
    simpa using x₀_le_x₁.trans (x₁_le_x₂.trans x₂_le_x₃)
  have hy₀₃ : (x₀ + y₀ * I : ℂ).im ≤ (x₃ + y₃ * I : ℂ).im := by
    simpa using y₀_le_y₁.trans (y₁_le_y₂.trans y₂_le_y₃)
  rw [rect_subset_iff, mem_Rect hx₀₃ hy₀₃, mem_Rect hx₀₃ hy₀₃]
  refine ⟨⟨?_, ?_, ?_, ?_⟩, ?_, ?_, ?_, ?_⟩ <;> simpa using by linarith
...
lemma square_mem_nhds (p : ℂ) {c : ℝ} (hc : c ≠ 0) :
  ...
  exact (Complex.nhds_hasBasis_square p).mem_of_mem hc_pos
