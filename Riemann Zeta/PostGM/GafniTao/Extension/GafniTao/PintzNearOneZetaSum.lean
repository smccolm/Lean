import GafniTao.PintzNearOneZetaAllBlocks
import GafniTao.FordDadaroRemainder
import GafniTao.HeathBrownLogAbsorption

/-!
# Pintz's coefficient-one-half near-one zeta estimate

The native block estimate is summed over the exact Dadaro dyadic partition.
One half of the requested epsilon absorbs the shell count; the finite
approximation remainder is retained until the last step.
-/
...
/-- Exact dyadic shell sum before logarithmic absorption. -/
theorem norm_fordDyadicWeightedShellSum_zero_le_pintz_nearOne
  ...
  obtain ⟨C, hC, hblock⟩ :=
    norm_fordShiftedWeightedBlock_zero_le_pintz_nearOne_all hepsilon
  refine ⟨C, hC, ?_⟩
  intro sigma t M hsigmaUpper hsigmaLower htOne hM hMtop
  ...
  calc
    ...
    _ ≤ ∑ _j ∈ Finset.range (Nat.clog 2 M),
      ...
      exact hblock sigma t (2 ^ j) (min M (2 ^ (j + 1)))
        hsigmaUpper hsigmaLower (pow_pos (by omega) _)
        hnonempty hupper hjt
    _ = (Nat.clog 2 M : ℝ) *
        (C * t ^ ((1 / 2 : ℝ) *
          (1 - sigma) ^ (3 / 2 : ℝ) + epsilon)) := by simp

/-- Dadaro finite approximation with the exact shell count still visible. -/
      ...
      ‖fordHurwitzFiniteApproximation sigma 1 t‖ ≤
        1 + (Nat.clog 2 (fordFiniteEndpoint t + 1) : ℝ) *
          (C * t ^ ((1 / 2 : ℝ) *
            (1 - sigma) ^ (3 / 2 : ℝ) + epsilon)) := by
  obtain ⟨C, hC, hshell⟩ :=
    norm_fordDyadicWeightedShellSum_zero_le_pintz_nearOne hepsilon
  refine ⟨C, hC, ?_⟩
  intro sigma t hsigmaUpper hsigmaLower ht
  let M : ℕ := fordFiniteEndpoint t + 1
  have hM : 1 ≤ M := by dsimp only [M]; omega
  rw [fordHurwitzFiniteApproximation_one_eq_partial_sum,
    fordDadaroCutoff_floor, partialZeta_eq_one_add_fordDyadic hM]
  calc
    ...
    _ ≤ 1 + (Nat.clog 2 M : ℝ) *
        (C * t ^ ((1 / 2 : ℝ) *
          (1 - sigma) ^ (3 / 2 : ℝ) + epsilon)) := by
      gcongr
      exact hshell sigma t M hsigmaUpper hsigmaLower
        (by linarith) hM (by simp [M])
    _ = 1 + (Nat.clog 2 (fordFiniteEndpoint t + 1) : ℝ) *
        (C * t ^ ((1 / 2 : ℝ) *
          (1 - sigma) ^ (3 / 2 : ℝ) + epsilon)) := by rfl

/-- Native form of Pintz (2.19) on the exact range later used to prove the
strict `23/24` zero-density cutoff. -/
theorem norm_riemannZeta_le_pintz_nearOne
  ...
  obtain ⟨C₀, hC₀, hfinite⟩ :=
    norm_fordHurwitzFiniteApproximation_one_le_pintz_nearOne hepsHalf
  ...
  have hlogTwoUpper : Real.log 2 ≤ 1 := by linarith [Real.log_two_lt_d9]
  ...
  have hclogScale : (Nat.clog 2 M : ℝ) ≤
    ...
    calc
      ...
      _ ≤ (Real.log 2)⁻¹ + Real.log (M : ℝ) / Real.log 2 := by gcongr
  ...
  have hfiniteRaw := hfinite sigma t hsigmaUpper hsigmaLower ht
  have hfiniteFinal : ‖fordHurwitzFiniteApproximation sigma 1 t‖ ≤
    ...
    calc
      ...
      _ = 1 + K * t ^ (q + epsilon) := by
        ...
        rw [← hpowEq]
        ring
      _ ≤ t ^ (q + epsilon) + K * t ^ (q + epsilon) := by linarith
  ...
  calc
    ...
    _ ≤ 15 * t ^ (q + epsilon) + (1 + K) * t ^ (q + epsilon) := by gcongr
    _ = C * t ^ ((1 / 2 : ℝ) *
        (1 - sigma) ^ (3 / 2 : ℝ) + epsilon) := by
      dsimp only [C, q]
      ring
