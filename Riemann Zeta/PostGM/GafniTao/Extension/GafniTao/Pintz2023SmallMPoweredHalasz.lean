...
/-!
# Pintz (2023), equation (4.19) for the surviving small-m block

This is the Halász--Montgomery consumer for the precise coefficient produced
by equations (4.13)--(4.16).  In particular, it cannot be instantiated with
the discarded large-`m` portion or with the original unsplit coefficient.
-/
...
/-- Equation (4.19), through the two off-diagonal absorptions, for the exact
small-`m` powered coefficient. -/
theorem exists_pintz2023_smallM_intervalPower_halasz_cardinality
    ...
    ∃ Ce Cd Co : ℝ, 0 < Ce ∧ 0 < Cd ∧ 0 < Co ∧
      ∀ (X U N : ℕ) (R : ℝ) (baseI Iset : Finset ℕ) (W : Finset ℝ)
  ...
  intro X U N R baseI Iset W xi eta lambda A T G etaAt gammaAt
  dsimp only
...
#print axioms exists_pintz2023_smallM_intervalPower_halasz_cardinality
