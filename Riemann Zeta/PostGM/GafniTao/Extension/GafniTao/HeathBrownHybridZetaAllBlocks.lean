...
/-- Source-range hybrid block estimate.  This replaces the provisional
near-one cutoff by the full `sigma ≥ 5/6` range required when Pintz applies
the zeta estimate at real part `1 - eta - eta_j`. -/
theorem norm_fordShiftedWeightedBlock_zero_le_hybrid_zeta_source
    {epsilon : ℝ} (hepsilon : 0 < epsilon) :
    ∃ C : ℝ, 0 < C ∧ ∀ (sigma t : ℝ) (N R : ℕ),
      sigma ≤ 1 → 5 / 6 ≤ sigma →
      1024 ≤ N → N < R → R ≤ 2 * N → (N : ℝ) ≤ t →
  ...
  refine ⟨C, hC, ?_⟩
  intro sigma t N R hsigmaUpper hsigmaRange hN hNR hR hNt
  ...
  · have hraw := hshort sigma t N R hsigma hsigmaUpper hN hNR hR hshortRange
    exact hraw.trans (by
      ...
      · dsimp only [C]
        linarith
      · positivity)
  · have hmiddle := norm_fordShiftedWeightedBlock_zero_le_middle_zeta
      hsigmaUpper hsigmaRange hN hNt (le_of_not_ge hshortRange) hNR hR
    have hpow : t ^ q ≤ t ^ (q + epsilon) :=
      Real.rpow_le_rpow_of_exponent_le htOne (by linarith)
    calc
      ‖fordShiftedWeightedBlock sigma N R 0 t‖ ≤ 130 * t ^ q := by
        simpa only [q] using hmiddle
      _ ≤ C * t ^ q := by
        apply mul_le_mul_of_nonneg_right
        · dsimp only [C]
          linarith
...
/-- Full source-range block estimate, including the finitely many blocks
below `1024`. -/
theorem norm_fordShiftedWeightedBlock_zero_le_all_zeta_source
    {epsilon : ℝ} (hepsilon : 0 < epsilon) :
    ∃ C : ℝ, 0 < C ∧ ∀ (sigma t : ℝ) (N R : ℕ),
      sigma ≤ 1 → 5 / 6 ≤ sigma →
      0 < N → N < R → R ≤ 2 * N → (N : ℝ) ≤ t →
      ‖fordShiftedWeightedBlock sigma N R 0 t‖ ≤
        C * t ^ (heathBrownHalfZetaKappa *
          (1 - sigma) ^ (3 / 2 : ℝ) + epsilon) := by
  obtain ⟨Chybrid, hChybrid, hhybrid⟩ :=
    norm_fordShiftedWeightedBlock_zero_le_hybrid_zeta_source hepsilon
  let C : ℝ := Chybrid + 1024 * fordQualitativeCoefficient
  have hC : 0 < C := by
    dsimp only [C]
    nlinarith [fordQualitativeCoefficient_nonneg]
  ...
  have htp : 1 ≤ t ^ p := Real.one_le_rpow htOne hp
  by_cases hlarge : 1024 ≤ N
  · have hraw := hhybrid sigma t N R hsigmaUpper hsigmaRange
      hlarge hNR hR hNt
    exact hraw.trans (by
      apply mul_le_mul_of_nonneg_right
      · dsimp only [C]
