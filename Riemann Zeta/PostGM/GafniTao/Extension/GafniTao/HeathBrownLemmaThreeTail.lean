import GafniTao.HeathBrownLemmaThreeEdges
...
is sufficient. -/
theorem exists_heathBrownCriticalZetaNorm_sq_le_quadratic :
    ∃ C : ℝ, 0 < C ∧ ∀ y : ℝ,
      heathBrownCriticalZetaNorm y ^ (2 : ℕ) ≤ C * (1 + y ^ (2 : ℕ)) := by
  obtain ⟨B, hB⟩ := isCompact_Icc.bddAbove_image
    continuous_heathBrownCriticalZetaNorm_sq.continuousOn
  let C : ℝ := max 100 B
  ...
  · have hyMem : y ∈ Set.Icc (-1 : ℝ) 1 := by
      exact (abs_le.mp hy)
    have hyB : heathBrownCriticalZetaNorm y ^ (2 : ℕ) ≤ B :=
      hB ⟨y, hyMem, rfl⟩
    have hBC : B ≤ C := le_max_right 100 B
    have hC : 0 ≤ C := (lt_of_lt_of_le (by norm_num) (le_max_left 100 B)).le
    calc
      heathBrownCriticalZetaNorm y ^ (2 : ℕ) ≤ C := hyB.trans hBC
      _ = C * 1 := by ring
      _ ≤ C * (1 + y ^ (2 : ℕ)) := by
  ...
  · have hyOne : 1 < |y| := lt_of_not_ge hy
    ...
    have hnorm : ‖z‖ ≤ 2 * |y| := by
      calc
        ...
        _ = (1 / 2 : ℝ) + |y| := by simp [z]
        _ ≤ 2 * |y| := by linarith
    have hzeta' : heathBrownCriticalZetaNorm y ≤ 10 * |y| := by
      unfold heathBrownCriticalZetaNorm
      change ‖riemannZeta z‖ ≤ _
      exact hzeta.trans (by nlinarith)
    have hsq : heathBrownCriticalZetaNorm y ^ (2 : ℕ) ≤ 100 * y ^ (2 : ℕ) := by
      calc
        heathBrownCriticalZetaNorm y ^ (2 : ℕ) ≤ (10 * |y|) ^ (2 : ℕ) :=
          pow_le_pow_left₀
    ...
    calc
      heathBrownCriticalZetaNorm y ^ (2 : ℕ) ≤ 100 * y ^ (2 : ℕ) := hsq
      _ ≤ C * y ^ (2 : ℕ) :=
...
theorem heathBrown_fullMoment_tail_pointwise
    {C t u : ℝ}
    (hC : ∀ y : ℝ,
      heathBrownCriticalZetaNorm y ^ (2 : ℕ) ≤ C * (1 + y ^ (2 : ℕ)))
  ...
  have hCnonneg : 0 ≤ C := by
    have hz0 := hC 0
    simpa using (le_trans (sq_nonneg (heathBrownCriticalZetaNorm 0)) hz0)
  calc
    Real.exp (-|u|) * heathBrownCriticalZetaNorm (t + u) ^ (2 : ℕ) ≤
...
theorem exists_heathBrownFullCriticalMoment_le_truncated :
  ...
  obtain ⟨C, hCpos, hC⟩ := exists_heathBrownCriticalZetaNorm_sq_le_quadratic
  let D : ℝ := 4 * (19 * C)
...
theorem heathBrownLemmaThree_native : HeathBrownLemmaThree := by
  ...
  · have htMem : t ∈ Set.Icc (10 : ℝ) K := by
    ...
    have htB : heathBrownCriticalZetaNorm t ^ (2 : ℕ) ≤ B :=
      hB ⟨t, htMem, rfl⟩
    have hBC : B ≤ C :=
      (le_max_right 1 B).trans (le_max_right C₀ (max 1 B))
    have hCnonneg : 0 ≤ C := hCpos.le
    calc
      heathBrownCriticalZetaNorm t ^ (2 : ℕ) ≤ B := htB
      _ ≤ C := hBC
      _ = C * 1 * 1 := by ring
      _ ≤ C * Real.log t * (1 + M) := by
        exact mul_le_mul
