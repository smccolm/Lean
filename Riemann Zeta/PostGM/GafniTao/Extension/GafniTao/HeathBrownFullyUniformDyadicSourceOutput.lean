...
/-!
# Fully uniform source entry for four dyadic zero shells

The logarithmic shell cover retains a single tuple of analytic constants for
all four independently selected physical heights.
-/
...
structure HeathBrownFullyUniformDyadicSourceOutput
  ...
  mixed : HeathBrownFullyUniformMixedSourceOutput sigma delta delta1 delta2
    eta epsilon K C Cp Cmv C0 C2 C4
...
theorem HeathBrownFullyUniformDyadicSourceOutput.shell_eq
    {sigma T delta delta1 delta2 eta epsilon K C Cp Cmv C0 C2 C4 : Real}
    {coverLabel : Fin 4 → Fin (dyadicZeroShellCount T)}
    (out : HeathBrownFullyUniformDyadicSourceOutput sigma T delta delta1 delta2
      eta epsilon K C Cp Cmv C0 C2 C4 coverLabel) (i : Fin 4) :
...
/-- Exhaustive source entry for any four shells, with every analytic constant
chosen before the shell heights. -/
theorem exists_heathBrownFullyUniformDyadicSourceCutoff
    ...
    ∃ K C Cp Cmv C0 C2 C4 B0 R0 : Real,
      ...
      ∀ (T : Real) (coverLabel : Fin 4 → Fin (dyadicZeroShellCount T)),
        ...
        Nonempty (HeathBrownFullyUniformDyadicSourceOutput sigma T delta delta1
          delta2 eta epsilon K C Cp Cmv C0 C2 C4 coverLabel) := by
  ...
  obtain ⟨K, C, Cp, Cmv, C0, C2, C4, B0, Uout, hK, hC, hCp,
      hCmv, hC0, hC2, hC4, hB0, hUout, hOutput⟩ :=
    eventually_heathBrownFullyUniformMixedSourceOutput
  ...
  refine ⟨K, C, Cp, Cmv, C0, C2, C4, B0, R0, hK, hC, hCp,
    hCmv, hC0, hC2, hC4, hB0, by dsimp only [R0]; linarith, ?_⟩
  intro T coverLabel
