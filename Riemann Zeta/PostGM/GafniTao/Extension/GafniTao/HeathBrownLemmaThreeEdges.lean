import GafniTao.HeathBrownLemmaThreeContour
import GafniTao.HeathBrownLemmaThreeDouble
import GafniTao.HeathBrownMellinHorizontal
...
theorem exists_eventually_heathBrownLemmaThree_fullMoment :
  ...
  calc
    heathBrownCriticalZetaNorm t ^ (2 : ℕ) = ‖riemannZeta s ^ 2‖ := by
      dsimp only [heathBrownCriticalZetaNorm, s]
      rw [norm_pow]
