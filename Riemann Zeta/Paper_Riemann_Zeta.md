...
The selected remaining interior engine is a finite zero-density-from-large-values route. The sharp approximation, dichotomy, exact powering infrastructure, coefficient growth, MHH/Weyl estimates, endpoint arithmetic, terminal restriction, and complete medium Type-I B-process are proved. The Type-I modules prove the exact source-faithful smooth partition and common-scale extraction, exact removal of the interior source boundary, scaled smooth Poisson summation, the corrected stationary point and dual window at scale `T/N`, a symmetric finite central window, a complete two-sided far tail bounded by `K(1+|t|)^102/(Q^101 M^100)`, and Fourier deweighting of both the short and reflected amplitudes to coefficient-one shifted polynomials. Exact Mellin inversion rescales every retained negative mode to a common interval and assembles those modes into one finite coefficient polynomial against a common weighted reflection integral. This removes the previously planned pointwise stationary-main-term approximation. The remaining Type-I theorem is the downstream finite-window/certificate consumer: it must route the zero and wrong-sign central modes and apply the existing MHH/Weyl certificates to the retained negative modes. The source/Lean correspondence and normalization correction are recorded in `Type I Reflection Proof Bijection.md`.
...
- **Focused Axiom/Output Audit Command**: `lake env lean RiemannZeta/Audit.lean` (1153/1153 existing dependency checks pass; the command exits nonzero on seven missing research outputs)
...
2. Consuming #15's exact finite B-process window (including its zero and wrong-sign central modes) with the endpoint certificates and existing MHH/Weyl estimates, then completing the actual-`X` Type-II powered MHH application, branch-to-slab reduction, and concrete Ingham/Huxley endpoints.
