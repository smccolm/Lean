...
The selected #18 route follows Maynard–Pratt Appendix C [5]. It first proves rapid uniform vertical decay for the Gamma kernel by logarithmic-variable Fourier analysis and repeated integration by parts. Mellin inversion and contour shifting for an exponentially smoothed Möbius detector then establish the Type-I/contour-Type-II cover. Gamma truncation, Hölder, separated extraction, local multiplicity, and bounded overlap reduce the residual count to the twisted fourth moment. The identity $|M\zeta|^4=|\zeta|^4|M^2|^2$ puts the squared mollifier at length $T^{1/50}$, within the $T^{1/11-\varepsilon}$ range of Hughes–Young [6]. Formalizing the required upper-bound corollary still requires an approximate functional equation for $\zeta^2$, diagonal evaluation, quadratic-divisor/off-diagonal control, and smoothing removal; none of these results is presently claimed.

For #19, the present interval-free incidence statement is false and the block contract has an off-by-one counterexample. They will be deleted. The source-faithful Guth–Maynard route [1] instead formalizes the sampling matrix, singular-value trace-cube reduction, Poisson expansion, the $S_1$, $S_2$, and $S_3$ estimates, Section 6 smooth reflection, the Heath–Brown difference-set mean-square estimate, affine transformations, tolerance-one approximate additive energy, and the final GCD/spacing assembly. Mathlib's Poisson and singular-value APIs are reusable; its exact additive-energy definition does not replace the required approximate real-energy theory.
...
`RiemannZeta/Audit.lean` explicitly lists all 852 exported source-level theorems across the production modules and computes their transitive axioms with `Lean.collectAxioms`. This includes the complete exported surface of the vendored PNT+ rectangle and sharp-zeta modules, discovered by module provenance despite their upstream global names. The audit checks that the explicit list matches the discovered theorem set and permits only `propext`, `Classical.choice`, and `Quot.sound`. At the current revision it exits nonzero only for `l2_decoupling_bound_native`. The zeta foundation, Jensen local count, classical mollifier/detector foundations, critical-line mollifier second moment, rectangle argument principle, sharp truncation at zeros, van der Corput/Weyl stack, finite Type-I/Type-II dichotomy, classical Montgomery–Halász–Huxley theorem, endpoint boundary cases, Montgomery mean value, beta removal, unconditional separated extraction, arithmetic coefficient estimates, and native Halász–Montgomery consequence are audit-clean. Two direct project axioms remain, both in the rejected `Decoupling.lean` model. The table below is a selected declaration map, not a clean-audit certificate.
...
- **Focused Axiom Audit Command**: `lake env lean RiemannZeta/Audit.lean` (currently expected to exit nonzero and identify one historical decoupling dependency failure across 852 synchronized declarations)
...
2. Completing #15's medium reflection, terminal estimate, finite reduction, and concrete Ingham/Huxley endpoints.
