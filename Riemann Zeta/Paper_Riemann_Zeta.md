...
In this work, we formalize finite Dirichlet polynomial conjugation identities, coordinate wrappers around Mathlib's completed Zeta symmetries, a complex-valued Hardy-type phase normalization, and provisional interfaces for the Guth–Maynard zero-density architecture [2, 3]. The F-01 through F-10 transfer is kernel-checked conditionally from ten explicit primitive inputs. Local zero multiplicity, Montgomery mean value, beta removal, divisor count, ordered-factorization count, and the finite classical Montgomery–Halász–Huxley large-values theorem are proved. The false historical decoupling model has been deleted and replaced by source-facing shared-language, matrix/trace, and Fourier/Poisson packages, including affine energy reindexing, a floor-bin bridge, the exact operator-norm conclusions of Guth–Maynard Lemmas 4.1–4.2, the genuine Schwartz trace kernel, its Mellin zero-mode decay, Fourier dilation, exact Poisson summation, and the complete Hilbert–Schmidt first-trace expansion with a uniform `N⁻¹⁰⁰` remainder. The exact-$\sigma$ classical density transfer, completed Type-II inputs, and the remaining `S₁/S₂/S₃` and energy portion of the Guth–Maynard large-values proof remain open.
...
The selected remaining interior engine is a finite zero-density-from-large-values route. `ZetaTruncation.lean` proves the exact Euler–Maclaurin truncation and its absolute tail bound, then uses a pinned, locally vendored PNT+ Poisson/Euler–Maclaurin closure to obtain the required oscillatory strengthening. At the half-integral cutoff $a=\lfloor4T\rfloor+3/2$, Lean proves $4T<a\le6T$, bounds the phase in $[1/12,1/4]$, bounds the explicit boundary and error coefficients by $14$ and $129$, and proves $\lVert\sum_{n\le a}n^{-\rho}\rVert\le149a^{-\Re(\rho)}$ for every zero in the positive dyadic slab. `SecondDerivative.lean`, `SecondOrderMeanValue.lean`, `Weyl.lean`, `WeylExplicit.lean`, and `WeylZeta.lean` prove the multi-period B-process, corrected A-process, the $(1/6,2/3)$ logarithmic Weyl estimate with uniform prefixes, and exact weighted-block bounds for every $\sigma\ge0$. `FiniteDensityTransfer.lean` proves exact-$\beta$ removal for the sharp Möbius tail, common multiplicity-aware dyadic extraction, normalization, and unrestricted finite MHH application. `ClassicalPowering.lean` and `FiniteDensityEndpoint.lean` prove exact finite power raising, factorization-based coefficient control, normalized powered-block extraction, and the corresponding MHH cardinality bound. `ClassicalDichotomy.lean` now proves the multiplicity-aware finite Type-I/Type-II alternative and the genuinely short Möbius-product branch. `FiniteDensityExponents.lean` proves that Ingham's exceptional short zeta-polynomial range is empty and Huxley's is either empty or lies below the proved Weyl threshold. `FiniteScaleAssembly.lean` proves the exact MHH exponent windows, bounded two-or-three-fold scale reduction in exponent and literal real-power form, the Corollary-11.10 subdivision envelope and power bound, and concrete Ingham/Huxley endpoint scale certificates. `TerminalTypeI.lean` proves uniform prefix Kusmin–Landau cancellation, Abel-summed $n^{-\sigma}$ control, the exact sharp-cutoff block identity, and the theorem forcing a large witness below the height. These results do not remove medium ordinary-zeta Type-I reflection: for example, at $\sigma=3/4$, $N=T^{2/3}$, and $V=N^{3/4}$, the direct MHH contribution is $T^{2/3}$ rather than the required $T^{3/5}$. `MediumReflection.lean` supplies partial infrastructure with exact Poisson/cosine representation, quantitative nonstationary expansion, stationary normal form, and two-sided quadratic control, but its uniform stationary main term and dual-block recovery remain necessary #15 work. The remaining path smooths each medium block, proves the stationary main term, recovers a dual block of length comparable to $T/N$, removes ordinate-dependent phases by finite Mellin/Fourier separation and pigeonholing, consumes the completed certificates, and assembles the two endpoints. Guth–Maynard's distinct smooth reflection principle remains separately tracked under #19 because it is an input to their Section 6 large-values argument.
...
The selected #18 route follows Maynard–Pratt Appendix C [5]. Uniform arbitrary-order Gamma decay and honest contour integrability are proved. The native fourth-moment reduction is complete: it constructs multiplicity-preserving ordinate fibers and representative zeros, performs Jensen-controlled growing-scale extraction, proves the exact contour-to-critical-line Gamma convolution and weighted fourth-power Hölder inequalities, absorbs the Gamma tail quantitatively, converts the translated windows by bounded overlap to $[T/2,3T]$, and assembles the final epsilon loss. Mellin inversion and contour shifting for an exponentially smoothed Möbius detector remain necessary for coverage. The identity $|M\zeta|^4=|\zeta|^4|M^2|^2$ puts the squared mollifier at length $T^{1/50}$, within the $T^{1/11-\varepsilon}$ range of Hughes–Young [6]. Formalizing the required upper-bound corollary still requires an approximate functional equation for $\zeta^2$, diagonal evaluation, and quadratic-divisor/off-diagonal control.

For #19, the false interval-free incidence and off-by-one block contracts have been deleted. The source-faithful Guth–Maynard route [1] now has its affine/approximate-energy language, exact sampling-matrix trace-dispersion reduction through Lemmas 4.1–4.2, and an analytic layer proving the actual `h_t` kernel is Schwartz, identifying and uniformly bounding its zero Fourier mode through Mellin/Fourier conversion, establishing exact dilation and Poisson identities, and proving the complete Hilbert–Schmidt first-trace expansion with arbitrary-power tail control. The remaining chain is the full two-parameter Lemma 4.3 bounds, cubic trace split, the $S_1$, $S_2$, and $S_3$ estimates, Section 6 smooth reflection, the Heath–Brown difference-set mean-square estimate, Proposition 9.1, and the final GCD/spacing assembly. Mathlib's exact additive energy is connected to the tolerance-one real energy through the proved seven-bin floor bridge.
...
- **Focused Axiom/Output Audit Command**: `lake env lean RiemannZeta/Audit.lean` (1008/1008 existing dependency checks pass; the command exits nonzero on eight missing research outputs)
...
3. Proving the Maynard–Pratt Type-I/Type-II cover and Hughes–Young-range twisted fourth moment required by #18; the contour-to-twisted-fourth-moment reduction is complete.
4. Completing the source Guth–Maynard proof after the proved matrix/dispersion and first-trace Poisson layers: the remaining Lemma 4.3 bounds, cubic trace split, `S₁/S₂/S₃`, reflection, Heath–Brown, affine transformation, energy, and final integration.
