...
In this work, we formalize finite Dirichlet polynomial conjugation identities, coordinate wrappers around Mathlib's completed Zeta symmetries, a complex-valued Hardy-type phase normalization, and provisional interfaces for the Guth–Maynard zero-density architecture [2, 3]. The F-01 through F-10 transfer is kernel-checked conditionally from ten explicit primitive inputs. Local zero multiplicity, Montgomery mean value, beta removal, divisor count, ordered-factorization count, and the finite classical Montgomery–Halász–Huxley large-values theorem are proved. The false historical decoupling model has been deleted and replaced by source-facing shared-language, matrix/trace, and initial Fourier/Poisson packages, including affine energy reindexing, a floor-bin bridge, the exact operator-norm conclusions of Guth–Maynard Lemmas 4.1–4.2, the genuine Schwartz trace kernel, its Mellin zero-mode decay, Fourier dilation, and exact Poisson summation. The exact-$\sigma$ classical density transfer, completed Type-II inputs, and the remaining `S₁/S₂/S₃` and energy portion of the Guth–Maynard large-values proof remain open.
...
For #19, the false interval-free incidence and off-by-one block contracts have been deleted. The source-faithful Guth–Maynard route [1] now has its affine/approximate-energy language, exact sampling-matrix trace-dispersion reduction through Lemmas 4.1–4.2, and an initial analytic layer proving the actual `h_t` kernel is Schwartz, identifying and uniformly bounding its zero Fourier mode through Mellin/Fourier conversion, and establishing exact dilation and Poisson identities. The remaining chain is the full two-parameter Lemma 4.3 bounds, trace split, the $S_1$, $S_2$, and $S_3$ estimates, Section 6 smooth reflection, the Heath–Brown difference-set mean-square estimate, Proposition 9.1, and the final GCD/spacing assembly. Mathlib's exact additive energy is connected to the tolerance-one real energy through the proved seven-bin floor bridge.
...
- **Focused Axiom/Output Audit Command**: `lake env lean RiemannZeta/Audit.lean` (999/999 existing dependency checks pass; the command exits nonzero on eight missing research outputs)
...
4. Completing the source Guth–Maynard proof after the proved matrix/dispersion and initial Poisson layers: the remaining Lemma 4.3 bounds, trace split, `S₁/S₂/S₃`, reflection, Heath–Brown, affine transformation, energy, and final integration.
