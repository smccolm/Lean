...
In this work, we formalize finite Dirichlet polynomial conjugation identities, coordinate wrappers around Mathlib's completed Zeta symmetries, a complex-valued Hardy-type phase normalization, and provisional interfaces for the Guth–Maynard zero-density architecture [2, 3]. The F-01 through F-10 transfer is kernel-checked conditionally from ten explicit primitive inputs. Local zero multiplicity, Montgomery mean value, beta removal, divisor count, ordered-factorization count, the finite classical Montgomery–Halász–Huxley large-values theorem, Appendix C contour coverage, and the Type-II fourth-moment reduction are proved. The false historical decoupling model has been deleted and replaced by source-facing shared-language, matrix/trace, and Fourier/Poisson packages, including affine energy reindexing, a floor-bin bridge, the exact operator-norm conclusions of Guth–Maynard Lemmas 4.1–4.2, the genuine Schwartz trace kernel, its Mellin zero-mode decay, Fourier dilation, exact Poisson summation, and the complete Hilbert–Schmidt first-trace expansion with a uniform `N⁻¹⁰⁰` remainder. The exact-$\sigma$ classical density transfer, twisted fourth moment, and the remaining `S₁/S₂/S₃` and energy portion of the Guth–Maynard large-values proof remain open.
...
For #19, the false interval-free incidence and off-by-one block contracts have been deleted. The source-faithful Guth–Maynard route [1] now has its affine/approximate-energy language, exact sampling-matrix trace-dispersion reduction through Lemmas 4.1–4.2, the complete Hilbert–Schmidt first-trace Poisson expansion, arbitrary-order uniform two-parameter Fourier decay, and a kernel-checked Section 6 smooth-reflection module. The latter proves exact cutoff Mellin inversion and Fubini rescaling, signed mode symmetry, the finite approximate functional equation `gmSmoothReflection_native`, its aggregate norm bound, the $10/\sqrt{\tau}$ reflection-integral estimate, and explicit $T^{-100}$ far-frequency control. The remaining chain is the cubic trace split, the $S_1$, $S_2$, and $S_3$ estimates, the Heath–Brown difference-set mean-square estimate, Proposition 9.1, and the final GCD/spacing assembly. Mathlib's exact additive energy is connected to the tolerance-one real energy through the proved seven-bin floor bridge.
...
- **Focused Axiom/Output Audit Command**: `lake env lean RiemannZeta/Audit.lean` (1052/1052 existing dependency checks pass; the command exits nonzero on seven missing research outputs)
...
4. Completing the source Guth–Maynard proof after the proved matrix/dispersion, first-trace Poisson, uniform Fourier-decay, and smooth-reflection layers: the remaining cubic trace split, `S₁/S₂/S₃`, Heath–Brown, affine transformation, energy, and final integration.
