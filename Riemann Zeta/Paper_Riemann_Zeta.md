...
date: "August 3, 2026"
abstract: |
  We present a Lean 4 library (pinned to Lean `v4.30.0` and Mathlib revision `c5ea00351c28e24afc9f0f84379aa41082b1188f`) of finite positive-index Dirichlet-polynomial conjugation lemmas, elementary products-of-norms identities, coordinate wrappers for existing Mathlib theorems about the completed Riemann Zeta function, and the exact native Guth–Maynard Sections 3–12 large-values theorem. We also define a complex-valued Hardy-type phase normalization on the critical line and prove norm and zero equivalence with the Riemann Zeta function. A two-reflection theorem packages the functional-equation images of two assumed conjugate zeros. The library does not yet prove the interior classical zero-density endpoints, the concrete final density theorems, a one-zero fourfold orbit, real-valuedness of the classical Hardy Z-function, or new zero-free regions. Verification details are tied to the repository release and its continuous integration audit.
---
...
In this work, we formalize finite Dirichlet polynomial conjugation identities, coordinate wrappers around Mathlib's completed Zeta symmetries, a complex-valued Hardy-type phase normalization, and the Guth–Maynard zero-density architecture [2, 3]. The F-01 through F-10 transfer is kernel-checked conditionally from ten explicit primitive inputs. Local zero multiplicity, Montgomery mean value, beta removal, divisor count, ordered-factorization count, the finite classical Montgomery–Halász–Huxley large-values theorem, the exact-$\sigma$ positive-slab density transfer, Appendix C contour coverage, the Type-II fourth-moment reduction, the DFI quadratic-divisor theorem, the Hughes–Young twisted fourth moment, and `guthMaynardLargeValues_native : GuthMaynardLargeValues` are proved. The source-faithful Sections 3–12 chain includes matrix/trace and Poisson packages, `S₁`, quantitative smooth reflection, native Heath–Brown, full `S₂`, localization, affine descent, refined `S₃`, Section 11 energy, and final root/source assembly. The symmetric Ingham/Huxley conversion and concrete density consumers remain open.
...
The primary long-term objective of this project is the zero-density bound of Guth and Maynard (2026). The conclusion-equivalent transfer axiom has been deleted. Its replacement `conditionalZeroDensityTransfer` is a kernel-checked primitive-input theorem: it derives separated extraction, powered-coefficient control, the complete central Type-I estimate, the residual Type-II estimate, the positive-slab partition, the F-01 dyadic-to-global reduction, and Huxley's high-$\sigma$ branch from individually named analytic/arithmetic inputs. Eight of its ten inputs are now discharged by native theorems: local-zero multiplicity, Montgomery mean value, beta removal, divisor count, ordered-factorization count, Appendix C coverage, the fourth-moment reduction, and `twistedZetaFourthMoment_native`. The two remaining inputs are native Huxley and `GuthMaynardLargeValues`.
...
$$
This has the `(T+O(X)) log X` strength of the critical-line mollifier input used in the source argument. The selected finite large-values route now proves the exact-$\sigma$ positive-slab density transfer and both endpoint-certificate specializations. Shitlist #15 nevertheless remains open because their conversion to the full symmetric Ingham and Huxley zero-density propositions is still unproved.
...
The selected remaining interior engine is a finite zero-density-from-large-values route. The sharp approximation, dichotomy, coefficient growth, arbitrary-`X` detector normalization and exact powering/MHH application, endpoint arithmetic, terminal restriction, and medium Type-I B-process are proved. The Type-I analytic modules prove the smooth partition, scaled Poisson reflection, complete two-sided far tail, and Fourier deweighting. `normalizedClassicalTypeICoeff` supplies the required `N^σ` source normalization; the direct, powered, reflected, and weighted-Weyl consumers use the corrected `N^τ=T` bridge. `classical_endpoint_positive_slab_native` then eliminates the actual zero/Type-I/Type-II dichotomy, derives a positive finite power whose scale lies in `[2τ₀/3,τ₀]`, preserves the original analytic multiplicities, absorbs every finite/logarithmic/harmonic loss, and proves the exact slab exponent `3(1-σ)/τ₀`. A 24 August semantic re-audit unfolded the proposition and dichotomy and traced these dependencies through the actual Type-I and Type-II witness consumers; the medium interface is strictly narrower and discharged natively. This closes `FR` without claiming the still-open symmetric endpoint conversion.
$$R\ll_\varepsilon T^\varepsilon\left(N^2V^{-2}+T\min(NV^{-2},N^4V^{-6})\right).$$
The theorem `classical_montgomery_halasz_huxley_native` proves this estimate for bounded coefficients on a dyadic interval and a finite one-separated set in $[0,T]$, with an explicit positive constant and the exact minimum of the second- and sixth-power branches. `ingham_endpoint_positive_slab_native` and `huxley_endpoint_positive_slab_native` already obtain Ingham's exponent $3(1-\sigma)/(2-\sigma)$ and Huxley's exponent $3(1-\sigma)/(3\sigma-1)$ on positive slabs from the concrete endpoint certificates. The remaining `ZD` transfer must specialize the positive-ordinate-to-symmetric machinery over the full endpoint intervals. At $\sigma=3/4$ the two exponents both equal $3/5$, so boundary compatibility is not a separate analytic obstruction. Neither fully quantified endpoint proposition is currently a Lean theorem for the project's symmetric multiplicity-weighted `N`.

The selected #18 route follows Maynard–Pratt Appendix C [5]. Uniform arbitrary-order Gamma decay, honest contour integrability, and the native coverage theorem are proved. The coverage proof establishes Mellin inversion for the exponentially smoothed Möbius detector, removes the apparent singularity at $s=0$, shifts the rectangle, computes the sole residue at $s=1-\rho$, controls all tails quantitatively, and assembles the exact dyadic Type-I/contour-Type-II dichotomy. The native fourth-moment reduction constructs multiplicity-preserving ordinate fibers and representative zeros, performs Jensen-controlled growing-scale extraction, proves the exact contour-to-critical-line Gamma convolution and weighted fourth-power Hölder inequalities, absorbs the Gamma tail quantitatively, converts the translated windows by bounded overlap to $[T/2,3T]$, and assembles the final epsilon loss. The identity $|M\zeta|^4=|\zeta|^4|M^2|^2$ puts the squared mollifier at length $T^{1/50}$, within the $T^{1/11-\varepsilon}$ range of Hughes–Young [6]. `SmoothZetaAFE.lean` proves the completed-$\zeta^2$ smooth contour identity, its right-line divisor-series opening, ordinary-zeta normalization, exact $hm=kn$ phase integration, and reduction of the weighted diagonal to the harmonic-power majorant. `GammaPairBound.lean` proves the required cancellation directly from Euler's finite Gamma products and passes to Gamma by a kernel-checked limit. With the fixed strength-$100$ admissible Gaussian, the exact coefficientwise archimedean factor is identified from the divisor contour and satisfies the uniform bound $e^{-84u^2}(2+8u^2)^4$ and the integrable envelope $16e^{-68u^2}$. The DFI chain formalizes equations (2) and (9)–(30), including source-weight Mellin/Bessel analysis, delta localization, signed central terms, and the optimized Theorem 1 error. Hughes–Young's equation-(80) consumer, dyadic and contour reassembly, and finite off-diagonal epsilon-power estimate then yield `twistedZetaFourthMoment_native : TwistedZetaFourthMomentProp`. Thus both the diagonal and off-diagonal twisted-moment nodes are complete; #18 remains open only for the one-premise Goal C zero-density specialization, which also depends on #15's native Huxley theorem.

For #19, the false interval-free incidence and off-by-one block contracts have been deleted. The source-facing route [1] proves an exact three-piece localization from `sourceDirichletPoly` to one common smoothed matrix family, sampling-matrix trace-dispersion reductions, the first-trace Poisson expansion, arbitrary-order Fourier decay, the cubic expansion and frequency partition, and Proposition 5.1 for $S_1$. `LargeValuesReflection.lean` and `QuantitativeSmoothReflection.lean` prove the exact signed Mellin/Fubini identities, complete nonzero integer-frequency series, arbitrary $O_A(T^{-A})$ tail, and scaled cubic-$S_2$ consumer. The subsequent native chain proves Heath–Brown's difference-set estimate, full Proposition 6.1, approximate-energy moments, Proposition 8.1 localization, Proposition 9.1 affine descent, Proposition 10.1 refined $S_3$, and Propositions 11.1–11.2. `LargeValuesFinal.lean` performs Sections 3 and 12 root extraction, one-separated/full-height transfer, exact source-polynomial localization, and the complementary low-$V$, high-$V$, near-height, and bounded-$N$ ranges. A semantic re-audit confirms that `guthMaynardLargeValues_native` concludes the exact public proposition from these native declarations rather than from a theorem-equivalent hypothesis.

## Theorem 11 (Large Values Estimate)
**Status**: Kernel-checked as `guthMaynardLargeValues_native : GuthMaynardLargeValues`.
...
`RiemannZeta/Audit.lean` explicitly lists all 7602 exported source-level theorems in the root-imported production graph and computes their transitive axioms with `Lean.collectAxioms`. The explicit list matches the discovered theorem set in that scope, and every audited declaration passes with only `propext`, `Classical.choice`, and `Quot.sound`. The audit explicitly passes the native DFI error theorem, its Hughes–Young consumers, `twistedZetaFourthMoment_native`, `classical_endpoint_positive_slab_native`, `gmQuantitativeSmoothReflection_native`, and `guthMaynardLargeValues_native`. A separate named-output gate rejects the five later #15/#18/#19 end theorems, so the executable audit remains nonzero until the research agenda is complete. No direct project axiom remains.

The default `RiemannZeta` root includes `ClassicalEndpointDensity`, the complete FR dependency chain, `QuantitativeSmoothReflection`, and the native large-values chain through `LargeValuesFinal`. The runner also builds selected production modules explicitly as a redundant check. Two retained subordinate Hughes–Young modules and 47 tracked top-level probes are outside runner elaboration; therefore the 7602-theorem audit and zero-warning build claim are scoped to the imported graph until #19 final verification repairs this coverage gap.
...
| Type II Analytic Inputs | `typeIContourTypeIICoverOn_native`, `typeIIFourthMomentReduction_native`, `twistedZetaFourthMoment_native` | `GuthMaynard/TypeIICoverage.lean`, `TypeIIFourthMomentReduction.lean`, `HughesYoungNativeCompletion.lean` | All three source-facing residual Type-II inputs proved natively |
| DFI Quadratic-Divisor Error | `exists_norm_dfiDyadicShiftedDivisorSum_sub_signedCentralSeries_le_theorem1ErrorScale_native` | `GuthMaynard/DFITheorem1.lean` | DFI equations (2), (9)–(30), optimized error assembly |
...
| Large Values Estimate | `guthMaynardLargeValues_native : GuthMaynardLargeValues` | `GuthMaynard/LargeValuesFinal.lean` | Native Sections 3–12 chain and complementary classical ranges |
...
- **Principal Runner Coverage**: five warning-failing stages covering the default production graph, explicit production-module redundancy, two designated examples, and the transitive axiom audit; 49 retained Lean files remain outside elaboration and are an open #19 verification obligation
- **Focused Axiom/Output Audit Command**: `lake env lean RiemannZeta/Audit.lean` (7602/7602 existing dependency checks pass; `twistedZetaFourthMoment_native`, `classical_endpoint_positive_slab_native`, `gmQuantitativeSmoothReflection_native`, and `guthMaynardLargeValues_native` pass their output gates; the command exits nonzero on five later missing research outputs)
- **Latest Verification Evidence**: `logs/overall_proof_20260828_140927.log` (direct final-module elaboration and focused final/energy/umbrella builds passed independently; runner stages 1–4 are warning-free, all integrity scans pass, and the native Guth–Maynard output gate passes)
...
2. Converting #15's completed multiplicity-aware positive-slab Ingham/Huxley specializations into the fully quantified symmetric endpoint propositions.
3. Completing #18's one-premise Goal C specialization now that the Maynard–Pratt coverage/reduction, smooth completed-$\zeta^2$ AFE, DFI off-diagonal theorem, Hughes–Young consumer, and `twistedZetaFourthMoment_native` are complete; the remaining specialization also depends on #15's native Huxley theorem.
4. Proving the concrete Guth–Maynard and combined density consumers now that the exact source large-values theorem is complete, then closing retained-file evaluation coverage and final integration.
