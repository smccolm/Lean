...
The selected remaining interior engine is a finite zero-density-from-large-values route. Its first three packages are complete. `ZetaTruncation.lean` proves the exact Euler–Maclaurin truncation on $\Re(s)>0$, specializes it at genuine off-axis zeta zeros, and bounds the error by $\lVert\rho\rVert b^{-\Re(\rho)}/\Re(\rho)$. `SecondDerivative.lean`, `SecondOrderMeanValue.lean`, `Weyl.lean`, `WeylExplicit.lean`, and `WeylZeta.lean` prove the multi-period B-process, corrected A-process, the $(1/6,2/3)$ logarithmic Weyl estimate with uniform prefixes, and the exact weighted critical-line block bound. `LogarithmicKernel.lean` and `ClassicalLargeValues.lean` prove the near/far logarithmic-kernel estimates, exact phase-aligned Gram expansion, separated harmonic-shell bound, local absorption, global subdivision, coefficient energy inequalities, and the full finite Montgomery–Halász–Huxley estimate. The only open #15 package is the finite exact-$\sigma$ density transfer. Guth–Maynard's smooth reflection principle is separately tracked under #19 because it is an input to their Section 6 large-values argument.
...
- **Focused Axiom Audit Command**: `lake env lean RiemannZeta/Audit.lean` (currently expected to exit nonzero and identify one decoupling dependency failure across 549 synchronized declarations)
