---
title: "A Lean 4 Formalization of the Guth--Maynard Large-Values and Zero-Density Chain"
author: "S. McColm"
date: "August 3, 2026"
abstract: |
...
In this work, we formalize finite Dirichlet-polynomial identities, coordinate wrappers around Mathlib's completed-zeta symmetries, a complex-valued Hardy-type phase normalization, and the Guth–Maynard zero-density argument [1--6]. The F-01 through F-10 transfer is kernel-checked from ten explicit primitive inputs, and the native specialization supplies each input from a named theorem. This includes local zero multiplicity, Montgomery mean value, beta removal, coefficient bounds, the finite classical Montgomery–Halász–Huxley theorem, the exact-$\sigma$ positive-slab transfer, native Ingham and Huxley bounds, Appendix C coverage, the Type-II fourth-moment reduction, the localized signed DFI estimate, the mollifier-specific twisted-fourth-moment upper bound, and the Guth–Maynard Sections 3–12 large-values proposition. The final concrete Guth–Maynard and combined zero-density propositions are kernel-checked and project-integrated.
...
5. **Formalized Published Inputs and Consumers**: Classical density, DFI/Hughes--Young upper-bound, and Guth--Maynard large-values arguments translated into project-specific formal statements with explicit convention bridges.
6. **New Formalization Infrastructure**: Analytic-multiplicity rectangles, epsilon-power closure lemmas, finite source localization, sign and endpoint bridges, floor/ceiling arithmetic, and executable dependency/output audits. These are implementation contributions, not claims of new paper mathematics.

## AI Tool Disclosure & Human Review Statement
Lean proof search, translations of paper arguments, refactoring, source discovery, audits, and manuscript drafting were assisted by language-model coding agents, including OpenAI Codex and earlier Antigravity/Gemini sessions. Lean checked accepted proof terms; it did not validate attribution, exposition, naturalness, or source fidelity. No AI system is an author. S. McColm retains responsibility for the mathematical content, semantic claims, citations, and any future release or submission.
...
$$
This has the `(T+O(X)) log X` strength of the critical-line mollifier input used in the source argument. The selected finite large-values route proves the exact-$\sigma$ positive-slab density transfer and both endpoint-certificate specializations. `ingham_zero_density_native` and `huxley_zero_density_native` complete their conversion to the full symmetric multiplicity-weighted propositions.
...
The selected interior engine is a finite zero-density-from-large-values route. The sharp approximation, dichotomy, coefficient growth, arbitrary-`X` detector normalization and exact powering/MHH application, endpoint arithmetic, terminal restriction, and medium Type-I B-process are proved. `classical_endpoint_positive_slab_native` eliminates the actual zero/Type-I/Type-II dichotomy, derives a positive finite power whose scale lies in `[2τ₀/3,τ₀]`, preserves analytic multiplicities, absorbs every finite/logarithmic/harmonic loss, and proves the exact slab exponent `3(1-σ)/τ₀`. The native endpoint consumers then perform the dyadic positive-to-symmetric conversion.
$$R\ll_\varepsilon T^\varepsilon\left(N^2V^{-2}+T\min(NV^{-2},N^4V^{-6})\right).$$
The theorem `classical_montgomery_halasz_huxley_native` proves this estimate for bounded coefficients on a dyadic interval and a finite one-separated set in $[0,T]$, with an explicit positive constant and the exact minimum of the second- and sixth-power branches. `ingham_endpoint_positive_slab_native` and `huxley_endpoint_positive_slab_native` obtain Ingham's exponent $3(1-\sigma)/(2-\sigma)$ and Huxley's exponent $3(1-\sigma)/(3\sigma-1)$ on positive slabs. `ingham_zero_density_native` and `huxley_zero_density_native` prove the corresponding fully quantified propositions for the project's symmetric multiplicity-weighted $N$; the shared $\sigma=3/4$ exponent is $3/5$.

The selected #18 route follows Maynard–Pratt Appendix C [5]. Uniform arbitrary-order Gamma decay, contour integrability, and the native coverage theorem are proved. The coverage proof establishes Mellin inversion for the exponentially smoothed Möbius detector, removes the apparent singularity at $s=0$, shifts the rectangle, computes the residue at $s=1-\rho$, controls tails, and assembles the dyadic Type-I/contour-Type-II dichotomy. The fourth-moment reduction constructs multiplicity-preserving ordinate fibers and representative zeros, performs growing-scale extraction, proves the contour-to-critical-line Gamma convolution and weighted fourth-power Hölder inequalities, absorbs the Gamma tail, and converts translated windows by bounded overlap to $[T/2,3T]$. The identity $|M\zeta|^4=|\zeta|^4|M^2|^2$ puts the squared mollifier at length $T^{1/50}$, within the $T^{1/11-\varepsilon}$ range of Hughes–Young [6]. `SmoothZetaAFE.lean` proves the smooth completed-$\zeta^2$ contour identity and diagonal majorant. The DFI chain supplies a localized signed dyadic specialization, with equation-(2) weight and scale assumptions explicit, by formalizing the equations used in its consumer. Hughes–Young's equation-(80) consumer, dyadic/contour reassembly, and finite off-diagonal estimate then yield `twistedZetaFourthMoment_native : TwistedZetaFourthMomentProp`. This conclusion is the mollifier-specific $T^{1+\varepsilon}$ upper bound required by the transfer; it is not the full Hughes–Young shifted asymptotic formula.

For #19, the false interval-free incidence and off-by-one block contracts have been deleted. The source-facing route [1] proves a three-piece localization from `sourceDirichletPoly` to one common smoothed matrix family, sampling-matrix trace-dispersion reductions, the first-trace Poisson expansion, arbitrary-order Fourier decay, the cubic expansion and frequency partition, and the project counterpart of Proposition 5.1 for $S_1$. `LargeValuesReflection.lean` and `QuantitativeSmoothReflection.lean` prove signed Mellin/Fubini identities, the complete nonzero integer-frequency series, arbitrary $O_A(T^{-A})$ tail, and a scaled cubic-$S_2$ consumer. The subsequent chain implements the cited difference-set estimate, Proposition 6.1, approximate-energy moments, Propositions 8.1, 9.1, 10.1, and 11.1--11.2 in the project's normalization. `LargeValuesFinal.lean` performs Sections 3 and 12 assembly and the complementary low-$V$, high-$V$, near-height, and bounded-$N$ ranges. The internal semantic audit confirms that `guthMaynardLargeValues_native` concludes the unfolded public proposition from these declarations rather than a theorem-equivalent hypothesis; independent source review remains open.
...
**Status**: Kernel-checked as `guthMaynardZeroDensity_native`; the reusable one-premise form is `guthMaynardZeroDensity_of_largeValues_native`.
...
**Status**: Kernel-checked as `combined_zero_density_native`.
Combining the native Ingham and Guth–Maynard bounds, for $\sigma \ge 1/2$:
...
We have constructed a Lean 4 library whose project-integrated declarations encode the large-values and zero-density chain described above. Kernel checking establishes formal derivability of those encoded statements. The separate publication-readiness audit records why this is a narrower claim than independent mathematical validation.

**Future Technical Extensions**:
1. Formalizing continuous branch choices for $\theta(t)$ to prove that $H(t)$ is real-valued.
2. Packaging reusable analytic components for potential upstream Mathlib or PNT+ contributions.
3. Extending the formalization to refinements beyond the completed #15–#19 acceptance contract.
4. Independent expert review of the theorem/source crosswalk and the ten dangerous convention bridges.
5. Human exposition review, a separately authorized preprint decision, peer review, and eventual community canonicalization.
