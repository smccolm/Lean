...
In this work, we formalize finite Dirichlet polynomial conjugation identities, coordinate wrappers around Mathlib's completed Zeta symmetries, a complex-valued Hardy-type phase normalization, and provisional interfaces for the Guth–Maynard zero-density architecture [2, 3]. The F-01 through F-10 transfer is kernel-checked conditionally from ten explicit primitive inputs. Local zero multiplicity, Montgomery mean value, beta removal, divisor count, ordered-factorization count, the finite classical Montgomery–Halász–Huxley large-values theorem, the exact-$\sigma$ positive-slab density transfer, Appendix C contour coverage, the Type-II fourth-moment reduction, the DFI quadratic-divisor theorem, and the Hughes–Young twisted fourth moment are proved. The false historical decoupling model has been deleted and replaced by source-facing shared-language, matrix/trace, and Fourier/Poisson packages. These now include the exact operator-norm conclusions of Guth–Maynard Lemmas 4.1–4.2, the first-trace expansion, the exact Lemma 4.5 cubic Poisson expansion with its diagonal `Oε(T⁻¹⁰⁰)` remainder, the `S₁/S₂/S₃` partition, Proposition 5.1 with `S₁ = Oε(T⁻¹⁰)`, and the complete quantitative smooth-reflection estimate for the nonzero integer-frequency sum. The symmetric Ingham/Huxley conversion and the remaining full `S₂/S₃` and energy portion of the Guth–Maynard large-values proof remain open.
...
For #19, the false interval-free incidence and off-by-one block contracts have been deleted. The source-facing route [1] has affine/approximate-energy language, an exact three-piece localization from `sourceDirichletPoly` to one common smoothed matrix family, sampling-matrix trace-dispersion reductions, the first-trace Poisson expansion, arbitrary-order Fourier decay, the cubic expansion and frequency partition, and Proposition 5.1 for $S_1$. `LargeValuesReflection.lean` proves the exact signed finite-mode and Mellin/Fubini identities. `QuantitativeSmoothReflection.lean` proves summability and the exact decomposition of the complete nonzero integer-frequency series, splits and integrates the Mellin core and tails, handles both signs of the ordinate, and chooses $\eta=\min(\varepsilon/16,1/16)$, $H=T^\eta$, and $M=\lceil(1+|t|)H/N\rceil$ so both omitted-frequency and Mellin errors are $O_A(T^{-A})$. The public `gmQuantitativeSmoothReflection_native` gives arbitrary $A$, its strength-$100$ specialization gives the source remainder, and the scaled theorem is consumed by the literal first cyclic `S_2` summand. A semantic re-audit unfolded these definitions and verified that the proof uses the exact Schwartz-transform series, the absolutely summable complement, and the source cubic factor rather than a finite proxy or an assumed conclusion. The complete Proposition 6.1 summation, Heath–Brown, $S_2/S_3$, Proposition 9.1, energy, and GCD/spacing assembly remain open.
...
`RiemannZeta/Audit.lean` explicitly lists all 6066 exported source-level theorems in the root-imported production graph and computes their transitive axioms with `Lean.collectAxioms`. The explicit list matches the discovered theorem set in that scope, and every audited declaration passes with only `propext`, `Classical.choice`, and `Quot.sound`. The audit explicitly passes the native DFI error theorem, its Hughes–Young consumers, `twistedZetaFourthMoment_native`, `classical_endpoint_positive_slab_native`, and `gmQuantitativeSmoothReflection_native`. A separate named-output gate rejects the six later #15/#18/#19 end theorems, so the executable audit remains nonzero until the research agenda is complete. No direct project axiom remains.

The default `RiemannZeta` root includes `ClassicalEndpointDensity`, the complete FR dependency chain, and `QuantitativeSmoothReflection`. The runner also builds selected production modules explicitly as a redundant check. Two retained subordinate Hughes–Young modules and 47 tracked top-level probes are outside runner elaboration; therefore the 6066-theorem audit and zero-warning build claim are scoped to the imported graph until #19 final verification repairs this coverage gap.
...
- **Focused Axiom/Output Audit Command**: `lake env lean RiemannZeta/Audit.lean` (6066/6066 existing dependency checks pass; `twistedZetaFourthMoment_native`, `classical_endpoint_positive_slab_native`, and `gmQuantitativeSmoothReflection_native` pass their output gates; the command exits nonzero on six later missing research outputs)
...
4. Completing the source Guth–Maynard proof after the proved matrix/dispersion, first- and cubic-trace Poisson, `S₁`, uniform Fourier-decay, and quantitative smooth-reflection layers: the remaining complete `S₂/S₃`, Heath–Brown, affine transformation, energy, and final integration.
