...
The selected remaining interior engine is a finite zero-density-from-large-values route. The sharp approximation, dichotomy, exact powering infrastructure, coefficient growth, MHH/Weyl estimates, endpoint arithmetic, terminal restriction, and medium Type-I B-process are proved. The Type-I analytic modules prove the smooth partition and common-scale extraction, removal of the interior source boundary, scaled Poisson summation, a symmetric finite central window, a complete two-sided far tail, and Fourier deweighting. `typeI_finite_window_resolution_native` is a generic coefficient-uniform MHH/cardinality helper paired with an abstract endpoint certificate. It does not consume the dichotomy, connect its free `τ` to the actual block scale, translate the enlarged ordinate interval, or discharge the terminal majorant. Consequently the actual Type-I branch consumer `ZR` remains open alongside actual-`X` Type II and the slab/endpoints.
...
For #19, the false interval-free incidence and off-by-one block contracts have been deleted. The source-facing route [1] has affine/approximate-energy language, an exact three-piece localization from `sourceDirichletPoly` to one common smoothed matrix family, sampling-matrix trace-dispersion reductions, the first-trace Poisson expansion, arbitrary-order Fourier decay, the cubic expansion and frequency partition, and Proposition 5.1 for $S_1$. `LargeValuesReflection.lean` proves exact finite signed-mode identities and cancellation-preserving estimates for the positive, negative, and complete retained signed window, with the Dirichlet polynomials kept inside the Mellin integrals; it does not yet extract the source-uniform $T_0^{-1/2}$ factor or sum the omitted modes into the Lemma 6.2 $T^{-100}$ remainder. Quantitative Lemma 6.2 precedes the remaining $S_2/S_3$, Heath–Brown, Proposition 9.1, energy, and GCD/spacing assembly.
...
- **Focused Axiom/Output Audit Command**: `lake env lean RiemannZeta/Audit.lean` (1177/1177 existing dependency checks pass; the command exits nonzero on seven missing research outputs)
...
2. Completing #15's actual-`X` Type-II powered MHH application and actual Type-I dichotomy/scale/majorant consumer, then consuming both in the branch-to-slab reduction and concrete Ingham/Huxley endpoints.
