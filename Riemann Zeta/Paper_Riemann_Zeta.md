...
The selected remaining interior engine is a finite zero-density-from-large-values route. Its first package is now complete: `ZetaTruncation.lean` proves the exact Euler–Maclaurin truncation on $\Re(s)>0$, specializes it at genuine off-axis zeta zeros, and bounds the error by $\lVert\rho\rVert b^{-\Re(\rho)}/\Re(\rho)$. The open packages are targeted van der Corput B/A processes giving exponent pairs $(1/2,1/2)$ and $(1/6,2/3)$, together with the Weyl critical-line zeta bound and medium-length reflection estimate; a finite exact-$\sigma$ density transfer; and the full classical Montgomery–Halász–Huxley large-values estimate
...
- **Focused Axiom Audit Command**: `lake env lean RiemannZeta/Audit.lean` (currently expected to exit nonzero and identify one decoupling dependency failure across 447 synchronized declarations)
