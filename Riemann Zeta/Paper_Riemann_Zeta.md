...
For #19, the false interval-free incidence and off-by-one block contracts have been deleted. The source-faithful Guth–Maynard route [1] now has its affine/approximate-energy language and exact sampling-matrix trace-dispersion reduction through Lemmas 4.1–4.2. The remaining chain is Poisson expansion, the $S_1$, $S_2$, and $S_3$ estimates, Section 6 smooth reflection, the Heath–Brown difference-set mean-square estimate, Proposition 9.1, and the final GCD/spacing assembly. Mathlib's exact additive energy is connected to the tolerance-one real energy through the proved seven-bin floor bridge.
...
- **Focused Axiom/Output Audit Command**: `lake env lean RiemannZeta/Audit.lean` (975/975 existing dependency checks pass; the command exits nonzero on eight missing research outputs)
