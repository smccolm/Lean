...
The primary long-term objective of this project is the zero-density bound of Guth and Maynard (2026). The conclusion-equivalent transfer axiom has now been deleted. The replacement `conditionalZeroDensityTransfer` is kernel-checked and genuinely combines the exact Type-I/residual partition, a provisional dyadic-to-global boundary, and Huxley's high-$\sigma$ branch. It remains an interim result rather than the completed Section 13.1 transfer: the uniform central Type-I slab estimate and the F-01 dyadic-to-global reduction are still explicit narrower parameters. The powered-block preparation and all F-09/F-10 exponent comparisons needed to discharge the first parameter are now formalized.
...
`RiemannZeta/Audit.lean` explicitly lists all 173 exported source-level theorems across the production modules and computes their transitive axioms with `Lean.collectAxioms`. It checks that the explicit list matches the discovered theorem set and permits only `propext`, `Classical.choice`, and `Quot.sound`. At the current revision the audit exits nonzero: 4 theorems depend on project-specific mathematical axioms. The compact-rectangle zeta-zero finiteness theorem, finite dyadic-scale and weighted extraction machinery, shifted-bin covering, phase-translation identities, raw and normalized conditional Type-I extraction theorems, detector/divisor-count reductions, powered-polynomial preparation, F-09/F-10 exponent theorems, the interim transfer, mean-value-to-large-values implication, and the generic plus concrete conditional Type II deductions pass. Thirteen direct project axioms remain. The table below is a selected declaration map, not a clean-audit certificate.
...
| Interim Conditional Transfer | `conditionalZeroDensityTransfer` | `GuthMaynard/Transfer.lean` | Audit-clean theorem from explicit Type-I slab, residual, F-01, and Huxley inputs; primitive-input assembly remains open |
