...
For #19, the false interval-free incidence and off-by-one block contracts have been deleted. The source-facing route [1] has affine/approximate-energy language, an exact three-piece localization from `sourceDirichletPoly` to one common smoothed matrix family, sampling-matrix trace-dispersion reductions, the first-trace Poisson expansion, arbitrary-order Fourier decay, the cubic expansion and frequency partition, and Proposition 5.1 for $S_1$. `LargeValuesReflection.lean` proves exact finite signed-mode identities, cancellation-preserving retained-window estimates, arbitrary Mellin decay, the elementary dual-tail bound, and pointwise uniform positive/negative core $T_0^{-1/2}$ estimates. It does not yet prove the measurable core/tail integral split or sum the complete omitted integer-frequency tail into the Lemma 6.2 $T^{-100}$ remainder. Quantitative Lemma 6.2 precedes the remaining $S_2/S_3$, Heath–Brown, Proposition 9.1, energy, and GCD/spacing assembly.
...
- **Focused Axiom/Output Audit Command**: `lake env lean RiemannZeta/Audit.lean` (6028/6028 existing dependency checks pass; `twistedZetaFourthMoment_native` and `classical_endpoint_positive_slab_native` pass their output gates; the command exits nonzero on six later missing research outputs)
