...
For the trivial branch of the optimized DFI scale split, choose the auxiliary
frequency parameter `P = T / (2Y)`.  Since every nonzero integral shift has
absolute value at least one, the corresponding DFI near family is empty.
Thus the exact source box is the equation-(65) complementary family.  Taking
zero integrations by parts gives a uniform finite-box estimate without any
spurious negative power of `P`.
-/
...
/-- A source-facing trivial estimate for one localized box.  It is the
`j = 0` case of Hughes--Young equation (65), after the exact finite shift
partition and gcd reduction.  It applies precisely where the optimized DFI
scale is too short for the hypothesis `Q ≥ 8`. -/
theorem exists_hughesYoungLocalizedOffDiagonalBox_smallScale_bound
    (ε : ℝ) (hε : 0 < ε) :
    ∃ Cγ D L : ℝ, 0 < Cγ ∧ 0 < D ∧ 0 < L ∧
      ...
      ‖hughesYoungLocalizedOffDiagonalBox T
        ...
        hughesYoungFarBoxMajorant Cw D L 0 T X Y ε h k M N := by
  obtain ⟨Cγ, D, L, hCγ, hD, hL, Cw, hCw, hfar⟩ :=
    exists_integrated_farShift_sum_full_bound ε hε 0
  ...
  simpa only [pow_zero, one_mul, hughesYoungFarBoxMajorant] using hbound
