/-
Contour core adapted from PNT+ commit 80c12dfd93 (Lean 4.30).
General residue section adapted from current PrimeNumberTheoremAnd/ResidueCalcOnRectangles.lean.
Copyright belongs to the upstream contributors. Released under Apache License 2.0:
https://github.com/AlexKontorovich/PrimeNumberTheoremAnd/blob/main/LICENSE
-/
import Mathlib
...
import RiemannZeta.External.PNT.Rectangle
...
lemma RectanglePullToNhdOfPole' [CompleteSpace E] {z₀ z₁ z₂ z₃ p : ℂ}
  ...
  rw [h₁, h₂, zero_add, h₃, h₄, add_zero, h₅, h₆, zero_add,
    h₇, h₈, add_zero]
...
lemma simplePole_sub_residue_isBigO_one {f : ℂ → ℂ} {p : ℂ}
  ...
  have hdslope : (fun z ↦ (z - p)⁻¹ * (g z - g p)) =O[nhdsWithin p {p}ᶜ] (1 : ℂ → ℂ) := by
    ...
    simpa [slope] using hbig_ne.congr'
      (dslope_eventuallyEq_slope_nhdsNE (f := g) (a := p)) .rfl
...
private lemma rectangleIntegral'_toMeromorphicNFOn_eq {f : ℂ → ℂ} {z w : ℂ}
  ...
  have hbot := HIntegral_congr_codiscreteWithin h_eq
    (by simpa [R] using mapsTo_rectangle_left_im z w)
  have htop := HIntegral_congr_codiscreteWithin h_eq
    (by simpa [R] using mapsTo_rectangle_right_im z w)
  have hright := VIntegral_congr_codiscreteWithin h_eq
    (by simpa [R] using mapsTo_rectangle_right_re z w)
  have hleft := VIntegral_congr_codiscreteWithin h_eq
