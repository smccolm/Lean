...
| `Audit.lean` checks transitive dependencies for the synchronized production theorem set. | #6 | **Complete for the cleaned tree's named public theorems:** the current list is 999/999, including the native #18 reduction and the #19 shared-language, matrix, trace-dispersion, and initial Fourier/Poisson surfaces. Every declaration passes with only the permitted logical dependencies. |
...
| The paper stated stale audit counts. | Documentation gate | **Resolved during #8 and kept synchronized:** the current audit passes across 999 synchronized declarations. |

Current clean facts: the production graph and both retained examples elaborate with zero warnings; the dependency audit synchronizes and passes 999 declarations; and all prohibited-proof and project-postulate scans are empty. The named output gate reports eight absent end theorems after accepting `typeIIFourthMomentReduction_native`; the overall result remains honestly `FAIL`/`1` until the remaining outputs exist.
...
| 2 | **Verified complete as cleanup** | The retained tree has 72 Lean files, no empty Lean file, a 69-file transitive production graph, `Audit.lean`, and exactly two non-production anonymous-example files. All listed obsolete tests, tombstones, and side-effect utilities are absent. |
...
| 6 | **Verified complete as audit infrastructure** | The explicit and discovered theorem sets agree at 999/999 and all transitive dependency checks pass. The separate completion gate fails on the eight absent #15/#18/#19 outputs. |
| 7 | **Verified complete** | The root import closure reaches all 68 subordinate production modules, giving a 69-file production graph. |
...
| 12 | **Conditional/concrete layer complete** | The concrete `ResidualZeroBoundProp` follows from three explicit source-facing propositions. #18 now discharges `TypeIIFourthMomentReductionProp`; contour coverage and the twisted fourth moment remain unproved. |
...
|---:|---|---|---|
| 15 | **Classical finite zero-density engine and endpoints — in progress** | Existing work proves exact-β removal for the sharp finite Möbius product, the native multiplicity-dominant Type-I/Type-II dichotomy and genuinely short Type-II product, common dyadic-witness extraction, bounded-coefficient normalization, exact finite power raising with #17 coefficient control, unrestricted finite MHH application, weighted Weyl bounds for every `σ ≥ 0`, endpoint-range arithmetic, the finite Corollaries 11.8–11.10 scale/exponent certificates, and the terminal Type-I estimate for the actual sharp-cutoff coefficients. The remaining analytic reduction is medium Type-I reflection, consumption of that result and the completed certificates, and the two endpoints. | The item stays uncrossed until audit-clean theorems prove both `HuxleyZeroDensity (fun σ T => N σ T)` and `InghamZeroDensity (fun σ T => N σ T)`. |
...
| 18 | **Type-II residual analysis and Goal C integration — in progress** | Completed: source-range contract repair; arbitrary-order Gamma decay; weighted whole-line Gamma and actual contour-integrand integrability; arbitrary-scale weighted separation and bounded overlap; the complete native fourth-moment counting reduction with Gamma-tail absorption and epsilon assembly; exact initial Möbius cancellation; exact `M²` convolution, support, eventual `T^(1/22)` length, uniform divisor coefficient bound, and moment identity. Remaining: Appendix C Mellin/rectangle shift, residue and tail comparison proving coverage; smooth `ζ²` AFE, diagonal estimate, and genuine DFI quadratic-divisor/off-diagonal theorem; generic twisted moment and native `M²` specialization; Goal C assembly after #15 supplies native Huxley. | The source-range coverage theorem and twisted fourth-moment proposition are native, audit-clean theorems, and a public audited theorem derives the concrete full-range zero-density result from `GuthMaynardLargeValues` alone. `typeIIFourthMomentReduction_native` already discharges the former third #18 input. No other analytic or arithmetic proposition parameter remains. |
| 19 | **Guth–Maynard large values and final project integration — in progress** | **Completed:** deletion of the false model; cutoff/derivative, matrix action, exact Gram kernel, positive spectrum, singular-value powers, trace-cube, sixth-dispersion, core approximate-energy, basic `R`, and the initial Fourier/Poisson foundations. **Remaining:** Hilbert–Schmidt and cubic trace expansion, uniform two-parameter Fourier decay, the `S₁/S₂/S₃` chain, native Heath–Brown, reflection, moment/energy bridge, affine and GCD/spacing theorems, Sections 3/12 assembly, and concrete transfers. | The concrete Guth–Maynard and combined zero-density theorems are project-theorem complete. Existing dependency and integrity gates pass, while the named output gate keeps the principal runner at `FAIL`/`1` until the end theorems exist. |
...
The completion of #18's native fourth-moment reduction and #19's initial Fourier/Poisson layer changes the most efficient implementation order without changing any Shitlist status. The following targets are ordered by estimated implementation difficulty, not by Shitlist number. The first row is deliberately one theorem-sized closeout; the former row that bundled all of Lemmas 4.3–4.5 with Proposition 5.1 was too broad to be a reliable next task.

| Rank | Owning item | Complete closeout target | Why it is now reachable | Exact boundary |
|---:|---:|---|---|---|
| 1 | 19 | **Hilbert–Schmidt trace expansion (first unfinished part of Package 4)** | `LargeValuesMatrix.lean` gives the exact Gram diagonal, while `LargeValuesPoisson.lean` gives the actual cutoff kernel, its `t = 0` Schwartz decay, exact dilation, Poisson identity, and finite integer support. This step does not require the still-open uniform two-parameter estimate. | Prove the exact Hilbert–Schmidt/first-trace Poisson expansion, isolate the zero Fourier mode, and bound the full nonzero-frequency tail with a constant uniform in the scale. Register the theorem in `Audit.lean`. |
| 2 | 18 | **Appendix C contour coverage** | Source-range interfaces, exact initial Möbius cancellation, arbitrary-order Gamma decay, contour integrability, local zero multiplicity, and the downstream fourth-moment reduction are complete. | Prove `typeIContourTypeIICoverOn_native` at `7/10`, including the smoothed series, removable cancellation, rectangle shift, residue, and quantitative tails. |
| 3 | 18 | **Smooth `ζ²` approximate functional equation and diagonal bound** | The exact `M²` expansion, support, coefficient bound, admissible length, and target integral are already fixed. This node is independent of the DFI off-diagonal theorem. | Close the `AF` node with a reusable smooth `ζ²` AFE and its full diagonal estimate; do not claim the twisted fourth moment before the separate off-diagonal node is proved. |
| 4 | 19 | **Package 6: smooth reflection** | The cutoff regularity, affine language, Mellin/Fourier infrastructure, and finite coefficient machinery are present. | Prove Guth–Maynard Lemma 6.2 with uniform truncation and tail bounds and the resulting short polynomial; `S₂` remains downstream of Heath–Brown. |
| 5 | 15 | **Medium Type-I reflection** | `MediumReflection.lean` already contains the exact Poisson formula, stationary point and phase normalization, dyadic frequency window, and two-sided quadratic control. All other #15 finite certificates and the terminal branch are complete. | Prove the uniform stationary main term, dual-block recovery, and ordinate-independent coefficient reduction. This is the highest-payoff closeout because it immediately unlocks the finite reduction and endpoint assembly, but it is harder than ranks 1–4. |
...
Shitlist #15 is one acceptance item with four explicit implementation packages and one final assembly gate. This is the selected path because it targets finite Dirichlet-polynomial and oscillatory-sum statements already close to the project's APIs. It does not require completing the entire classical Littlewood–Gabriel–fourth-moment contour stack.
...
3. **Finite detector, normalization, powering, and dichotomy — complete:** `ClassicalDetector.lean` proves the finite zeta–Möbius cancellation and raw tail witness. `FiniteDensityTransfer.lean` realizes the tail as a finite Dirichlet polynomial, proves coefficient-mass bounds, extracts a common dyadic block with analytic multiplicity, normalizes its coefficients, and applies unrestricted finite MHH. `ClassicalPowering.lean` and `FiniteDensityEndpoint.lean` prove exact finite power raising, #17 factorization bounds, normalization, common powered-block extraction, and the corresponding MHH cardinality estimate. `ClassicalDichotomy.lean` now proves the finite ANTEDB-Lemma-11.5 detector split at `Y = ⌊T^δ₁⌋`: either the complementary interval `(Y,A]` is a large ordinary-zeta Type-I tail, or the short sum through `Y`, multiplied by the Möbius polynomial through `X = ⌊T^(δ₂/2)⌋`, yields a large genuinely short Type-II tail supported in `(X,X*Y]`. The public native theorem includes the exact pointwise split, eventual thresholds, branch-specific exact-`σ` localization, dominant analytic-multiplicity branch, and common dyadic scale.
4. **Endpoint-range reduction — arithmetic complete; dependency interpretation corrected:** `FiniteDensityExponents.lean` proves `ingham_zeta_polynomial_range_empty`, `huxley_zeta_polynomial_range_empty`, and `huxley_zeta_polynomial_range_below_weyl`. These theorems settle the exceptional short zeta-polynomial ranges after the detector decomposition; they do not eliminate the medium ordinary-zeta Type-I branch. The obstruction is already visible at `σ = 3/4`, `N = T^(2/3)`, `V = N^(3/4)`: the direct MHH bound has exponent `2/3`, larger than the required `3/5`. `MediumReflection.lean` must therefore use a smooth dyadic partition and the existing Poisson phase geometry to prove a uniform stationary main term, recover a dual block of length comparable to `T/N`, and remove the ordinate-dependent phase by a truncated Mellin/Fourier separation followed by finite pigeonholing. The resulting common coefficients must be independent of the ordinate, with every truncation, separation, and logarithmic loss explicit.
...
7. This package owns the classical transfer required for Ingham and Huxley. It may later be reused by #18, but it does not by itself discharge #18's separately specified contour coverage, fourth-moment reduction, or twisted fourth-moment propositions.
...
8. **Verification and scope boundary:** the focused and root builds are warning-free. All 25 dichotomy declarations and all five terminal declarations are registered in the synchronized 999-declaration transitive audit and use only `propext`, `Classical.choice`, and `Quot.sound`. The certificate leaves the medium Type-I scale unclassified; short-scale estimation and the completed terminal case go directly to the finite reduction.

#### Endpoint assembly and acceptance

1. **Ingham:** set `τ₀ = 2 - σ`. Prove the finite Type-I/Type-II scale alternatives, apply the direct MHH term on short scales, reflect every medium Type-I scale to length comparable to `T/N`, and use the terminal estimate at the upper edge. The endpoint arithmetic must yield `3 * (1 - σ) / (2 - σ)` before combining with the already proved `σ = 1/2` and `σ = 1` cases.
2. **Huxley:** set `τ₀ = 3 * σ - 1`. Use the sixth-power MHH branch on the short Type-II windows and on the reflected medium Type-I windows; reserve the completed Weyl theorem for the exceptional short zeta-polynomial range identified by `FiniteDensityExponents.lean`. At `σ = 3/4`, derive the boundary from Ingham because both exponents equal `3/5`.
...
The already completed sharp and absolute truncation theorems, finite Möbius-product algebra and native detector dichotomy, exact-β removal, common multiplicity extraction, finite normalization and powering, mollifier, Jensen, critical-line second moment, rectangle argument principle, strip growth, finite differencing, multi-period B-process bounds, general-`σ` weighted Weyl bound, classical large-values theorem, endpoint-range arithmetic, finite scale/exponent assembly, terminal prefix cancellation and sharp-block control, exact Poisson/cosine reflection identity, nonstationary expansion, stationary normal form, and two-sided quadratic phase bounds remain valid reusable library work. Weighted Littlewood, Gabriel interpolation, the standalone zeta fourth moment, and right/horizontal contour moments are not mandatory #15 deliverables. A uniform stationary reflection theorem and dual-block recovery **are** mandatory for the selected finite Type-I/Type-II route. Packages A–C and the detector, endpoint-range arithmetic, finite scale/exponent, and terminal nodes of Package D are complete; Package D still requires medium reflection, the finite reduction consuming it and the completed certificates, and the two endpoints.
...
| Downstream work | Concrete Huxley and concrete Ingham remain unproved. Exact-β removal, the full finite Type-I/Type-II dichotomy and short Type-II product, common multiplicity extraction, normalization, generic exact powering, coefficient control, finite MHH application, endpoint range arithmetic, finite scale/exponent assembly, and the terminal Type-I estimate are complete. The selected route still requires completed medium Type-I reflection, the finite reduction consuming it and the completed certificates, and final symmetric-count specialization. |
| Verification | The focused terminal module and root production build pass without warnings. The current audit passes 999/999 dependency checks and all integrity scans but fails the output gate on the two absent #15 endpoints and six other agenda outputs. |
...
1. **Coverage boundary — contract complete, theorem open.** `TypeIContourTypeIICoverOnProp σ₀` now exists, `TypeIContourTypeIICoverProp` is its `σ₀ = 7/10` specialization, and the finite bridge consumes exactly that range. The analytic Appendix C proof of the proposition remains in step 3.
...
3. **Appendix C coverage.** Define the infinite exponentially smoothed detector, prove absolute convergence and the finite Möbius-convolution identity, establish `a(1)=1`, cancellation for `2 ≤ n ≤ M`, the divisor bound, exponential-tail control, and the dyadic decomposition into existing Type-I blocks. Use Mathlib's proved `mellinInv_mellin_eq`, not a new Mellin inversion development. Regularize the product at `s=0` using analytic factorization of zeta at its zero, shift the rectangle, compute the sole simple residue at `s=1-ρ`, send the horizontal edges to infinity, and compare the two detector formulas to prove the source-range coverage theorem.
...
5. **Generic twisted-fourth-moment theorem.** Rewrite `|M(s)ζ(s)|⁴` as `|ζ(s)|⁴ |M(s)²|²`. Prove a reusable smooth upper bound for arbitrary coefficient-controlled Dirichlet polynomials of length at most `T^(1/22)`. Its proof requires a smooth approximate functional equation for `ζ²`, diagonal evaluation, and a specialized Duke–Friedlander–Iwaniec quadratic-divisor theorem for the off-diagonal. Hughes–Young's full six-main-term asymptotic is unnecessary, but its off-diagonal analytic content is not optional. A fixed nonnegative Mathlib bump majorizing `[T/2,3T]` avoids a separate smooth-weight-removal theorem.
...
Acceptance requires native proofs of the two remaining Type-II inputs after the completed coverage-interface repair and native fourth-moment reduction, explicit convergence for every whole-line integral, no theorem-equivalent premise hidden behind a generic wrapper, and an audit-clean one-premise Goal C theorem. Principal references are [Maynard–Pratt Appendix C](https://ora.ox.ac.uk/objects/uuid%3Ab2cc1c2e-6cec-495a-ba6a-61ba08acc723/files/sj38609141), [Mathlib's proved Mellin inversion](https://github.com/leanprover-community/mathlib4/blob/master/Mathlib/Analysis/MellinInversion.lean), [Hughes–Young](https://arxiv.org/abs/0709.2345), the stronger but less suitable [Bettin–Bui–Li–Radziwiłł route](https://arxiv.org/abs/1609.02539), and [PNT+](https://github.com/AlexKontorovich/PrimeNumberTheoremAnd). The remaining difficulty order is contour coverage, then the twisted fourth moment; the latter is expected to be the larger package.

### Source-Faithful Completion Plan for Shitlist #19 — 10 August 2026

**Implementation status, 10 August 2026:** Packages 1–3 are complete. Package 2 includes exact translation invariance, nonzero scaling covariance, affine reindexing, the seven-bin floor partition of tolerance-one energy, and the injection of its zero-defect bin into Mathlib exact additive energy for one-separated sets. Package 3 proves the exact operator-norm conclusions of Guth–Maynard Lemmas 4.1–4.2, including the source cardinality estimate and sixth-root trace-dispersion bound. Package 4 is partially implemented in `LargeValuesPoisson.lean`: the actual `h_t`, its Schwartz realization, endpoint vanishing, Mellin/Fourier zero-mode identity, arbitrary-order uniform zero-mode decay, exact Fourier dilation, source-normalized Poisson formula, and finite-support reduction are proved. Its full two-parameter Lemma 4.3 estimates, trace split, and Proposition 5.1 remain open. Packages 5–14 remain open. The audit and integrity portion of Package 14 passes, but final integration does not because the native large-values and concrete density theorems are absent.
...
4. **Formalize Fourier decay, Poisson, and `S₁` — in progress.** `LargeValuesPoisson.lean` proves the chosen source kernel is Schwartz, its fixed-ordinate arbitrary-frequency decay, the Mellin/Fourier representation and arbitrary-order uniform decay of `ĥ_t(0)`, exact dilation `𝓕(h(·/N))(ξ)=N ĥ(Nξ)`, source-normalized Poisson summation, cutoff endpoint vanishing, and finite integer support. Complete the uniform two-parameter parts of Lemma 4.3, expand the Hilbert–Schmidt and cubic traces as in Lemmas 4.4–4.5, split the nonzero-frequency configurations into `S₁`, `S₂`, and `S₃`, and prove Proposition 5.1 for `S₁`.
...
6. **Prove smooth reflection.** Formalize Guth–Maynard Lemma 6.2 by Mellin inversion of the fixed smooth cutoff: prove Mellin-transform decay, shift/truncate the integral with explicit uniform tails, and identify the resulting short Dirichlet polynomial. Reuse the audited PNT+ Mellin/Fourier stack where its hypotheses match.
...
| Audit impact | The current explicit and discovered theorem sets agree at 999/999. Every beta/localization theorem, `extractSeparated_native`, every classical-moment theorem, the vendored PNT+ proof closure, the native #18 reduction, and the #19 shared-language/matrix/dispersion/Poisson declarations pass with only `propext`, `Classical.choice`, and `Quot.sound`. |
| Principal verification | `logs/overall_proof_20260810_122112.log` passes all four build/example stages warning-free, passes 999/999 dependency checks, and finds no admitted proof, unsafe bypass, or project postulate. The named output gate fails on eight absent agenda theorems, so the overall result remains honestly `FAIL`/`1`. |
...
| Verification | The focused arithmetic module and root production closure build with zero warnings. The current audit synchronizes 999/999 declarations; all five #17 declarations and the new #18/#19 foundations pass with only permitted logical dependencies. |

## Historical Files Ordered by Estimated Repair Difficulty

This section now separates historical cleanup ranks from the files that still carry research obligations. The repository-wide audit read all 72 retained Lean files: 69 are in the `RiemannZeta.lean` import closure, `RiemannZeta/Audit.lean` is the executable audit, and `TestExp.lean` plus `test_separated.lean` are the two retained anonymous examples. No retained Lean file currently needs an integrity repair; the remaining work is theorem completion in the groups below. Historical deletion records remain evidence about earlier states, not descriptions of the present tree.
...
| 6 | ~~`RiemannZeta/Audit.lean`~~ | **Completed for all named public theorems in the current tree:** the explicit and discovered sets synchronize at 999/999, every existing theorem has only the permitted logical dependencies, and the separate completion gate reports eight absent output names. | Easy–moderate—complete |
| 7 | ~~`RiemannZeta.lean`~~ | **Completed and maintained:** the root reaches all 68 subordinate production modules, including the six vendored PNT+ files and the current #15/#18/#19 modules. The deleted false `Decoupling` module is not imported. | Easy—complete |
...
| 12 | ~~`RiemannZeta/GuthMaynard/HalaszMontgomery.lean`, `ZeroDetector.lean`, and `TypeIIZeros.lean`~~ | **Conditional/concrete layer completed:** coverage is eventual in height and source-range restricted; the generic count is identified with the concrete analytic-multiplicity residual count; and the concrete `ResidualZeroBoundProp` follows from three explicit analytic inputs. #18 has proved the fourth-moment reduction input; coverage and the twisted fourth moment remain Goal C work. | Very hard—conditional/concrete layer complete |
...
| 15 | `MediumReflection.lean`, followed by `FiniteDensityTransfer.lean`, `FiniteDensityEndpoint.lean`, and `InghamBound.lean` | Complete the stationary main term, dual-block recovery, common-coefficient reduction, finite endpoint assembly, and both native endpoint theorems. The other current #15 modules are verified prerequisites, not repair targets. | Very hard—open |
...
| 18 | `TypeIICoverage.lean`, `TypeIIContour.lean`, `TwistedMoment.lean`, `TypeIIZeros.lean`, and new narrowly scoped AFE/DFI modules | Prove native Appendix C coverage, the smooth `ζ²` approximate functional equation and diagonal estimate, the genuine quadratic-divisor/off-diagonal theorem, the twisted fourth moment, and the one-premise Goal C theorem. | Extreme—open |
| 19 | `LargeValuesPoisson.lean`, `LargeValues.lean`, the proved matrix/language prerequisites, and new source-section modules | The next repair is the Hilbert–Schmidt trace expansion. It is followed by uniform two-parameter Fourier decay, cubic trace split, `S₁/S₂/S₃`, reflection, Heath–Brown, affine/energy/GCD theorems, native large values, and concrete transfers. | Hardest overall; first trace theorem is the easiest current proof—open |
...
|---:|---|---|---|
| 1 | `RiemannZeta/GuthMaynard/LargeValuesPoisson.lean` (with a small bridge to `LargeValuesMatrix.lean`) | Hilbert–Schmidt/first-trace Poisson expansion with zero-mode main term and a uniform nonzero-mode tail. | Most accessible: all needed objects and fixed-`t` decay are already kernel-checked. |
| 2 | `RiemannZeta/GuthMaynard/TypeIICoverage.lean`, `TypeIIContour.lean` | `typeIContourTypeIICoverOn_native`. | Hard: substantial contour shifting, but the Gamma, integrability, cancellation, and multiplicity layers exist. |
| 3 | `RiemannZeta/GuthMaynard/TwistedMoment.lean` plus a new AFE module | Smooth `ζ²` approximate functional equation and complete diagonal estimate. | Hard–extreme: self-contained before the separate DFI off-diagonal theorem. |
| 4 | `RiemannZeta/GuthMaynard/MediumReflection.lean` | Uniform stationary main term, dual block of scale `T/N`, and ordinate-independent coefficient recovery. | Very hard, but it has the highest immediate #15 payoff. |
| 5 | `RiemannZeta/GuthMaynard/LargeValuesPoisson.lean` and subsequent new #19 modules | Uniform two-parameter Lemma 4.3, cubic trace split, and Proposition 5.1 for `S₁`. | Very hard; no longer mislabeled as one small closeout. |
| 6 | New #19 Heath–Brown, affine, energy, and GCD/spacing modules, then `LargeValues.lean` | Remaining `S₂/S₃` chain and `guthMaynardLargeValues_native`. | Hardest. |
...
1. **Completed for the production perimeter and kept synchronized through 10 August 2026:** created an explicit, synchronization-checked list, currently containing 999 exported source-level theorems.
...
4. **#15 second pass:** complete the smooth medium reflection, finite certificate-consuming reduction, and concrete Ingham/Huxley inputs. The terminal estimate, finite Type-I/Type-II dichotomy, and genuinely short Type-II construction are already complete and must not be relisted as open work. Do not cross out #15 before both endpoint theorems exist.
...
3. Prove `S₁`, the Section 6 smooth reflection theorem, the Heath–Brown difference-set mean-square input, `S₂`, the affine-transformation estimates, `S₃`, and the GCD/spacing energy assembly.
...
| Honest analytic boundary | The coverage boundary is repaired to `TypeIContourTypeIICoverOnProp (7/10)`, and `TypeIIFourthMomentReductionProp` is discharged by the native theorem. `TypeIContourTypeIICoverProp` and `TwistedZetaFourthMomentProp` remain unproved proposition specifications. The conditional bridge still exposes those two inputs until their native theorems exist. |
...
| Coverage | At that iteration the tree contained 44 Lean files and a 41-file production graph. The current superseding inventory is 72 Lean files: a 69-file root closure, `Audit.lean`, and two separately executed examples. No retained Lean file is outside the human-facing evaluation. |
...
| Remaining obstruction at that iteration | #8 had no remaining warning or retained-file coverage defect. The superseding 10 August audit has 999/999 passes and zero direct project axioms. |
...
| F-03 Type I/Type II | **Complete conditionally.** The exact partition is assembled and the residual bound is derived internally. | The interface exposes three source-facing Type-II inputs. `typeIIFourthMomentReduction_native` discharges one; contour coverage and the twisted fourth moment remain open. |
...
| Honest remaining boundary | #14 is **complete conditionally, not unconditionally**. Its signature retains ten primitive inputs, but native theorems discharge six; the four unresolved inputs are Guth–Maynard large values, Huxley, Type-II contour coverage, and the twisted fourth moment. `TypeIPositiveSlabBoundProp` is a derived internal intermediate, and `DyadicToGlobalZeroCountProp` has been deleted. |
...
1. **#15 — Classical finite zero-density endpoints (in progress).** The zero-count foundation, unit-zero theorem, Montgomery mean value, basic Halász–Montgomery, mollifier/rectangle infrastructure, strip growth, coefficient bounds, symmetric-count bridge, detector-strength sharp truncation, exact-β removal, the native finite Type-I/Type-II dichotomy and genuinely short Type-II product, common multiplicity extraction, normalized finite powering, pointwise/general-weight van der Corput–Weyl stack, Package-C Montgomery–Halász–Huxley estimate including `T * N^4 * V⁻⁶`, endpoint range arithmetic, finite scale/exponent assembly, and terminal Type-I sharp-block estimate are proved. The remaining critical path is medium Type-I reflection, the finite reduction consuming it and the completed assembly certificates, and the two concrete endpoint theorems. This classical large-values package is distinct from #19's `GuthMaynardLargeValues`, which is the sole remaining premise of the Central Type I estimate after applying the native mean-value, extraction, and coefficient theorems.
2. **#18 — Type II and Goal C integration.** The coverage boundary, uniform vertical Gamma decay, contour integrability, scaled extraction, and concrete fourth-moment reduction are complete. Prove source-range Maynard–Pratt coverage and a generic Hughes–Young-range twisted moment through the required quadratic-divisor theorem; specialize it to `M²`, then reduce the public zero-density theorem to the single premise `GuthMaynardLargeValues`.
3. **#19 — Large values and final integration.** The false decoupling contracts are deleted; the source definitions, sampling matrix, trace-dispersion algebra, and initial Fourier/Poisson foundations are proved. Next prove the Hilbert–Schmidt trace expansion, then the uniform two-parameter/cubic trace and `S₁` layer, reflection, Heath–Brown, affine-transformation, approximate-energy, `S₂/S₃`, and GCD/spacing chain; prove `GuthMaynardLargeValues`; derive the concrete Guth–Maynard and combined zero-density theorems; and make the principal runner pass.
