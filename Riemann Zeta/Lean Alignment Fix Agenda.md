...
| `Audit.lean` checks transitive dependencies for the synchronized production theorem set. | #6 | **Complete for the cleaned tree's named public theorems:** the current list is 1069/1069, including the complete #19 cubic-trace, `S₁`, uniform Fourier-decay, and smooth-reflection surfaces. Every declaration passes with only the permitted logical dependencies. |
...
| The paper stated stale audit counts. | Documentation gate | **Resolved during #8 and kept synchronized:** the current audit passes across 1069 synchronized declarations. |

Current clean facts: the production graph and both retained examples elaborate with zero warnings; the dependency audit synchronizes and passes 1069 declarations; and all prohibited-proof and project-postulate scans are empty. The named output gate reports seven absent end theorems after accepting native #18 coverage and reduction; the overall result remains honestly `FAIL`/`1` until the remaining outputs exist.
...
| 6 | **Verified complete as audit infrastructure** | The explicit and discovered theorem sets agree at 1069/1069 and all transitive dependency checks pass. The separate completion gate fails on the seven absent #15/#18/#19 outputs. |
...
|---:|---|---|---|
| 15 | **Classical finite zero-density engine and endpoints — in progress** | Existing work proves exact-β removal for the sharp finite Möbius product, the native multiplicity-dominant Type-I/Type-II dichotomy and genuinely short Type-II product, common dyadic-witness extraction, bounded-coefficient normalization, exact finite power raising, the `X=1` ordinary sharp-zeta powered MHH theorem, #17 coefficient growth, weighted Weyl bounds, endpoint arithmetic, scale/exponent certificates, and terminal Type-I control. The remaining route is split into discrete interfaces: actual-`X` Type-II powering/MHH; Type-I cutoff removal/smoothing and scale classification; medium-only B-process; shared Fourier deweighting; Type-I range resolution; a branch-to-slab density reduction consuming the dichotomy and endpoint certificates; and the two endpoints. | The item stays uncrossed until audit-clean theorems prove both `HuxleyZeroDensity (fun σ T => N σ T)` and `InghamZeroDensity (fun σ T => N σ T)`. |
...
| 19 | **Guth–Maynard large values and final project integration — in progress** | **Completed:** deletion of the false model; cutoff/derivative, matrix action, exact Gram kernel, positive spectrum, singular-value powers, trace-cube, sixth-dispersion, core approximate-energy, basic `R`, Fourier/Poisson foundations, the complete Hilbert–Schmidt first-trace expansion, arbitrary-order uniform two-parameter Fourier decay, the exact Lemma 4.5 cubic Poisson expansion and `S₁/S₂/S₃` partition, Proposition 5.1 with `S₁ = Oε(T⁻¹⁰)`, and the native signed smooth-reflection theorem. **Remaining:** native Heath–Brown, `S₂`, moment/energy bridge, `S₃` localization and refinement, affine and GCD/spacing theorems, Sections 3/12 assembly, and concrete transfers. | The concrete Guth–Maynard and combined zero-density theorems are project-theorem complete. Existing dependency and integrity gates pass, while the named output gate keeps the principal runner at `FAIL`/`1` until the end theorems exist. |
...
|---:|---:|---|---|---|
| 1 | 15 | **Actual-`X` Type-II powered MHH application** | The current endpoint theorem already proves the complete extraction/MHH pattern for `X=1`; the actual dichotomy provides the general `X`, and #17 provides its subpolynomial coefficient control. | Generalize the finite theorem without an analytic premise and apply it to `X = ⌊T^(δ₂/2)⌋`. This closes `IIE` only. |
| 2 | 15 | **Type-I cutoff removal, smoothing, and scale split** | The dichotomy supplies the actual sharp Type-I witness and `TerminalTypeI.lean` forces a large witness below its ordinate. | Prove cutoff inactivity, common smooth-block extraction, and the short/medium split. This closes `PS` only. |
| 3 | 18 | **Smooth `ζ²` approximate functional equation and diagonal bound** | The exact `M²` expansion, support, coefficient bound, admissible length, target integral, and full Appendix C coverage are fixed. | Close `AF`; keep the DFI off-diagonal theorem separate. |
| 4 | 15 | **Medium Type-I B-process** | `MediumReflection.lean` contains the Poisson formula, stationary geometry, frequency window, quadratic control, and exact rescaling. | For the medium output of `PS`, prove the uniform `x^{-σ-it}` stationary formula and weighted dual block at scale `T/N`. Shared deweighting remains downstream. |

Completed closeout: **#19 Package 6 smooth reflection.** `LargeValuesReflection.lean` proves `gmSmoothReflection_native` and `norm_gmSmoothReflection_native_le`; `S₂` remains downstream of the open Heath–Brown theorem.
...
3. **Finite detector, normalization, powering infrastructure, and dichotomy — complete within their exact signatures:** `ClassicalDetector.lean` proves finite zeta–Möbius cancellation and the raw tail witness. `FiniteDensityTransfer.lean` proves coefficient-mass bounds, common dyadic extraction, normalization, and unrestricted finite MHH. `ClassicalPowering.lean` proves exact finite powering and unit-bounded coefficient control. `FiniteDensityEndpoint.lean` proves `powered_sharp_block_large_values_bound`, but its coefficient is hard-coded to `sharpMollifiedLineCoeff A 1`; at `X=1` this is the ordinary sharp-zeta block. It does **not** yet apply to the actual Type-II cutoff `X = ⌊T^(δ₂/2)⌋` returned by `ClassicalDichotomy.lean`. The dichotomy itself is complete and returns the source Type-I/Type-II alternatives with analytic multiplicity.
...
8. **Type-I smoothing and scale split — open, next finite interface:** from the actual Type-I alternative and `typeI_scale_lt_height_of_large`, prove that the sharp coefficient cutoff is inactive on the retained nonterminal block, replace the sharp dyadic indicator by one of finitely many source-faithful smooth blocks on a common weighted subset, and divide into `N ≲ √T` and `√T ≪ N < T`. This theorem must preserve the original analytic-multiplicity accounting and record the smoothing/pigeonhole loss.
9. **Medium B-process — open, medium branch only:** for the medium smooth block, generalize the existing phase geometry to the actual amplitude `x^(-σ-it)`, prove a uniform stationary formula with quantitative nonstationary error, and recover a weighted dual block supported at `M ≍ T/N`. `MediumReflection.lean` owns this theorem. It does not own the next Fourier deweighting step.
10. **Shared Type-I Fourier deweighting — open:** both the original short smooth block and the reflected dual smooth block must feed one theorem that removes the smooth/real-part weight by a finite logarithmic Fourier or Mellin decomposition. It must return one ordinary zeta polynomial with coefficients independent of the sampled ordinate on a common separated subset. This is the missing source step represented by ANTEDB Lemma 11.5 after equations (11.3) and (11.4); it is not presently a downstream theorem in the repository.
11. **Actual Type-II powered MHH application — open, finite glue:** generalize the `X=1` theorem to `sharpMollifiedLineCoeff Y X σ` with the dichotomy's genuine `X`, use #17 to absorb its divisor-size base coefficients before powering, and apply the endpoint certificate. No new analytic estimate should be postulated.
12. **Type-I range resolution and branch-to-slab reduction — open:** apply the completed MHH/powering or Weyl result according to the `EndpointScaleCertificate`, combine it with the actual-`X` Type-II theorem, and convert the dichotomy's witness-set bound back to a positive-slab analytic-multiplicity zero count. This theorem must be the first actual consumer of both `ClassicalTypeITypeIIDichotomyConclusion` and `EndpointScaleCertificate`.
13. This package owns the classical transfer required for Ingham and Huxley. It may later be reused by #18, but it does not by itself discharge #18's separately specified contour coverage, fourth-moment reduction, or twisted fourth-moment propositions.
...
8. **Verification and scope boundary:** the focused and root builds are warning-free. All 25 dichotomy declarations and all five terminal declarations are registered in the synchronized 1069-declaration transitive audit and use only `propext`, `Classical.choice`, and `Quot.sound`. Repository-wide symbol tracing shows that no theorem outside `ClassicalDichotomy.lean` currently consumes `ClassicalTypeITypeIIDichotomyConclusion`, and no density theorem consumes `EndpointScaleCertificate`. The former diagram edge sending “short branches” directly to the finite reduction therefore represented planned mathematics, not compiled dependency. Short Type I still needs smoothing and shared Fourier deweighting; terminal control only proves the scale restriction needed before that split.

#### Endpoint assembly and acceptance

1. **Ingham:** set `τ₀ = 2 - σ`. Send Type II through the actual-`X` powered MHH theorem still to be proved. For Type I, use terminal control, smooth the retained block, send the short case directly and the medium case through B-process reflection, then send both through shared deweighting and range resolution. The arithmetic must yield `3 * (1 - σ) / (2 - σ)` before combining with the proved boundary cases.
2. **Huxley:** set `τ₀ = 3 * σ - 1`. Use the sixth-power MHH branch on the Type-II windows and on the deweighted short/reflected Type-I windows; reserve the completed Weyl theorem for the exceptional short zeta-polynomial range identified by `FiniteDensityExponents.lean`. At `σ = 3/4`, derive the boundary from Ingham because both exponents equal `3/5`.
...
The completed truncation, detector dichotomy, exact-β removal, extraction, normalization/powering infrastructure, MHH/Weyl estimates, endpoint arithmetic, terminal control, and reflection geometry remain valid. Weighted Littlewood, Gabriel interpolation, the standalone zeta fourth moment, and contour-edge moments are not mandatory. The mandatory open path is explicit: actual-`X` Type-II powered MHH; Type-I smoothing/scale split; medium B-process; shared deweighting; Type-I resolution; branch-to-slab reduction; and endpoints. Completing the B-process alone would not close the short-Type-I or Type-II interfaces or make the certificates consume themselves.
...
| Downstream work | Concrete Huxley and Ingham remain unproved. Exact-β removal, the dichotomy, common extraction, normalization, exact powering, the `X=1` ordinary sharp-zeta powered theorem, coefficient growth, endpoint arithmetic, and terminal restriction are complete. The actual-`X` Type-II application, Type-I smoothing/scale split, medium B-process, shared deweighting, Type-I range resolution, branch-to-slab consumer, and symmetric specialization remain. |
| Verification | The focused terminal module and root production build pass without warnings. The current audit passes 1069/1069 dependency checks and all integrity scans but fails the output gate on the two absent #15 endpoints and five other agenda outputs. |
...
2. ~~**#19 smooth reflection.**~~ **Closed 10 August 2026.** `LargeValuesReflection.lean` proves arbitrary-order uniform two-parameter decay, explicit `T⁻¹⁰⁰` far-frequency control, exact cutoff Mellin inversion and Fubini transfer to the common interval, a fully proved first/second-derivative bound `10 / √τ`, signed-mode conjugation, the exact finite positive and negative reflected sums, and the aggregate norm inequality. The public closeout is `gmSmoothReflection_native`; the later cubic trace and `S₁` package is now also closed independently in `LargeValuesCubic.lean`.
3. **#15 Type-I interface chain.** First prove a finite `typeISmoothScaleSplit_native` theorem from the actual dichotomy witness and terminal bound: remove the inactive sharp cutoff, select one smooth block on a common subset, and split short from medium scales. Next prove `mediumTypeIBProcess_native` only for the medium smooth block, using `partial_zeta_block_poisson`, the existing stationary rescaling, and quantitative nonstationary control to recover a weighted dual block at scale `T/N`. Finally prove a shared `typeIFourierDeweight_native` theorem that accepts either a short original smooth block or a reflected dual block and returns one fixed-coefficient ordinary zeta polynomial on a common separated subset. These names describe planned theorem boundaries, not existing declarations.
...
**Implementation result.** `reflectionOscillatoryIntegral_stationary_rescale` proves #15's exact interval-integral change to stationary coordinates. The completed #19 theorem `norm_gmReflectionIntegral_le_ten_div_sqrt` can support #15's stationary estimate after the sigma-weighted kernel generalization, but it does not reflect the actual Type-I block. No proof-status change is claimed for `PS`, `MR`, `FD`, `ZR`, or `FR`. `TwistedDiagonal.lean` proves #18's finite `hm = kn` phase cancellation, exact interval integral, coefficient-sum evaluation, and norm bound. `LargeValuesReflection.lean` completes #19 smooth reflection. `LargeValuesCubic.lean` completes Lemma 4.5 and Proposition 5.1.

### Source-Faithful Completion Plan for Shitlist #19 — 10 August 2026

**Implementation status, 10 August 2026:** Packages 1–4 and 6 are complete. Package 2 includes exact translation invariance, nonzero scaling covariance, affine reindexing, the seven-bin floor partition of tolerance-one energy, and the injection of its zero-defect bin into Mathlib exact additive energy for one-separated sets. Package 3 proves the exact operator-norm conclusions of Guth–Maynard Lemmas 4.1–4.2. Package 4 is closed by `LargeValuesCubic.lean`: it proves the complete Hilbert–Schmidt first-trace layer, exact cubic Poisson expansion, diagonal main term with `Oε(T⁻¹⁰⁰)` remainder, exact `S₁/S₂/S₃` split, and Proposition 5.1 in its epsilon-separated `Oε(T⁻¹⁰)` form. Package 6 is closed by `LargeValuesReflection.lean`. Packages 5 and 7–14 remain open. The audit and integrity portion of Package 14 passes, but final integration does not because the native large-values and concrete density theorems are absent.
...
| Audit impact | The current explicit and discovered theorem sets agree at 1069/1069. Every beta/localization theorem, `extractSeparated_native`, every classical-moment theorem, the vendored PNT+ proof closure, native #18 coverage and reduction, and the #15/#18/#19 closeout foundations pass with only `propext`, `Classical.choice`, and `Quot.sound`. |
| Principal verification | The current runner passes all build/example stages warning-free, passes 1069/1069 dependency checks, and finds no admitted proof, unsafe bypass, or project postulate. The named output gate fails on seven absent agenda theorems, so the overall result remains honestly `FAIL`/`1`. |
...
| Verification | The focused arithmetic module and root production closure build with zero warnings. The current audit synchronizes 1069/1069 declarations; all five #17 declarations and the #15/#18/#19 foundations pass with only permitted logical dependencies. |
...
| 6 | ~~`RiemannZeta/Audit.lean`~~ | **Completed for all named public theorems in the current tree:** the explicit and discovered sets synchronize at 1069/1069, every existing theorem has only the permitted logical dependencies, and the separate completion gate reports seven absent output names. | Easy–moderate—complete |
...
| 15 | `ClassicalDichotomy.lean`, `TerminalTypeI.lean`, `MediumReflection.lean`, `FiniteDensityTransfer.lean`, `FiniteDensityEndpoint.lean`, `FiniteScaleAssembly.lean`, `InghamBound.lean`, plus narrowly scoped Type-I smoothing/deweighting modules | The dichotomy, terminal restriction, `X=1` powered ordinary-zeta theorem, and endpoint arithmetic are proved but disconnected. Add the actual-`X` Type-II theorem, Type-I smoothing/scale interface, medium B-process, shared deweighting, Type-I resolution, branch-to-slab consumer, and endpoints. | Very hard—open; medium reflection is one interior dependency |
...
| 19 | `LargeValuesPoisson.lean`, `LargeValuesCubic.lean`, `LargeValuesReflection.lean`, `LargeValues.lean`, and the proved matrix/language prerequisites | The first-trace expansion, exact cubic split, `S₁` estimate, uniform Fourier decay, and complete signed smooth-reflection package are kernel-checked. The remaining repairs are Heath–Brown, `S₂`, energy/localization/refined `S₃`, affine/GCD theorems, native large values, and concrete transfers. | Hardest overall; trace/Poisson, `S₁`, and smooth reflection complete, later source packages open |
...
| 2 | `FiniteDensityEndpoint.lean`, `ClassicalDichotomy.lean`, and #17 coefficient lemmas | Generalize the powered MHH application from `X=1` to the dichotomy's actual Type-II cutoff. | Moderate–hard finite glue; most accessible #15 closeout. |
| 3 | `ClassicalDichotomy.lean`, `TerminalTypeI.lean`, plus a narrowly scoped Type-I smoothing module | Prove cutoff inactivity, common smoothing, and the short/medium split. | Hard; closes only `PS`. |
| 4 | `RiemannZeta/GuthMaynard/MediumReflection.lean` | Uniform sigma-weighted stationary formula and weighted dual block at scale `T/N`. | Very hard; closes `MR` only. |
| 5 | A Type-I deweighting module, then `FiniteDensityEndpoint.lean` and a finite density consumer | Fixed-coefficient deweighting, Type-I resolution, and slab assembly. | Very hard; `FD → ZR → FR`. |
...
1. **Completed for the production perimeter and kept synchronized through 10 August 2026:** created an explicit, synchronization-checked list, currently containing 1069 exported source-level theorems.
...
4. **#15 second pass:** complete the actual-`X` Type-II powered MHH application, Type-I smoothing/scale split, medium B-process, shared deweighting, Type-I resolution, branch-to-slab consumer, and endpoints. The terminal estimate, dichotomy, short Type-II construction, `X=1` powered theorem, and endpoint arithmetic are complete. Do not cross out #15 before both endpoint theorems exist.
...
3. With the cubic trace split, `S₁`, and smooth reflection complete, prove the Heath–Brown difference-set mean-square input, `S₂`, the affine-transformation estimates, `S₃`, and the GCD/spacing energy assembly.
...
| Remaining obstruction at that iteration | #8 had no remaining warning or retained-file coverage defect. The superseding 10 August audit has 1069/1069 passes and zero direct project axioms. |
...
1. **#15 — Classical finite zero-density endpoints (in progress).** The dichotomy, common extraction, exact powering infrastructure, `X=1` ordinary sharp-zeta MHH theorem, coefficient growth, MHH/Weyl estimates, endpoint arithmetic, and terminal Type-I estimate are proved. The actual Type-II branch still needs `IIE`, because `powered_sharp_block_large_values_bound` is hard-coded to `X=1` while the dichotomy uses `X=⌊T^(δ₂/2)⌋`. The Type-I path is `PS → (short or MR) → FD → ZR`; both join at `FR → ZD`. This classical package is distinct from #19's `GuthMaynardLargeValues`.
2. **#18 — Type II and Goal C integration.** Source-range Maynard–Pratt coverage, uniform vertical Gamma decay, contour integrability, scaled extraction, and the concrete fourth-moment reduction are complete. Prove the smooth `ζ²` AFE/weighted diagonal and genuine quadratic-divisor off-diagonal theorem, assemble the generic Hughes–Young-range twisted moment, specialize it to `M²`, then reduce the public zero-density theorem to the single premise `GuthMaynardLargeValues` after #15 supplies Huxley.
3. **#19 — Large values and final integration.** The false decoupling contracts are deleted; the source definitions, sampling matrix, trace-dispersion algebra, Fourier/Poisson foundations, first- and cubic-trace expansions, Proposition 5.1 for `S₁`, uniform two-parameter Fourier decay, and smooth reflection are proved. Next prove Heath–Brown, affine-transformation, approximate-energy, `S₂/S₃`, and the GCD/spacing chain; prove `GuthMaynardLargeValues`; derive the concrete Guth–Maynard and combined zero-density theorems; and make the principal runner pass.
