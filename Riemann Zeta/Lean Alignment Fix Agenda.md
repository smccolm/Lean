...
| `Audit.lean` checks transitive dependencies for the synchronized production theorem set. | #6 | **Complete for the cleaned tree's named public theorems:** the current list is 269/269; the only auxiliary Lean declarations are anonymous examples. The audit correctly remains red on one project-axiom-dependent declaration. |
...
| 6 | **Verified complete as audit infrastructure** | The explicit and discovered theorem sets agree at 269/269 and transitive dependencies are checked. One failure is reported rather than hidden. |
...
|---:|---|---|---|
| 15 | **Zeta zero-count foundations and classical density endpoints — in progress** | The zero-count foundation, pole-safe vertical growth, Jensen local multiplicity theorem, unconditional Montgomery mean value, and the generic positive-slab-to-symmetric-count bridge are kernel-checked. The remaining work has two explicit parts. **Classical density analysis:** prove a multiplicity-aware Littlewood rectangle theorem, finite Gabriel vertical-integral convexity, the critical-line zeta fourth moment and mollifier boundary moments, and the full classical Montgomery–Halász–Huxley large-values estimate containing the `T * N^4 * V⁻⁶` branch. **Endpoint assembly:** derive Ingham and Huxley on positive ordinates, perform the exact exponent arithmetic and endpoint cases, and specialize the existing bridge to the project's symmetric multiplicity-weighted `N`. | No project axiom remains in the zero-count layer; `zeta_growth_bound_native`, `zeta_lower_bound_native`, `localZeroMultiplicityBound_native`, and `montgomery_mean_value_native` pass the transitive audit. The item remains uncrossed until audit-clean theorems prove both `HuxleyZeroDensity (fun σ T => N σ T)` and `InghamZeroDensity (fun σ T => N σ T)`. The basic `halasz_montgomery_lemma_native` alone does not satisfy this test. |
...
2. **Mollifier algebra:** define the finite Möbius mollifier `M_X`, the error `f_X = ζ M_X - 1`, and `h_X = 1 - f_X^2`; prove analyticity on the required pole-safe rectangles, conjugation, exact finite coefficient expansions, and inclusion of every zeta zero in the zero divisor of `h_X` with at least the same analytic multiplicity.
...
6. **Mollifier boundary moments:** prove the mollifier mean square, right-edge mollified-error moment, horizontal-edge bounds, and absorption of all logarithmic losses into `T^ε`.
...
1. **Ingham assembly:** apply Littlewood to `h_X`, insert Gabriel interpolation and the fourth-moment/mollifier estimates, choose the mollifier scale, absorb logarithms, and prove the exponent `3 * (1 - σ) / (2 - σ)` on positive ordinates. Prove the `σ = 1/2` and `σ = 1` boundary cases explicitly, then specialize the symmetric-count bridge.
...
4. **Huxley assembly:** apply the full classical large-values theorem, select the `N^4 * V⁻⁶` branch on `3/4 ≤ σ ≤ 1`, optimize the polynomial length, and prove the exponent `3 * (1 - σ) / (3 * σ - 1)`, including the `σ = 3/4` and `σ = 1` boundary cases. Then specialize the symmetric-count bridge.
...
The foundation and mean-value subpasses have removed the final `ZeroCount.lean` axiom, proved the local-zero theorem, and supplied the unconditional Montgomery mean-value and basic Halász–Montgomery inputs. They have not proved the stronger classical `N^4 * V⁻⁶` large-values branch. The #16 pass has also proved beta removal and unconditional separated extraction. The synchronized audit now covers 269 declarations and has only the unrelated decoupling failure. Completing #15 still requires both Part A analytic engines and both Part B endpoint theorems. The repository remains globally incomplete until #15 and #17–#19 pass their own tests.
...
#### #15 foundation implementation audit — 8 August 2026
...
| Mean-value dependency | `montgomery_mean_value_native` and `halasz_montgomery_lemma_native` are kernel-checked and discharge the classical discrete mean-value premise. |
...
| Verification | The focused mean-value and Halász–Montgomery builds pass with no warnings or tactic diagnostics. The latest principal run `logs/overall_proof_20260808_212117.log` passed stages 1–4 warning-free, synchronized 269/269 declarations, reported only the unrelated decoupling audit failure, and exited `1`. |
...
| Audit impact | The explicit and discovered theorem sets agree at 269/269. Every new beta/localization theorem and `extractSeparated_native` passes with only `propext`, `Classical.choice`, and `Quot.sound`. The only audit failure is the unrelated `l2_decoupling_bound_native`. |
| Principal verification | `logs/overall_proof_20260808_212117.log` records warning-free success for stages 1–4. Stage 5 reports the synchronized 269-theorem audit and the single decoupling failure. The prohibited-proof and unsafe scans pass; the direct-axiom scan finds only the two #19 `Decoupling.lean` axioms. Overall exit code remains honestly `1`. |
...
| 6 | ~~`RiemannZeta/Audit.lean`~~ | **Completed for all named public theorems in the cleaned tree:** transitive checks cover the synchronized 269-theorem list; the only auxiliary declarations are anonymous examples. One current dependency failure is reported honestly. | Easy–moderate—complete |
...
1. **Completed for the production perimeter 8 August 2026:** created an explicit, synchronization-checked list, currently containing 269 exported source-level theorems.
...
1. **#15 — Classical density endpoints (in progress).** The zero-count foundation, unit-zero theorem, Montgomery mean value, and generic symmetric-count bridge are proved. Part A must prove Littlewood/Gabriel, the critical-line fourth moment and mollifier moments, and the full classical Montgomery–Halász–Huxley estimate including `T * N^4 * V⁻⁶`. Part B must prove the concrete Ingham and Huxley endpoints for `N`.
