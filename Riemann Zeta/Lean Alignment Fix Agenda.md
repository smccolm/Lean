...
|---:|---|---|---|
| 15 | **Classical finite zero-density engine and endpoints — in progress** | Existing work already proves zeta growth, local multiplicity, the finite Möbius detector, the critical-line mollifier second moment, the rectangle argument principle, Montgomery mean value, basic Halász–Montgomery, and the symmetric-count bridge; completed #17 supplies coefficient bounds. **Selected completion route:** (A) a uniform Euler–Maclaurin truncation of zeta at its zeros, (B) the targeted van der Corput B/A processes giving the exponent pairs `(1/2,1/2)` and `(1/6,2/3)` plus the Weyl zeta bound, (C) the full Montgomery–Halász–Huxley large-values estimate including `T * N^4 * V⁻⁶`, and (D) a finite, exact-`σ` zero-density-from-large-values transfer. The former weighted-Littlewood/Gabriel/fourth-moment route remains useful optional infrastructure, not a #15 acceptance dependency. | All 437 public production theorems, including the 107-theorem imported rectangle stack, remain in the synchronized transitive audit. The item stays uncrossed until audit-clean theorems prove both `HuxleyZeroDensity (fun σ T => N σ T)` and `InghamZeroDensity (fun σ T => N σ T)`. |
...
#### Package A — Truncated zeta relation

1. Add a focused `ZetaTruncation.lean` module by adapting the Euler–Maclaurin infrastructure in PNT+ rather than reimplementing complex zeta continuation. Prove a uniform finite approximation on the required strip and height range.
2. Specialize at a zeta zero `ρ = β + it` with `|t| ≍ T` to obtain the quantitative truncated relation needed by zero detection, with explicit error small enough for `EpsilonPowerBound`.
3. Keep all interval, floor, branch, and threshold hypotheses explicit. The package is complete only when the zero-specialized theorem is public, warning-free, audited, and has no conclusion-shaped premise.

#### Package B — Targeted van der Corput and Weyl estimates

1. Port only the reusable logarithmic-phase and Euler–Maclaurin lemmas needed from the Lean `expdb` project; do not add it as a blind dependency because its current Lean toolchain differs from this repository's pinned version.
...
The already completed mollifier, Jensen, critical-line second-moment, rectangle-argument-principle, and strip-growth results remain valid reusable library work and an alternative route if the finite large-values route fails. Weighted Littlewood, Gabriel interpolation, the standalone zeta fourth moment, and right/horizontal contour moments are no longer listed as mandatory #15 deliverables. No mathematical status has changed: both endpoint propositions are still absent, the audit still covers 437 declarations, and the repository remains globally incomplete until #15, #18, and #19 pass their tests.

Primary implementation references are the [ANTEDB finite zero-density chapter](https://teorth.github.io/expdb/blueprint/zero-density-chapter.html), the Lean [`expdb` repository](https://github.com/teorth/expdb), PNT+'s [`ZetaBounds.lean`](https://github.com/AlexKontorovich/PrimeNumberTheoremAnd/blob/main/PrimeNumberTheoremAnd/ZetaBounds.lean), the source [Guth–Maynard paper](https://arxiv.org/html/2405.20552v2), and the explicit [Ingham zero-density formulation](https://arxiv.org/abs/2507.15184). These sources guide the implementation; only the pinned Lean build and dependency audit determine completion.
...
| Downstream work | Concrete Huxley and concrete Ingham remain unproved. The selected route now requires the zero-specialized truncated-zeta theorem, targeted van der Corput/Weyl estimates, the full Montgomery–Halász–Huxley `T * N^4 * V⁻⁶` branch, the finite exact-`σ` density transfer, and final assembly for symmetric multiplicity-weighted `N`. The completed contour infrastructure is retained but is not on the critical path. |
...
1. **#15 — Classical finite zero-density endpoints (in progress).** The zero-count foundation, unit-zero theorem, Montgomery mean value, basic Halász–Montgomery, mollifier/rectangle infrastructure, strip growth, coefficient bounds, and symmetric-count bridge are proved. The selected critical path is now the four bounded packages above: truncated zeta at zeros; targeted van der Corput/Weyl estimates; the full Montgomery–Halász–Huxley estimate including `T * N^4 * V⁻⁶`; and the finite exact-`σ` transfer followed by the two concrete endpoint theorems.
