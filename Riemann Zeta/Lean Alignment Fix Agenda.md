...
| `Audit.lean` checks transitive dependencies for the synchronized production theorem set. | #6 | **Complete for the cleaned tree's named public theorems:** the current list is 432/432, including all 107 public theorems in the vendored PNT+ modules; the only auxiliary Lean declarations are anonymous examples. The audit correctly remains red on one project-axiom-dependent declaration. |
...
| 2 | **Verified complete as cleanup** | The retained tree has 36 Lean files, no empty Lean file, and only two non-production anonymous-example files. All listed obsolete tests, tombstones, and side-effect utilities are absent. |
...
| 6 | **Verified complete as audit infrastructure** | The explicit and discovered theorem sets agree at 432/432 and transitive dependencies are checked. One failure is reported rather than hidden. |
| 7 | **Verified complete** | The root import graph reaches all 32 subordinate production modules, giving a 33-file production graph. |
...
## Four Remaining Items: Exhaustive Completion Contract

Shitlist #15 and #17–#19 remain; #16 is complete. They are bounded work packages, not a strict serial order; the dependency graph requires a first and second pass through #15 around #17. Each open item has a discrete mathematical responsibility and an objective completion test. The list is exhaustive: every current direct project axiom, every remaining primitive premise of #14, the Guth–Maynard large-values theorem, final integration, audit synchronization, documentation, and the principal runner are assigned below.
...
|---:|---|---|---|
| 15 | **Zeta zero-count foundations and classical density endpoints — in progress** | `ClassicalDensity.lean` proves the actual finite Möbius mollifier, exact convolution and cancellation, pole-free Ingham detector, multiplicity/Jensen bridge, unit-bin estimate, dyadic `O(T log T)` bound, and three symmetric endpoint cases. `ClassicalMoments.lean` proves the source-strength critical-line mollifier second moment. The Lean-4.30 PNT+ port and `ClassicalArgumentPrinciple.lean` now prove the unweighted multiplicity-aware rectangle argument principle, its pole-free detector specialization, and linear zeta growth on the classical strip. **Still required:** the weighted Littlewood/boundary-perturbation theorem, finite Gabriel convexity, the zeta fourth moment, right-edge mollified-error and horizontal-edge estimates, the full Montgomery–Halász–Huxley `T * N^4 * V⁻⁶` branch, the interior assemblies, and Huxley at `σ = 3/4`. | All 432 public production theorems, including the 107-theorem imported rectangle stack, are in the synchronized transitive audit. The item remains uncrossed until audit-clean theorems prove both `HuxleyZeroDensity (fun σ T => N σ T)` and `InghamZeroDensity (fun σ T => N σ T)`. |
...
| 17 | **Arithmetic coefficient bounds** | Prove `DivisorCountBoundProp` and `FactorizationCountBoundProp` in their current uniform quantified forms, repairing the statements first if source comparison exposes an error. `MontgomeryMeanValue` is already discharged by `montgomery_mean_value_native`. | Both arithmetic propositions have named kernel-checked witnesses; the powered-coefficient downstream theorem specializes without either hypothesis. The already-native mean-value and basic Halász–Montgomery results remain warning-free and audit-clean. |
...
Completing the four open rows is, by definition, completion of the repository's current research agenda: Goals A–D are proved in Lean, the useful Goal E infrastructure is retained and documented, every integrity gate passes, and no untracked mathematical hypothesis remains. Upstream submission to Mathlib is optional follow-on work and is not a blocker for this repository-completion claim.

### Conclusive Two-Part Plan for Shitlist #15

Shitlist #15 is one discrete acceptance item with two diagram nodes and an internal dependency break around #17; it is not an additional Shitlist task. Execute and verify it through the following bounded work packages.

#### Part A — Classical density analysis

1. **Statement and count gate:** audit the rectangle and analytic-multiplicity conventions, sigma ranges, signs, constants, and eventual thresholds. Keep the already proved conjugation and dyadic machinery as the sole bridge from positive ordinates to symmetric `N`.
2. **Mollifier algebra — completed:** `ClassicalDensity.lean` defines `M_X`, `f_X`, `h_X`, and its entire pole-cleared form; proves the exact arithmetic-function/L-series convolution, coefficient cancellation through `X`, analyticity, nontriviality, and multiplicity inheritance; and exposes a Jensen divisor bound for finite zeta-zero families.
3. **Littlewood rectangle theorem — argument-principle core completed:** the ported PNT+ theorem proves that the normalized rectangle integral of `logDeriv f` equals the finite sum of analytic orders, and `ClassicalArgumentPrinciple.lean` specializes it to the pole-free Ingham detector. The finite-order and boundary-nonvanishing hypotheses remain explicit. Still prove the weighted logarithmic Littlewood formula/inequality, horizontal argument control, and a boundary-avoiding perturbation theorem before this package is complete.
4. **Gabriel interpolation:** derive the finite vertical-integral convexity theorem needed to interpolate moments of `f_X` between the critical line and the right boundary. Mathlib complex convexity may be used as a foundation, but a bare three-lines citation is not the required finite-integral theorem.
5. **Critical-line fourth moment:** prove an audit-clean estimate of the form `∫₀ᵀ ‖ζ(1/2 + it)‖^4 dt ≪ T (log T)^4`, or the equivalent `T^(1+ε)` form sufficient for `EpsilonPowerBound`. Reuse the native Dirichlet-polynomial mean-value theorem and consume #17's proved divisor estimates explicitly where the source proof requires them.
6. **Mollifier boundary moments — partially completed:** `ClassicalMoments.lean` proves the exact full-interval continuous mean-square estimate and specializes it to
   `∫₀ᵀ ‖M_X(1/2+it)‖² dt ≤ (T + 2(2π+1)X)(1 + log X)`.
   Still prove the right-edge mollified-error moment, horizontal-edge bounds, and absorption of the combined logarithmic losses into `T^ε`.
7. **Full classical large-values theorem:** prove the Montgomery–Halász–Huxley estimate with both the mean-value branch and the stronger `T * N^4 * V⁻⁶` branch. The existing `halasz_montgomery_lemma_native` proves only the basic `V⁻²` consequence and is a dependency, not completion of this gate.
8. **Part-A completion test:** the Littlewood, Gabriel, fourth-moment, mollifier-moment, and full classical large-values theorems are public, warning-free, and transitive-audit clean. Only then may the `#15 Classical density analysis` diagram node be marked done.

#### Part B — Ingham and Huxley

1. **Ingham assembly:** the symmetric `σ = 1/2` and `σ = 1` boundary cases are completed in `ClassicalDensity.lean`. For `1/2 < σ < 1`, apply Littlewood to `h_X`, insert Gabriel interpolation and the fourth-moment/mollifier estimates, choose the mollifier scale, absorb logarithms, and prove the exponent `3 * (1 - σ) / (2 - σ)` before specializing the symmetric-count bridge.
2. **Ingham output:** prove `ingham_zero_density_native : InghamZeroDensity (fun σ T => N σ T)` without a project axiom or theorem-equivalent premise.
3. **Huxley reduction:** formalize the classical zero-detection reduction to separated large values. Reuse existing separation and polynomial-power infrastructure only where its hypotheses exactly match the classical argument; do not import #18's still-open Type-II propositions into #15.
4. **Huxley assembly:** the symmetric `σ = 1` boundary case is completed in `ClassicalDensity.lean`. Apply the full classical large-values theorem, select the `N^4 * V⁻⁶` branch on `3/4 ≤ σ < 1`, optimize the polynomial length, and prove the exponent `3 * (1 - σ) / (3 * σ - 1)`, including the `σ = 3/4` boundary. Then specialize the symmetric-count bridge.
5. **Huxley output:** prove `huxley_zero_density_native : HuxleyZeroDensity (fun σ T => N σ T)` without using the Guth–Maynard large-values theorem or #18's Type-II assumptions.
6. **Endpoint package and acceptance:** expose an audited theorem packaging both endpoints, discharge the Huxley premise of `conditionalZeroDensityTransfer` and the Ingham premise of the combined transfer, add every new public theorem to `Audit.lean`, build every affected module with zero warnings, and run `run_lake_build.bat --no-pause`. Only then may the `#15 Ingham and Huxley` node and Shitlist #15 be marked done.

The foundation and mean-value subpasses have removed the final `ZeroCount.lean` axiom, proved the local-zero theorem, supplied the unconditional Montgomery mean-value and basic Halász–Montgomery inputs, and added the mollifier/detector algebra, exact boundary estimates, the critical-line mollifier second moment, the rectangle argument principle, and a linear classical-strip zeta bound. They have not proved the weighted Littlewood identity, boundary perturbation, right-edge detector moment, zeta fourth moment, stronger classical `N^4 * V⁻⁶` large-values branch, or either complete endpoint proposition. The #16 pass has also proved beta removal and unconditional separated extraction. The synchronized audit now covers 432 declarations and has only the unrelated decoupling failure. The repository remains globally incomplete until #15 and #17–#19 pass their own tests.
...
| Downstream work | Concrete Huxley and concrete Ingham remain unproved. Part A now requires the weighted Littlewood/boundary-perturbation extension, Gabriel interpolation, the classical fourth moment, and the full Montgomery–Halász–Huxley estimate, especially its `T * N^4 * V⁻⁶` branch. Part B must assemble the two exact exponents and transfer them to symmetric multiplicity-weighted `N`. |
| Verification | Focused builds of all four vendored modules and `ClassicalArgumentPrinciple.lean`, plus the root production build, pass without warnings. The direct audit synchronizes 432/432 declarations, explicitly checks all 107 public imported theorems, and reports only the unrelated decoupling dependency failure. |
...
| Audit impact | The explicit and discovered theorem sets agree at 432/432. Every beta/localization theorem, `extractSeparated_native`, every classical-moment theorem, all 107 public PNT+ port theorems, and the project argument-principle specialization pass with only `propext`, `Classical.choice`, and `Quot.sound`. The only audit failure is the unrelated `l2_decoupling_bound_native`. |
| Principal verification | `logs/overall_proof_20260809_014149.log` passes stages 1–4 warning-free, synchronizes 432/432 declarations, and reports only the single decoupling audit failure. The integrity scan finds no `sorry`, `admit`, `sorryAx`, or unsafe bypass and exactly the two #19 `Decoupling.lean` axioms. Overall exit code is honestly `1`. |
...
| 6 | ~~`RiemannZeta/Audit.lean`~~ | **Completed for all named public theorems in the cleaned tree:** transitive checks cover the synchronized 432-theorem list, including the globally named public declarations in the vendored modules; the only auxiliary declarations are anonymous examples. One current dependency failure is reported honestly. | Easy–moderate—complete |
...
| 15 | Classical density analysis and the Ingham/Huxley endpoints | See the authoritative four-item completion contract and two-part #15 plan above. | Very hard—open |
| ~~16~~ | ~~Beta removal~~ | Completed by the rational Phragmén–Lindelöf implementation recorded above. | Very hard—complete |
| 17 | Arithmetic coefficient bounds | See the authoritative four-item completion contract above. | Very hard—open |
| 18 | Type-II residual analysis and Goal C integration | See the authoritative four-item completion contract above. | Extreme—open |
| 19 | Large values and final integration | See the authoritative four-item completion contract above. | Hardest—open |
...
1. **Completed for the production perimeter and kept synchronized through 9 August 2026:** created an explicit, synchronization-checked list, currently containing 432 exported source-level theorems.
...
| Coverage | The current tree contains 36 Lean files. The root build covers its 33-file production import graph, the audit is executed explicitly, and the two remaining files are executed explicitly as examples. No retained Lean file is outside the human-facing evaluation. |
...
Exactly four items remain, in dependency order:

1. **#15 — Classical density endpoints (in progress).** The zero-count foundation, unit-zero theorem, Montgomery mean value, critical-line mollifier second moment, unweighted rectangle argument principle, linear zeta strip growth, and generic symmetric-count bridge are proved. Part A must still prove the weighted Littlewood/boundary-perturbation and Gabriel steps, the zeta fourth moment, the right-edge/horizontal detector estimates, and the full classical Montgomery–Halász–Huxley estimate including `T * N^4 * V⁻⁶`. Part B must prove the concrete Ingham and Huxley endpoints for `N`.
2. **#17 — Arithmetic coefficient bounds.** `MontgomeryMeanValue` is already proved; prove the remaining divisor-count and factorization-count propositions.
3. **#18 — Type II and Goal C integration.** Prove the three residual inputs and reduce the public zero-density theorem to the single premise `GuthMaynardLargeValues`.
4. **#19 — Large values and final integration.** Discharge the two decoupling axioms, prove `GuthMaynardLargeValues`, derive the concrete Guth–Maynard and combined zero-density theorems, and make the principal runner pass.

These four tasks are exhaustive. No fifth mathematical cleanup item is deferred beyond them. When #15 and #17–#19 meet their stated completion tests, the overall research agenda is complete.
