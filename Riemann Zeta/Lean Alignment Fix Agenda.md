...
| `Audit.lean` checks transitive dependencies for the synchronized production theorem set. | #6 | **Complete for the cleaned tree's named public theorems:** the current list is 887/887, including the expanded vendored PNT+ sharp-zeta dependency closure, finite-density transfer/powering/dichotomy surface, endpoint and scale arithmetic, medium-reflection API, terminal Type-I estimate, and the new #18 support surface; the only auxiliary Lean declarations are anonymous examples. The audit correctly remains red on one project-axiom-dependent declaration. |
...
| 6 | **Verified complete as audit infrastructure** | The explicit and discovered theorem sets agree at 887/887 and transitive dependencies are checked. One failure is reported rather than hidden. |
...
| 18 | **Type-II residual analysis and Goal C integration — in progress** | Completed: source-range contract repair; arbitrary-order Gamma decay; weighted whole-line Gamma and actual contour-integrand integrability; arbitrary-scale weighted separation and bounded overlap; exact initial Möbius cancellation; exact `M²` convolution, support, eventual `T^(1/22)` length, uniform divisor coefficient bound, and moment identity. Remaining: Appendix C Mellin/rectangle shift, residue and tail comparison proving coverage; the concrete counting reduction; smooth `ζ²` AFE, diagonal estimate, and genuine DFI quadratic-divisor/off-diagonal theorem; generic twisted moment and native `M²` specialization; Goal C assembly after #15 supplies native Huxley. | The source-range coverage theorem and the two Type-II propositions are native, audit-clean theorems, and a public audited theorem derives the concrete full-range zero-density result from `GuthMaynardLargeValues` alone. No other analytic or arithmetic proposition parameter remains. |
...
| Verification | The focused terminal module and root production build pass without warnings. Principal run `logs/overall_proof_20260810_025105.log` passes stages 1–4 warning-free and synchronizes 887/887 declarations; every new dichotomy, terminal, and #18 support theorem passes with only permitted logical dependencies. Its sole audit failure and the two direct-axiom findings are the unrelated, tracked #19 decoupling obligations. |
...
4. **Scaled separated extraction complete; fourth-moment reduction open.** `ScaledSeparated.lean` proves weighted selection for arbitrary positive `G` without losing fiber weights and proves overlap at most one when `G > 2H`. The remaining work is the source specialization `G=(log T)³`, Gamma truncation, Hölder, local analytic multiplicity, and epsilon-power assembly proving `TypeIIFourthMomentReductionProp`.
...
| Audit impact | The current explicit and discovered theorem sets agree at 887/887. Every beta/localization theorem, `extractSeparated_native`, every classical-moment theorem, the complete vendored PNT+ proof closure, the project argument-principle specialization, the #17 arithmetic theorems, the #15 sharp-truncation, finite-transfer, dichotomy, powering, Weyl, kernel, classical-large-values, endpoint-range, finite-scale, medium-reflection, and terminal theorems, and the new #18 support theorems pass with only `propext`, `Classical.choice`, and `Quot.sound`. The only audit failure is the unrelated `l2_decoupling_bound_native`. |
| Principal verification | `logs/overall_proof_20260810_025105.log` passes stages 1–4 warning-free, synchronizes 887/887 declarations, and reports only the single decoupling dependency failure. Its integrity scan finds no prohibited admitted-proof text or unsafe bypass and exactly the two tracked #19 `Decoupling.lean` axioms, so it remains honestly `FAIL`/`1` until #19. |
...
| Verification | The focused arithmetic module and the current root production closure build with zero warnings. The audit synchronizes 887/887 declarations; all five #17 declarations pass with only permitted logical dependencies. The remaining failures are the two pre-existing #19 decoupling axioms and their single transitive audit failure. |
...
1. **Completed for the production perimeter and kept synchronized through 10 August 2026:** created an explicit, synchronization-checked list, currently containing 887 exported source-level theorems.
