...
| `Audit.lean` checks transitive dependencies for the synchronized production theorem set. | #6 | **Complete for the cleaned tree's named public theorems:** the current list is 522/522, including all 107 public theorems in the vendored PNT+ modules; the only auxiliary Lean declarations are anonymous examples. The audit correctly remains red on one project-axiom-dependent declaration. |
...
| 6 | **Verified complete as audit infrastructure** | The explicit and discovered theorem sets agree at 522/522 and transitive dependencies are checked. One failure is reported rather than hidden. |
...
|---:|---|---|---|
| 15 | **Classical finite zero-density engine and endpoints — in progress** | Existing work proves zeta growth, local multiplicity, the finite Möbius detector, the critical-line mollifier second moment, the rectangle argument principle, Montgomery mean value, basic Halász–Montgomery, the symmetric-count bridge, and #17 coefficient bounds. **Packages A and B are complete.** Package B proves the multi-period `(1/2,1/2)` B-process, the corrected uniform-correlation A-process, the floor-selected `(1/6,2/3)` Weyl estimate with uniform prefixes, and the exact critical-line weighted Dirichlet block bound. Packages C and D remain open. | All 522 public production theorems, including the 107-theorem imported rectangle stack, are synchronized in the transitive audit. The item stays uncrossed until audit-clean theorems prove both `HuxleyZeroDensity (fun σ T => N σ T)` and `InghamZeroDensity (fun σ T => N σ T)`. |
...
4. `WeylZeta.lean` proves finite Abel summation for antitone weights, identifies the weighted block exactly with `Σ (A+n)^(-1/2-iY³)`, and proves its norm is at most `30 √Y = 30 t^(1/6)` throughout the medium range. The smooth reflection principle in Guth–Maynard Lemma 6.2 is a separate #19 large-values dependency; any medium-length alternative needed by the classical finite transfer remains inside Package D. No weaker symmetry theorem is relabeled as that analytic input.
...
The already completed truncation, mollifier, Jensen, critical-line second-moment, rectangle-argument-principle, strip-growth, finite differencing, multi-period B-process, explicit Weyl exponent, and weighted critical-line block results remain valid reusable library work. Weighted Littlewood, Gabriel interpolation, the standalone zeta fourth moment, and right/horizontal contour moments are not mandatory #15 deliverables. Packages A and B are complete; both endpoint propositions are still absent because Packages C and D remain open. The audit synchronizes 522 declarations and reports only the unrelated #19 decoupling dependency failure.

Primary implementation references are the [ANTEDB finite zero-density chapter](https://teorth.github.io/expdb/blueprint/zero-density-chapter.html), the [ANTEDB source repository](https://github.com/teorth/expdb), PNT+'s [`ZetaAppendix.lean`](https://github.com/AlexKontorovich/PrimeNumberTheoremAnd/blob/main/PrimeNumberTheoremAnd/IEANTN/ZetaAppendix.lean), and the source [Guth–Maynard paper](https://arxiv.org/html/2405.20552v2). Current ANTEDB does contain Lean foundation modules, but no reusable B-process, A-process, or Weyl-transform proof; its toolchain is also newer than this project. PNT+ supplies a fully proved sharp partial-zeta Poisson/Euler–Maclaurin stack, but no van der Corput derivative test. These sources guide implementation; only this repository's pinned Lean build and dependency audit determine completion.
...
| Downstream work | Concrete Huxley and concrete Ingham remain unproved. The selected route now requires the full Montgomery–Halász–Huxley `T * N^4 * V⁻⁶` branch, the finite exact-`σ` density transfer, and final assembly for symmetric multiplicity-weighted `N`. The van der Corput/Weyl and contour infrastructure is complete and retained. |
| Verification | The focused Weyl builds and root production build pass without warnings. `logs/overall_proof_20260809_114914.log` passes stages 1–4 warning-free. The current direct audit synchronizes 522/522 declarations, explicitly checks all 107 public imported theorems and the complete new #15 Weyl stack, and reports only the unrelated decoupling dependency failure. |
...
| Audit impact | The current explicit and discovered theorem sets agree at 522/522. Every beta/localization theorem, `extractSeparated_native`, every classical-moment theorem, all 107 public PNT+ port theorems, the project argument-principle specialization, the #17 arithmetic theorems, and the #15 truncation and Weyl theorems pass with only `propext`, `Classical.choice`, and `Quot.sound`. The only audit failure is the unrelated `l2_decoupling_bound_native`. |
| Principal verification | `logs/overall_proof_20260809_114914.log` passes stages 1–4 warning-free, synchronizes 522/522 declarations, and reports only the single decoupling audit failure. The integrity scan finds no prohibited admitted-proof text or unsafe bypass and exactly the two #19 `Decoupling.lean` axioms. Overall exit code is honestly `1`. |
...
| 6 | ~~`RiemannZeta/Audit.lean`~~ | **Completed for all named public theorems in the cleaned tree:** transitive checks cover the synchronized 522-theorem list, including the globally named public declarations in the vendored modules; the only auxiliary declarations are anonymous examples. One current dependency failure is reported honestly. | Easy–moderate—complete |
...
1. **Completed for the production perimeter and kept synchronized through 9 August 2026:** created an explicit, synchronization-checked list, currently containing 522 exported source-level theorems.
...
1. **#15 — Classical finite zero-density endpoints (in progress).** The zero-count foundation, unit-zero theorem, Montgomery mean value, basic Halász–Montgomery, mollifier/rectangle infrastructure, strip growth, coefficient bounds, symmetric-count bridge, Package A truncated-zeta theorem, and Package B van der Corput/Weyl theorem stack are proved. The remaining critical path is the full Montgomery–Halász–Huxley estimate including `T * N^4 * V⁻⁶`, followed by the finite exact-`σ` transfer and the two concrete endpoint theorems. This classical large-values package is distinct from #19's `GuthMaynardLargeValues`, which is the sole remaining premise of the Central Type I estimate after applying the native mean-value, extraction, and coefficient theorems.
