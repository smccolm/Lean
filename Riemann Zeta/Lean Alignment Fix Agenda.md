...
| `Audit.lean` checks transitive dependencies for the synchronized production theorem set. | #6 | **Complete for the cleaned tree's named public theorems:** the current list is 549/549, including all 107 public theorems in the vendored PNT+ modules; the only auxiliary Lean declarations are anonymous examples. The audit correctly remains red on one project-axiom-dependent declaration. |
...
| 6 | **Verified complete as audit infrastructure** | The explicit and discovered theorem sets agree at 549/549 and transitive dependencies are checked. One failure is reported rather than hidden. |
...
|---:|---|---|---|
| 15 | **Classical finite zero-density engine and endpoints — in progress** | Existing work proves zeta growth, local multiplicity, the finite Möbius detector, the critical-line mollifier second moment, the rectangle argument principle, Montgomery mean value, the symmetric-count bridge, and #17 coefficient bounds. **Packages A, B, and C are complete.** `classical_montgomery_halasz_huxley_native` proves the full bounded-coefficient finite MHH estimate, including `T * N^4 * V⁻⁶`, from the native mean-value theorem and proved logarithmic-kernel estimates. Package D and endpoint assembly remain open. | All 549 public production theorems, including the 107-theorem imported rectangle stack, are synchronized in the transitive audit. The item stays uncrossed until audit-clean theorems prove both `HuxleyZeroDensity (fun σ T => N σ T)` and `InghamZeroDensity (fun σ T => N σ T)`. |
...
The already completed truncation, mollifier, Jensen, critical-line second-moment, rectangle-argument-principle, strip-growth, finite differencing, multi-period B-process, explicit Weyl exponent, weighted critical-line block, and classical large-values results remain valid reusable library work. Weighted Littlewood, Gabriel interpolation, the standalone zeta fourth moment, and right/horizontal contour moments are not mandatory #15 deliverables. Packages A, B, and C are complete; both endpoint propositions are still absent because Package D and endpoint assembly remain open. The audit synchronizes 549 declarations and reports only the unrelated #19 decoupling dependency failure.
...
| Downstream work | Concrete Huxley and concrete Ingham remain unproved. The selected route now requires the finite exact-`σ` density transfer and final assembly for symmetric multiplicity-weighted `N`. The classical large-values, van der Corput/Weyl, and contour infrastructure is complete and retained. |
| Verification | The focused Package-C and root production builds pass without warnings. The principal run `logs/overall_proof_20260809_133023.log` passes stages 1–4 with zero warnings, synchronizes 549/549 declarations, explicitly checks all 107 public imported theorems and the complete #15 Package-C stack, and reports only the unrelated decoupling dependency failure. |
...
| Audit impact | The current explicit and discovered theorem sets agree at 549/549. Every beta/localization theorem, `extractSeparated_native`, every classical-moment theorem, all 107 public PNT+ port theorems, the project argument-principle specialization, the #17 arithmetic theorems, and the #15 truncation, Weyl, kernel, and classical-large-values theorems pass with only `propext`, `Classical.choice`, and `Quot.sound`. The only audit failure is the unrelated `l2_decoupling_bound_native`. |
| Principal verification | `logs/overall_proof_20260809_133023.log` synchronizes 549/549 declarations and reports only the single decoupling audit failure. Its integrity scan finds no prohibited admitted-proof text or unsafe bypass and exactly the two #19 `Decoupling.lean` axioms; the principal runner therefore remains honestly `FAIL`/`1` until #19. |
...
| Verification | The focused arithmetic module and the current root production closure build with zero warnings. The audit synchronizes 549/549 declarations; all five #17 declarations pass with only permitted logical dependencies. The remaining failures are the two pre-existing #19 decoupling axioms and their single transitive audit failure. |
...
| 6 | ~~`RiemannZeta/Audit.lean`~~ | **Completed for all named public theorems in the cleaned tree:** transitive checks cover the synchronized 549-theorem list, including the globally named public declarations in the vendored modules; the only auxiliary declarations are anonymous examples. One current dependency failure is reported honestly. | Easy–moderate—complete |
...
1. **Completed for the production perimeter and kept synchronized through 9 August 2026:** created an explicit, synchronization-checked list, currently containing 549 exported source-level theorems.
