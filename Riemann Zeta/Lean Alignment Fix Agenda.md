...
| `Audit.lean` checks transitive dependencies for the synchronized production theorem set. | #6 | **Complete for the cleaned tree's named public theorems:** the current list is 1078/1078, including the new #15 Type-I finite estimates and #19 cubic-trace, `S₁`, uniform Fourier-decay, and smooth-reflection surfaces. Every declaration passes with only the permitted logical dependencies. |
...
| The paper stated stale audit counts. | Documentation gate | **Resolved during #8 and kept synchronized:** the current audit passes across 1078 synchronized declarations. |

Current clean facts: the production graph and both retained examples elaborate with zero warnings; the dependency audit synchronizes and passes 1078 declarations; and all prohibited-proof and project-postulate scans are empty. The named output gate reports seven absent end theorems after accepting native #18 coverage and reduction; the overall result remains honestly `FAIL`/`1` until the remaining outputs exist.
...
| 2 | **Verified complete as cleanup** | The retained tree has 76 Lean files, no empty Lean file, a 73-file transitive production graph, `Audit.lean`, and exactly two non-production anonymous-example files. All listed obsolete tests, tombstones, and side-effect utilities are absent. |
...
| 6 | **Verified complete as audit infrastructure** | The explicit and discovered theorem sets agree at 1078/1078 and all transitive dependency checks pass. The separate completion gate fails on the seven absent #15/#18/#19 outputs. |
| 7 | **Verified complete** | The root import closure reaches all 72 subordinate production modules, giving a 73-file production graph. |
...
|---:|---|---|---|
| 15 | **Classical finite zero-density engine and endpoints — in progress** | Existing work proves exact-β removal, the native multiplicity-dominant dichotomy, common extraction, normalization and finite powering, the `X=1` powered theorem, #17 coefficient growth, MHH/Weyl bounds, endpoint certificates, terminal control, and the finite Type-I estimates in `TypeIFiniteEstimates.lean`. The latter give scale classification, fixed/unit-bounded sharp coefficients, a uniform prefix bound, its Abel-weighted sharp-cutoff form, and a conditional medium exclusion. Remaining: source-faithful Type-I smoothing; stationary reflection to a `T/N` dual block; reflected-block deweighting; actual-`X` Type-II powering/MHH; branch-to-slab reduction; and the two endpoints. | The item stays uncrossed until audit-clean theorems prove both `HuxleyZeroDensity (fun σ T => N σ T)` and `InghamZeroDensity (fun σ T => N σ T)`. |
...
| 2 | 18 | **Smooth `ζ²` approximate functional equation and diagonal bound** | The exact `M²` expansion, support, coefficient bound, admissible length, target integral, and full Appendix C coverage are fixed. | Close `AF`; keep the DFI off-diagonal theorem separate. |
...
8. **Type-I smoothing and scale split — partial 10 August 2026:** `typeISmoothScaleSplit_native` consumes the actual pointwise witness data and terminal majorant, proves `N ≤ T`, and returns the exhaustive `N² ≤ T` or `T < N²` split. The source-faithful common smooth-block extraction and its loss accounting remain open; the sharp dichotomy alone does not supply the smooth kernel needed by stationary reflection.
9. **Medium B-process — partial 10 August 2026:** `norm_logarithmicPhase_prefix_le_sqrt` gives every prefix an explicit `100√t` second-derivative bound, `norm_mediumWeightedBlock_le` transfers it by Abel summation, and `mediumTypeIBProcess_native` handles the actual cutoff. These theorems are genuine, but they do not replace the stationary dual polynomial: at `σ=3/4`, `N=T^(2/3)`, the weighted majorant is of order one and cannot dominate the detector threshold. The exact reflected block at scale `T/N` remains open.
10. **Shared Type-I deweighting — partial 10 August 2026:** `norm_classicalZetaLongLineCoeff_le_one` proves the sharp coefficients are fixed and unit-bounded, and `typeIFourierDeweight_native` identifies a full below-cutoff block with `n^{-σ}`. The Fourier/Mellin removal of the ordinate-dependent weight produced after stationary reflection remains open.
...
8. **Verification and scope boundary:** the focused and root builds are warning-free. The dichotomy, terminal, and Type-I finite-estimate declarations are registered in the synchronized 1078-declaration transitive audit and use only `propext`, `Classical.choice`, and `Quot.sound`. Smooth extraction, stationary reflection, reflected deweighting, and the later branch-to-slab consumer remain open.

#### Endpoint assembly and acceptance

1. **Ingham:** set `τ₀ = 2 - σ`. Send Type II through the actual-`X` powered MHH theorem. For Type I, extract the common smooth block; use direct deweighting on the short side and stationary reflection followed by shared deweighting on the medium side. Then apply the appropriate fixed-coefficient large-values estimate to obtain `3 * (1 - σ) / (2 - σ)`.
2. **Huxley:** set `τ₀ = 3 * σ - 1`. Use the sixth-power MHH branch on Type-II and short Type-I windows, the reflected short block on the medium side, and Weyl for the exceptional short range identified by `FiniteDensityExponents.lean`. At `σ = 3/4`, derive the boundary from Ingham because both exponents equal `3/5`.
...
The completed truncation, detector dichotomy, exact-β removal, extraction, normalization/powering infrastructure, MHH/Weyl estimates, endpoint arithmetic, terminal control, and finite Type-I prefix/Abel estimates remain valid. Weighted Littlewood, Gabriel interpolation, the standalone zeta fourth moment, and contour-edge moments are not mandatory for #15. Stationary Type-I reflection is mandatory. The open path is: common smoothing; stationary `T/N` reflection; shared reflected deweighting; actual-`X` Type-II powered MHH; range and branch-to-slab reduction; and endpoints.
...
| Downstream work | Concrete Huxley and Ingham remain unproved. Exact-β removal, the dichotomy, common extraction, normalization, exact powering, the `X=1` ordinary sharp-zeta powered theorem, coefficient growth, endpoint arithmetic, terminal restriction, scale split, and finite prefix/Abel bounds are complete. Smooth extraction, stationary reflection, reflected deweighting, the actual-`X` Type-II application, range resolution, branch-to-slab consumer, and symmetric specialization remain. |
| Verification | The focused Type-I finite-estimate module and root production build pass without warnings. The current audit passes 1078/1078 dependency checks and all integrity scans but fails the output gate on the two absent #15 endpoints and five other agenda outputs. |
...
3. **#15 Type-I interface chain — open.** `TypeIFiniteEstimates.lean` proves `typeISmoothScaleSplit_native`, `typeIFourierDeweight_native`, the unit coefficient bound, and uniform prefix/Abel-weighted estimates. Critical review showed that the coarse `100√t` bound cannot close the full medium range. Acceptance still requires the common smooth-block extraction, stationary dual polynomial at scale `T/N`, and Fourier/Mellin deweighting of the reflected weight.
...
**Implementation result.** The stationary-rescaling work remains valid reference infrastructure. `TypeIFiniteEstimates.lean` adds substantive scale, coefficient, prefix, Abel, and conditional-exclusion results, but `PS`, `MR`, and `FD` remain open at their source acceptance boundaries. `TwistedDiagonal.lean` proves #18's finite `hm = kn` phase cancellation, exact interval integral, coefficient-sum evaluation, and norm bound. `LargeValuesReflection.lean` completes #19 smooth reflection. `LargeValuesCubic.lean` completes Lemma 4.5 and Proposition 5.1.
...
| Audit impact | The current explicit and discovered theorem sets agree at 1078/1078. Every beta/localization theorem, `extractSeparated_native`, every classical-moment theorem, the vendored PNT+ proof closure, native #18 coverage and reduction, and the #15/#18/#19 closeout foundations pass with only `propext`, `Classical.choice`, and `Quot.sound`. |
| Principal verification | `logs/overall_proof_20260810_230616.log` passes all build/example stages warning-free, passes 1078/1078 dependency checks, and finds no admitted proof, unsafe bypass, or project postulate. The named output gate fails on seven absent agenda theorems, so the overall result remains honestly `FAIL`/`1`. |
...
| Verification | The focused arithmetic module and root production closure build with zero warnings. The current audit synchronizes 1078/1078 declarations; all five #17 declarations and the #15/#18/#19 foundations pass with only permitted logical dependencies. |
...
| 6 | ~~`RiemannZeta/Audit.lean`~~ | **Completed for all named public theorems in the current tree:** the explicit and discovered sets synchronize at 1078/1078, every existing theorem has only the permitted logical dependencies, and the separate completion gate reports seven absent output names. | Easy–moderate—complete |
| 7 | ~~`RiemannZeta.lean`~~ | **Completed and maintained:** the root reaches all 72 subordinate production modules, including the six vendored PNT+ files and the current #15/#18/#19 modules. The deleted false `Decoupling` module is not imported. | Easy—complete |
...
| 15 | `TypeIFiniteEstimates.lean`, `MediumReflection.lean`, `ClassicalDichotomy.lean`, `FiniteDensityEndpoint.lean`, `FiniteScaleAssembly.lean`, and `InghamBound.lean` | Scale split and finite sharp-block estimates are complete. Add smooth extraction, the stationary `T/N` dual block, reflected deweighting, the actual-`X` Type-II theorem, branch-to-slab reduction, and endpoints. | Very hard—open; Type-I analytic interface incomplete |
...
| 3 | `RiemannZeta/GuthMaynard/TypeIFiniteEstimates.lean`, `MediumReflection.lean` | Common smooth-block extraction, stationary `T/N` dual block, and reflected Fourier/Mellin deweighting. | Extreme; `PS`, `MR`, and `FD` remain open, with finite estimates complete. |
| 4 | `TypeIFiniteEstimates.lean`, `FiniteScaleAssembly.lean`, and a finite density consumer | Prove the explicit threshold/certificate comparisons and Type-I range resolution after reflection. | Hard; `ZR`. |
| 5 | A finite density consumer over `ClassicalDichotomy.lean` | Combine Type-I and actual-`X` Type-II bounds with multiplicity and certificates. | Very hard; `FR`. |
...
1. **Completed for the production perimeter and kept synchronized through 10 August 2026:** created an explicit, synchronization-checked list, currently containing 1078 exported source-level theorems.
...
| Coverage | At that iteration the tree contained 44 Lean files and a 41-file production graph. The current superseding inventory is 76 Lean files: a 73-file root closure, `Audit.lean`, and two separately executed examples. No retained Lean file is outside the human-facing evaluation. |
...
| Remaining obstruction at that iteration | #8 had no remaining warning or retained-file coverage defect. The superseding 10 August audit has 1078/1078 passes and zero direct project axioms. |
...
1. **#15 — Classical finite zero-density endpoints (in progress).** The dichotomy, common extraction, exact powering infrastructure, `X=1` theorem, coefficient growth, MHH/Weyl estimates, endpoint arithmetic, terminal Type-I estimate, scale split, sharp coefficient identities, and coarse prefix/Abel bounds are proved. Type I still needs smooth extraction, stationary `T/N` reflection, and reflected deweighting before `ZR`; Type II still needs `IIE` because `powered_sharp_block_large_values_bound` is hard-coded to `X=1`. Both branches join at `FR → ZD`. This classical package is distinct from #19's `GuthMaynardLargeValues`.
