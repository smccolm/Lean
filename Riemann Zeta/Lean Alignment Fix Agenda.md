...
| `Audit.lean` checks transitive dependencies for the synchronized production theorem set. | #6 | **Complete for the cleaned tree's named public theorems:** the current list is 1141/1141, including the #15 smooth-extraction, exact Mellin-reflection, fixed-block Fourier-tail, Poisson-geometry, and two-sided deweighting surfaces. Every declaration passes with only the permitted logical dependencies. |
...
| The paper stated stale audit counts. | Documentation gate | **Resolved during #8 and kept synchronized:** the current audit passes across 1141 synchronized declarations. |

Current clean facts: the production graph and both retained examples elaborate with zero warnings; the dependency audit synchronizes and passes 1141 declarations; and all prohibited-proof and project-postulate scans are empty. The named output gate still reports seven absent end theorems; the overall result remains honestly `FAIL`/`1` until the remaining outputs exist.
...
| 6 | **Verified complete as audit infrastructure** | The explicit and discovered theorem sets agree at 1141/1141 and all transitive dependency checks pass. The separate completion gate fails on the seven absent #15/#18/#19 outputs. |
...
|---:|---|---|---|
| 15 | **Classical finite zero-density engine and endpoints — in progress** | Existing work now includes the exact source-faithful smooth partition, common-scale extraction with cardinality loss, the literal smooth Poisson formula, corrected stationary point/window at scale `T/N`, exact Mellin factorization of every retained critical mode on a common interval, and exact Fourier deweighting of both the short and reflected amplitudes. The human-to-Lean correspondence is recorded in `Type I Reflection Proof Bijection.md`. Remaining in this chain is one substantive medium theorem: a uniform estimate for the endpoint blocks and the zero, wrong-sign, and summed far-frequency modes. The actual-`X` Type-II application, range/branch reduction, and both endpoints also remain. | The item stays uncrossed until audit-clean theorems prove both `HuxleyZeroDensity (fun σ T => N σ T)` and `InghamZeroDensity (fun σ T => N σ T)`. |
...
9. **Medium B-process — exact retained-mode Mellin reflection complete; noncritical remainder open 11 August 2026:** the earlier `100√t` estimate remains a valid coarse bound. `TypeIReflection.lean` now proves Mellin inversion for the literal dyadic cutoff, absolute Fubini integrability, the exact `v=m*x` rescaling, a common interval `[Q/2,2MQ]`, and `sum_typeIDyadicPhysicalIntegral_eq_reflectedMellinPolynomial`. `LargeValuesReflection.lean` supplies the weighted `10/√τ` reflection bound. This removes the planned Fresnel/stationary-main-term dependency. The remaining theorem must retain the exact source boundary and uniformly sum the zero, wrong-sign, and frequencies beyond `T^ε T/Q` below the detector threshold.
...
8. **Verification and scope boundary:** the focused Type-I builds are warning-free. The smoothing, exact retained-mode Mellin reflection, fixed-block Fourier decay/summability, and deweighting declarations are registered in the synchronized 1141-declaration transitive audit and use only `propext`, `Classical.choice`, and `Quot.sound`. The parameter-uniform full-source endpoint/noncritical-mode remainder theorem and the later branch-to-slab consumer remain open.
...
| Downstream work | Concrete Huxley and Ingham remain unproved. Exact-β removal, the dichotomy, common extraction, normalization, exact powering, the `X=1` theorem, coefficient growth, endpoint arithmetic, terminal restriction, source smoothing/common-scale extraction, exact Poisson geometry, exact retained-mode Mellin reflection, and both deweighting identities are complete. The full-source boundary/wrong-sign/far-frequency remainder estimate, actual-`X` Type-II application, range resolution, branch-to-slab consumer, and symmetric specialization remain. |
| Verification | The focused Type-I modules pass without warnings. The current audit passes 1141/1141 dependency checks but fails the output gate on the two absent #15 endpoints and five other agenda outputs. |
...
3. **#15 Type-I interface chain — retained modes exact, full-source remainder open.** `TypeISmoothing.lean` closes the source-faithful common smooth-block extraction. `TypeIReflection.lean` proves exact Poisson summation, the corrected stationary-frequency window, literal cutoff Mellin inversion, physical rescaling, a common reflection interval, and finite critical-mode polynomial assembly. `TypeIFourierDeweight.lean` closes exact deweighting for both the source and the concrete reflected ANTEDB amplitude. The sole open interface theorem is now the uniform endpoint/zero/wrong-sign/far-frequency remainder that converts the complete Poisson series into that finite reflected block at scale `T/N`; no approximate stationary main term is required for the retained modes.
...
**Implementation result.** `PS` and `FD` are complete and audit-clean. `MR` now contains the exact smooth Poisson identity, mode/phase identification, stationary normalization, corrected dual window, cutoff Mellin inversion, common-interval weighted reflection, and finite critical-mode polynomial assembly. It remains open solely at the uniform full-source endpoint/noncritical-mode remainder theorem. The complete source/Lean bijection and normalization correction are in `Type I Reflection Proof Bijection.md`. `TwistedDiagonal.lean` proves #18's finite `hm = kn` phase cancellation, while `LargeValuesReflection.lean` and `LargeValuesCubic.lean` close their stated #19 packages.
...
| Verification | The focused arithmetic module and root production closure build with zero warnings. The current audit synchronizes 1141/1141 declarations; all five #17 declarations and the #15/#18/#19 foundations pass with only permitted logical dependencies. |
...
| 6 | ~~`RiemannZeta/Audit.lean`~~ | **Completed for all named public theorems in the current tree:** the explicit and discovered sets synchronize at 1141/1141, every existing theorem has only the permitted logical dependencies, and the separate completion gate reports seven absent output names. | Easy–moderate—complete |
...
| 15 | `TypeISmoothing.lean`, `TypeIReflection.lean`, `TypeIFourierDeweight.lean`, `MediumReflection.lean`, and the finite-density consumers | Smooth extraction, exact Poisson geometry, exact retained-mode Mellin reflection, and short/reflected deweighting are complete. Add the uniform full-source endpoint/noncritical-mode remainder, actual-`X` Type-II theorem, branch-to-slab reduction, and endpoints. | Very hard—open; one Type-I analytic theorem remains before finite assembly |
...
| 3 | `RiemannZeta/GuthMaynard/TypeIReflection.lean`, `MediumReflection.lean` | Bound the full-source endpoint blocks and the zero, wrong-sign, and frequencies beyond the corrected `T^ε T/N` window. The retained critical window is already an exact common Mellin-reflection polynomial. | Extreme; `PS`, retained-mode `MR`, and `FD` are complete, and this summed uniform remainder is the sole remaining `MR` obligation. |
...
1. **Completed for the production perimeter and kept synchronized through 11 August 2026:** created an explicit, synchronization-checked list, currently containing 1141 exported source-level theorems.
...
| Remaining obstruction at that iteration | #8 had no remaining warning or retained-file coverage defect. The superseding 11 August audit has 1141/1141 passes and zero direct project axioms. |
