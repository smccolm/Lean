...
| `Audit.lean` checks transitive dependencies for the synchronized production theorem set. | #6 | **Complete for the cleaned tree's named public theorems:** the current list is 1018/1018, including the new #15 stationary-rescaling, #18 finite-diagonal, and #19 Mellin-inversion surfaces. Every declaration passes with only the permitted logical dependencies. |
...
| The paper stated stale audit counts. | Documentation gate | **Resolved during #8 and kept synchronized:** the current audit passes across 1018 synchronized declarations. |

Current clean facts: the production graph and both retained examples elaborate with zero warnings; the dependency audit synchronizes and passes 1018 declarations; and all prohibited-proof and project-postulate scans are empty. The named output gate reports seven absent end theorems after accepting native #18 coverage and reduction; the overall result remains honestly `FAIL`/`1` until the remaining outputs exist.
...
| 6 | **Verified complete as audit infrastructure** | The explicit and discovered theorem sets agree at 1018/1018 and all transitive dependency checks pass. The separate completion gate fails on the seven absent #15/#18/#19 outputs. |
...
| 18 | **Type-II residual analysis and Goal C integration — in progress** | Completed: source-range contract repair; arbitrary-order Gamma decay; weighted whole-line Gamma and actual contour-integrand integrability; the complete native fourth-moment counting reduction; and the full Appendix C coverage theorem, including Mellin inversion, regularized zero cancellation, rectangle shifting, residue computation, quantitative tails, exact detector decomposition, and dyadic assembly. `TwistedMoment.lean` proves the exact `M²` convolution, support, eventual `T^(1/22)` length, uniform divisor coefficient bound, and moment identity. `TwistedDiagonal.lean` now proves exact `hm = kn` phase cancellation, its integral over `[T/2,3T]`, the complete finite diagonal evaluation, and a general norm bound. Remaining: the completed-`ζ²` smooth AFE, the weighted asymptotic diagonal bound after inserting its weights, genuine DFI quadratic-divisor/off-diagonal theorem, generic twisted moment and native specialization, and Goal C assembly after #15 supplies native Huxley. | `typeIContourTypeIICoverOn_native` and `typeIIFourthMomentReduction_native` are native and audit-clean. Acceptance still requires `twistedZetaFourthMoment_native` and an audited theorem deriving the concrete full-range zero-density result from `GuthMaynardLargeValues` alone. |
| 19 | **Guth–Maynard large values and final project integration — in progress** | **Completed:** deletion of the false model; cutoff/derivative, matrix action, exact Gram kernel, positive spectrum, singular-value powers, trace-cube, sixth-dispersion, core approximate-energy, basic `R`, Fourier/Poisson foundations, and the complete Hilbert–Schmidt first-trace expansion with a uniform `N⁻¹⁰⁰` tail. The smooth-reflection package now also has absolute Mellin convergence of `w²`, vertical integrability of its transform, exact pointwise inversion, and substitution into the trace kernel. **Remaining:** cubic trace expansion, uniform two-parameter Fourier decay, the `S₁/S₂/S₃` chain, native Heath–Brown, the frequency truncation and mode summation completing reflection, moment/energy bridge, affine and GCD/spacing theorems, Sections 3/12 assembly, and concrete transfers. | The concrete Guth–Maynard and combined zero-density theorems are project-theorem complete. Existing dependency and integrity gates pass, while the named output gate keeps the principal runner at `FAIL`/`1` until the end theorems exist. |
...
The completion of #18's native fourth-moment reduction, Appendix C coverage, and #19's Hilbert–Schmidt first-trace Poisson layer changes the most efficient implementation order without closing either owning Shitlist item. The following targets are ordered by estimated implementation difficulty, not by Shitlist number. The completed coverage and first-trace rows have been removed.

| Rank | Owning item | Complete closeout target | Why it is now reachable | Exact boundary |
|---:|---:|---|---|---|
| 1 | 18 | **Smooth `ζ²` approximate functional equation and diagonal bound** | The exact `M²` expansion, support, coefficient bound, admissible length, target integral, and full Appendix C coverage are now fixed. This node is independent of the DFI off-diagonal theorem. | Close the `AF` node with a reusable smooth `ζ²` AFE and its full diagonal estimate; do not claim the twisted fourth moment before the separate off-diagonal node is proved. |
| 2 | 19 | **Package 6: smooth reflection** | The cutoff regularity, affine language, Mellin/Fourier infrastructure, and finite coefficient machinery are present. | Prove Guth–Maynard Lemma 6.2 with uniform truncation and tail bounds and the resulting short polynomial; `S₂` remains downstream of Heath–Brown. |
| 3 | 15 | **Medium Type-I reflection** | `MediumReflection.lean` already contains the exact Poisson formula, stationary point and phase normalization, dyadic frequency window, and two-sided quadratic control. All other #15 finite certificates and the terminal branch are complete. | Prove the uniform stationary main term, dual-block recovery, and ordinate-independent coefficient reduction. This is the highest-payoff classical closeout because it immediately unlocks the finite reduction and endpoint assembly. |
...
8. **Verification and scope boundary:** the focused and root builds are warning-free. All 25 dichotomy declarations and all five terminal declarations are registered in the synchronized 1018-declaration transitive audit and use only `propext`, `Classical.choice`, and `Quot.sound`. The certificate leaves the medium Type-I scale unclassified; short-scale estimation and the completed terminal case go directly to the finite reduction.
...
| Verification | The focused terminal module and root production build pass without warnings. The current audit passes 1018/1018 dependency checks and all integrity scans but fails the output gate on the two absent #15 endpoints and five other agenda outputs. |
...
1. **#18 smooth `ζ²` AFE and diagonal.** Add a dedicated AFE module. Define the Hughes–Young weight by a vertical Mellin integral, prove its convergence and decay, shift the completed-`ζ²` contour, take the pole at the origin, and obtain the two-piece approximate functional equation on the critical line. Expand the smoothed twisted moment and prove the complete `hm = kn` diagonal estimate with constants uniform for coefficient-controlled twists of length `≤ T^(1/22)`. Public outputs: `smoothZetaSqAFE_native` and `twistedFourthMomentDiagonal_native`. The DFI quadratic-divisor estimate and every `hm ≠ kn` term remain in the separately drawn `QD` node.
2. **#19 smooth reflection.** First finish the part of Guth–Maynard Lemma 4.3 actually consumed by Lemma 6.2: uniform decay of `ĥ_t(ξ)` outside `|ξ| ≲ |t|`. Then prove the paper's Mellin representation of `w²`, truncate the Mellin variable using rapid decay, rescale the oscillatory integral, apply the proved first/second derivative bounds on every dyadic `v`-block, and sum positive and negative Fourier modes. Public output: `gmSmoothReflection_native`, an explicit epsilon-form Lemma 6.2 estimate. This research corrects the architecture with the necessary edge `UF → RF`; the cubic-trace and `S₁` parts of `UF` remain separate.
3. **#15 medium Type-I reflection.** Smooth a medium ordinary-zeta block, use `partial_zeta_block_poisson`, isolate exactly the positive frequencies whose stationary points lie in the block, evaluate them uniformly after `reflectionPhase_stationary_rescale`, and control negative/nonstationary frequencies with the existing PNT+ first-derivative expansion. Pigeonhole the truncated Mellin variable so the recovered dual block has one coefficient sequence independent of the ordinate. Public output: `mediumTypeIReflection_native`, with dual support comparable to `T/N`, unit-normalizable coefficients, and a threshold/cardinality loss already in the project's epsilon-power language.
...
**Implementation result of this pass.** `reflectionOscillatoryIntegral_stationary_rescale` proves #15's exact interval-integral change to stationary coordinates. New `TwistedDiagonal.lean` proves #18's complete finite `hm = kn` phase cancellation, exact interval integral, coefficient-sum evaluation, and norm bound. `mellinConvergent_gmCutoffSq_one`, `verticalIntegrable_mellin_gmCutoffSq_one`, `gmCutoffSq_mellinInversion`, and `gmTraceKernel_mellinInversion` prove #19's cutoff inversion without assumptions. These results are audit targets, but none is the named closeout theorem. The source-faithful remaining boundaries are now exact: a uniform stationary-phase/B-process theorem with common-coefficient recovery for #15; the completed-`ζ²` contour shift, gamma-ratio bounds, and weighted diagonal estimate for #18; and arbitrary-order two-parameter Fourier decay plus the positive/negative mode summation for #19.

### Source-Faithful Completion Plan for Shitlist #19 — 10 August 2026

**Implementation status, 10 August 2026:** Packages 1–3 are complete. Package 2 includes exact translation invariance, nonzero scaling covariance, affine reindexing, the seven-bin floor partition of tolerance-one energy, and the injection of its zero-defect bin into Mathlib exact additive energy for one-separated sets. Package 3 proves the exact operator-norm conclusions of Guth–Maynard Lemmas 4.1–4.2, including the source cardinality estimate and sixth-root trace-dispersion bound. Package 4 is partially implemented in `LargeValuesPoisson.lean`: the actual `h_t`, its Schwartz realization, endpoint vanishing, Mellin/Fourier zero-mode identity, arbitrary-order uniform zero-mode decay, exact Fourier dilation, source-normalized Poisson formula, finite-support reduction, and the complete Hilbert–Schmidt first-trace expansion of Lemma 4.4 are proved. The last result identifies the zero mode exactly and controls the entire nonzero-frequency tail uniformly by `O(N⁻¹⁰⁰)` after polynomial cardinality loss. Package 6 now has the exact Mellin-convergence, vertical-integrability, inversion, and kernel-substitution foundation for `w²`; its uniform Lemma 4.3 truncation and Lemma 6.2 mode sum remain open. The full two-parameter Lemma 4.3 estimates, cubic trace split, and Proposition 5.1 remain open. Packages 5 and 7–14 remain open. The audit and integrity portion of Package 14 passes, but final integration does not because the native large-values and concrete density theorems are absent.
...
6. **Prove smooth reflection — inversion foundation complete, estimate open.** `mellinConvergent_gmCutoffSq_one`, `verticalIntegrable_mellin_gmCutoffSq_one`, `gmCutoffSq_mellinInversion`, and `gmTraceKernel_mellinInversion` establish the exact fixed-cutoff Mellin representation used by Guth–Maynard Lemma 6.2. Complete arbitrary-order uniform decay of `ĥ_t(ξ)` outside `|ξ| ≲ |t|`, truncate with an explicit `T⁻¹⁰⁰` tail, perform the rescaled first/second-derivative estimates, and identify the resulting positive/negative short Dirichlet polynomials.
...
| Audit impact | The current explicit and discovered theorem sets agree at 1018/1018. Every beta/localization theorem, `extractSeparated_native`, every classical-moment theorem, the vendored PNT+ proof closure, native #18 coverage and reduction, and the #15/#18/#19 closeout foundations pass with only `propext`, `Classical.choice`, and `Quot.sound`. |
| Principal verification | The current runner passes all build/example stages warning-free, passes 1018/1018 dependency checks, and finds no admitted proof, unsafe bypass, or project postulate. The named output gate fails on seven absent agenda theorems, so the overall result remains honestly `FAIL`/`1`. |
...
| Verification | The focused arithmetic module and root production closure build with zero warnings. The current audit synchronizes 1018/1018 declarations; all five #17 declarations and the #15/#18/#19 foundations pass with only permitted logical dependencies. |
...
| 6 | ~~`RiemannZeta/Audit.lean`~~ | **Completed for all named public theorems in the current tree:** the explicit and discovered sets synchronize at 1018/1018, every existing theorem has only the permitted logical dependencies, and the separate completion gate reports seven absent output names. | Easy–moderate—complete |
...
| 18 | `TwistedMoment.lean`, new `TwistedDiagonal.lean`, `TypeIIZeros.lean`, and new narrowly scoped AFE/DFI modules | Native Appendix C coverage and the complete finite `hm = kn` phase/integral/norm evaluation are complete. Prove the completed-`ζ²` smooth AFE and weighted asymptotic diagonal estimate, the genuine quadratic-divisor/off-diagonal theorem, the twisted fourth moment, and the one-premise Goal C theorem. | Extreme—in progress |
| 19 | `LargeValuesPoisson.lean`, `LargeValues.lean`, the proved matrix/language prerequisites, and new source-section modules | The Hilbert–Schmidt first-trace expansion and exact fixed-cutoff Mellin inversion are complete. The remaining repairs are uniform two-parameter Fourier decay, cubic trace split, `S₁/S₂/S₃`, the Lemma 6.2 truncation/mode sum, Heath–Brown, affine/energy/GCD theorems, native large values, and concrete transfers. | Hardest overall; first trace and reflection-inversion foundations complete, later source packages open |
...
| 1 | `RiemannZeta/GuthMaynard/TwistedMoment.lean`, `TwistedDiagonal.lean`, plus a new AFE module | Completed-`ζ²` smooth approximate functional equation and the weighted asymptotic diagonal estimate. The finite diagonal evaluation is complete. | Hard–extreme: self-contained before the separate DFI off-diagonal theorem. |
| 2 | `RiemannZeta/GuthMaynard/MediumReflection.lean` | Uniform stationary main term, dual block of scale `T/N`, and ordinate-independent coefficient recovery. Exact stationary-coordinate rescaling is complete. | Very hard, but it has the highest immediate #15 payoff. |
| 3 | `RiemannZeta/GuthMaynard/LargeValuesPoisson.lean` and subsequent new #19 modules | Uniform two-parameter Lemma 4.3, Lemma 6.2 truncation/mode sum, cubic trace split, and Proposition 5.1 for `S₁`; first trace and exact cutoff inversion are complete. | Very hard; no longer mislabeled as one small closeout. |
| 4 | New #19 Heath–Brown, affine, energy, and GCD/spacing modules, then `LargeValues.lean` | Remaining `S₂/S₃` chain and `guthMaynardLargeValues_native`. | Hardest. |
...
1. **Completed for the production perimeter and kept synchronized through 10 August 2026:** created an explicit, synchronization-checked list, currently containing 1018 exported source-level theorems.
...
| Remaining obstruction at that iteration | #8 had no remaining warning or retained-file coverage defect. The superseding 10 August audit has 1018/1018 passes and zero direct project axioms. |
