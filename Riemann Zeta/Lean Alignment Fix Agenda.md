...
| `Audit.lean` checks transitive dependencies for the synchronized root-imported theorem set. | #6 | **Complete as scoped integrity infrastructure:** the current list is 7602/7602. It includes the full DFI/Hughes–Young chain, the native #15 positive-slab chain, quantitative smooth reflection, and the exact Guth–Maynard large-values proof. Every audited declaration passes with only the permitted logical dependencies; this does not make the five later output theorems or the retained-file coverage gap complete. |
| `GuthMaynardLargeValues` formerly omitted positivity/eventual quantifiers and used the opposite exponential sign without a proof. | #14, #19, Goal A | **Resolved completely:** the source-positive form has the correct quantifiers and `V > 0`; the negative-sign form is proved by coefficient conjugation; and `guthMaynardLargeValues_native` now proves the exact proposition from the native Sections 3–12 chain and complementary classical ranges. |
...
| The paper stated stale audit counts. | Documentation gate | **Resolved during #8 and kept synchronized:** the current audit passes across 7602 synchronized declarations. |

Current scoped facts: the root-imported production graph elaborates with zero warnings; its dependency audit synchronizes and passes 7602 declarations; and the repository-wide prohibited-proof and direct-postulate scans are empty. The named output gate now also passes `guthMaynardLargeValues_native`, alongside the earlier twisted-moment, positive-slab, and quantitative-reflection outputs, while reporting five later end theorems absent. The fresh 28 August stress run `logs/overall_proof_20260828_140927.log` passes direct final-module elaboration, focused final/energy/umbrella builds, the 8853-job root build, stages 1–4 warning-free, all 7602 dependency checks, the GM output gate, and every integrity scan; stage 5 and the global runner return the expected `FAIL`/`1` solely because those five later outputs are absent. Retained-file elaboration coverage remains separately assigned to #19 final verification.

### Post-Completion Semantic Re-Audit of FR — 24 August 2026

The FR closeout was re-checked against the unfolded proposition, the literal detector-dichotomy datatype, and the bodies of its public consumers rather than accepted from compilation or audit counts alone. The result remains **complete at the positive-slab boundary**:

- `classical_endpoint_positive_slab_native` has exactly the requested hypotheses `1 / 2 < σ`, `σ < 1`, and `EndpointScaleCertificate σ τ₀`; its conclusion unfolds to the analytic-multiplicity count `zeroCountRect σ 1 T (2 * T)` with exponent `3 * (1 - σ) / τ₀`.
- Its proof calls `classical_endpoint_positive_slab_of_medium_native`, which invokes `classical_typeI_typeII_dichotomy_native` and eliminates the literal zero, Type-I, and Type-II alternatives. The zero branch uses the original count equality rather than an empty proxy.
- The Type-I branch preserves the dichotomy's original multiplicity inequality and dispatches the actual witness through the direct, powered, weighted-Weyl, short-gap, and native medium consumers. `classical_medium_typeI_witness_consumer_native` discharges the only narrower intermediate interface from the selected smooth source block; it is not a theorem-equivalent assumption.
- The Type-II branch calls `actual_typeII_dichotomy_witness_consumer`, which in turn uses `actual_typeII_powered_mhh_bounded_uniform_native`, selects a positive bounded power, proves the powered scale lies in `[2 * τ₀ / 3, τ₀]`, and absorbs the `Nat.clog`, displacement, multiplicity, harmonic, coefficient-normalization, and finite-power losses into the same epsilon-power chain.
- `ingham_endpoint_positive_slab_native` and `huxley_endpoint_positive_slab_native` are immediate certificate specializations. They do not yet prove the symmetric, fully quantified `InghamZeroDensity` and `HuxleyZeroDensity` propositions; that conversion remains the open `ZD` node and keeps Shitlist #15 uncrossed.

The fresh principal evaluation recorded below again checks the theorem and its transitive dependencies. This semantic re-audit therefore changes no mathematical status; it records why the green FR node satisfies the repository's anti-overstatement gate.
...
| 2 | **Historical cleanup verified; old inventory superseded** | The files deleted in the 8 August cleanup remain absent. The current tree has 335 tracked Lean files and 47 later top-level probes beyond the two runner examples, so the old 83-file/two-auxiliary inventory is no longer a current coverage claim. |
...
| 6 | **Verified complete as scoped audit infrastructure** | The explicit and discovered theorem sets agree at 7602/7602 within the root-imported graph and all dependency checks in that scope pass. The separate completion gate also passes native GM large values and fails on five later #15/#18/#19 outputs. |
| 7 | **Historical root-import repair verified; current regression open in #19** | The present root closure includes `ClassicalEndpointDensity` and the full FR dependency chain. `HughesYoungEquation96CoprimeBound.lean` and `HughesYoungZeroShiftC.lean` are retained but outside it. |
| 8 | **Configured gates verified; retained-file coverage open in #19** | The runner warning-checks its root, focused-module, example, and audit stages and scans all Lean text for prohibited constructs. It does not elaborate 47 other top-level probes or the two subordinate modules outside the root closure. |
...
| 12 | **Conditional/concrete layer complete; inputs discharged** | The concrete `ResidualZeroBoundProp` follows from three explicit source-facing propositions. #18 now discharges coverage, reduction, and the twisted fourth moment with native theorems. |
...
Shitlist #15, #18, and #19 remain; #16 and #17 are complete. They are bounded work packages, not a strict serial order. The exact Guth–Maynard large-values theorem inside #19 is now complete; the remaining work consists of the named density outputs and final retained-file coverage. Existing dependency checks, warning checks, and direct-postulate scans pass; the research-output gate currently reports five absent end theorems.
...
|---:|---|---|---|
| 15 | **Classical finite zero-density engine and endpoints — in progress** | The actual-`X` Type-II and actual Type-I packages now feed the real zero/Type-I/Type-II alternatives of `classical_typeI_typeII_dichotomy_native`. `classical_endpoint_positive_slab_native` proves the exact multiplicity-weighted slab count with exponent `3 * (1 - σ) / τ₀`; its Ingham and Huxley certificate specializations are also kernel-checked. Remaining are the positive-slab-to-full endpoint theorems. | The item stays uncrossed until audit-clean theorems prove both `HuxleyZeroDensity (fun σ T => N σ T)` and `InghamZeroDensity (fun σ T => N σ T)`. |
...
| 18 | **Type-II residual analysis and Goal C integration — in progress** | Completed: source-range repair, Gamma/convergence, Appendix C coverage, native fourth-moment reduction, `M²` arithmetic, smooth `ζ²` AFE/diagonal, DFI, Hughes–Young, and `twistedZetaFourthMoment_native`. | `AF`, `QD`, and `TM` are crossed off. Since `guthMaynardLargeValues_native` now discharges the former sole premise, the remaining #18 acceptance output is the concrete `guthMaynardZeroDensity_of_largeValues_native` consumer; full-range completion also waits on #15's native Huxley theorem. |
| 19 | **Guth–Maynard large values and final project integration — in progress** | **Large-values theorem complete:** the source-localization, matrix/trace, Poisson, `S₁`, quantitative reflection, native Heath–Brown, full Proposition 6.1 `S₂`, Proposition 8.1 localization, Proposition 9.1 affine descent, Proposition 10.1 refined `S₃`, Propositions 11.1–11.2 energy chain, Sections 3/12 root extraction, one-separated/full-height transfers, and complementary classical ranges culminate in `guthMaynardLargeValues_native : GuthMaynardLargeValues`. | The large-values theorem and Central Type I premise are discharged. #19 remains open only for `guthMaynardZeroDensity_native`, `combined_zero_density_native`, and restoration of all-retained-Lean-file runner/audit coverage; the exact theorem and all support declarations are kernel-checked in the synchronized audit. |
...
| ~~2~~ | 15 | **Actual Type-I finite absorption and endpoint routing — complete** | `normalizedClassicalTypeICoeff` supplies the missing `N^σ` normalization; `actual_typeI_dichotomy_witness_mhh_absorbed_native` absorbs the harmonic loss into `(3T)^ε`; `powered_actual_typeI_block_large_values_bound` realizes exact two/three-fold powering; and the weighted Weyl and endpoint-certificate routes are packaged by `actual_typeI_branch_resolution_native`. | Kernel-checked and included in the synchronized 7602-declaration audit. This closes `ZR`; the subsequently completed `FR` theorem performs threshold substitution and multiplicity assembly. |
| ~~3~~ | 19 | **Source-to-matrix entry bridge — complete** | `sourceDirichletPoly_eq_three_gmSmooth` proves an exact three-piece source identity; `source_large_values_localize_to_matrix` selects one fixed scale/coefficient family on at least one third of the ordinates and applies the matrix bound. The published interval is already `(N,2N]`; a separate theorem records compatibility with the alternate closed convention. | Kernel-checked and included in the 7602-declaration audit. |
| ~~4~~ | 19 | **Quantitative smooth reflection — complete and semantically re-audited** | `gmQuantitativeSmoothReflection_native` assembles the complete nonzero integer-frequency `tsum`, the exact signed retained window, measurable Mellin core/tail splitting, both ordinate signs, explicit `η = min (ε/16) (1/16)`, `H = T^η`, and `M = ⌈(1+|t|)H/N⌉`, and arbitrary `T⁻ᴬ` error. | Its strength-100 and scaled forms feed the now-complete Proposition 6.1 `S₂` theorem and final Sections 3–12 assembly. All are kernel-checked and audit-listed. |
...
Completed package: **#19 quantitative smooth reflection and downstream `S₂`.** `LargeValuesReflection.lean` and `QuantitativeSmoothReflection.lean` supply the exact signed identities, complete integer-frequency summability/decomposition, measurable core/tail assembly, both-sign conjugation, arbitrary-power absorption, strength-100 result, and scaled trace-tail theorem. `LargeValuesS2.lean` now consumes these results together with native Heath–Brown and proves the complete three-cyclic-summand Proposition 6.1 estimate.

The former #19 analytic critical path is complete: native Heath–Brown, `S₂`, the approximate-energy moments, affine Proposition 9.1, refined `S₃`, Section 11, and Sections 3/12 now feed the exact native large-values theorem. The remaining open nodes are the later density consumers and retained-file evaluation coverage.
...
13. **Branch-to-slab reduction — complete 24 August 2026:** `classical_endpoint_positive_slab_native` consumes `classical_typeI_typeII_dichotomy_native` itself and eliminates its zero, Type-I, and Type-II alternatives. `classical_medium_typeI_witness_consumer_native` and the short-gap consumer unpack the literal Type-I witnesses through the direct, powered, reflected, and Weyl machinery; `actual_typeII_dichotomy_witness_consumer` invokes `actual_typeII_powered_mhh_native`. The proof selects the positive finite power, establishes the physical `τ/k` endpoint window, preserves the original analytic multiplicity through the `Nat.clog` cover, and absorbs displacement, multiplicity, logarithmic, harmonic, coefficient-normalization, and finite-power losses in one epsilon budget. `ingham_endpoint_positive_slab_native` and `huxley_endpoint_positive_slab_native` are immediate certificate specializations. This closes `FR`, but not the later positive-slab-to-global endpoint node `ZD`.
...
The completed truncation, detector dichotomy, exact-β removal, actual-`X` Type-II powering/MHH, normalized Type-I direct/powered/Weyl routes, endpoint arithmetic, terminal control, smoothing, exact `T/N` reflection, deweighting, and multiplicity-aware branch-to-slab reduction remain valid. Weighted Littlewood, Gabriel interpolation, the standalone zeta fourth moment, and contour-edge moments are not mandatory for #15. The open path is now the `ZD` conversion from the proved endpoint positive slabs to the fully quantified Ingham and Huxley zero-density propositions.
...
| Verification | The focused analytic modules and full root graph pass without warnings. The current audit passes 7602/7602 dependency checks and the native GM output gate, and fails only on the two absent #15 full endpoints plus three later #18/#19 density outputs. The principal run preserves the honest overall `FAIL`/`1` until those outputs exist. |
...
5. **Generic twisted-fourth-moment theorem — complete.** The DFI equation chain discharges the source weights, delta localization, Bessel/Mellin transforms, and optimized error. Hughes–Young's equation-(80) consumer and finite off-diagonal assembly combine this with the smooth `ζ²` AFE and diagonal to prove the required epsilon-power moment bound.
6. **Möbius-square arithmetic specialization — complete.** `TwistedMoment.lean` proves the exact convolution, support, eventual `cutoff(T)² ≤ T^(1/22)`, divisor-count coefficient bound, and exact `|Mζ|⁴ = |ζ|⁴ |M²|²` integral reduction. `twistedZetaFourthMoment_native` now applies the completed Hughes–Young/DFI chain.
...
The remaining #18 acceptance clause is the audit-clean one-premise Goal C theorem. The analytic Type-II inputs and every whole-line convergence obligation used by the twisted moment are discharged. Principal references are [Maynard–Pratt Appendix C](https://ora.ox.ac.uk/objects/uuid%3Ab2cc1c2e-6cec-495a-ba6a-61ba08acc723/files/sj38609141), [Mathlib's proved Mellin inversion](https://github.com/leanprover-community/mathlib4/blob/master/Mathlib/Analysis/MellinInversion.lean), [Hughes–Young](https://arxiv.org/abs/0709.2345), [Duke–Friedlander–Iwaniec](https://www.math.ucla.edu/~wdduke/preprints/quadraticdiv.pdf), and [PNT+](https://github.com/AlexKontorovich/PrimeNumberTheoremAnd).

#### Historical quadratic-divisor deep-research result — 12 August 2026

This pre-implementation inventory is retained as research history and is superseded by the 22 August completion audit below. Its statements that DFI equations, the error theorem, or the Hughes–Young native moment are absent are no longer current.
...
| DFI Theorem 1 error | **Historical 12 August status:** not formalized then. **Superseding 22 August status:** complete in `exists_norm_dfiDyadicShiftedDivisorSum_sub_signedCentralSeries_le_theorem1ErrorScale_native`. |
| Hughes–Young summation and native moment | **Historical 12 August status:** not formalized then. **Superseding 22 August status:** complete through the equation-(80) and finite off-diagonal consumers, concluding `twistedZetaFourthMoment_native : TwistedZetaFourthMomentProp`. |
...
Verification at that historical research pass covered 1296 declarations and seven then-absent outputs. The current superseding audit covers 7602/7602 declarations and additionally passes `guthMaynardLargeValues_native`; five later output names remain absent.

#### DFI/Hughes–Young implementation completion — 22 August 2026

| Acceptance layer | Kernel-checked public evidence |
|---|---|
| DFI source chain | Equations (2) and (9)–(30) are formalized across the `DFI*.lean` modules, including source cutoffs, delta/Euler–Maclaurin reduction, Voronoi, Bessel/Mellin transformations, central terms, tails, and parameter optimization. |
| Exact DFI error | `exists_norm_dfiDyadicShiftedDivisorSum_sub_signedCentralSeries_le_theorem1ErrorScale_native` proves the native signed Theorem 1 bound with the published scale. |
| Hughes–Young pointwise consumer | `exists_norm_hughesYoungActiveLargeDFIPointwiseDiscrepancy_le_equation80` realizes the equation-(80) bridge. |
| Finite off-diagonal consumer | `hughesYoungConductorActiveFiniteOffDiagonal_epsilonPowerBound_native` proves the complete active finite off-diagonal epsilon-power estimate. |
| Native moment | `twistedZetaFourthMoment_native : TwistedZetaFourthMomentProp` consumes the sharp-to-smooth majorant and complete DFI/Hughes–Young assembly. |
| Audit and runner | The synchronized imported-graph audit passes 7602/7602 declarations; the completed milestones use only `propext`, `Classical.choice`, and `Quot.sound`. The output audit now passes `guthMaynardLargeValues_native` as well as the earlier milestones; it reports exit `1` on five later missing output theorems, while retained-file elaboration coverage remains a #19 verification defect. |
...
**Implementation result.** `PS`, `MR`, `FD`, `IIE`, `ZR`, `AF`, and quantitative `RF` are complete and audit-clean. `MR` exports `mediumTypeIExactBProcess_native`; `TypeIFiniteWindow.lean` supplies the corrected `N^σ` normalization, absorbed direct MHH route, exact powered route, weighted Weyl route, and final branch package. The source/Lean bijection is in `Type I Reflection Proof Bijection.md`. `GammaPairBound.lean` and `SmoothZetaAFE.lean` close the contour, ordinary-zeta normalization, coefficientwise Gamma-ratio weight, Gaussian decay, and weighted-diagonal layers of `AF`. `LargeValuesCubic.lean` closes its cubic/`S₁` package; `LargeValuesReflection.lean` and `QuantitativeSmoothReflection.lean` close exact and quantitative Lemma 6.2 respectively.

### Source-Faithful Completion Plan for Shitlist #19 — 10 August 2026

**Implementation status, updated 28 August 2026:** Packages 1–13 are complete. The exact source-entry localization, matrix/trace and Poisson packages, Proposition 5.1, quantitative reflection, native Heath–Brown, full Proposition 6.1, approximate-energy moments, Proposition 8.1 localization, Proposition 9.1 affine descent, Proposition 10.1 refined `S₃`, Propositions 11.1–11.2, and Sections 3/12 assembly culminate in `guthMaynardLargeValues_native : GuthMaynardLargeValues`. Package 14 remains open only for the concrete density outputs and retained-file evaluation coverage; its imported-graph audit and integrity portion passes at 7602/7602 declarations. The dated package descriptions below preserve the implementation decomposition, but any older “open” wording inside them is superseded by this status paragraph and the authoritative table above.
...
6. **Prove quantitative smooth reflection — complete and semantically re-audited 24 August 2026.** `LargeValuesReflection.lean` proves uniform Fourier decay, exact Mellin/Fubini rescaling, the critical oscillatory estimate, signed positive/negative reflected-mode identities, cancellation-preserving aggregate bounds, arbitrary Mellin decay, an elementary dual-tail bound, and pointwise source-uniform positive/negative core `T₀⁻¹ᐟ²` estimates. `QuantitativeSmoothReflection.lean` proves summability of the complete nonzero integer series, exact central/far decomposition, measurable core/tail assembly, both ordinate signs, the explicit scales `η = min (ε/16) (1/16)`, `H = T^η`, and `M = ⌈(1+|t|)H/N⌉`, arbitrary `T⁻ᴬ`, the `A=100` specialization, scaled trace-tail transfer, and a direct first-`S₂`-summand consumer. The audit verified the actual complete series and exact cubic factor rather than a finite proxy or theorem-equivalent hypothesis.
...
The screened Lean ecosystem supplied lower-level components rather than the target proof: Mathlib Poisson/Schwartz/matrix/energy APIs, PNT+ Mellin infrastructure, and Carleson oscillatory tools. Broad searches found no public Lean implementation of Guth–Maynard Theorem 1.1, Heath–Brown's difference-set theorem, or Proposition 9.1. Those source theorems and their final assembly are now implemented locally and kernel-checked; #19 remains open only at its later density and retained-file coverage acceptance boundaries.
...
| Audit impact | The current explicit and discovered theorem sets agree at 7602/7602. The native #15 slab, #18 DFI/Hughes–Young chain, and complete #19 Sections 3–12 large-values chain all pass with only `propext`, `Classical.choice`, and `Quot.sound`. |
| Principal verification | `logs/overall_proof_20260828_140927.log` records the fresh warning-free root and explicit production builds plus both retained examples; all integrity scans pass. Direct final-module elaboration and focused final/energy/umbrella builds also passed before the runner. The 7602/7602 dependency audit passes every existing theorem and the named GM gate passes. Stage 5 and the final runner exit remain honestly `FAIL`/`1` solely on five later agenda outputs. |
...
| Verification | The focused arithmetic, DFI/Hughes–Young, reflection, and large-values modules and root production closure build with zero warnings. The current audit synchronizes 7602/7602 declarations; all proved #15/#18/#19 foundations and `guthMaynardLargeValues_native` pass with only permitted logical dependencies. |

## Historical Files Ordered by Estimated Repair Difficulty

This section separates historical cleanup ranks from files that still carry research obligations. The synchronized 7602-declaration audit covers the root-imported `RiemannZeta.lean` project closure, including the complete native large-values chain, and the runner separately elaborates `Audit.lean`, `TestExp.lean`, and `test_separated.lean`. Retained-file coverage outside that scope remains assigned to #19 final verification. Historical deletion records remain evidence about earlier states, not descriptions of the present tree.
...
| 6 | ~~`RiemannZeta/Audit.lean`~~ | **Completed for the current root-imported graph:** the explicit and discovered sets synchronize at 7602/7602, every audited theorem has only the permitted logical dependencies, and the completion gate passes FR, the twisted moment, quantitative reflection, and native GM large values while reporting five later output names absent. All-retained-file coverage remains under #19. | Easy–moderate—scoped audit complete |
| 7 | `RiemannZeta.lean` coverage | **Historical repair completed, current regression open under #19:** the root now includes `ClassicalEndpointDensity` and the full FR dependency chain, but two retained subordinate Hughes–Young modules are outside that closure. | Easy—coverage repair required |
| 8 | Warning-producing files and `run_lake_build.bat` | **Gate behavior completed; all-file coverage open under #19:** every configured Lean stage fails on warnings and integrity scans use `--no-ignore`, but 47 top-level probes and the two unimported subordinate modules are not elaborated. | Easy–moderate—coverage repair required |
...
| 12 | ~~`RiemannZeta/GuthMaynard/HalaszMontgomery.lean`, `ZeroDetector.lean`, and `TypeIIZeros.lean`~~ | **Conditional/concrete layer completed and inputs discharged:** coverage is eventual in height and source-range restricted; the generic count is identified with the concrete analytic-multiplicity residual count; and the concrete `ResidualZeroBoundProp` follows from three explicit analytic inputs, all now supplied by native #18 theorems. | Very hard—conditional/concrete layer complete |
...
| 15 | `ClassicalEndpointDensity.lean`, `InghamBound.lean`, and the endpoint consumers | The real dichotomy, actual Type-I/Type-II branches, multiplicity, and losses now assemble into `classical_endpoint_positive_slab_native`; both certificate specializations are proved. Convert those slabs to the complete Ingham and Huxley propositions. | Very hard—open; `ZD` is next |
...
| 18 | `TwistedMoment.lean`, `SmoothZetaAFE.lean`, the `DFI*.lean` and `HughesYoung*.lean` chains, and `TypeIIZeros.lean` | Native Appendix C coverage/reduction, smooth `ζ²` AFE/diagonal, DFI equations (2) and (9)–(30), optimized signed DFI error, Hughes–Young consumers, and `twistedZetaFourthMoment_native` are proved. Only the one-premise Goal C specialization remains. | Extreme—in progress; analytic Type-II chain complete, Goal C open |
| 19 | `LargeValuesLocalization.lean` through `LargeValuesFinal.lean` | **Analytic chain complete:** localization, matrix/trace, Poisson, reflection, native Heath–Brown, full `S₂`, affine/refined-`S₃`, approximate-energy and GCD estimates, Sections 3/12, and `guthMaynardLargeValues_native` are kernel-checked. Missing are the concrete density consumers and retained-file coverage. | Hard analytic core complete; final integration/coverage remains |
...
| ~~3~~ | `TypeIFiniteEstimates.lean`, `FiniteScaleAssembly.lean`, `TypeIFiniteWindow.lean`, and a finite branch consumer | Join the actual dichotomy Type-I witness to the correctly normalized cardinality theorem and every endpoint scale branch. | **Complete:** `actual_typeI_branch_resolution_native` closes `ZR`; `FR` was subsequently completed as the separate consumer below. |
| ~~3~~ | `ClassicalEndpointDensity.lean` | Combine completed Type-I and actual-`X` Type-II bounds with multiplicity and certificates. | **Complete:** `classical_endpoint_positive_slab_native` closes `FR`; `ZD` remains separate. |
| ~~5~~ | ~~`QuadraticDivisor.lean`, `Estermann.lean`, `DivisorVoronoi.lean`, the `DFI*.lean` chain, and `HughesYoungNativeCompletion.lean`~~ | **Complete:** DFI equations (2) and (9)–(30), the optimized signed error theorem, Hughes–Young equation-(80) and finite off-diagonal consumers, and `twistedZetaFourthMoment_native`. | Extreme—complete and audit-clean. |
| ~~6~~ | Subsequent #19 modules | **Completed:** native Heath–Brown, `S₂`, energy/affine/refined-`S₃`. | Very hard—complete. |
| ~~7~~ | New #19 Heath–Brown, affine, energy, GCD/spacing, and final modules | **Completed:** remaining `S₂/S₃` chain and `guthMaynardLargeValues_native`. | Hardest analytic package—complete. |
...
- [x] Auxiliary scratch/test cleanup — **completed for the 8 August 2026 inventory:** removed the then-obsolete, failing, empty, tombstone, duplicate, and side-effectful Lean files and retained two compiling anonymous examples. This historical checkbox does not cover the 47 probes added later.
...
**Current re-audit status:** The historical Phase 0 deletion evidence remains valid, but its inventory statement is superseded. `TestExp.lean` and `test_separated.lean` elaborate with exit code 0 and no warnings; 47 later tracked top-level probe files remain outside runner elaboration and must be handled by #19 final verification.
...
1. **Completed for the production perimeter and kept synchronized through 28 August 2026:** the explicit, synchronization-checked list currently contains 7602 exported source-level theorems.
2. **Completed for the 8 August 2026 inventory:** imported every then-intended production module into the audit after making the three previously omitted modules compilable. The two current subordinate import omissions are tracked under #19 final verification.
...
6. **Completed for the 8 August 2026 inventory:** extended the root import graph to the then-intended production modules after their compilation repairs; current closure repair remains under #19.
7. **Completed during reopened #2 for that inventory:** removed the obsolete auxiliary named declarations and directly elaborated the two retained examples. Later probes are not covered by this historical result.
...
The synchronized audit covers every named public theorem in the current root-imported graph, including the full FR chain. It does not cover the two subordinate modules outside that graph or establish elaboration of the 47 top-level probes.
...
**Current re-audit status:** The configured warning gates pass, but Phase 1A no longer has all-retained-file evidence. The 49-file elaboration gap identified on 22 August 2026 must be closed under #19 before the runner can serve as repository-wide proof evidence.
...
3. **Completed:** source localization, cubic trace split, `S₁`, quantitative Lemma 6.2, native Heath–Brown, complete `S₂`, affine transformation, `S₃`, and the GCD/spacing energy assembly.
4. **Completed:** `guthMaynardLargeValues_native` assembles the exact `GuthMaynardLargeValues` statement used by Central Type I and the transfer.
...
| Honest analytic boundary | At the #12 repair, the coverage boundary was corrected to `TypeIContourTypeIICoverOnProp (7/10)` while coverage and `TwistedZetaFourthMomentProp` remained explicit inputs. Later #18 work proved `typeIContourTypeIICoverOn_native`, `typeIIFourthMomentReduction_native`, and `twistedZetaFourthMoment_native`, so no analytic input remains open in this residual chain. |
...
| Remaining obstruction | None within #12's residual Type-II chain: native source coverage, Appendix C reduction, and the twisted fourth moment now discharge all three inputs. The separate #18 Goal C specialization remains open because it also requires #15 Huxley and #19 large values. |
...
| Coverage | At that iteration the tree contained 44 Lean files and a 41-file production graph. That later 83-file superseding inventory is itself obsolete: the 22 August inventory has 335 tracked Lean files, a 283-module root closure, two subordinate modules outside it, and 47 top-level probes outside runner elaboration. |
...
| F-03 Type I/Type II | **Complete conditionally, with all analytic inputs discharged.** The exact partition is assembled and the residual bound is derived internally. | The interface exposes three source-facing Type-II inputs. Native theorems now discharge coverage, the fourth-moment reduction, and the twisted fourth moment. |
...
| F-09 large-values case | **Complete unconditionally at the native input boundary.** The normalized fixed block is passed to `guthMaynardLargeValues_native` and every loss is absorbed. | The former sole unresolved Central Type-I premise is discharged. |
...
| Honest remaining boundary | #14 is **complete conditionally, not yet a concrete full-range density theorem**. Native theorems now discharge Guth–Maynard large values and the twisted fourth moment; Huxley remains the only unresolved primitive analytic input. `TypeIPositiveSlabBoundProp` is a derived internal intermediate, and `DyadicToGlobalZeroCountProp` has been deleted. |
...
- #6's synchronized transitive audit covers every named public theorem in the root-imported graph, including FR; two retained subordinate modules and 47 top-level probes are outside its elaborated scope;
- #8's warning gate covers both designated examples and uses ignore-resistant integrity scans, but retained-file elaboration coverage has regressed and is assigned to #19 final verification;
...
- #12 now reaches the concrete `ResidualZeroBoundProp` from honest source-facing inputs; native #18 theorems discharge all three. #13's conditional/normalized extraction layer is fully specialized by the proved #15 unit-zero and #16 beta-removal inputs.
- #14 removes the circular conclusion-equivalent assumption and completes the primitive-input conditional transfer; native Guth–Maynard large values and the twisted moment are now discharged, leaving native Huxley assigned to #15.
...
1. **#15 — Classical finite zero-density endpoints (in progress).** The dichotomy, common extraction, exact powering infrastructure, arbitrary-`X` Type-II application, coefficient growth, endpoint arithmetic, terminal Type-I estimate, smoothing, medium B-process, Mellin reflection, deweighting, normalized direct/powered/Weyl Type-I package, and the multiplicity-aware positive-slab consumer are proved. The corrected scale is `τ=log_N T`, satisfying `N^τ=T`, and MHH is applied at the correct threshold `N^σ V`. `IIE`, `ZR`, and `FR` are complete; `ZD`, the conversion to the full Ingham and Huxley propositions, is next. This classical package is distinct from #19's `GuthMaynardLargeValues`.
2. **#18 — Type II and Goal C integration.** All analytic inputs, including `twistedZetaFourthMoment_native` and `guthMaynardLargeValues_native`, are complete. Construct the concrete `guthMaynardZeroDensity_of_largeValues_native` consumer after #15 supplies Huxley.
3. **#19 — Final integration.** The entire Guth–Maynard Sections 3–12 analytic chain and `guthMaynardLargeValues_native` are proved. Derive `guthMaynardZeroDensity_native` and `combined_zero_density_native`, restore retained-file evaluation coverage, and make the principal runner pass.
