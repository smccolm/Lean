...
| `Audit.lean` checks transitive dependencies for the synchronized production theorem set. | #6 | **Complete for the cleaned tree's named public theorems:** the current list is 857/857, including the expanded vendored PNT+ sharp-zeta dependency closure, finite-density transfer/powering/dichotomy surface, endpoint and scale arithmetic, medium-reflection API, and terminal Type-I estimate; the only auxiliary Lean declarations are anonymous examples. The audit correctly remains red on one project-axiom-dependent declaration. |
...
| `l2_parabola_incidence_bound` is false for arbitrary finite 1-separated sets without a containing interval, and `DecouplingProp` has an off-by-one counterexample. | #19 | **Deletion required:** Guth–Maynard does not use this decoupling model. Replace it with the source sampling-matrix/Poisson/reflection/approximate-energy chain. |
...
| The paper stated stale audit counts. | Documentation gate | **Resolved during #8 and kept synchronized:** the reproducibility section now reports one failure across the current 857 declarations. |

Current clean facts are narrower: the production graph and both retained examples elaborate with zero warnings under the five-stage principal runner; the production audit synchronizes 857 declarations and reports one forbidden dependency failure; and the source contains two direct project axioms, both in `Decoupling.lean`. The runner covers every retained Lean file but correctly exits nonzero on the audit and axiom gates.
...
| 2 | **Verified complete as cleanup** | The retained tree has 43 Lean files, no empty Lean file, a 40-file transitive production graph, and only two non-production anonymous-example files. All listed obsolete tests, tombstones, and side-effect utilities are absent. |
...
| 6 | **Verified complete as audit infrastructure** | The explicit and discovered theorem sets agree at 857/857 and transitive dependencies are checked. One failure is reported rather than hidden. |
| 7 | **Verified complete** | The root import closure reaches all 39 subordinate production modules, giving a 40-file production graph. |
...
| 18 | **Type-II residual analysis and Goal C integration** | Repair the coverage contract to the source range actually consumed by the transfer (`Re ρ ≥ 7/10`), then prove uniform vertical Gamma decay and whole-line contour integrability, Maynard–Pratt Appendix C coverage, the concrete fourth-moment reduction with `log³ T`-separated weighted extraction, and `TwistedZetaFourthMomentProp`. The moment route proves a generic short-polynomial upper-bound corollary of Hughes–Young through a smooth `ζ²` approximate functional equation, diagonal analysis, and a genuine Duke–Friedlander–Iwaniec quadratic-divisor/off-diagonal theorem, then specializes it to `M²`. Finally specialize `conditionalZeroDensityTransfer` using #15–#18 so its only remaining premise is `GuthMaynardLargeValues`. | The repaired source-range coverage theorem and the two existing Type-II propositions are native, audit-clean theorems; every whole-line integral has an explicit integrability proof; and a public audited theorem derives the concrete full-range zero-density result from `GuthMaynardLargeValues` alone. No other analytic or arithmetic proposition parameter remains. |
| 19 | **Guth–Maynard large values and final project integration** | Delete the two false `Decoupling.lean` contracts and their axioms; replace that abandoned model with the source proof: sampling matrix and singular values, trace-cube and Poisson reduction, `S₁`/`S₂`/`S₃`, smooth reflection, the Heath–Brown difference-set estimate, affine transformations, approximate additive energy, and the GCD/spacing assembly. Prove `GuthMaynardLargeValues`, apply the #18 theorem and existing combined transfer, and close every audit and runner gate. | The concrete Guth–Maynard and combined zero-density theorems are project-theorem complete; all direct-axiom and prohibited-proof scans are empty; all public audits pass; every build is warning-free; and `run_lake_build.bat` returns `PASS`/`0`. |
...
| Verification | The focused terminal module and root production build pass without warnings. Principal run `logs/overall_proof_20260810_010725.log` passes stages 1–4 warning-free and synchronizes 857/857 declarations; every new dichotomy and terminal theorem passes with only permitted logical dependencies. Its sole audit failure and the two direct-axiom findings are the unrelated, tracked #19 decoupling obligations. |
...
1. **Repair the coverage boundary before proving it.** The current `TypeIContourTypeIICoverProp` quantifies over all zeros with `Re ρ ≥ 1/2`, while the Appendix C proof begins at `Re ρ ≥ 1/2 + 1/log T`; the transfer only consumes zeros with `Re ρ ≥ 7/10`. Introduce a reusable `TypeIContourTypeIICoverOnProp σ₀`, prove it at `σ₀ = 7/10`, and make the finite coverage bridge consume that source-faithful contract. The stronger old proposition may remain only if its additional low-real-part branch is separately proved.
2. **Uniform vertical Gamma decay and honest integrability.** On logarithmic coordinates,
   `Gamma (a + it)` is the Fourier transform of `x ↦ exp (-exp x) * exp (a*x)`. Prove uniform `Cₖ |t|⁻ᵏ` bounds on a positive compact strip by repeated integration by parts, then transport them to the required strips with Gamma recurrence. Derive explicit `Integrable` and tail-limit theorems for `typeIIContourIntegrand`; the totalized Bochner integral is not itself evidence of convergence. PNT+ supplies strip bounds but not this decay theorem.
...
4. **Scaled separated extraction and fourth-moment reduction.** Generalize the existing one-separated weighted selection theorem to arbitrary real width `G ≥ 1`, specialize to `G=(log T)³`, and prove bounded overlap of the radius-`(log T)²` translated windows. Combine Gamma truncation, Hölder, the native local analytic-multiplicity bound, and this extraction theorem to derive the concrete `TypeIIFourthMomentReductionProp` with every logarithmic loss absorbed through `EpsilonPowerBound`.
...
6. **Möbius-square specialization.** Define the coefficients of `M²`, prove the exact convolution expansion, support at the squared detector cutoff, the eventual inequality `cutoff(T)² ≤ T^(1/22)`, and the needed coefficient bound from #17. Apply the generic theorem to prove `twistedZetaFourthMoment_native : TwistedZetaFourthMomentProp`.
...
Acceptance requires native proofs of all three Type-II inputs after the coverage interface repair, explicit convergence for every whole-line integral, no theorem-equivalent premise hidden behind a generic wrapper, and an audit-clean one-premise Goal C theorem. Principal references are [Maynard–Pratt Appendix C](https://ora.ox.ac.uk/objects/uuid%3Ab2cc1c2e-6cec-495a-ba6a-61ba08acc723/files/sj38609141), [Mathlib's proved Mellin inversion](https://github.com/leanprover-community/mathlib4/blob/master/Mathlib/Analysis/MellinInversion.lean), [Hughes–Young](https://arxiv.org/abs/0709.2345), the stronger but less suitable [Bettin–Bui–Li–Radziwiłł route](https://arxiv.org/abs/1609.02539), and [PNT+](https://github.com/AlexKontorovich/PrimeNumberTheoremAnd). The realistic difficulty order is Gamma decay, scaled separation/reduction, contour coverage, then the twisted fourth moment; the last package is expected to exceed the other three combined.

### Source-Faithful Completion Plan for Shitlist #19 — 10 August 2026

The two current `Decoupling.lean` interfaces are not valid weakened forms of Guth–Maynard. `l2_parabola_incidence_bound` permits an arbitrary finite separated set while its bound has no dependence on a containing interval or height. `DecouplingProp` also has an off-by-one counterexample: take `K=N` and support the coefficient at `N+1`; the left side is positive while its block family begins at `N+2`, so the right side is zero. Guth–Maynard's paper does not use a parabola-decoupling theorem. Both declarations must therefore be deleted, not cosmetically repaired or replaced by an unrelated easy block inequality.

The replacement is divided into independently auditable packages:

1. **Contract deletion and source interfaces:** delete both axioms and every dependent false theorem; state the actual finite smooth Dirichlet-polynomial large-value problem and its sampling matrix `M_W`.
2. **Matrix reduction:** formalize the singular-value/trace-cube argument of Sections 3–4, including the finite matrix identities and the reduction to three correlation sums.
3. **Poisson and `S₁`:** use Mathlib's Poisson-summation API to prove the source Fourier expansion and dispose of the rapidly decaying term.
4. **Smooth reflection and `S₂`:** prove Guth–Maynard Lemma 6.2 as a genuine smooth approximate functional equation, formalize the Heath–Brown difference-set mean-square estimate it consumes, and close the `S₂` bound.
5. **Affine transformations and `S₃`:** formalize the transformations in Sections 7–10, their Jacobian/range bookkeeping, and the resulting correlation estimates.
6. **Approximate energy and arithmetic assembly:** define tolerance-one additive energy for real ordinates—Mathlib's existing additive-energy API concerns exact group equalities—and prove the Section 11 smoothing, discrete third moment, GCD decomposition, and spacing bounds.
7. **Large-values theorem:** assemble Sections 3 and 12 into the exact quantified and sign-correct `GuthMaynardLargeValues` proposition already consumed by Central Type I.
8. **Final integration:** apply the #18 Goal C specialization and existing combined-exponent transfer, synchronize the audit and documents, and require `run_lake_build.bat --no-pause` to return `PASS`/`0` with zero warnings.

Useful formal infrastructure exists in [Mathlib Poisson summation](https://leanprover-community.github.io/mathlib4_docs/Mathlib/Analysis/Fourier/PoissonSummation.html), [Mathlib singular values](https://leanprover-community.github.io/mathlib4_docs/Mathlib/Analysis/InnerProductSpace/SingularValues.html), and [Mathlib additive energy](https://leanprover-community.github.io/mathlib4_docs/Mathlib/Combinatorics/Additive/Energy.html). The source proof is [Guth–Maynard](https://arxiv.org/html/2405.20552v2). The Carleson project's [van der Corput development](https://github.com/fpvandoorn/carleson/blob/master/Carleson/Classical/VanDerCorput.lean) may help with nonstationary tails, but it does not supply stationary phase, the smooth reflection lemma, or the large-values theorem. No screened public Lean repository contains a complete substitute.
...
| Audit impact | The current explicit and discovered theorem sets agree at 857/857. Every beta/localization theorem, `extractSeparated_native`, every classical-moment theorem, the complete vendored PNT+ proof closure, the project argument-principle specialization, the #17 arithmetic theorems, and the #15 sharp-truncation, finite-transfer, dichotomy, powering, Weyl, kernel, classical-large-values, endpoint-range, finite-scale, medium-reflection, and terminal theorems pass with only `propext`, `Classical.choice`, and `Quot.sound`. The only audit failure is the unrelated `l2_decoupling_bound_native`. |
| Principal verification | `logs/overall_proof_20260810_010725.log` passes stages 1–4 warning-free, synchronizes 857/857 declarations, and reports only the single decoupling dependency failure. Its integrity scan finds no prohibited admitted-proof text or unsafe bypass and exactly the two tracked #19 `Decoupling.lean` axioms, so it remains honestly `FAIL`/`1` until #19. |
...
| Verification | The focused arithmetic module and the current root production closure build with zero warnings. The audit synchronizes 857/857 declarations; all five #17 declarations pass with only permitted logical dependencies. The remaining failures are the two pre-existing #19 decoupling axioms and their single transitive audit failure. |
...
| 6 | ~~`RiemannZeta/Audit.lean`~~ | **Completed for all named public theorems in the cleaned tree:** transitive checks cover the synchronized 857-theorem list, including the globally named public declarations in the vendored modules; the only auxiliary declarations are anonymous examples. One current dependency failure is reported honestly. | Easy–moderate—complete |
...
| 15 | Classical finite zero-density engine and the Ingham/Huxley endpoints | See the authoritative three-item completion contract and four-package #15 plan above. | Very hard—open |
...
| 18 | Type-II residual analysis and Goal C integration | See the authoritative completion contract and seven-step researched #18 plan above. | Extreme—open |
| 19 | Large values and final integration | See the authoritative three-item completion contract above. | Hardest—open |
...
1. **Completed for the production perimeter and kept synchronized through 10 August 2026:** created an explicit, synchronization-checked list, currently containing 857 exported source-level theorems.
...
| Honest analytic boundary | The current overbroad `TypeIContourTypeIICoverProp`, `TypeIIFourthMomentReductionProp`, and `TwistedZetaFourthMomentProp` remain unproved proposition specifications. #18 must repair coverage to `TypeIContourTypeIICoverOnProp (7/10)` and adapt the conditional bridge. The reduction records the Appendix C Gamma-decay, integrability, Hölder, scaled-extraction, and local-multiplicity work; the concrete theorem only composes those inputs and does not prove them. |
...
| Coverage | The current tree contains 43 Lean files. The root build covers its 40-file transitive production graph, the audit is executed explicitly, and the two remaining files are executed explicitly as examples. No retained Lean file is outside the human-facing evaluation. |
...
| Remaining obstruction at that iteration | #8 had no remaining warning or retained-file coverage defect. The current superseding count is one audited dependency failure and two direct project axioms. |
...
| F-04 beta removal | **At this pre-implementation audit:** `DetectorBetaShiftProp` was source-facing but unproved. | Goal B accepted it explicitly at that stage. Later #16 deleted the malformed declarations and proved the proposition by rational Phragmén–Lindelöf localization. |
...
Exactly three items remain, in dependency order:
...
2. **#18 — Type II and Goal C integration.** Repair the coverage boundary; prove uniform vertical Gamma decay and contour integrability, source-range Maynard–Pratt coverage, scaled extraction and the concrete fourth-moment reduction, and a generic Hughes–Young-range twisted moment through the required quadratic-divisor theorem; specialize it to `M²`, then reduce the public zero-density theorem to the single premise `GuthMaynardLargeValues`.
3. **#19 — Large values and final integration.** Delete the two false decoupling contracts and axioms; prove the source sampling-matrix, Poisson, reflection, Heath–Brown, affine-transformation, and approximate-energy chain; prove `GuthMaynardLargeValues`; derive the concrete Guth–Maynard and combined zero-density theorems; and make the principal runner pass.
