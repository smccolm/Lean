...
| 6 | ~~`RiemannZeta/Audit.lean`~~ | **Completed for all named public theorems in the cleaned tree:** transitive checks cover the synchronized list, currently 140 production theorems; the only auxiliary declarations are anonymous examples. | Easy–moderate—complete |
...
1. **Completed for the production perimeter 8 August 2026:** created an explicit, synchronization-checked list, currently containing 140 exported source-level theorems.
...
| Audit impact | Added eight public theorems. The synchronized transitive audit now lists and discovers 140/140 declarations; all eight additions pass with only `propext`, `Classical.choice`, and `Quot.sound`. The same six unrelated dependency failures and fourteen direct project axioms remain. |
