...
| `Audit.lean` checks transitive dependencies for the synchronized production theorem set. | #6 | **Complete for the cleaned tree's named public theorems:** the current list is 308/308; the only auxiliary Lean declarations are anonymous examples. The audit correctly remains red on one project-axiom-dependent declaration. |
...
| 6 | **Verified complete as audit infrastructure** | The explicit and discovered theorem sets agree at 308/308 and transitive dependencies are checked. One failure is reported rather than hidden. |
...
|---:|---|---|---|
| 15 | **Zeta zero-count foundations and classical density endpoints — in progress** | `ClassicalDensity.lean` now proves the actual finite Möbius mollifier, exact convolution and tail cancellation, a globally analytic pole-free Ingham detector, multiplicity inheritance, a detector-to-Jensen divisor bridge, a full unit-bin Jensen estimate down to `σ = 1/2`, an explicit dyadic `O(T log T)` bound, and the symmetric Ingham boundary cases `σ = 1/2, 1` plus the Huxley boundary `σ = 1`. **Still required:** the multiplicity-aware Littlewood rectangle theorem, finite Gabriel vertical-integral convexity, critical-line fourth moment and mollifier boundary moments, the full Montgomery–Halász–Huxley `T * N^4 * V⁻⁶` branch, the interior endpoint assemblies, and the Huxley `σ = 3/4` boundary. | All 39 new public theorems are in the root graph and transitive audit. The item remains uncrossed until audit-clean theorems prove both `HuxleyZeroDensity (fun σ T => N σ T)` and `InghamZeroDensity (fun σ T => N σ T)`. The basic `halasz_montgomery_lemma_native` and the newly proved boundary cases alone do not satisfy this test. |
...
The foundation and mean-value subpasses have removed the final `ZeroCount.lean` axiom, proved the local-zero theorem, supplied the unconditional Montgomery mean-value and basic Halász–Montgomery inputs, and now added the mollifier/detector algebra and exact boundary estimates described above. They have not proved the stronger classical `N^4 * V⁻⁶` large-values branch or either complete endpoint proposition. The #16 pass has also proved beta removal and unconditional separated extraction. The synchronized audit now covers 308 declarations and has only the unrelated decoupling failure. The repository remains globally incomplete until #15 and #17–#19 pass their own tests.
...
| Verification | The focused `ClassicalDensity.lean` elaboration and full production build pass with no warnings or tactic diagnostics. The latest principal run `logs/overall_proof_20260809_000419.log` passed stages 1–4 warning-free, synchronized 308/308 declarations, reported only the unrelated decoupling audit failure, and exited `1`. |
...
| Audit impact | The explicit and discovered theorem sets agree at 308/308. Every beta/localization theorem and `extractSeparated_native` passes with only `propext`, `Classical.choice`, and `Quot.sound`. The only audit failure is the unrelated `l2_decoupling_bound_native`. |
| Principal verification | The current evidence is `logs/overall_proof_20260809_000419.log`: stages 1–4 pass warning-free, while stage 5 reports the synchronized 308-theorem audit and the single decoupling failure. The prohibited-proof and unsafe scans pass; the direct-axiom scan finds only the two #19 `Decoupling.lean` axioms. Overall exit code remains honestly `1`. |
...
| 6 | ~~`RiemannZeta/Audit.lean`~~ | **Completed for all named public theorems in the cleaned tree:** transitive checks cover the synchronized 308-theorem list; the only auxiliary declarations are anonymous examples. One current dependency failure is reported honestly. | Easy–moderate—complete |
...
1. **Completed for the production perimeter and kept synchronized through 9 August 2026:** created an explicit, synchronization-checked list, currently containing 308 exported source-level theorems.
