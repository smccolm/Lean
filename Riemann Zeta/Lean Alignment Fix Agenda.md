...
| `Audit.lean` checks transitive dependencies for the synchronized production theorem set. | #6 | **Complete for the cleaned tree's named public theorems:** the current list is 1153/1153, including the #15 exact medium B-process, uniform summed far tail, smooth extraction, Poisson geometry, Mellin reflection, and two-sided deweighting surfaces. Every declaration passes with only the permitted logical dependencies. |
...
| The paper stated stale audit counts. | Documentation gate | **Resolved during #8 and kept synchronized:** the current audit passes across 1153 synchronized declarations. |

Current clean facts: the production graph and both retained examples elaborate with zero warnings; the dependency audit synchronizes and passes 1153 declarations; and all prohibited-proof and project-postulate scans are empty. The named output gate still reports seven absent end theorems; the overall result remains honestly `FAIL`/`1` until the remaining outputs exist.
...
| 6 | **Verified complete as audit infrastructure** | The explicit and discovered theorem sets agree at 1153/1153 and all transitive dependency checks pass. The separate completion gate fails on the seven absent #15/#18/#19 outputs. |
...
|---:|---|---|---|
| 15 | **Classical finite zero-density engine and endpoints — in progress** | The Type-I B-process is now complete in `mediumTypeIExactBProcess_native`: interior source boundaries are removed exactly, scaled Poisson summation yields a finite symmetric dual window, retained negative modes have the exact common Mellin-polynomial formula, and the complete two-sided far tail has a uniform `Q^-101 M^-100` bound. Smooth extraction and both deweighting identities are also complete. The remaining Type-I work is finite-window resolution of the zero/wrong-sign central modes and certificate consumption in `ZR`; the actual-`X` Type-II application, branch-to-slab reduction, and both endpoints also remain. | The item stays uncrossed until audit-clean theorems prove both `HuxleyZeroDensity (fun σ T => N σ T)` and `InghamZeroDensity (fun σ T => N σ T)`. |
...
8. **Verification and scope boundary:** the focused Type-I build is warning-free. The exact B-process, uniform summed tail, smoothing, Mellin reflection, and deweighting declarations are registered in the synchronized 1153-declaration transitive audit and use only `propext`, `Classical.choice`, and `Quot.sound`. The finite central-window consumer and later branch-to-slab theorem remain open.
...
| Downstream work | Concrete Huxley and Ingham remain unproved. Exact-β removal, the dichotomy, common extraction, normalization, exact powering, the `X=1` theorem, coefficient growth, endpoint arithmetic, terminal restriction, source smoothing/common-scale extraction, the exact medium B-process with uniform summed tail, and both deweighting identities are complete. The retained finite-window resolution, actual-`X` Type-II application, range resolution, branch-to-slab consumer, and symmetric specialization remain. |
| Verification | The focused Type-I module passes without warnings. The current audit passes 1153/1153 dependency checks but fails the output gate on the two absent #15 endpoints and five other agenda outputs. |
...
3. ~~**#15 Type-I interface chain.**~~ **Closed 11 August 2026.** `TypeISmoothing.lean` closes source-faithful common smooth-block extraction; `TypeIReflection.lean` proves exact boundary removal, scaled Poisson summation, the symmetric finite dual window, exact Mellin retained-mode assembly, and a uniform summed `Q^-101 M^-100` far tail; `TypeIFourierDeweight.lean` closes both deweighting identities. The next open task is the finite central-window/certificate consumer in `ZR`, followed by the slab reduction.
...
**Implementation result.** `PS`, `MR`, and `FD` are complete and audit-clean. `MR` exports `mediumTypeIExactBProcess_native`, with exact source-boundary removal, the complete finite-window/far-tail Poisson split, and a summed uniform remainder; the exact retained-mode Mellin polynomial remains its algebraic identification. The complete source/Lean bijection and normalization correction are in `Type I Reflection Proof Bijection.md`. `TwistedDiagonal.lean` proves #18's finite `hm = kn` phase cancellation, while `LargeValuesReflection.lean` and `LargeValuesCubic.lean` close their stated #19 packages.
...
| Audit impact | The current explicit and discovered theorem sets agree at 1153/1153. Every beta/localization theorem, `extractSeparated_native`, every classical-moment theorem, the vendored PNT+ proof closure, native #18 coverage and reduction, and the #15/#18/#19 closeout foundations pass with only `propext`, `Classical.choice`, and `Quot.sound`. |
| Principal verification | `logs/overall_proof_20260811_113043.log` passes all build/example stages warning-free, passes 1153/1153 dependency checks, and finds no admitted proof, unsafe bypass, or project postulate. The named output gate fails on seven absent agenda theorems, so the overall result remains honestly `FAIL`/`1`. |
...
| Verification | The focused arithmetic module and root production closure build with zero warnings. The current audit synchronizes 1153/1153 declarations; all five #17 declarations and the #15/#18/#19 foundations pass with only permitted logical dependencies. |
...
| 6 | ~~`RiemannZeta/Audit.lean`~~ | **Completed for all named public theorems in the current tree:** the explicit and discovered sets synchronize at 1153/1153, every existing theorem has only the permitted logical dependencies, and the separate completion gate reports seven absent output names. | Easy–moderate—complete |
...
| 15 | `TypeISmoothing.lean`, `TypeIReflection.lean`, `TypeIFourierDeweight.lean`, `MediumReflection.lean`, and the finite-density consumers | Smooth extraction, exact medium B-process (including the summed far tail), Mellin retained-mode reflection, and short/reflected deweighting are complete. Add the finite central-window/certificate consumer, actual-`X` Type-II theorem, branch-to-slab reduction, and endpoints. | Very hard—open; the analytic transform is complete and finite integration remains |
...
| 3 | `RiemannZeta/GuthMaynard/FiniteDensityEndpoint.lean` and the Type-I range consumer | Consume `mediumTypeIExactBProcess_native`: bound/select within its finite central window, route zero/wrong-sign members, and apply the endpoint certificate and existing MHH/Weyl estimates. | Hard finite integration; the source boundary and complete far tail are already discharged by `MR`. |
...
1. **Completed for the production perimeter and kept synchronized through 11 August 2026:** created an explicit, synchronization-checked list, currently containing 1153 exported source-level theorems.
...
| Remaining obstruction at that iteration | #8 had no remaining warning or retained-file coverage defect. The superseding 11 August audit has 1153/1153 passes and zero direct project axioms. |
