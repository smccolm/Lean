...
| `Audit.lean` checks transitive dependencies for the synchronized root-imported theorem set. | #6 | **Complete as scoped integrity infrastructure:** the current list is 5694/5694. It includes the full DFI/Hughes–Young chain and `twistedZetaFourthMoment_native`. Every audited declaration passes with only the permitted logical dependencies; this does not make the six missing output theorems or the 49-file coverage gap complete. |
...
| The paper stated stale audit counts. | Documentation gate | **Resolved during #8 and kept synchronized:** the current audit passes across 5694 synchronized declarations. |

Current scoped facts: the root-imported production graph elaborates with zero warnings; its dependency audit synchronizes and passes 5694 declarations; and the repository-wide prohibited-proof and direct-postulate scans are empty. The named output gate passes `twistedZetaFourthMoment_native` and reports six absent end theorems. The runner still lacks elaboration coverage for 47 tracked top-level probes and two subordinate Hughes–Young modules outside the root closure; that regression is assigned to #19 final verification.
...
| 6 | **Verified complete as scoped audit infrastructure** | The explicit and discovered theorem sets agree at 5694/5694 within the root-imported graph and all checks in that scope pass. The separate completion gate fails on the six absent #15/#18/#19 outputs. |
| 7 | **Historical root-import repair verified; current regression open in #19** | The present root closure contains 283 project modules including the root. `HughesYoungEquation96CoprimeBound.lean` and `HughesYoungZeroShiftC.lean` are retained but outside it. |
...
| ~~2~~ | 15 | **Actual Type-I finite absorption and endpoint routing — complete** | `normalizedClassicalTypeICoeff` supplies the missing `N^σ` normalization; `actual_typeI_dichotomy_witness_mhh_absorbed_native` absorbs the harmonic loss into `(3T)^ε`; `powered_actual_typeI_block_large_values_bound` realizes exact two/three-fold powering; and the weighted Weyl and endpoint-certificate routes are packaged by `actual_typeI_branch_resolution_native`. | Kernel-checked and included in the synchronized 5694-declaration audit. This closes `ZR`; threshold substitution, multiplicity, and the slab estimate remain the separate `FR` node. |
| ~~3~~ | 19 | **Source-to-matrix entry bridge — complete** | `sourceDirichletPoly_eq_three_gmSmooth` proves an exact three-piece source identity; `source_large_values_localize_to_matrix` selects one fixed scale/coefficient family on at least one third of the ordinates and applies the matrix bound. The published interval is already `(N,2N]`; a separate theorem records compatibility with the alternate closed convention. | Kernel-checked and included in the 5694-declaration audit. |
...
| Verification | The focused Type-I and full DFI/Hughes–Young modules pass without warnings. The current audit passes 5694/5694 dependency checks and fails the output gate on the two absent #15 endpoints and four other agenda outputs. The principal run preserves the honest overall `FAIL`/`1` until those outputs exist. |
...
Verification at that historical research pass covered 1296 declarations and seven then-absent outputs. The current superseding audit covers 5694/5694 declarations and passes `twistedZetaFourthMoment_native`; six unrelated output names remain absent.
...
| Audit and runner | The synchronized imported-graph audit passes 5694/5694 declarations; the four milestones above use only `propext`, `Classical.choice`, and `Quot.sound`. `run_lake_build.bat --no-pause` passes its configured build, example, integrity, and `twistedZetaFourthMoment_native` gates. The current log is `logs/overall_proof_20260822_215218.log`; it reports exit `1` on six unrelated missing output theorems, while the separately recorded 49-file elaboration-coverage gap remains a #19 verification defect not detected by that run. |
...
| Audit impact | The current explicit and discovered theorem sets agree at 5694/5694. Every beta/localization theorem, `extractSeparated_native`, the vendored PNT+ proof closure, the complete DFI/Hughes–Young chain, and the #15/#18/#19 proved foundations pass with only `propext`, `Classical.choice`, and `Quot.sound`. |
| Principal verification | The current focused audit synchronizes and passes 5694/5694 existing declarations. Its named output gate passes `twistedZetaFourthMoment_native` and fails on six absent agenda theorems, so the overall result remains honestly `FAIL`/`1`. |
...
| Verification | The focused arithmetic, paired-Gamma, smooth-AFE, DFI/Hughes–Young modules and root production closure build with zero warnings. The current audit synchronizes 5694/5694 declarations; all #17 declarations and the proved #15/#18/#19 foundations pass with only permitted logical dependencies. |

## Historical Files Ordered by Estimated Repair Difficulty

This section separates historical cleanup ranks from files that still carry research obligations. The synchronized 5694-declaration audit covers the 283-module `RiemannZeta.lean` project closure, and the runner separately elaborates `Audit.lean`, `TestExp.lean`, and `test_separated.lean`. It does not elaborate 47 other tracked top-level probes or the retained subordinate modules `HughesYoungEquation96CoprimeBound.lean` and `HughesYoungZeroShiftC.lean`; closing that coverage gap belongs to #19 final verification. Historical deletion records remain evidence about earlier states, not descriptions of the present tree.
...
| 6 | ~~`RiemannZeta/Audit.lean`~~ | **Completed for the current root-imported graph:** the explicit and discovered sets synchronize at 5694/5694, every audited theorem has only the permitted logical dependencies, and the separate completion gate reports six absent output names. All-retained-file coverage remains under #19. | Easy–moderate—scoped audit complete |
| 7 | `RiemannZeta.lean` coverage | **Historical repair completed, current regression open under #19:** the root now reaches 283 project modules including itself, but two retained subordinate Hughes–Young modules are outside that closure. | Easy—coverage repair required |
...
1. **Completed for the production perimeter and kept synchronized through 22 August 2026:** created an explicit, synchronization-checked list, currently containing 5694 exported source-level theorems.
...
The synchronized audit covers every named public theorem in the current 283-module root-imported graph. It does not cover the two subordinate modules outside that graph or establish elaboration of the 47 top-level probes.
...
- #6's synchronized transitive audit covers every named public theorem in the 283-module root-imported graph; two retained subordinate modules and 47 top-level probes are outside its elaborated scope;
