...
| `Audit.lean` checks transitive dependencies for the synchronized production theorem set. | #6 | **Complete for the cleaned tree's named public theorems:** the current list is 827/827, including the expanded vendored PNT+ sharp-zeta dependency closure, finite-density transfer/powering surface, endpoint and scale arithmetic, and medium-reflection API; the only auxiliary Lean declarations are anonymous examples. The audit correctly remains red on one project-axiom-dependent declaration. |
...
| 6 | **Verified complete as audit infrastructure** | The explicit and discovered theorem sets agree at 827/827 and transitive dependencies are checked. One failure is reported rather than hidden. |
...
|---:|---|---|---|
| 15 | **Classical finite zero-density engine and endpoints — in progress** | Existing work proves exact-β removal for the sharp finite Möbius product, multiplicity-aware common dyadic-witness extraction, bounded-coefficient normalization, exact finite power raising with #17 coefficient control, unrestricted finite MHH application, weighted Weyl bounds for every `σ ≥ 0`, endpoint-range arithmetic, and the finite Corollaries 11.8–11.10 scale/exponent certificates. The remaining analytic reduction is separately identified: complete the finite Type-I/Type-II detector dichotomy and short Type-II product, medium Type-I reflection, terminal estimate, consumption of the completed assembly certificates, and the two endpoints. | The item stays uncrossed until audit-clean theorems prove both `HuxleyZeroDensity (fun σ T => N σ T)` and `InghamZeroDensity (fun σ T => N σ T)`. |
...
3. **Finite detector, normalization, and powering — substantial completion; dichotomy open:** `ClassicalDetector.lean` proves the finite zeta–Möbius cancellation and raw tail witness. `FiniteDensityTransfer.lean` realizes the tail as a finite Dirichlet polynomial, proves coefficient-mass bounds, extracts a common dyadic block with analytic multiplicity, normalizes its coefficients, and applies unrestricted finite MHH. `ClassicalPowering.lean` and `FiniteDensityEndpoint.lean` prove exact finite power raising, #17 factorization bounds, normalization, common powered-block extraction, and the corresponding MHH cardinality estimate. What remains is the source-faithful finite Type-I/Type-II dichotomy: a long ordinary-zeta Type-I branch and a genuinely short Type-II branch formed by multiplying a short zeta sum by a short Möbius polynomial.
4. **Endpoint-range reduction — arithmetic complete; dependency interpretation corrected:** `FiniteDensityExponents.lean` proves `ingham_zeta_polynomial_range_empty`, `huxley_zeta_polynomial_range_empty`, and `huxley_zeta_polynomial_range_below_weyl`. These theorems settle the exceptional short zeta-polynomial ranges after the detector decomposition; they do not eliminate the medium ordinary-zeta Type-I branch. The obstruction is already visible at `σ = 3/4`, `N = T^(2/3)`, `V = N^(3/4)`: the direct MHH bound has exponent `2/3`, larger than the required `3/5`. `MediumReflection.lean` must therefore be completed through a uniform stationary main-term theorem and dual-block recovery at length comparable to `T/N`, with common coefficients independent of the ordinate and controlled error/separation losses.
5. **Multiplicity-aware common-witness extraction — complete for an identified sharp block:** `extract_sharpMollified_dyadic_witness_native` and `extracted_sharpMollified_block_card_bound` combine the native Jensen cap, weighted finite pigeonholing, separation, normalization, and unrestricted MHH while preserving analytic multiplicity. The remaining selection problem is the global detector branch/scale choice recorded in step 3, not the finite extraction mechanism.
6. **Finite scale/exponent assembly — complete 9 August 2026:** `FiniteScaleAssembly.lean` defines the exact Montgomery–Halász–Huxley exponent and Corollary-11.10 subdivision envelope, proves the bounded two-or-three-fold scale reduction in both exponent and literal real-power form, proves both direct MHH windows and the final power-envelope bound, and constructs concrete `EndpointScaleCertificate`s for `τ₀ = 2 - σ` and `τ₀ = 3*σ - 1`. The public theorem `classical_finite_scale_exponent_assembly_native` packages the actual finite MHH theorem with both endpoint certificates. This closes the arithmetic assembly node without pretending that it proves ANTEDB Lemma 11.5's detector dichotomy, medium reflection, terminal branch, or their finite zero-density reduction; those are the separately open analytic inputs that must consume these certificates.
...
| Downstream work | Concrete Huxley and concrete Ingham remain unproved. Exact-β removal, common multiplicity extraction, normalization, generic exact powering, coefficient control, finite MHH application, endpoint range arithmetic, and finite scale/exponent assembly are complete. The selected route still requires the finite Type-I/Type-II dichotomy, short Type-II product, completed medium Type-I reflection, terminal estimate, the finite reduction consuming the completed certificates, and final symmetric-count specialization. |
| Verification | The focused finite-scale module and root production build pass without warnings. Principal run `logs/overall_proof_20260809_220105.log` passes stages 1–4 warning-free and synchronizes 827/827 declarations; every finite-scale theorem and certificate projection passes with only permitted logical dependencies. Its sole audit failure and two direct-axiom findings are the unrelated, tracked #19 decoupling obligations. |
...
| Audit impact | The current explicit and discovered theorem sets agree at 827/827. Every beta/localization theorem, `extractSeparated_native`, every classical-moment theorem, the complete vendored PNT+ proof closure, the project argument-principle specialization, the #17 arithmetic theorems, and the #15 sharp-truncation, finite-transfer, powering, Weyl, kernel, classical-large-values, endpoint-range, finite-scale, and medium-reflection theorems pass with only `propext`, `Classical.choice`, and `Quot.sound`. The only audit failure is the unrelated `l2_decoupling_bound_native`. |
| Principal verification | `logs/overall_proof_20260809_220105.log` passes stages 1–4 warning-free, synchronizes 827/827 declarations, and reports only the single decoupling dependency failure. Its integrity scan finds no prohibited admitted-proof text or unsafe bypass and exactly the two tracked #19 `Decoupling.lean` axioms, so it remains honestly `FAIL`/`1` until #19. |
...
| Verification | The focused arithmetic module and the current root production closure build with zero warnings. The audit synchronizes 827/827 declarations; all five #17 declarations pass with only permitted logical dependencies. The remaining failures are the two pre-existing #19 decoupling axioms and their single transitive audit failure. |
...
| 6 | ~~`RiemannZeta/Audit.lean`~~ | **Completed for all named public theorems in the cleaned tree:** transitive checks cover the synchronized 827-theorem list, including the globally named public declarations in the vendored modules; the only auxiliary declarations are anonymous examples. One current dependency failure is reported honestly. | Easy–moderate—complete |
...
1. **Completed for the production perimeter and kept synchronized through 9 August 2026:** created an explicit, synchronization-checked list, currently containing 827 exported source-level theorems.
...
4. **#15 second pass:** use #17's completed divisor/factorization estimates, detector-strength sharp truncation, completed exact-β witness extraction and powering, endpoint range arithmetic, and the existing mean-value/MHH/Weyl machinery to prove the concrete Ingham and Huxley inputs. Do not cross out #15 before this pass is complete.
...
1. **#15 — Classical finite zero-density endpoints (in progress).** The zero-count foundation, unit-zero theorem, Montgomery mean value, basic Halász–Montgomery, mollifier/rectangle infrastructure, strip growth, coefficient bounds, symmetric-count bridge, detector-strength sharp truncation, exact-β removal, common multiplicity extraction, normalized finite powering, pointwise/general-weight van der Corput–Weyl stack, Package-C Montgomery–Halász–Huxley estimate including `T * N^4 * V⁻⁶`, endpoint range arithmetic, and finite scale/exponent assembly are proved. The remaining critical path is the finite Type-I/Type-II detector dichotomy, short Type-II product, medium Type-I reflection and terminal estimate, the finite reduction consuming the completed assembly certificates, and the two concrete endpoint theorems. This classical large-values package is distinct from #19's `GuthMaynardLargeValues`, which is the sole remaining premise of the Central Type I estimate after applying the native mean-value, extraction, and coefficient theorems.
2. **#18 — Type II and Goal C integration.** Prove the three residual inputs and reduce the public zero-density theorem to the single premise `GuthMaynardLargeValues`.
3. **#19 — Large values and final integration.** Discharge the two decoupling axioms, prove `GuthMaynardLargeValues`, derive the concrete Guth–Maynard and combined zero-density theorems, and make the principal runner pass.
