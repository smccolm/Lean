...
| `Audit.lean` checks transitive dependencies for the synchronized production theorem set. | #6 | **Complete for the cleaned tree's named public theorems:** the current list is 1112/1112, including the new #15 smooth-extraction, exact Poisson, stationary-geometry, and two-sided deweighting surfaces. Every declaration passes with only the permitted logical dependencies. |
...
| The paper stated stale audit counts. | Documentation gate | **Resolved during #8 and kept synchronized:** the current audit passes across 1112 synchronized declarations. |

Current clean facts: the production graph and both retained examples elaborate with zero warnings; the dependency audit synchronizes and passes 1112 declarations; and all prohibited-proof and project-postulate scans are empty. `logs/overall_proof_20260811_001717.log` records those passes and the named output gate's seven absent end theorems; the overall result remains honestly `FAIL`/`1` until the remaining outputs exist.
...
| 6 | **Verified complete as audit infrastructure** | The explicit and discovered theorem sets agree at 1112/1112 and all transitive dependency checks pass. The separate completion gate fails on the seven absent #15/#18/#19 outputs. |
...
|---:|---|---|---|
| 15 | **Classical finite zero-density engine and endpoints — in progress** | Existing work now includes the exact source-faithful smooth partition, common-scale extraction with cardinality loss, the literal smooth Poisson formula, corrected stationary point/window at scale `T/N`, and exact Fourier deweighting of both the short and reflected amplitudes. The human-to-Lean correspondence is recorded in `Type I Reflection Proof Bijection.md`. Remaining in this chain is one substantive theorem: the uniform stationary main term with summed error small enough to yield the reflected block. The actual-`X` Type-II application, range/branch reduction, and both endpoints also remain. | The item stays uncrossed until audit-clean theorems prove both `HuxleyZeroDensity (fun σ T => N σ T)` and `InghamZeroDensity (fun σ T => N σ T)`. |
...
9. **Medium B-process — exact transform layer complete, stationary estimate open 11 August 2026:** the earlier `100√t` estimate remains a valid coarse bound. `TypeIReflection.lean` now constructs the actual smooth kernel, proves literal Poisson summation, identifies every negative mode with `reflectionPhase`, and proves the corrected critical-frequency window comparable to `t/(2^rY)`. The remaining theorem must uniformly evaluate those critical integrals and sum an error below the detector threshold; phase geometry alone is not the dual-polynomial conclusion.
...
8. **Verification and scope boundary:** the focused and root builds are warning-free. The new smoothing, exact reflection, and deweighting declarations are registered in the synchronized 1112-declaration transitive audit and use only `propext`, `Classical.choice`, and `Quot.sound`. The uniform stationary main-term/error theorem and the later branch-to-slab consumer remain open.
...
| Downstream work | Concrete Huxley and Ingham remain unproved. Exact-β removal, the dichotomy, common extraction, normalization, exact powering, the `X=1` theorem, coefficient growth, endpoint arithmetic, terminal restriction, source smoothing/common-scale extraction, exact Poisson/stationary geometry, and both deweighting identities are complete. The uniform stationary main-term/error theorem, actual-`X` Type-II application, range resolution, branch-to-slab consumer, and symmetric specialization remain. |
| Verification | The focused Type-I modules and root production build pass without warnings. The current audit passes 1112/1112 dependency checks and all integrity scans but fails the output gate on the two absent #15 endpoints and five other agenda outputs. |
...
3. **#15 Type-I interface chain — two of three interfaces closed.** `TypeISmoothing.lean` closes the source-faithful common smooth-block extraction. `TypeIReflection.lean` proves exact Poisson summation and the corrected stationary-frequency window. `TypeIFourierDeweight.lean` closes exact deweighting for both the source and the concrete reflected ANTEDB amplitude. The sole open interface theorem is the uniform stationary main term plus summed remainder converting the complete Poisson series into the finite reflected block at scale `T/N`.
...
**Implementation result.** `PS` and `FD` are complete and audit-clean. `MR` now contains the exact smooth Poisson identity, mode/phase identification, stationary normalization, and corrected dual window; it remains open solely at the uniform stationary main-term-and-remainder theorem. The complete source/Lean bijection and normalization correction are in `Type I Reflection Proof Bijection.md`. `TwistedDiagonal.lean` proves #18's finite `hm = kn` phase cancellation, while `LargeValuesReflection.lean` and `LargeValuesCubic.lean` close their stated #19 packages.
...
| Verification | The focused arithmetic module and root production closure build with zero warnings. The current audit synchronizes 1112/1112 declarations; all five #17 declarations and the #15/#18/#19 foundations pass with only permitted logical dependencies. |
...
| 6 | ~~`RiemannZeta/Audit.lean`~~ | **Completed for all named public theorems in the current tree:** the explicit and discovered sets synchronize at 1112/1112, every existing theorem has only the permitted logical dependencies, and the separate completion gate reports seven absent output names. | Easy–moderate—complete |
...
| 3 | `RiemannZeta/GuthMaynard/TypeIReflection.lean`, `MediumReflection.lean` | Prove the uniform stationary main term and sum its remainder over the corrected `T/N` window. | Extreme; `PS` and `FD` are complete, and this is the sole remaining `MR` obligation. |
...
1. **Completed for the production perimeter and kept synchronized through 11 August 2026:** created an explicit, synchronization-checked list, currently containing 1112 exported source-level theorems.
...
| Remaining obstruction at that iteration | #8 had no remaining warning or retained-file coverage defect. The superseding 11 August audit has 1112/1112 passes and zero direct project axioms. |
