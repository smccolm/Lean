...
| `Audit.lean` checks transitive dependencies for the synchronized root-imported theorem set. | #6 | **Complete as scoped integrity infrastructure:** the current list is 6066/6066. It includes the full DFI/Hughes–Young chain, `twistedZetaFourthMoment_native`, the native #15 positive-slab chain, and the quantitative smooth-reflection chain. Every audited declaration passes with only the permitted logical dependencies; this does not make the six later output theorems or the 49-file coverage gap complete. |
...
| The paper stated stale audit counts. | Documentation gate | **Resolved during #8 and kept synchronized:** the current audit passes across 6066 synchronized declarations. |

Current scoped facts: the root-imported production graph elaborates with zero warnings; its dependency audit synchronizes and passes 6066 declarations; and the repository-wide prohibited-proof and direct-postulate scans are empty. The named output gate now passes `twistedZetaFourthMoment_native`, `classical_endpoint_positive_slab_native`, and `gmQuantitativeSmoothReflection_native`, while reporting six later end theorems absent. The 24 August quantitative-reflection run is `logs/overall_proof_20260824_202645.log`: stages 1–4 passed warning-free, all 6066 dependency checks passed, the FR, twisted-moment, and quantitative-reflection gates passed, the integrity scans passed, and the audit stage/global runner returned the expected `FAIL`/`1` solely because those six later outputs are absent. The runner still lacks elaboration coverage for 47 tracked top-level probes and two subordinate Hughes–Young modules outside the root closure; that regression is assigned to #19 final verification.
...
| 6 | **Verified complete as scoped audit infrastructure** | The explicit and discovered theorem sets agree at 6066/6066 within the root-imported graph and all dependency checks in that scope pass. The separate completion gate passes the #15 slab theorem, the #18 twisted moment, and #19 quantitative smooth reflection, and fails on six later #15/#18/#19 outputs. |
...
| 18 | **Type-II residual analysis and Goal C integration — in progress** | Completed: source-range repair, Gamma/convergence, Appendix C coverage, native fourth-moment reduction, `M²` arithmetic, smooth `ζ²` AFE/diagonal, DFI equations (2) and (9)–(30), the optimized native signed DFI error theorem, its Hughes–Young equation-(80) and finite off-diagonal consumers, and `twistedZetaFourthMoment_native`. | `AF`, `QD`, and `TM` are crossed off. The remaining #18 acceptance output is `guthMaynardZeroDensity_of_largeValues_native`, whose sole mathematical premise is `GuthMaynardLargeValues`; it also waits on #15's native Huxley theorem. |
| 19 | **Guth–Maynard large values and final project integration — in progress** | **Completed:** deletion of the false model; cutoff/derivative infrastructure; exact three-piece localization from the published sharp polynomial to one common smoothed matrix family; sampling-matrix and trace reductions; Fourier/Poisson foundations; the Hilbert–Schmidt first-trace expansion; uniform two-parameter Fourier decay; the exact cubic expansion and `S₁/S₂/S₃` partition; Proposition 5.1 for `S₁`; and quantitative Lemma 6.2 for the complete nonzero integer-frequency series, including arbitrary `T⁻ᴬ`, the `A=100` specialization, scaled trace-tail transfer, and a direct first-`S₂`-summand consumer. **Remaining:** native Heath–Brown, the complete Proposition 6.1 `S₂` summation, moment/energy bridge, `S₃`, affine/GCD estimates, Sections 3/12 assembly, concrete transfers, and restoration of all-retained-Lean-file runner/audit coverage. | The large-values, concrete Guth–Maynard zero-density, and combined zero-density theorems are absent. The item is complete only when all three are kernel-checked, audited, documented, every retained Lean file is elaborated or removed, the root/audit coverage is complete, and the principal runner returns `PASS`/`0`. |
...
| ~~2~~ | 15 | **Actual Type-I finite absorption and endpoint routing — complete** | `normalizedClassicalTypeICoeff` supplies the missing `N^σ` normalization; `actual_typeI_dichotomy_witness_mhh_absorbed_native` absorbs the harmonic loss into `(3T)^ε`; `powered_actual_typeI_block_large_values_bound` realizes exact two/three-fold powering; and the weighted Weyl and endpoint-certificate routes are packaged by `actual_typeI_branch_resolution_native`. | Kernel-checked and included in the synchronized 6066-declaration audit. This closes `ZR`; the subsequently completed `FR` theorem performs threshold substitution and multiplicity assembly. |
| ~~3~~ | 19 | **Source-to-matrix entry bridge — complete** | `sourceDirichletPoly_eq_three_gmSmooth` proves an exact three-piece source identity; `source_large_values_localize_to_matrix` selects one fixed scale/coefficient family on at least one third of the ordinates and applies the matrix bound. The published interval is already `(N,2N]`; a separate theorem records compatibility with the alternate closed convention. | Kernel-checked and included in the 6066-declaration audit. |
| ~~4~~ | 19 | **Quantitative smooth reflection — complete and semantically re-audited** | `gmQuantitativeSmoothReflection_native` assembles the complete nonzero integer-frequency `tsum`, the exact signed retained window, measurable Mellin core/tail splitting, both ordinate signs, explicit `η = min (ε/16) (1/16)`, `H = T^η`, and `M = ⌈(1+|t|)H/N⌉`, and arbitrary `T⁻ᴬ` error. | `gmQuantitativeSmoothReflection_hundred` supplies `T⁻¹⁰⁰`; `gmScaledQuantitativeSmoothReflection_native` transfers to the literal trace tail; `gmCubicS2FirstSummand_reflection_native` is a direct first-cyclic-`S₂` consumer. All are kernel-checked and audit-listed. The full Proposition 6.1 summation remains open. |
...
Completed package: **#19 quantitative smooth reflection.** `LargeValuesReflection.lean` supplies the exact signed identities, Mellin/Fubini formulas, decay, and stationary core estimates. `QuantitativeSmoothReflection.lean` proves the complete integer-frequency summability/decomposition, measurable core/tail integral assembly, both-sign conjugation, the exact reserve `η = min (ε/16) (1/16)`, arbitrary-power error absorption, the strength-100 result, scaled trace-tail theorem, and direct first-`S₂`-summand consumer. The post-completion semantic audit unfolded `GMQuantitativeSmoothReflection`, traced `gmTraceNonzeroFourierSum` back to the literal `gmTraceFourier` Schwartz transform, checked the exact retained-window plus absolutely summable complement identity, and followed the proof through `gmScaledQuantitativeSmoothReflection_native` into `gmCubicS2FirstSummand`. This closes Lemma 6.2 without claiming the still-open full Proposition 6.1 summation.

The remaining open nodes are not all standalone closeout candidates. The complete approximate-energy moment node still consumes Heath–Brown; `S₂`, refined `S₃`, the energy theorem, Goal C, and the final density outputs consume other open nodes. The DFI/twisted-moment chain is complete. Heath–Brown and Guth–Maynard Proposition 9.1 remain independent major source theorems rather than high-confidence near-term closeouts.
...
| Verification | The focused Type-I, DFI/Hughes–Young, and quantitative-reflection modules pass without warnings. The current audit passes 6066/6066 dependency checks, passes the native #15 slab and #19 reflection gates, and fails the output gate on the two absent #15 full endpoints and four other agenda outputs. The principal run preserves the honest overall `FAIL`/`1` until those outputs exist. |
...
Verification at that historical research pass covered 1296 declarations and seven then-absent outputs. The current superseding audit covers 6066/6066 declarations and passes `twistedZetaFourthMoment_native`, `classical_endpoint_positive_slab_native`, and `gmQuantitativeSmoothReflection_native`; six later output names remain absent.
...
| Audit and runner | The synchronized imported-graph audit passes 6066/6066 declarations; the completed milestones use only `propext`, `Classical.choice`, and `Quot.sound`. The output audit passes `twistedZetaFourthMoment_native`, `classical_endpoint_positive_slab_native`, and `gmQuantitativeSmoothReflection_native`; it reports exit `1` on six later missing output theorems, while the separately recorded 49-file elaboration-coverage gap remains a #19 verification defect not detected by that run. |
...
**Implementation status, updated 24 August 2026:** Packages 1–4 and 6 are complete. The source-entry localization feeding Package 3 is also complete. Package 2 includes exact translation invariance, nonzero scaling covariance, affine reindexing, the seven-bin floor partition of tolerance-one energy, and the injection of its zero-defect bin into Mathlib exact additive energy for one-separated sets. Package 3 proves the exact operator-norm conclusions of Guth–Maynard Lemmas 4.1–4.2. Package 4 is closed by `LargeValuesCubic.lean`: it proves the complete Hilbert–Schmidt first-trace layer, exact cubic Poisson expansion, diagonal main term with `Oε(T⁻¹⁰⁰)` remainder, exact `S₁/S₂/S₃` split, and Proposition 5.1 in its epsilon-separated `Oε(T⁻¹⁰)` form. Package 6 is closed by `gmQuantitativeSmoothReflection_native` and its scaled cubic-`S₂` consumer. Packages 5 and 7–14 remain open. The audit and integrity portion of Package 14 passes, but final integration does not because the native large-values and concrete density theorems are absent.
...
| Audit impact | The current explicit and discovered theorem sets agree at 6066/6066. Every beta/localization theorem, `extractSeparated_native`, the vendored PNT+ proof closure, the complete DFI/Hughes–Young chain, the native #15 slab chain, quantitative smooth reflection, and the other proved foundations pass with only `propext`, `Classical.choice`, and `Quot.sound`. |
| Principal verification | `logs/overall_proof_20260824_202645.log` records the 24 August quantitative-reflection run: warning-free root and explicit production builds plus both retained examples; all integrity scans pass. The 6066/6066 dependency audit passes every existing theorem, including the full reflection chain, and its named gate passes `twistedZetaFourthMoment_native`, `classical_endpoint_positive_slab_native`, and `gmQuantitativeSmoothReflection_native`. Stage 5 and the final runner exit remain honestly `FAIL`/`1` solely on six later agenda outputs. |
...
| Verification | The focused arithmetic, paired-Gamma, smooth-AFE, DFI/Hughes–Young, and quantitative-reflection modules and root production closure build with zero warnings. The current audit synchronizes 6066/6066 declarations; all #17 declarations and the proved #15/#18/#19 foundations pass with only permitted logical dependencies. |

## Historical Files Ordered by Estimated Repair Difficulty

This section separates historical cleanup ranks from files that still carry research obligations. The synchronized 6066-declaration audit covers the root-imported `RiemannZeta.lean` project closure, and the runner separately elaborates `Audit.lean`, `TestExp.lean`, and `test_separated.lean`. It does not elaborate 47 other tracked top-level probes or the retained subordinate modules `HughesYoungEquation96CoprimeBound.lean` and `HughesYoungZeroShiftC.lean`; closing that coverage gap belongs to #19 final verification. Historical deletion records remain evidence about earlier states, not descriptions of the present tree.
...
| 6 | ~~`RiemannZeta/Audit.lean`~~ | **Completed for the current root-imported graph:** the explicit and discovered sets synchronize at 6066/6066, every audited theorem has only the permitted logical dependencies, and the separate completion gate passes the FR slab, twisted-moment, and quantitative-reflection results while reporting six later output names absent. All-retained-file coverage remains under #19. | Easy–moderate—scoped audit complete |
...
| 19 | `LargeValuesLocalization.lean`, `LargeValuesMatrix.lean`, `LargeValuesPoisson.lean`, `LargeValuesCubic.lean`, `LargeValuesReflection.lean`, `QuantitativeSmoothReflection.lean`, and `LargeValues.lean` | The published interval convention, exact three-piece source-to-smooth localization, matrix entry theorem, trace/Poisson, exact cubic split, `S₁`, and complete arbitrary-power quantitative smooth reflection are kernel-checked. Missing are Heath–Brown, the complete `S₂` summation, energy/`S₃`, affine/GCD theorems, native large values, and transfers. | Hardest overall; Heath–Brown and the remaining source theorems precede the later packages |
...
1. **Completed for the production perimeter and kept synchronized through 24 August 2026:** created an explicit, synchronization-checked list, currently containing 6066 exported source-level theorems.
...
3. With the source localization, cubic trace split, `S₁`, and quantitative Lemma 6.2 complete, prove the Heath–Brown difference-set mean-square input, the complete `S₂` estimate, the affine-transformation estimates, `S₃`, and the GCD/spacing energy assembly.
...
- #14 removes the circular conclusion-equivalent assumption and completes the primitive-input conditional transfer; eight of its ten source-facing hypotheses are now discharged. Native Huxley remains assigned to #15 and Guth–Maynard large values to #19.
