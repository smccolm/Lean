...
| `Audit.lean` checks transitive dependencies for the synchronized production theorem set. | #6 | **Complete for the cleaned tree's named public theorems:** the current list is 437/437, including all 107 public theorems in the vendored PNT+ modules; the only auxiliary Lean declarations are anonymous examples. The audit correctly remains red on one project-axiom-dependent declaration. |
...
| 2 | **Verified complete as cleanup** | The retained tree has 42 Lean files, no empty Lean file, a 39-file transitive production graph, and only two non-production anonymous-example files. All listed obsolete tests, tombstones, and side-effect utilities are absent. |
...
| 6 | **Verified complete as audit infrastructure** | The explicit and discovered theorem sets agree at 437/437 and transitive dependencies are checked. One failure is reported rather than hidden. |
| 7 | **Verified complete** | The root import closure reaches all 38 subordinate production modules, giving a 39-file production graph. |
...
|---:|---|---|---|
| 15 | **Zeta zero-count foundations and classical density endpoints — in progress** | `ClassicalDensity.lean` proves the actual finite Möbius mollifier, exact convolution and cancellation, pole-free Ingham detector, multiplicity/Jensen bridge, unit-bin estimate, dyadic `O(T log T)` bound, and three symmetric endpoint cases. `ClassicalMoments.lean` proves the source-strength critical-line mollifier second moment. The Lean-4.30 PNT+ port and `ClassicalArgumentPrinciple.lean` prove the unweighted multiplicity-aware rectangle argument principle, its pole-free detector specialization, and linear zeta growth on the classical strip. Completed #17 supplies the divisor and factorization estimates. **Still required:** the weighted Littlewood/boundary-perturbation theorem, finite Gabriel convexity, the zeta fourth moment, right-edge mollified-error and horizontal-edge estimates, the full Montgomery–Halász–Huxley `T * N^4 * V⁻⁶` branch, the interior assemblies, and Huxley at `σ = 3/4`. | All 437 public production theorems, including the 107-theorem imported rectangle stack, are in the synchronized transitive audit. The item remains uncrossed until audit-clean theorems prove both `HuxleyZeroDensity (fun σ T => N σ T)` and `InghamZeroDensity (fun σ T => N σ T)`. |
...
Shitlist #15 is one discrete acceptance item with two diagram nodes and a proved incoming dependency from completed #17; it is not an additional Shitlist task. Execute and verify it through the following bounded work packages.
...
The foundation and mean-value subpasses have removed the final `ZeroCount.lean` axiom, proved the local-zero theorem, supplied the unconditional Montgomery mean-value and basic Halász–Montgomery inputs, and added the mollifier/detector algebra, exact boundary estimates, the critical-line mollifier second moment, the rectangle argument principle, and a linear classical-strip zeta bound. #17 now supplies the required divisor and ordered-factorization estimates. The project has not proved the weighted Littlewood identity, boundary perturbation, right-edge detector moment, zeta fourth moment, stronger classical `N^4 * V⁻⁶` large-values branch, or either complete endpoint proposition. The #16 pass has also proved beta removal and unconditional separated extraction. The synchronized audit now covers 437 declarations and has only the unrelated decoupling failure. The repository remains globally incomplete until #15, #18, and #19 pass their own tests.
...
| Verification | Focused builds of all four vendored modules and `ClassicalArgumentPrinciple.lean`, plus the root production build, pass without warnings. The current direct audit synchronizes 437/437 declarations, explicitly checks all 107 public imported theorems, and reports only the unrelated decoupling dependency failure. |
...
| Audit impact | The current explicit and discovered theorem sets agree at 437/437. Every beta/localization theorem, `extractSeparated_native`, every classical-moment theorem, all 107 public PNT+ port theorems, the project argument-principle specialization, and the later #17 arithmetic theorems pass with only `propext`, `Classical.choice`, and `Quot.sound`. The only audit failure is the unrelated `l2_decoupling_bound_native`. |
| Principal verification | `logs/overall_proof_20260809_021343.log` passes stages 1–4 warning-free, synchronizes 437/437 declarations, and reports only the single decoupling audit failure. The integrity scan finds no `sorry`, `admit`, `sorryAx`, or unsafe bypass and exactly the two #19 `Decoupling.lean` axioms. Overall exit code is honestly `1`. |
...
| Verification | The focused arithmetic module and the 39-file root production closure build with zero warnings. The audit synchronizes 437/437 declarations; all five #17 declarations pass with only permitted logical dependencies. `logs/overall_proof_20260809_021343.log` passes stages 1–4 warning-free and fails only on the two pre-existing #19 decoupling axioms and their single transitive audit failure. |
...
| 6 | ~~`RiemannZeta/Audit.lean`~~ | **Completed for all named public theorems in the cleaned tree:** transitive checks cover the synchronized 437-theorem list, including the globally named public declarations in the vendored modules; the only auxiliary declarations are anonymous examples. One current dependency failure is reported honestly. | Easy–moderate—complete |
...
1. **Completed for the production perimeter and kept synchronized through 9 August 2026:** created an explicit, synchronization-checked list, currently containing 437 exported source-level theorems.
...
4. **#15 second pass:** use #17's completed divisor/factorization estimates and the existing mean-value machinery to prove the concrete Ingham and Huxley inputs. Do not cross out #15 before this pass is complete.
...
| Coverage | The current tree contains 42 Lean files. The root build covers its 39-file transitive production graph, the audit is executed explicitly, and the two remaining files are executed explicitly as examples. No retained Lean file is outside the human-facing evaluation. |
