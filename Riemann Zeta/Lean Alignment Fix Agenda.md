...
| `Audit.lean` checks transitive dependencies for 132 synchronized production theorems. | #6 | **Complete for the cleaned tree's named public theorems:** the only auxiliary Lean declarations are anonymous examples, and both are covered by the runner. |
...
| #12 and #13 contain valid finite/conditional deductions, but their strongest proposition inputs package hard analytic reductions. In #13, `LocalZeroMultiplicityBoundProp` also assumes the exact shifted, scale-filtered occupancy estimate that the recorded plan intended to derive from an ordinary unit-interval zero count. | #12, #13 | **#12 reaches the concrete residual-zero target conditionally. #13's finite extraction core is valid, but its local input must be narrowed and its enlarged interval and logarithmic losses normalized before it is a downstream-ready conditional interface.** |
| The paper stated an expected audit count of 17 failures after the current production audit had fallen to 6. | Documentation gate | **Resolved during #8:** the reproducibility section reports 6 failures and is synchronized at 132 declarations after the repaired #12 concrete bridge. |

Current clean facts are narrower: the repository has 30 Lean files, no empty/tombstone Lean file, and no Lean file containing `sorry`, `admit`, or `sorryAx`; the production graph and both auxiliary examples elaborate with zero warnings under the five-stage principal runner; the production audit synchronizes 132 declarations and reports 6 forbidden dependency failures; and the source contains 14 direct project axioms. The runner covers every retained Lean file but correctly exits nonzero on the audit and axiom gates.
...
| 6 | ~~`RiemannZeta/Audit.lean`~~ | **Completed for all named public theorems in the cleaned tree:** transitive checks cover the synchronized list, currently 132 production theorems; the only auxiliary declarations are anonymous examples. | Easy–moderate—complete |
...
| 13 | `RiemannZeta/GuthMaynard/ExtractSeparated.lean`, with supporting changes in `Separated.lean`, `DirichletPolynomial.lean`, `ZeroDetector.lean`, and `ZeroCount.lean` | **Reopened after critical interface audit:** preserve the proved zero finiteness, dyadic pigeonholing, multiplicity aggregation, and weighted selection. Replace the overstrong shifted-bin input with an unshifted unit-zero input plus a proved pullback/finite-cover theorem; parameterize the raw shift loss; translate the enlarged interval together with the required coefficient phase twist; and absorb all logarithmic losses into the requested epsilon. `DetectorBetaShiftProp` remains a #16 analytic boundary. | Hard–very hard—partial |
...
1. **Completed for the production perimeter 8 August 2026:** created an explicit, synchronization-checked list, currently containing 132 exported source-level theorems.
...
| Remaining obstruction | #8 has no remaining warning or retained-file coverage defect. The overall runner cannot pass until later Shitlist items remove the six audited dependency failures and all fourteen direct project axioms. |
...
- #12 now reaches the concrete `ResidualZeroBoundProp` from honest source-facing inputs; those hard analytic inputs remain open. #13 has a valid finite extraction core, but its shifted-bin hypothesis is broader than the pre-implementation plan allowed, and its interval and loss outputs are not yet normalized for #14.
...
1. finish the reopened #13 conditional/normalized layer: narrow the local-zero input, prove shifted-bin covering, translate both the set and polynomial coefficients, and absorb the logarithmic losses;
2. repair `GuthMaynardLargeValues` and prove the sign-convention bridge needed for exact source correspondence; and
3. repair the full equation (13.1) `k`-selection theorem; the structural-power/explicit-coefficient expansion is now complete.
