...
| `Audit.lean` checks transitive dependencies for the synchronized production theorem set. | #6 | **Complete for the cleaned tree's named public theorems:** the current list is 1052/1052, including the complete #19 uniform Fourier-decay and smooth-reflection surfaces. Every declaration passes with only the permitted logical dependencies. |
...
| The paper stated stale audit counts. | Documentation gate | **Resolved during #8 and kept synchronized:** the current audit passes across 1052 synchronized declarations. |

Current clean facts: the production graph and both retained examples elaborate with zero warnings; the dependency audit synchronizes and passes 1052 declarations; and all prohibited-proof and project-postulate scans are empty. The named output gate reports seven absent end theorems after accepting native #18 coverage and reduction; the overall result remains honestly `FAIL`/`1` until the remaining outputs exist.
...
| 6 | **Verified complete as audit infrastructure** | The explicit and discovered theorem sets agree at 1052/1052 and all transitive dependency checks pass. The separate completion gate fails on the seven absent #15/#18/#19 outputs. |
...
| 2 | 19 | ~~**Package 6: smooth reflection**~~ | **Complete:** `LargeValuesReflection.lean` proves the exact signed finite-mode approximate functional equation, its aggregate norm estimate, positive/negative symmetry, the common Mellin interval, the uniform `T₀⁻¹/²` reflection-integral estimate, and explicit `T⁻¹⁰⁰` far-frequency decay. | Closed by `gmSmoothReflection_native` and `norm_gmSmoothReflection_native_le`; `S₂` remains downstream of the still-open Heath–Brown theorem. |
...
8. **Verification and scope boundary:** the focused and root builds are warning-free. All 25 dichotomy declarations and all five terminal declarations are registered in the synchronized 1052-declaration transitive audit and use only `propext`, `Classical.choice`, and `Quot.sound`. The certificate leaves the medium Type-I scale unclassified; short-scale estimation and the completed terminal case go directly to the finite reduction.
...
| Verification | The focused terminal module and root production build pass without warnings. The current audit passes 1052/1052 dependency checks and all integrity scans but fails the output gate on the two absent #15 endpoints and five other agenda outputs. |
...
2. ~~**#19 smooth reflection.**~~ **Closed 10 August 2026.** `LargeValuesReflection.lean` proves arbitrary-order uniform two-parameter decay, explicit `T⁻¹⁰⁰` far-frequency control, exact cutoff Mellin inversion and Fubini transfer to the common interval, a fully proved first/second-derivative bound `10 / √τ`, signed-mode conjugation, the exact finite positive and negative reflected sums, and the aggregate norm inequality. The public closeout is `gmSmoothReflection_native`; the cubic trace split and `S₁` remain separate open work.
...
**Implementation result.** `reflectionOscillatoryIntegral_stationary_rescale` proves #15's exact interval-integral change to stationary coordinates. `TwistedDiagonal.lean` proves #18's finite `hm = kn` phase cancellation, exact interval integral, coefficient-sum evaluation, and norm bound. `LargeValuesReflection.lean` completes #19 smooth reflection with the named signed finite theorem `gmSmoothReflection_native`, its aggregate norm bound, the `10 / √τ` analytic estimate, and uniform far-frequency control. The remaining closeout boundaries are the #15 common-coefficient stationary reflection and the #18 completed-`ζ²` contour/gamma/off-diagonal package; #19's next analytic task is the cubic trace split.

### Source-Faithful Completion Plan for Shitlist #19 — 10 August 2026

**Implementation status, 10 August 2026:** Packages 1–3 and 6 are complete. Package 2 includes exact translation invariance, nonzero scaling covariance, affine reindexing, the seven-bin floor partition of tolerance-one energy, and the injection of its zero-defect bin into Mathlib exact additive energy for one-separated sets. Package 3 proves the exact operator-norm conclusions of Guth–Maynard Lemmas 4.1–4.2. Package 4 has the complete Hilbert–Schmidt first-trace expansion and the uniform two-parameter Fourier-decay component; its cubic trace split and Proposition 5.1 remain open. Package 6 is closed by `LargeValuesReflection.lean`, including the exact signed finite mode theorem and aggregate estimate. Packages 5 and 7–14 remain open. The audit and integrity portion of Package 14 passes, but final integration does not because the native large-values and concrete density theorems are absent.
...
4. **Formalize Fourier decay, Poisson, and `S₁` — in progress; Lemma 4.4 and uniform Lemma 4.3 decay complete.** `LargeValuesPoisson.lean` proves the trace kernel, Poisson identities, first-trace formula, zero mode, and `N⁻¹⁰⁰` first-trace tail. `LargeValuesReflection.lean` proves the arbitrary-order uniform two-parameter derivative/Fourier estimate and its explicit far-frequency consequence. Expand the cubic trace as in Lemma 4.5, split its nonzero-frequency configurations into `S₁`, `S₂`, and `S₃`, and prove Proposition 5.1 for `S₁`.
...
6. ~~**Prove smooth reflection.**~~ **Complete.** `LargeValuesReflection.lean` proves the uniform Fourier truncation, exact Mellin/Fubini rescaling, critical oscillatory estimate, signed positive/negative reflected modes, native finite sum, and aggregate norm bound. The public closeout is `gmSmoothReflection_native`.
...
| Audit impact | The current explicit and discovered theorem sets agree at 1052/1052. Every beta/localization theorem, `extractSeparated_native`, every classical-moment theorem, the vendored PNT+ proof closure, native #18 coverage and reduction, and the #15/#18/#19 closeout foundations pass with only `propext`, `Classical.choice`, and `Quot.sound`. |
| Principal verification | The current runner passes all build/example stages warning-free, passes 1052/1052 dependency checks, and finds no admitted proof, unsafe bypass, or project postulate. The named output gate fails on seven absent agenda theorems, so the overall result remains honestly `FAIL`/`1`. |
...
| Verification | The focused arithmetic module and root production closure build with zero warnings. The current audit synchronizes 1052/1052 declarations; all five #17 declarations and the #15/#18/#19 foundations pass with only permitted logical dependencies. |
...
| 6 | ~~`RiemannZeta/Audit.lean`~~ | **Completed for all named public theorems in the current tree:** the explicit and discovered sets synchronize at 1052/1052, every existing theorem has only the permitted logical dependencies, and the separate completion gate reports seven absent output names. | Easy–moderate—complete |
...
| 19 | `LargeValuesPoisson.lean`, `LargeValuesReflection.lean`, `LargeValues.lean`, and the proved matrix/language prerequisites | The Hilbert–Schmidt first-trace expansion, uniform two-parameter Fourier decay, and complete signed smooth-reflection package are kernel-checked. The remaining repairs are the cubic trace split, `S₁/S₂/S₃`, Heath–Brown, affine/energy/GCD theorems, native large values, and concrete transfers. | Hardest overall; first trace and smooth reflection complete, later source packages open |
...
| 3 | `RiemannZeta/GuthMaynard/LargeValuesPoisson.lean`, `LargeValuesReflection.lean`, and subsequent #19 modules | Uniform two-parameter Lemma 4.3 decay and Lemma 6.2 smooth reflection are complete. The next repairs are the cubic trace split and Proposition 5.1 for `S₁`. | Very hard; two analytic packages complete, cubic trace remains. |
...
1. **Completed for the production perimeter and kept synchronized through 10 August 2026:** created an explicit, synchronization-checked list, currently containing 1052 exported source-level theorems.
...
3. Prove the cubic trace split and `S₁`, then the Heath–Brown difference-set mean-square input, `S₂`, the affine-transformation estimates, `S₃`, and the GCD/spacing energy assembly. The Section 6 smooth-reflection theorem is complete.
...
| Remaining obstruction at that iteration | #8 had no remaining warning or retained-file coverage defect. The superseding 10 August audit has 1052/1052 passes and zero direct project axioms. |
...
3. **#19 — Large values and final integration.** The false decoupling contracts are deleted; the source definitions, sampling matrix, trace-dispersion algebra, Fourier/Poisson foundations, Hilbert–Schmidt first-trace expansion, uniform two-parameter Fourier decay, and smooth reflection are proved. Next prove the cubic trace and `S₁` layer, Heath–Brown, affine-transformation, approximate-energy, `S₂/S₃`, and GCD/spacing chain; prove `GuthMaynardLargeValues`; derive the concrete Guth–Maynard and combined zero-density theorems; and make the principal runner pass.
