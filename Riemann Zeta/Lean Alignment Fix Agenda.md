...
| 2 | **Verified complete as cleanup** | The retained tree has 44 Lean files, no empty Lean file, a 41-file transitive production graph, and only two non-production anonymous-example files. All listed obsolete tests, tombstones, and side-effect utilities are absent. |
...
| 7 | **Verified complete** | The root import closure reaches all 40 subordinate production modules, giving a 41-file production graph. |
...
| 19 | **Guth–Maynard large values and final project integration — in progress** | **Completed:** delete the false model; define and prove the cutoff/derivative, matrix action, exact Gram kernel, positive spectrum, singular-value power, trace-cube, sixth-dispersion, core approximate-energy, and basic `R` layers. **Remaining:** Poisson and `S₁`; native Heath–Brown; reflection and `S₂`; moment/energy bridge; localization, affine and refined `S₃`; GCD/spacing theorem; Sections 3/12 assembly; concrete transfers. | The concrete Guth–Maynard and combined zero-density theorems are project-theorem complete. Existing dependency and integrity gates pass, while the named output gate keeps the principal runner at `FAIL`/`1` until the end theorems exist. |
...
### Immediate Complete-Closeout Queue — 10 August 2026

The completion of #18's native fourth-moment reduction and #19 Packages 2–3 changes the most efficient implementation order without changing any theorem status. The following five open packages have all of their formal project dependencies available and are bounded enough to target as complete theorem-level closeouts. They are ordered by estimated implementation difficulty, not by Shitlist number.

| Rank | Owning item | Complete closeout target | Why it is now reachable | Exact boundary |
|---:|---:|---|---|---|
| 1 | 19 | **Package 4: Fourier decay, Poisson trace expansion, and `S₁`** | The cutoff derivatives, sampling matrix, exact Gram kernel, trace identities, and operator-norm conclusion are proved; Mathlib and the pinned PNT+ layer provide the underlying Fourier/Poisson APIs. | Close the entire Lemmas 4.3–4.5/Proposition 5.1 package, including the `S₁/S₂/S₃` split and the complete `S₁` estimate—not merely a Fourier-transform helper. |
...
| 6 | ~~`RiemannZeta/Audit.lean`~~ | **Completed for all named public theorems in the cleaned tree:** transitive checks cover the synchronized 887-theorem list, including the globally named public declarations in the vendored modules; the only auxiliary declarations are anonymous examples. One current dependency failure is reported honestly. | Easy–moderate—complete |
...
| Coverage | The current tree contains 44 Lean files. The root build covers its 41-file transitive production graph, the audit is executed explicitly, and the two remaining files are executed explicitly as examples. No retained Lean file is outside the human-facing evaluation. |
