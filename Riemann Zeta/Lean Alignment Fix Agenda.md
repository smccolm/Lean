...
| `Audit.lean` checks transitive dependencies for the synchronized production theorem set. | #6 | **Complete for the cleaned tree's named public theorems:** the current list is 322/322; the only auxiliary Lean declarations are anonymous examples. The audit correctly remains red on one project-axiom-dependent declaration. |
...
| 6 | **Verified complete as audit infrastructure** | The explicit and discovered theorem sets agree at 322/322 and transitive dependencies are checked. One failure is reported rather than hidden. |
...
|---:|---|---|---|
| 15 | **Zeta zero-count foundations and classical density endpoints — in progress** | `ClassicalDensity.lean` proves the actual finite Möbius mollifier, exact convolution and cancellation, pole-free Ingham detector, multiplicity/Jensen bridge, unit-bin estimate, dyadic `O(T log T)` bound, and three symmetric endpoint cases. `ClassicalMoments.lean` now proves the source-strength critical-line mollifier second moment `(T + O(X))(1 + log X)`. **Still required:** Littlewood's rectangle theorem, finite Gabriel convexity, the zeta fourth moment, right-edge mollified-error and horizontal-edge estimates, the full Montgomery–Halász–Huxley `T * N^4 * V⁻⁶` branch, the interior assemblies, and Huxley at `σ = 3/4`. | All 53 #15 public theorems are in the root graph and transitive audit. The item remains uncrossed until audit-clean theorems prove both `HuxleyZeroDensity (fun σ T => N σ T)` and `InghamZeroDensity (fun σ T => N σ T)`. |
...
3. **Littlewood rectangle theorem:** prove a project-native rectangle formula/inequality that counts zeros with analytic multiplicity, controls them by vertical logarithmic integrals and horizontal argument variation, and removes boundary nonvanishing assumptions through a proved perturbation argument.
...
The foundation and mean-value subpasses have removed the final `ZeroCount.lean` axiom, proved the local-zero theorem, supplied the unconditional Montgomery mean-value and basic Halász–Montgomery inputs, and added the mollifier/detector algebra, exact boundary estimates, and the critical-line mollifier second moment. They have not proved the right-edge detector moment, zeta fourth moment, stronger classical `N^4 * V⁻⁶` large-values branch, or either complete endpoint proposition. The #16 pass has also proved beta removal and unconditional separated extraction. The synchronized audit now covers 322 declarations and has only the unrelated decoupling failure. The repository remains globally incomplete until #15 and #17–#19 pass their own tests.
...
| Downstream work | Concrete Huxley and concrete Ingham remain unproved. Part A requires the classical fourth-moment/Littlewood–Gabriel chain and the full Montgomery–Halász–Huxley estimate, especially its `T * N^4 * V⁻⁶` branch. Part B must assemble the two exact exponents and transfer them to symmetric multiplicity-weighted `N`. |
| Verification | Focused elaboration of `ClassicalDensity.lean` and `ClassicalMoments.lean`, plus the full production build, pass with no warnings or tactic diagnostics. The direct audit synchronizes 322/322 declarations; all 14 new moment theorems pass, and only the unrelated decoupling dependency fails. |
...
| Audit impact | The explicit and discovered theorem sets agree at 322/322. Every beta/localization theorem, `extractSeparated_native`, and every new classical-moment theorem passes with only `propext`, `Classical.choice`, and `Quot.sound`. The only audit failure is the unrelated `l2_decoupling_bound_native`. |
| Principal verification | `logs/overall_proof_20260809_004244.log` passes stages 1–4 warning-free, synchronizes 322/322 declarations, and reports only the single decoupling audit failure. The integrity scan finds no `sorry`, `admit`, `sorryAx`, or unsafe bypass and exactly the two #19 `Decoupling.lean` axioms. Overall exit code is honestly `1`. |
...
| 6 | ~~`RiemannZeta/Audit.lean`~~ | **Completed for all named public theorems in the cleaned tree:** transitive checks cover the synchronized 322-theorem list; the only auxiliary declarations are anonymous examples. One current dependency failure is reported honestly. | Easy–moderate—complete |
...
1. **Completed for the production perimeter and kept synchronized through 9 August 2026:** created an explicit, synchronization-checked list, currently containing 322 exported source-level theorems.
...
1. **#15 — Classical density endpoints (in progress).** The zero-count foundation, unit-zero theorem, Montgomery mean value, critical-line mollifier second moment, and generic symmetric-count bridge are proved. Part A must still prove Littlewood/Gabriel, the zeta fourth moment, the right-edge/horizontal detector estimates, and the full classical Montgomery–Halász–Huxley estimate including `T * N^4 * V⁻⁶`. Part B must prove the concrete Ingham and Huxley endpoints for `N`.
