...
| `Audit.lean` checks transitive dependencies for the synchronized root-imported theorem set. | #6 | **Complete as scoped integrity infrastructure:** the current list is 6028/6028. It includes the full DFI/Hughes–Young chain, `twistedZetaFourthMoment_native`, and the native #15 positive-slab chain. Every audited declaration passes with only the permitted logical dependencies; this does not make the six later output theorems or the 49-file coverage gap complete. |
...
| The paper stated stale audit counts. | Documentation gate | **Resolved during #8 and kept synchronized:** the current audit passes across 6028 synchronized declarations. |

Current scoped facts: the root-imported production graph elaborates with zero warnings; its dependency audit synchronizes and passes 6028 declarations; and the repository-wide prohibited-proof and direct-postulate scans are empty. The named output gate now passes both `twistedZetaFourthMoment_native` and `classical_endpoint_positive_slab_native`, while reporting six later end theorems absent. The 24 August re-audit run is `logs/overall_proof_20260824_171054.log`: stages 1–4 passed warning-free, all 6028 dependency checks passed, FR passed its output gate, the integrity scans passed, and the audit stage/global runner returned the expected `FAIL`/`1` solely because those six later outputs are absent. The runner still lacks elaboration coverage for 47 tracked top-level probes and two subordinate Hughes–Young modules outside the root closure; that regression is assigned to #19 final verification.
...
| 6 | **Verified complete as scoped audit infrastructure** | The explicit and discovered theorem sets agree at 6028/6028 within the root-imported graph and all dependency checks in that scope pass. The separate completion gate passes the new #15 slab theorem and fails on six later #15/#18/#19 outputs. |
...
| ~~2~~ | 15 | **Actual Type-I finite absorption and endpoint routing — complete** | `normalizedClassicalTypeICoeff` supplies the missing `N^σ` normalization; `actual_typeI_dichotomy_witness_mhh_absorbed_native` absorbs the harmonic loss into `(3T)^ε`; `powered_actual_typeI_block_large_values_bound` realizes exact two/three-fold powering; and the weighted Weyl and endpoint-certificate routes are packaged by `actual_typeI_branch_resolution_native`. | Kernel-checked and included in the synchronized 6028-declaration audit. This closes `ZR`; the subsequently completed `FR` theorem performs threshold substitution and multiplicity assembly. |
| ~~3~~ | 19 | **Source-to-matrix entry bridge — complete** | `sourceDirichletPoly_eq_three_gmSmooth` proves an exact three-piece source identity; `source_large_values_localize_to_matrix` selects one fixed scale/coefficient family on at least one third of the ordinates and applies the matrix bound. The published interval is already `(N,2N]`; a separate theorem records compatibility with the alternate closed convention. | Kernel-checked and included in the 6028-declaration audit. |
...
| Verification | The focused Type-I and full DFI/Hughes–Young modules pass without warnings. The current audit passes 6028/6028 dependency checks, passes the native #15 slab gate, and fails the output gate on the two absent #15 full endpoints and four other agenda outputs. The principal run preserves the honest overall `FAIL`/`1` until those outputs exist. |
...
Verification at that historical research pass covered 1296 declarations and seven then-absent outputs. The current superseding audit covers 6028/6028 declarations and passes `twistedZetaFourthMoment_native` and `classical_endpoint_positive_slab_native`; six later output names remain absent.
...
| Audit and runner | The synchronized imported-graph audit passes 6028/6028 declarations; the completed milestones use only `propext`, `Classical.choice`, and `Quot.sound`. The output audit passes `twistedZetaFourthMoment_native` and `classical_endpoint_positive_slab_native`; it reports exit `1` on six later missing output theorems, while the separately recorded 49-file elaboration-coverage gap remains a #19 verification defect not detected by that run. |
...
| Audit impact | The current explicit and discovered theorem sets agree at 6028/6028. Every beta/localization theorem, `extractSeparated_native`, the vendored PNT+ proof closure, the complete DFI/Hughes–Young chain, the native #15 slab chain, and the other proved foundations pass with only `propext`, `Classical.choice`, and `Quot.sound`. |
| Principal verification | `logs/overall_proof_20260824_171054.log` records the 24 August semantic re-audit: warning-free root and explicit production builds plus both retained examples; all integrity scans pass. The 6028/6028 dependency audit passes every existing theorem, including the full FR chain, and its named gate passes `twistedZetaFourthMoment_native` and `classical_endpoint_positive_slab_native`. Stage 5 and the final runner exit remain honestly `FAIL`/`1` solely on six later agenda outputs. |
...
| Verification | The focused arithmetic, paired-Gamma, smooth-AFE, DFI/Hughes–Young modules and root production closure build with zero warnings. The current audit synchronizes 6028/6028 declarations; all #17 declarations and the proved #15/#18/#19 foundations pass with only permitted logical dependencies. |

## Historical Files Ordered by Estimated Repair Difficulty

This section separates historical cleanup ranks from files that still carry research obligations. The synchronized 6028-declaration audit covers the root-imported `RiemannZeta.lean` project closure, and the runner separately elaborates `Audit.lean`, `TestExp.lean`, and `test_separated.lean`. It does not elaborate 47 other tracked top-level probes or the retained subordinate modules `HughesYoungEquation96CoprimeBound.lean` and `HughesYoungZeroShiftC.lean`; closing that coverage gap belongs to #19 final verification. Historical deletion records remain evidence about earlier states, not descriptions of the present tree.
...
1. **Completed for the production perimeter and kept synchronized through 24 August 2026:** created an explicit, synchronization-checked list, currently containing 6028 exported source-level theorems.
