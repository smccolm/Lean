...
1. **Delete the false model and freeze conventions.** Remove `l2_parabola_incidence_bound`, `l2_decoupling_bound_unconditional`, `l2_decoupling_bound_native`, and the obsolete `DecouplingProp`. Fix the source interval `(N,2N]`, positive phase, `T^ε` separation, and the smooth cutoff conventions in one interface module.
2. **Build the shared quantitative language.** Define compactly supported smooth-cutoff data with explicit derivative constants, the finite sampling operator `GMMatrix`, the exponential sum `R_W(v)`, and tolerance-`η` approximate additive energy `ApproxAddEnergy η W`. Prove finiteness, monotonicity, reindexing, translation, scaling, and the floor-binning bridge to exact finite additive combinatorics.
3. **Prove the trace-dispersion algebra.** First prove the scalar inequality behind Lemma 4.2 for a finite nonnegative eigenvalue sequence. Then model `GMMatrix` as a map between finite Euclidean spaces, prove its Gram operator is positive Hermitian, identify traces of its first and third powers with the required finite sums, and obtain the Lemmas 4.1–4.2 singular-value reduction. This uses Mathlib's Hermitian spectral theorem and singular-value API; a new rectangular-matrix SVD is unnecessary.
4. **Formalize Fourier decay, Poisson, and `S₁`.** Prove arbitrary-order Fourier decay for the chosen compactly supported smooth kernels, instantiate real Poisson summation, expand the Hilbert–Schmidt and cubic traces as in Lemmas 4.3–4.5, split the nonzero-frequency configurations into `S₁`, `S₂`, and `S₃`, and prove Proposition 5.1 for `S₁`.
