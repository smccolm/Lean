...
| `Audit.lean` checks transitive dependencies for the synchronized production theorem set. | #6 | **Complete for the cleaned tree's named public theorems:** the current list is 852/852, including the expanded vendored PNT+ sharp-zeta dependency closure, finite-density transfer/powering/dichotomy surface, endpoint and scale arithmetic, and medium-reflection API; the only auxiliary Lean declarations are anonymous examples. The audit correctly remains red on one project-axiom-dependent declaration. |
...
| 6 | **Verified complete as audit infrastructure** | The explicit and discovered theorem sets agree at 852/852 and transitive dependencies are checked. One failure is reported rather than hidden. |
...
| 18 | **Type-II residual analysis and Goal C integration** | Prove rapid uniform vertical Gamma decay, `TypeIContourTypeIICoverProp`, the concrete `TypeIIFourthMomentReductionProp`, and `TwistedZetaFourthMomentProp`. The selected moment route rewrites `‖Mζ‖⁴` as `‖ζ‖⁴ ‖M²‖²` and formalizes the needed short-polynomial upper-bound corollary of Hughes–Young. Then specialize `conditionalZeroDensityTransfer` using #15–#18 so its only remaining premise is `GuthMaynardLargeValues`. | A public audited theorem derives the concrete full-range zero-density result from `GuthMaynardLargeValues` alone. No other analytic or arithmetic proposition parameter remains. |
...
7. **Verification and scope boundary:** the focused and root builds are warning-free. All 25 public declarations are registered in the synchronized 852-declaration transitive audit and use only `propext`, `Classical.choice`, and `Quot.sound`. The certificate leaves the Type-I scale unclassified: short-scale estimation goes directly to the finite reduction, while medium reflection and terminal Euler–Maclaurin/nonstationary control remain the separately displayed downstream #15 obligations.
...
| Downstream work | Concrete Huxley and concrete Ingham remain unproved. Exact-β removal, the full finite Type-I/Type-II dichotomy and short Type-II product, common multiplicity extraction, normalization, generic exact powering, coefficient control, finite MHH application, endpoint range arithmetic, and finite scale/exponent assembly are complete. The selected route still requires completed medium Type-I reflection, the terminal estimate, the finite reduction consuming the completed certificates, and final symmetric-count specialization. |
| Verification | The focused dichotomy module and root production build pass without warnings. Principal run `logs/overall_proof_20260809_232948.log` passes stages 1–4 warning-free and synchronizes 852/852 declarations; every new dichotomy theorem passes with only permitted logical dependencies. Its sole audit failure and the two direct-axiom findings are the unrelated, tracked #19 decoupling obligations. |
...
The public search found no drop-in Lean proof of the Maynard–Pratt Appendix C reduction or the required twisted fourth moment. The viable route combines project infrastructure with narrowly selected external mathematics; it does not treat a cited paper as a Lean dependency.

1. **Uniform vertical Gamma decay:** put the Gamma kernel on logarithmic coordinates, use the Gamma recurrence to control derivatives, and integrate by parts an arbitrary finite number of times. Package an eventual bound uniform in the real parameter range used by `IsContourTypeIIZero`. The vendored PNT+ development provides Mellin, zeta, and Poisson infrastructure but not this rapid vertical-decay theorem.
2. **Type-I/contour-Type-II coverage:** follow Maynard–Pratt Appendix C. Introduce the exponentially smoothed Möbius detector, express it by Mellin inversion, shift the contour across the zeta factor, prove cancellation at the relevant zero, and bound the residue and truncated tails. The output must be exactly `TypeIContourTypeIICoverProp`, not a new conclusion-shaped surrogate.
3. **Fourth-moment reduction:** truncate the Gamma integral using the new decay theorem; apply Hölder; extract a `log³ T`-separated ordinate set with the proved local multiplicity bound; and prove bounded overlap of the translated integration windows. This must derive the existing concrete `TypeIIFourthMomentReductionProp`.
4. **Twisted fourth moment:** use `|M(s) ζ(s)|⁴ = |ζ(s)|⁴ |M(s)²|²`. Since the project mollifier has length `T^(1/100)`, its square has length `T^(1/50)`, inside the `T^(1/11-ε)` range of Hughes–Young. Formalize only the upper-bound corollary needed here: an approximate functional equation for `ζ²`, diagonal evaluation, the quadratic-divisor/off-diagonal estimate, and removal of smooth weights. This is #18's deepest isolated theorem; #17 supplies the coefficient-growth bounds.
5. **Goal C specialization:** discharge all three `TypeIIZeros.lean` inputs, combine them with #15 and the already proved #16/#17 results, and prove one public theorem whose sole hypothesis is `GuthMaynardLargeValues`.

Acceptance requires the three existing proposition specifications to be theorems, not axioms or parameters hidden behind renamed wrappers. Principal references are [Maynard–Pratt's paper and Appendix C](https://ora.ox.ac.uk/objects/uuid%3Ab2cc1c2e-6cec-495a-ba6a-61ba08acc723), [Hughes–Young's twisted fourth-moment theorem](https://arxiv.org/abs/0709.2345), and the [PNT+ repository](https://github.com/AlexKontorovich/PrimeNumberTheoremAnd).
...
| Audit impact | The current explicit and discovered theorem sets agree at 852/852. Every beta/localization theorem, `extractSeparated_native`, every classical-moment theorem, the complete vendored PNT+ proof closure, the project argument-principle specialization, the #17 arithmetic theorems, and the #15 sharp-truncation, finite-transfer, dichotomy, powering, Weyl, kernel, classical-large-values, endpoint-range, finite-scale, and medium-reflection theorems pass with only `propext`, `Classical.choice`, and `Quot.sound`. The only audit failure is the unrelated `l2_decoupling_bound_native`. |
| Principal verification | `logs/overall_proof_20260809_232948.log` passes stages 1–4 warning-free, synchronizes 852/852 declarations, and reports only the single decoupling dependency failure. Its integrity scan finds no prohibited admitted-proof text or unsafe bypass and exactly the two tracked #19 `Decoupling.lean` axioms, so it remains honestly `FAIL`/`1` until #19. |
...
| Verification | The focused arithmetic module and the current root production closure build with zero warnings. The audit synchronizes 852/852 declarations; all five #17 declarations pass with only permitted logical dependencies. The remaining failures are the two pre-existing #19 decoupling axioms and their single transitive audit failure. |
...
| 18 | Type-II residual analysis and Goal C integration | See the authoritative three-item completion contract above. | Extreme—open |
...
1. **Completed for the production perimeter and kept synchronized through 9 August 2026:** created an explicit, synchronization-checked list, currently containing 852 exported source-level theorems.
...
4. **#15 second pass:** complete the smooth medium reflection, terminal estimate, finite certificate-consuming reduction, and concrete Ingham/Huxley inputs. The finite Type-I/Type-II dichotomy and genuinely short Type-II construction are already complete and must not be relisted as open work. Do not cross out #15 before both endpoint theorems exist.
5. **#18:** prove uniform vertical Gamma decay, Appendix C coverage and fourth-moment reduction, the Hughes–Young-range twisted fourth-moment upper bound, and a specialization whose only premise is `GuthMaynardLargeValues`.
