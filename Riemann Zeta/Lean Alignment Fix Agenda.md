...
| `Audit.lean` checks transitive dependencies for the synchronized production theorem set. | #6 | **Complete for the cleaned tree's named public theorems:** the current list is 267/267; the only auxiliary Lean declarations are anonymous examples. The audit correctly remains red on two project-axiom-dependent declarations. |
...
| The paper stated stale audit counts. | Documentation gate | **Resolved during #8 and kept synchronized:** the reproducibility section now reports two failures across the current 267 declarations. |

Current clean facts are narrower: the production graph and both auxiliary examples elaborate with zero warnings under the five-stage principal runner; the production audit synchronizes 267 declarations and reports two forbidden dependency failures; and the source contains nine direct project axioms. The runner covers every retained Lean file but correctly exits nonzero on the audit and axiom gates.
...
| 6 | **Verified complete as audit infrastructure** | The explicit and discovered theorem sets agree at 267/267 and transitive dependencies are checked. Two failures are reported rather than hidden. |
...
The foundation and mean-value subpasses have removed the final `ZeroCount.lean` axiom, proved the local-zero theorem, and supplied the unconditional Montgomery/Halász–Montgomery input. The synchronized audit now covers 267 declarations and has only the two unrelated beta-removal and decoupling failures. Completing #15 still requires the concrete Huxley and Ingham propositions. The repository will remain globally incomplete until #16–#19 pass their own tests.
...
| Verification | The focused mean-value and Halász–Montgomery builds pass with no warnings or tactic diagnostics. The principal run `logs/overall_proof_20260808_200533.log` passed stages 1–4 warning-free, synchronized 267/267 declarations, reported only the two unrelated beta and decoupling audit failures, and exited `1`. |
...
| 6 | ~~`RiemannZeta/Audit.lean`~~ | **Completed for all named public theorems in the cleaned tree:** transitive checks cover the synchronized 267-theorem list; the only auxiliary declarations are anonymous examples. Two current dependency failures are reported honestly. | Easy–moderate—complete |
...
1. **Completed for the production perimeter 8 August 2026:** created an explicit, synchronization-checked list, currently containing 267 exported source-level theorems.
...
| Honest analytic boundary at the #13 iteration | `DetectorBetaShiftProp` and `LocalZeroMultiplicityBoundProp` were then unproved proposition specifications. The later #15 work has discharged the local-zero proposition; beta removal remains #16 work. |
...
| Remaining obstruction | #8 has no remaining warning or retained-file coverage defect. The overall runner cannot pass until later Shitlist work removes the two audited dependency failures and all nine direct project axioms. |
...
3. **#17 — Arithmetic and mean value.** `MontgomeryMeanValue` is proved; prove the remaining divisor-count and factorization-count propositions.
