...
| `Audit.lean` checks transitive dependencies for the synchronized production theorem set. | #6 | **Complete for the cleaned tree's named public theorems:** the current list is 447/447, including all 107 public theorems in the vendored PNT+ modules; the only auxiliary Lean declarations are anonymous examples. The audit correctly remains red on one project-axiom-dependent declaration. |
...
| 6 | **Verified complete as audit infrastructure** | The explicit and discovered theorem sets agree at 447/447 and transitive dependencies are checked. One failure is reported rather than hidden. |
...
|---:|---|---|---|
| 15 | **Classical finite zero-density engine and endpoints — in progress** | Existing work proves zeta growth, local multiplicity, the finite Möbius detector, the critical-line mollifier second moment, the rectangle argument principle, Montgomery mean value, basic Halász–Montgomery, the symmetric-count bridge, and #17 coefficient bounds. **Package A is complete:** `ZetaTruncation.lean` proves the exact Euler–Maclaurin truncation, its specialization at genuine off-axis zeta zeros, and the uniform error bound. **Remaining route:** (B) the targeted van der Corput B/A processes giving exponent pairs `(1/2,1/2)` and `(1/6,2/3)` plus the Weyl zeta bound, (C) the full Montgomery–Halász–Huxley large-values estimate including `T * N^4 * V⁻⁶`, and (D) a finite, exact-`σ` zero-density-from-large-values transfer. The former weighted-Littlewood/Gabriel/fourth-moment route remains useful optional infrastructure, not a #15 acceptance dependency. | All 447 public production theorems, including the 107-theorem imported rectangle stack, remain in the synchronized transitive audit. The item stays uncrossed until audit-clean theorems prove both `HuxleyZeroDensity (fun σ T => N σ T)` and `InghamZeroDensity (fun σ T => N σ T)`. |
...
#### Package A — Truncated zeta relation — complete 9 August 2026
...
#### Package B — Targeted van der Corput and Weyl estimates

1. Implement the reusable logarithmic-phase estimates from the ANTEDB blueprint. The `teorth/expdb` repository is TeX plus Python, not a Lean project, so there is no Lean module to port or toolchain mismatch to solve.
2. Prove the van der Corput B-process instance yielding exponent pair `(1/2,1/2)`, then the required A-process instance yielding `(1/6,2/3)`. General exponent-pair automation is outside #15.
3. Derive the concrete Weyl-strength zeta estimate on the critical line and the medium-length Dirichlet-polynomial reflection estimate used by the density transfer.
4. This package is complete when those concrete estimates compile against the pinned toolchain, are audited, and expose exactly the uniformity used in Packages C and D.

#### Package C — Classical large values

1. Extend `halasz_montgomery_lemma_native` to the full finite Montgomery–Halász–Huxley estimate
   `R ≪ T^ε (N^2 V⁻² + T * min (N V⁻²) (N^4 V⁻⁶))`
   for the project's separated finite ordinate sets and bounded coefficients.
2. Use completed #17 for divisor/factorization and powered-coefficient bounds, and Package B for the off-diagonal oscillatory-sum estimate. The existing `V⁻²` theorem is an input, not completion of this package.
3. The package is complete only when the full `N^4 V⁻⁶` branch is present in a public warning-free theorem and passes the transitive dependency audit.

#### Package D — Finite zero-density transfer

1. Formalize a project-native finite version of the classical zero-density-from-large-values argument: dyadic zero detection from Package A, separated selection with multiplicity control, Type-I/Type-II polynomial alternatives, removal of smooth weights by Fourier translation, medium-length reflection from Package B, and application of Package C.
2. State the result directly with `EpsilonPowerBound` and the project's exact `σ`; prove the small real-part displacement is absorbed by epsilon slack instead of leaving a hidden `σ - δ` premise.
3. This package owns the classical transfer required for Ingham and Huxley. It may later be reused by #18, but it does not by itself discharge #18's separately specified contour coverage, fourth-moment reduction, or twisted fourth-moment propositions.

#### Endpoint assembly and acceptance

1. **Ingham:** use the basic mean-value branch with `τ₀ = 2 - σ` to prove the interior exponent `3 * (1 - σ) / (2 - σ)`, then combine it with the already proved `σ = 1/2` and `σ = 1` cases.
2. **Huxley:** use the full Package-C branch and the Weyl estimate with `τ₀ = 3 * σ - 1` to prove `3 * (1 - σ) / (3 * σ - 1)` on the interior. At `σ = 3/4`, the Ingham and Huxley exponents are both `3/5`, so derive this boundary from the Ingham theorem; it is not a separate analytic package.
...
The already completed truncation, mollifier, Jensen, critical-line second-moment, rectangle-argument-principle, and strip-growth results remain valid reusable library work and an alternative route if the finite large-values route fails. Weighted Littlewood, Gabriel interpolation, the standalone zeta fourth moment, and right/horizontal contour moments are not mandatory #15 deliverables. Package A's status has changed to complete, but both endpoint propositions are still absent; the audit now covers 447 declarations, and the repository remains globally incomplete until #15, #18, and #19 pass their tests.

Primary implementation references are the [ANTEDB finite zero-density chapter](https://teorth.github.io/expdb/blueprint/zero-density-chapter.html), the [ANTEDB source repository](https://github.com/teorth/expdb), PNT+'s [`ZetaBounds.lean`](https://github.com/AlexKontorovich/PrimeNumberTheoremAnd/blob/main/PrimeNumberTheoremAnd/ZetaBounds.lean), the source [Guth–Maynard paper](https://arxiv.org/html/2405.20552v2), and the explicit [Ingham zero-density formulation](https://arxiv.org/abs/2507.15184). ANTEDB provides TeX and executable Python rather than Lean. PNT+ contains proved supporting analysis but its current explicit zero-density file retains admitted gaps. These sources guide implementation; only the pinned Lean build and dependency audit determine completion.
...
| Truncated zeta at zeros | `ZetaTruncation.lean` proves the exact Euler–Maclaurin relation from Mathlib finite Abel summation and the project's existing Abel continuation. `zeta_zero_truncation` and `norm_zeta_zero_truncation_error_le` give the public zero-specialized identity and uniform quantitative error. Package A is complete. |
...
| Downstream work | Concrete Huxley and concrete Ingham remain unproved. The selected route now requires targeted van der Corput/Weyl estimates, the full Montgomery–Halász–Huxley `T * N^4 * V⁻⁶` branch, the finite exact-`σ` density transfer, and final assembly for symmetric multiplicity-weighted `N`. The completed contour infrastructure is retained but is not on the critical path. |
| Verification | The focused truncation build and root production build pass without warnings. `logs/overall_proof_20260809_032335.log` passes stages 1–4 warning-free. The current direct audit synchronizes 447/447 declarations, explicitly checks all 107 public imported theorems and all ten new #15 declarations, and reports only the unrelated decoupling dependency failure. |
...
| Audit impact | The current explicit and discovered theorem sets agree at 447/447. Every beta/localization theorem, `extractSeparated_native`, every classical-moment theorem, all 107 public PNT+ port theorems, the project argument-principle specialization, the #17 arithmetic theorems, and the new #15 truncation theorems pass with only `propext`, `Classical.choice`, and `Quot.sound`. The only audit failure is the unrelated `l2_decoupling_bound_native`. |
| Principal verification | `logs/overall_proof_20260809_032335.log` passes stages 1–4 warning-free, synchronizes 447/447 declarations, and reports only the single decoupling audit failure. The integrity scan finds no prohibited admitted-proof text or unsafe bypass and exactly the two #19 `Decoupling.lean` axioms. Overall exit code is honestly `1`. |
...
| Verification | The focused arithmetic module and the current root production closure build with zero warnings. The audit synchronizes 447/447 declarations; all five #17 declarations pass with only permitted logical dependencies. `logs/overall_proof_20260809_032335.log` passes stages 1–4 warning-free and fails only on the two pre-existing #19 decoupling axioms and their single transitive audit failure. |
...
| 6 | ~~`RiemannZeta/Audit.lean`~~ | **Completed for all named public theorems in the cleaned tree:** transitive checks cover the synchronized 447-theorem list, including the globally named public declarations in the vendored modules; the only auxiliary declarations are anonymous examples. One current dependency failure is reported honestly. | Easy–moderate—complete |
...
1. **Completed for the production perimeter and kept synchronized through 9 August 2026:** created an explicit, synchronization-checked list, currently containing 447 exported source-level theorems.
...
1. **#15 — Classical finite zero-density endpoints (in progress).** The zero-count foundation, unit-zero theorem, Montgomery mean value, basic Halász–Montgomery, mollifier/rectangle infrastructure, strip growth, coefficient bounds, symmetric-count bridge, and Package A truncated-zeta theorem are proved. The remaining critical path is the three bounded packages above: targeted van der Corput/Weyl estimates; the full Montgomery–Halász–Huxley estimate including `T * N^4 * V⁻⁶`; and the finite exact-`σ` transfer followed by the two concrete endpoint theorems. This classical large-values package is distinct from #19's `GuthMaynardLargeValues`, which is the sole remaining premise of the Central Type I estimate after applying the native mean-value, extraction, and coefficient theorems.
