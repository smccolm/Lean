...
| `Audit.lean` checks transitive dependencies for the synchronized production theorem set. | #6 | **Complete for the cleaned tree's named public theorems:** the current list is 812/812, including the expanded vendored PNT+ sharp-zeta dependency closure, finite-density transfer/powering surface, endpoint arithmetic, and medium-reflection API; the only auxiliary Lean declarations are anonymous examples. The audit correctly remains red on one project-axiom-dependent declaration. |
...
| 6 | **Verified complete as audit infrastructure** | The explicit and discovered theorem sets agree at 812/812 and transitive dependencies are checked. One failure is reported rather than hidden. |
...
|---:|---|---|---|
| 15 | **Classical finite zero-density engine and endpoints — in progress** | Existing work proves exact-β removal for the sharp finite Möbius product, multiplicity-aware common dyadic-witness extraction, bounded-coefficient normalization, exact finite power raising with #17 coefficient control, unrestricted finite MHH application, weighted Weyl bounds for every `σ ≥ 0`, and the exact endpoint range arithmetic. `FiniteDensityExponents.lean` proves that Ingham's zeta-polynomial range is empty and Huxley's is either empty or below the proved Weyl threshold, so uniform stationary reflection is not an endpoint dependency. Complete detector branch/mollifier-scale/power selection, finite exponent assembly, and the two endpoints remain open. | The item stays uncrossed until audit-clean theorems prove both `HuxleyZeroDensity (fun σ T => N σ T)` and `InghamZeroDensity (fun σ T => N σ T)`. |
...
4. `WeylZeta.lean` proves finite Abel summation for antitone weights, identifies the weighted block exactly with `Σ (A+n)^(-1/2-iY³)`, and proves its norm is at most `30 √Y = 30 t^(1/6)` throughout the medium range. `FiniteDensityExponents.lean` now proves that this pointwise threshold covers the only nonempty Huxley zeta-polynomial range, while Ingham's corresponding range is empty. Thus the classical stationary-phase reflection is useful supporting infrastructure but no longer an endpoint dependency. Guth–Maynard Lemma 6.2 is a distinct smooth reflection input for #19 and remains open there.
...
3. **Finite detector, normalization, and powering — substantial completion; branch/scale selection open:** `ClassicalDetector.lean` proves the finite zeta–Möbius cancellation and raw tail witness. `FiniteDensityTransfer.lean` realizes the tail as a finite Dirichlet polynomial, proves coefficient-mass bounds, extracts a common dyadic block with analytic multiplicity, normalizes its coefficients, and applies unrestricted finite MHH. `ClassicalPowering.lean` and `FiniteDensityEndpoint.lean` prove exact finite power raising, #17 factorization bounds, normalization, common powered-block extraction, and the corresponding MHH cardinality estimate. What remains is the global choice of detector branch, mollifier length, power, and scale needed by the endpoint exponents.
4. **Endpoint-range reduction — complete; stationary reflection removed from the acceptance path:** `FiniteDensityExponents.lean` proves `ingham_zeta_polynomial_range_empty`, `huxley_zeta_polynomial_range_empty`, and `huxley_zeta_polynomial_range_below_weyl`. Thus Ingham never enters the zeta-polynomial range; Huxley does not enter it for `σ ≤ 5/6`; and for `σ > 5/6` the entire range lies below `6σ-3`, where the completed Weyl estimate excludes a large zeta block. `MediumReflection.lean` remains audit-clean supporting infrastructure—exact Poisson/cosine representation, public nonstationary expansion, stationary normal form, and two-sided quadratic control—but its unfinished uniform stationary main-term theorem is not required for the two #15 endpoints.
...
6. **Finite Corollary 11.10 and exponent assembly — open:** assemble the completed β-removal, extraction, powering, coefficient, mean-value, MHH, Weyl, and endpoint-range inputs into a direct finite theorem rather than introducing an abstract `LV` supremum framework. Its slab conclusion must use `EpsilonPowerBound`, the project's exact `σ`, and `zeroCountRect σ 1 T (2*T)`.
...
1. **Ingham:** set `τ₀ = 2 - σ`. The generic range ends at `τ = 1` and is discharged by `halasz_montgomery_lemma_native`; the zeta-polynomial range `2 ≤ τ < 4*τ₀/3` is empty for `σ > 1/2`. Prove the interior exponent `3 * (1 - σ) / (2 - σ)`, then combine it with the already proved `σ = 1/2` and `σ = 1` cases.
2. **Huxley:** set `τ₀ = 3 * σ - 1`. Use the basic mean-value theorem for `τ ≤ 1` and the sixth-power Package-C branch for `1 ≤ τ ≤ 4*σ - 2`. The zeta-polynomial range is empty for `σ ≤ 5/6`; for `σ > 5/6`, use the unweighted Weyl-prefix estimate to exclude large zeta blocks for `τ < 6*σ - 3`, together with `4*(3*σ - 1)/3 < 6*σ - 3`. At `σ = 3/4`, derive the boundary from Ingham because both exponents equal `3/5`.
...
The already completed sharp and absolute truncation theorems, finite Möbius-product algebra and raw tail witness, exact-β removal, common multiplicity extraction, finite normalization and powering, mollifier, Jensen, critical-line second moment, rectangle argument principle, strip growth, finite differencing, multi-period B-process bounds, general-`σ` weighted Weyl bound, classical large-values theorem, exact endpoint-range arithmetic, exact Poisson/cosine reflection identity, nonstationary expansion, stationary normal form, and two-sided quadratic phase bounds remain valid reusable library work. Weighted Littlewood, Gabriel interpolation, the standalone zeta fourth moment, right/horizontal contour moments, and a uniform stationary reflection theorem are not mandatory #15 deliverables. Packages A–C and the endpoint-range reduction are complete; Package D still requires global scale/exponent assembly and the two endpoints.
...
| Downstream work | Concrete Huxley and concrete Ingham remain unproved. Exact-β removal, common multiplicity extraction, normalization, generic exact powering, coefficient control, finite MHH application, and endpoint range arithmetic are complete. The selected route now requires global branch/mollifier-scale/power selection, finite Corollary 11.10 exponent assembly, and final symmetric-count specialization. |
| Verification | The focused #15 modules and root production build pass without warnings. Principal run `logs/overall_proof_20260809_194340.log` passes stages 1–4 with zero warnings, synchronizes 812/812 declarations, and reports only the unrelated decoupling dependency failure plus the two direct #19 axioms. |
...
| Audit impact | The current explicit and discovered theorem sets agree at 812/812. Every beta/localization theorem, `extractSeparated_native`, every classical-moment theorem, the complete vendored PNT+ proof closure, the project argument-principle specialization, the #17 arithmetic theorems, and the #15 sharp-truncation, finite-transfer, powering, Weyl, kernel, classical-large-values, endpoint-range, and medium-reflection theorems pass with only `propext`, `Classical.choice`, and `Quot.sound`. The only audit failure is the unrelated `l2_decoupling_bound_native`. |
| Principal verification | `logs/overall_proof_20260809_194340.log` passes stages 1–4 with zero warnings, synchronizes 812/812 declarations, and reports only the single decoupling audit failure. Its integrity scan finds no prohibited admitted-proof text or unsafe bypass and exactly the two #19 `Decoupling.lean` axioms; the principal runner therefore remains honestly `FAIL`/`1` until #19. |
...
| Verification | The focused arithmetic module and the current root production closure build with zero warnings. The audit synchronizes 812/812 declarations; all five #17 declarations pass with only permitted logical dependencies. The remaining failures are the two pre-existing #19 decoupling axioms and their single transitive audit failure. |
...
| 6 | ~~`RiemannZeta/Audit.lean`~~ | **Completed for all named public theorems in the cleaned tree:** transitive checks cover the synchronized 812-theorem list, including the globally named public declarations in the vendored modules; the only auxiliary declarations are anonymous examples. One current dependency failure is reported honestly. | Easy–moderate—complete |
...
1. **Completed for the production perimeter and kept synchronized through 9 August 2026:** created an explicit, synchronization-checked list, currently containing 812 exported source-level theorems.
...
1. **#15 — Classical finite zero-density endpoints (in progress).** The zero-count foundation, unit-zero theorem, Montgomery mean value, basic Halász–Montgomery, mollifier/rectangle infrastructure, strip growth, coefficient bounds, symmetric-count bridge, detector-strength sharp truncation, exact-β removal, common multiplicity extraction, normalized finite powering, pointwise/general-weight van der Corput–Weyl stack, Package-C Montgomery–Halász–Huxley estimate including `T * N^4 * V⁻⁶`, and endpoint range arithmetic are proved. The remaining critical path is global detector branch/mollifier-scale/power selection, finite exponent assembly, and the two concrete endpoint theorems. Medium-length stationary reflection is no longer a dependency. This classical large-values package is distinct from #19's `GuthMaynardLargeValues`, which is the sole remaining premise of the Central Type I estimate after applying the native mean-value, extraction, and coefficient theorems.
