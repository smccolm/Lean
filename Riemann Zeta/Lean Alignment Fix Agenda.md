...
| `Audit.lean` checks transitive dependencies for the synchronized production theorem set. | #6 | **Complete for the cleaned tree's named public theorems:** the current list is 1008/1008, including the native #18 reduction and the #19 shared-language, matrix, trace-dispersion, and first-trace Poisson surfaces. Every declaration passes with only the permitted logical dependencies. |
...
| The paper stated stale audit counts. | Documentation gate | **Resolved during #8 and kept synchronized:** the current audit passes across 1008 synchronized declarations. |

Current clean facts: the production graph and both retained examples elaborate with zero warnings; the dependency audit synchronizes and passes 1008 declarations; and all prohibited-proof and project-postulate scans are empty. The named output gate reports eight absent end theorems after accepting `typeIIFourthMomentReduction_native`; the overall result remains honestly `FAIL`/`1` until the remaining outputs exist.
...
| 6 | **Verified complete as audit infrastructure** | The explicit and discovered theorem sets agree at 1008/1008 and all transitive dependency checks pass. The separate completion gate fails on the eight absent #15/#18/#19 outputs. |
...
The completion of #18's native fourth-moment reduction and #19's Hilbert–Schmidt first-trace Poisson layer changes the most efficient implementation order without closing either owning Shitlist item. The following targets are ordered by estimated implementation difficulty, not by Shitlist number. The former first-trace row has been removed because it is now proved; the remaining uniform two-parameter and cubic-trace work stays separate from the smaller closeouts below.

| Rank | Owning item | Complete closeout target | Why it is now reachable | Exact boundary |
|---:|---:|---|---|---|
| 1 | 18 | **Appendix C contour coverage** | Source-range interfaces, exact initial Möbius cancellation, arbitrary-order Gamma decay, contour integrability, local zero multiplicity, and the downstream fourth-moment reduction are complete. | Prove `typeIContourTypeIICoverOn_native` at `7/10`, including the smoothed series, removable cancellation, rectangle shift, residue, and quantitative tails. |
...
| 3 | 19 | **Package 6: smooth reflection** | The cutoff regularity, affine language, Mellin/Fourier infrastructure, and finite coefficient machinery are present. | Prove Guth–Maynard Lemma 6.2 with uniform truncation and tail bounds and the resulting short polynomial; `S₂` remains downstream of Heath–Brown. |
| 4 | 15 | **Medium Type-I reflection** | `MediumReflection.lean` already contains the exact Poisson formula, stationary point and phase normalization, dyadic frequency window, and two-sided quadratic control. All other #15 finite certificates and the terminal branch are complete. | Prove the uniform stationary main term, dual-block recovery, and ordinate-independent coefficient reduction. This is the highest-payoff closeout because it immediately unlocks the finite reduction and endpoint assembly, but it remains harder than ranks 1–3. |
...
8. **Verification and scope boundary:** the focused and root builds are warning-free. All 25 dichotomy declarations and all five terminal declarations are registered in the synchronized 1008-declaration transitive audit and use only `propext`, `Classical.choice`, and `Quot.sound`. The certificate leaves the medium Type-I scale unclassified; short-scale estimation and the completed terminal case go directly to the finite reduction.
...
| Verification | The focused terminal module and root production build pass without warnings. The current audit passes 1008/1008 dependency checks and all integrity scans but fails the output gate on the two absent #15 endpoints and six other agenda outputs. |
...
**Implementation status, 10 August 2026:** Packages 1–3 are complete. Package 2 includes exact translation invariance, nonzero scaling covariance, affine reindexing, the seven-bin floor partition of tolerance-one energy, and the injection of its zero-defect bin into Mathlib exact additive energy for one-separated sets. Package 3 proves the exact operator-norm conclusions of Guth–Maynard Lemmas 4.1–4.2, including the source cardinality estimate and sixth-root trace-dispersion bound. Package 4 is partially implemented in `LargeValuesPoisson.lean`: the actual `h_t`, its Schwartz realization, endpoint vanishing, Mellin/Fourier zero-mode identity, arbitrary-order uniform zero-mode decay, exact Fourier dilation, source-normalized Poisson formula, finite-support reduction, and the complete Hilbert–Schmidt first-trace expansion of Lemma 4.4 are proved. The last result identifies the zero mode exactly and controls the entire nonzero-frequency tail uniformly by `O(N⁻¹⁰⁰)` after polynomial cardinality loss. The full two-parameter Lemma 4.3 estimates, cubic trace split, and Proposition 5.1 remain open. Packages 5–14 remain open. The audit and integrity portion of Package 14 passes, but final integration does not because the native large-values and concrete density theorems are absent.
...
4. **Formalize Fourier decay, Poisson, and `S₁` — in progress; Lemma 4.4 complete.** `LargeValuesPoisson.lean` proves the chosen source kernel is Schwartz, its fixed-ordinate arbitrary-frequency decay, the Mellin/Fourier representation and arbitrary-order uniform decay of `ĥ_t(0)`, exact dilation `𝓕(h(·/N))(ξ)=N ĥ(Nξ)`, source-normalized Poisson summation, cutoff endpoint vanishing, finite integer support, the exact Gram-trace/Poisson identity, the zero-mode main term, and a uniform `N⁻¹⁰⁰` bound for the complete nonzero-frequency tail under polynomial cardinality. Complete the uniform two-parameter parts of Lemma 4.3, expand the cubic trace as in Lemma 4.5, split the nonzero-frequency configurations into `S₁`, `S₂`, and `S₃`, and prove Proposition 5.1 for `S₁`.
...
| Audit impact | The current explicit and discovered theorem sets agree at 1008/1008. Every beta/localization theorem, `extractSeparated_native`, every classical-moment theorem, the vendored PNT+ proof closure, the native #18 reduction, and the #19 shared-language/matrix/dispersion/Poisson declarations pass with only `propext`, `Classical.choice`, and `Quot.sound`. |
| Principal verification | `logs/overall_proof_20260810_130604.log` passes all four build/example stages warning-free, passes 1008/1008 dependency checks, and finds no admitted proof, unsafe bypass, or project postulate. The named output gate fails on eight absent agenda theorems, so the overall result remains honestly `FAIL`/`1`. |
...
| Verification | The focused arithmetic module and root production closure build with zero warnings. The current audit synchronizes 1008/1008 declarations; all five #17 declarations and the new #18/#19 foundations pass with only permitted logical dependencies. |
...
| 6 | ~~`RiemannZeta/Audit.lean`~~ | **Completed for all named public theorems in the current tree:** the explicit and discovered sets synchronize at 1008/1008, every existing theorem has only the permitted logical dependencies, and the separate completion gate reports eight absent output names. | Easy–moderate—complete |
...
| 19 | `LargeValuesPoisson.lean`, `LargeValues.lean`, the proved matrix/language prerequisites, and new source-section modules | The Hilbert–Schmidt first-trace expansion is complete. The remaining repairs are uniform two-parameter Fourier decay, cubic trace split, `S₁/S₂/S₃`, reflection, Heath–Brown, affine/energy/GCD theorems, native large values, and concrete transfers. | Hardest overall; first trace theorem complete, later source packages open |
...
|---:|---|---|---|
| 1 | `RiemannZeta/GuthMaynard/TypeIICoverage.lean`, `TypeIIContour.lean` | `typeIContourTypeIICoverOn_native`. | Hard: substantial contour shifting, but the Gamma, integrability, cancellation, and multiplicity layers exist. |
| 2 | `RiemannZeta/GuthMaynard/TwistedMoment.lean` plus a new AFE module | Smooth `ζ²` approximate functional equation and complete diagonal estimate. | Hard–extreme: self-contained before the separate DFI off-diagonal theorem. |
| 3 | `RiemannZeta/GuthMaynard/MediumReflection.lean` | Uniform stationary main term, dual block of scale `T/N`, and ordinate-independent coefficient recovery. | Very hard, but it has the highest immediate #15 payoff. |
| 4 | `RiemannZeta/GuthMaynard/LargeValuesPoisson.lean` and subsequent new #19 modules | Uniform two-parameter Lemma 4.3, cubic trace split, and Proposition 5.1 for `S₁`; the first-trace theorem is already complete. | Very hard; no longer mislabeled as one small closeout. |
| 5 | New #19 Heath–Brown, affine, energy, and GCD/spacing modules, then `LargeValues.lean` | Remaining `S₂/S₃` chain and `guthMaynardLargeValues_native`. | Hardest. |
...
1. **Completed for the production perimeter and kept synchronized through 10 August 2026:** created an explicit, synchronization-checked list, currently containing 1008 exported source-level theorems.
...
| Remaining obstruction at that iteration | #8 had no remaining warning or retained-file coverage defect. The superseding 10 August audit has 1008/1008 passes and zero direct project axioms. |
