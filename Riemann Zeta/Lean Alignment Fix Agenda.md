...
|---:|---|---|---|
| 15 | **Zeta zero-count foundations and classical density endpoints — in progress** | The zero-count foundation, pole-safe vertical growth, Jensen local multiplicity theorem, and unconditional Montgomery mean-value dependency are now kernel-checked. Formalize the remaining source chain: the critical-line zeta fourth moment and Littlewood/Gabriel mollifier argument for Ingham; the approximate functional equation and powered large-values estimate for Huxley; then transfer both positive-ordinate results to the project's symmetric multiplicity-weighted `N`. | No project axiom remains in the zero-count layer; `zeta_growth_bound_native`, `zeta_lower_bound_native`, `localZeroMultiplicityBound_native`, `montgomery_mean_value_native`, `HuxleyZeroDensity (fun σ T => N σ T)`, and `InghamZeroDensity (fun σ T => N σ T)` all pass the transitive audit. The item remains uncrossed until both density endpoints are proved. |
...
2. **Zeta-foundation gate — complete:** `ZetaBounds.lean` proves an Abel-integral continuation and the pole-safe polynomial vertical estimate used by Jensen. `ZeroCount.lean` proves `zeta_growth_bound_native`, the right-half-plane bound, and the Euler/Möbius lower bound without project axioms.
3. **Local-zero gate — complete:** `typeI_unit_bin_sum_le_jensen` bridges Mathlib's Jensen divisor sum to the project's analytic multiplicity, and `localZeroMultiplicityBound_native` proves the required uniform unit-bin proposition.
4. **Mean-value dependency gate — complete:** `MeanValueProof.lean` proves `montgomery_mean_value_native` from a continuous mean-square identity, reciprocal-log Hilbert inequality, and local Sobolev sampling. `halasz_montgomery_lemma_native` discharges the downstream premise.
5. **Density gate — open:** primary-source comparison shows that the endpoint names conceal additional indispensable theorems. Ingham requires a critical-line fourth-moment estimate plus the mollified Littlewood/Gabriel zero-detection argument. Huxley requires a source-faithful approximate functional equation and the powered Dirichlet-polynomial large-values estimate. These are internal #15 obligations, not new Shitlist items. After proving them, transfer the positive-ordinate results to symmetric `N` using the already proved conjugation/multiplicity bridge.
...
The foundation and mean-value subpasses have removed the final `ZeroCount.lean` axiom, proved the local-zero theorem, and supplied the unconditional Montgomery/Halász–Montgomery input. The #16 pass has also proved beta removal and unconditional separated extraction. The synchronized audit now covers 269 declarations and has only the unrelated decoupling failure. Completing #15 still requires the concrete Huxley and Ingham propositions. The repository will remain globally incomplete until #15 and #17–#19 pass their own tests.
...
| Downstream work | Concrete Huxley and concrete Ingham remain unproved. Their newly explicit internal obligations are the classical fourth-moment/Littlewood–Gabriel and approximate-functional-equation/powered-large-values arguments. |
...
1. **#15 — Classical density endpoints (in progress).** The zero-count foundation, unit-zero theorem, and Montgomery mean value are proved. Complete the critical-line fourth-moment/zero-detection chain and prove the concrete Huxley and Ingham inputs.
2. **#17 — Arithmetic and mean value.** `MontgomeryMeanValue` is proved; prove the remaining divisor-count and factorization-count propositions.
