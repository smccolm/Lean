...
| `Audit.lean` checks transitive dependencies for the synchronized production theorem set. | #6 | **Complete as integrity infrastructure:** the current list is 1177/1177. It includes the #15 exact medium B-process, the interval-normalized Type-I MHH bridge, the #19 source-to-matrix localization, and cancellation-preserving signed-reflection estimates. Every existing declaration passes with only the permitted logical dependencies; this does not make the missing output theorems complete. |
...
| The paper stated stale audit counts. | Documentation gate | **Resolved during #8 and kept synchronized:** the current audit passes across 1177 synchronized declarations. |

Current clean facts: the production graph elaborates with zero warnings; the dependency audit synchronizes and passes 1177 declarations; and the prohibited-proof and project-postulate scans are empty. The named output gate still reports seven absent end theorems; the overall result remains honestly `FAIL`/`1` until the remaining outputs exist.
...
| 2 | **Verified complete as cleanup** | The retained tree has 80 Lean files, no empty Lean file, a 77-file transitive production graph, `Audit.lean`, and exactly two non-production anonymous-example files. All listed obsolete tests, tombstones, and side-effect utilities are absent. |
...
| 6 | **Verified complete as audit infrastructure** | The explicit and discovered theorem sets agree at 1177/1177 and all transitive dependency checks pass. The separate completion gate fails on the seven absent #15/#18/#19 outputs. |
| 7 | **Verified complete** | The root import closure reaches all 76 subordinate production modules, giving a 77-file production graph. |
...
|---:|---|---|---|
| 15 | **Classical finite zero-density engine and endpoints — in progress** | The smooth B-process, Mellin reflection, deweighting, coefficient-uniform MHH estimate, endpoint-certificate arithmetic, and enlarged-interval-to-`[0,3T]` MHH consumer are proved. `typeI_dichotomy_witness_mhh_bound_native` still accepts a separately supplied witness family and terminal majorant: it does not unpack `ClassicalTypeITypeIIDichotomyConclusion`, relate `τ` to `N` and `T`, discharge the majorant, or route every endpoint branch. The actual Type-I branch consumer, actual-`X` Type-II application, branch-to-slab reduction, and both endpoints remain. | The item stays uncrossed until audit-clean theorems prove both `HuxleyZeroDensity (fun σ T => N σ T)` and `InghamZeroDensity (fun σ T => N σ T)`. |
...
| 2 | 15 | **Actual Type-I dichotomy consumer** | The generic MHH theorem, endpoint arithmetic, and enlarged-interval normalization are present, but no theorem joins the dichotomy structure to its physical scale and discharges all branch hypotheses. | Unpack the Type-I branch, define and prove the logarithmic scale relation, discharge the terminal majorant, and route direct, powered, and Weyl subranges. |
| ~~3~~ | 19 | **Source-to-matrix entry bridge — complete** | `sourceDirichletPoly_eq_three_gmSmooth` proves an exact three-piece source identity; `source_large_values_localize_to_matrix` selects one fixed scale/coefficient family on at least one third of the ordinates and applies the matrix bound. The published interval is already `(N,2N]`; a separate theorem records compatibility with the alternate closed convention. | Kernel-checked and included in the 1177-declaration audit. |
| 4 | 19 | **Quantitative smooth reflection** | Exact signed modes, Mellin factorization, pointwise stationary bounds, far-frequency decay, and cancellation-preserving positive, negative, and signed aggregate bounds are present. | Extract the uniform `T₀⁻¹ᐟ²` factor without destroying the retained polynomial, sum both omitted tails uniformly, and prove the complete Lemma 6.2 inequality. |
| 5 | 18 | **Smooth `ζ²` approximate functional equation and diagonal bound** | The exact `M²` expansion, support, coefficient bound, admissible length, target integral, and full Appendix C coverage are fixed. | Close `AF`; keep the DFI off-diagonal theorem separate. |

Completed supporting layer: **#19 exact reflection identities and cancellation preservation.** `LargeValuesReflection.lean` proves `gmSmoothReflection_native`, identifies the old `norm_gmSmoothReflection_native_le` as a coarse factor-count estimate, and supplies replacement positive, negative, and signed bounds with no mode-cardinality loss. These do not yet extract the uniform `T₀⁻¹ᐟ²` factor or sum omitted modes into the quantitative Lemma 6.2 remainder, so the smooth-reflection closeout remains open.
...
11. **Type-I finite-window helpers — corrected audit 11 August 2026:** `typeI_finite_window_resolution_native` packages the coefficient-uniform MHH estimate with an independently supplied endpoint certificate. `typeI_dichotomy_witness_mhh_bound_native` additionally translates the dichotomy's enlarged ordinate interval into `[0,3T]`. Neither theorem consumes `ClassicalTypeITypeIIDichotomyConclusion`, links `τ` to the physical scale, or proves the terminal-majorant hypothesis. The helpers are kernel-checked; the actual Type-I branch resolution `ZR` is open.
12. **Actual Type-II powered MHH application — open, finite glue:** generalize the `X=1` theorem to `sharpMollifiedLineCoeff Y X σ` with the dichotomy's genuine `X`, use #17 to absorb its divisor-size base coefficients before powering, and apply the endpoint certificate. No new analytic estimate should be postulated.
...
8. **Verification and scope boundary:** the focused Type-I build is warning-free. The exact B-process, uniform summed tail, smoothing, Mellin reflection, deweighting, generic finite-window/certificate declarations, and enlarged-interval MHH consumer are registered in the synchronized 1177-declaration transitive audit and use only `propext`, `Classical.choice`, and `Quot.sound`. `ZR` remains open because no theorem consumes the dichotomy structure, relates its scale, discharges the terminal majorant, and routes every endpoint branch. The actual-`X` Type-II theorem and later branch-to-slab theorem also remain open.
...
The completed truncation, detector dichotomy, exact-β removal, extraction, normalization/powering infrastructure, MHH/Weyl estimates, endpoint arithmetic, terminal control, smoothing, exact `T/N` reflection, deweighting, and finite Type-I interval normalization remain valid. Weighted Littlewood, Gabriel interpolation, the standalone zeta fourth moment, and contour-edge moments are not mandatory for #15. The open path is: consume the dichotomy and discharge its scale/majorant/endpoint branches; prove the actual-`X` Type-II powered MHH application; perform the branch-to-slab reduction; and prove both endpoints.
...
| Verification | The focused Type-I module passes without warnings. The current audit passes 1177/1177 dependency checks but fails the output gate on the two absent #15 endpoints and five other agenda outputs. |
...
2. **#19 quantitative smooth reflection — open.** `LargeValuesReflection.lean` proves arbitrary-order uniform two-parameter decay, explicit pointwise far-frequency control, exact cutoff Mellin inversion and Fubini transfer, a first/second-derivative bound `10 / √τ`, signed-mode conjugation, exact finite reflected sums, and cancellation-preserving positive, negative, and signed aggregate bounds with the complete Dirichlet polynomials inside the integrals. The public `gmSmoothReflection_native` is an identity, not the quantitative Lemma 6.2 closeout. Uniform aggregate `T₀⁻¹ᐟ²` extraction and the omitted-mode `T⁻¹⁰⁰` estimate remain.
3. **#15 Type-I interface chain — partly closed.** `TypeISmoothing.lean` closes source-faithful common smooth-block extraction; `TypeIReflection.lean` proves exact boundary removal, scaled Poisson summation, the symmetric finite dual window, exact Mellin retained-mode assembly, and a uniform summed `Q^-101 M^-100` far tail; `TypeIFourierDeweight.lean` closes both deweighting identities. `TypeIFiniteWindow.lean` proves the generic MHH/certificate helper and enlarged-interval normalization, but not `ZR`: the dichotomy consumer, scale link, terminal-majorant discharge, and endpoint routing remain. The actual-`X` Type-II application and slab reduction remain afterward.
...
**Implementation result.** `PS`, `MR`, and `FD` are complete and audit-clean. `MR` exports `mediumTypeIExactBProcess_native`, with exact source-boundary removal, the complete finite-window/far-tail Poisson split, and a summed uniform remainder; `TypeIFiniteWindow.lean` adds an enlarged-interval MHH bridge but leaves `ZR` open. The source/Lean bijection and normalization correction are in `Type I Reflection Proof Bijection.md`. `TwistedDiagonal.lean` proves #18's finite `hm = kn` phase cancellation. `LargeValuesCubic.lean` closes its cubic/`S₁` package; `LargeValuesReflection.lean` closes only the exact reflection-identity layer, not quantitative Lemma 6.2.
...
6. **Prove quantitative smooth reflection — in progress.** `LargeValuesReflection.lean` proves uniform Fourier decay, exact Mellin/Fubini rescaling, the critical oscillatory estimate, signed positive/negative reflected-mode identities, and cancellation-preserving aggregate bounds retaining the finite Dirichlet polynomials inside the integrals. The new signed theorem removes the mode-cardinality loss from the old coarse `norm_gmSmoothReflection_native_le`; it still does not supply the source-uniform `T₀⁻¹ᐟ²` extraction or the summed two-sided `T⁻¹⁰⁰` omitted-mode remainder. Those are the acceptance boundary for this package.
...
| Audit impact | The current explicit and discovered theorem sets agree at 1177/1177. Every beta/localization theorem, `extractSeparated_native`, every classical-moment theorem, the vendored PNT+ proof closure, native #18 coverage and reduction, and the #15/#18/#19 proved foundations pass with only `propext`, `Classical.choice`, and `Quot.sound`. |
| Principal verification | `logs/overall_proof_20260811_203341.log` records the current runner: stages 1–4 pass warning-free, the audit synchronizes and passes 1177/1177 existing declarations, and all integrity scans pass. Its named output gate fails on seven absent agenda theorems, so the overall result remains honestly `FAIL`/`1`. |
...
| Verification | The focused arithmetic module and root production closure build with zero warnings. The current audit synchronizes 1177/1177 declarations; all five #17 declarations and the proved #15/#18/#19 foundations pass with only permitted logical dependencies. |
...
| 6 | ~~`RiemannZeta/Audit.lean`~~ | **Completed for all named public theorems in the current tree:** the explicit and discovered sets synchronize at 1177/1177, every existing theorem has only the permitted logical dependencies, and the separate completion gate reports seven absent output names. | Easy–moderate—complete |
| 7 | ~~`RiemannZeta.lean`~~ | **Completed and maintained:** the root reaches all 76 subordinate production modules, including the six vendored PNT+ files and the current #15/#18/#19 modules. The deleted false `Decoupling` module is not imported. | Easy—complete |
...
| 15 | `TypeIFiniteWindow.lean`, `FiniteDensityEndpoint.lean`, `ClassicalDichotomy.lean`, and the finite-density consumers | Smooth extraction, exact medium B-process, Mellin reflection, deweighting, generic Type-I MHH, and endpoint arithmetic are complete. Add the actual Type-I dichotomy consumer, actual-`X` Type-II theorem, branch-to-slab reduction, and endpoints. | Very hard—open; both branch consumers precede the slab theorem |
...
| 19 | `LargeValuesLocalization.lean`, `LargeValuesMatrix.lean`, `LargeValuesPoisson.lean`, `LargeValuesCubic.lean`, `LargeValuesReflection.lean`, and `LargeValues.lean` | The published interval convention, exact three-piece source-to-smooth localization, matrix entry theorem, trace/Poisson, exact cubic split, `S₁`, pointwise reflection, finite signed-mode identities, and cancellation-preserving signed aggregate bounds are kernel-checked. Missing are the uniform `T₀⁻¹ᐟ²`/summed-tail quantitative Lemma 6.2 conclusion, Heath–Brown, `S₂`, energy/`S₃`, affine/GCD theorems, native large values, and transfers. | Hardest overall; quantitative reflection and source theorems precede the later packages |
...
| 3 | `TypeIFiniteEstimates.lean`, `FiniteScaleAssembly.lean`, `TypeIFiniteWindow.lean`, and a finite branch consumer | Join the actual dichotomy Type-I witness to the generic cardinality theorem and every endpoint scale branch. | **Open:** the existing theorem is a generic helper and does not close `ZR`. |
...
1. **Completed for the production perimeter and kept synchronized through 11 August 2026:** created an explicit, synchronization-checked list, currently containing 1177 exported source-level theorems.
...
| Coverage | At that iteration the tree contained 44 Lean files and a 41-file production graph. The current superseding inventory is 80 Lean files: a 77-file root closure, `Audit.lean`, and two separately executed examples. No retained Lean file is outside the human-facing evaluation. |
...
| Remaining obstruction at that iteration | #8 had no remaining warning or retained-file coverage defect. The superseding 11 August audit has 1177/1177 passes and zero direct project axioms. |
...
1. **#15 — Classical finite zero-density endpoints (in progress).** The dichotomy, common extraction, exact powering infrastructure, `X=1` theorem, coefficient growth, MHH/Weyl estimates, endpoint arithmetic, terminal Type-I estimate, exact smoothing, complete medium B-process with summed far tail, Mellin retained-mode reflection, both deweighting identities, generic sharp-polynomial Type-I MHH helper, and enlarged-interval normalization are proved. `ZR` remains open at the dichotomy/scale/majorant/endpoint consumer. Type II still needs `IIE` because `powered_sharp_block_large_values_bound` is hard-coded to `X=1`; both branch consumers then meet at `FR → ZD`. This classical package is distinct from #19's `GuthMaynardLargeValues`.
