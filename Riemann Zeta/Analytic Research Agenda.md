...
- Shitlist #15's interior classical density analysis remains open. The detector-strength sharp truncation, exact-β removal, multiplicity-aware common dyadic extraction, bounded-coefficient normalization, exact finite power raising, #17 factorization control, unrestricted finite MHH application, weighted Weyl estimates for every `σ ≥ 0`, and endpoint range arithmetic are kernel-checked. The range arithmetic shows that uniform stationary reflection is not required for either endpoint. The remaining critical path is global detector branch/mollifier-scale/power selection, a finite exact-`σ` Corollary-11.10 exponent assembly, and the concrete endpoints. Medium-reflection infrastructure, weighted Littlewood, Gabriel interpolation, a standalone zeta fourth moment, and the remaining contour-edge moments are optional supporting work rather than #15 blockers. The `σ = 3/4` Huxley boundary will be inherited from Ingham because both target exponents equal `3/5` there.
...
1. Complete #15 through global detector scale/power selection, a finite exact-`σ` Corollary-11.10 exponent assembly, and the concrete Ingham and Huxley propositions. Exact-β removal, multiplicity extraction, normalization, finite powering, coefficient control, weighted Weyl, the full classical Montgomery–Halász–Huxley theorem, and the endpoint range reduction are kernel-checked and audited. The range reduction shows that medium-length stationary reflection is not required for either endpoint.
