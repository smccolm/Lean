...
- Shitlist #15 remains open. Exact powering infrastructure and `powered_sharp_block_large_values_bound` are proved, but that theorem uses cutoff `X=1`; the actual Type-II dichotomy uses `X=⌊T^(δ₂/2)⌋`, so its powered MHH application is still open. The Type-I chain is cutoff removal/common smoothing, scale split, medium-only B-process, shared deweighting, and MHH/Weyl resolution. Both branches then need a certificate-consuming slab theorem and the endpoints.
...
2. **#15 Type-I smoothing and scale split:** prove cutoff inactivity and common smoothing from the dichotomy and terminal restriction.
3. **#18 smooth `ζ²` AFE and diagonal:** close this independent node while keeping DFI separate.
4. **#15 medium B-process and shared deweighting:** recover the weighted dual `T/N` block, then use a separate theorem shared with the short branch.
...
- [PNT+](https://github.com/AlexKontorovich/PrimeNumberTheoremAnd): reusable Mellin, zeta, Poisson, and rectangle-contour infrastructure. It does not supply the needed rapid Gamma decay, `ζ²` approximate functional equation, quadratic-divisor theorem, or stationary-phase theorem.
