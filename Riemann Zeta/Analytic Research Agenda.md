...
- Shitlist #15's interior classical density analysis remains open, but its zero-specialized Euler–Maclaurin, targeted van der Corput/Weyl, and full finite Montgomery–Halász–Huxley large-values packages are complete. The public large-values theorem includes the decisive `T * N^4 * V⁻⁶` branch. The remaining selected route is the exact-`σ` finite transfer to the project's multiplicity-weighted count and endpoint assembly. Weighted Littlewood, Gabriel interpolation, a standalone zeta fourth moment, and the remaining contour-edge moments are retained as an optional alternative route rather than mandatory #15 blockers. The `σ = 3/4` Huxley boundary will be inherited from Ingham because both target exponents equal `3/5` there.
...
1. Complete #15 through its remaining finite exact-`σ` density transfer and assemble the concrete Ingham and Huxley propositions. The truncated-zeta, van der Corput/Weyl, and full classical Montgomery–Halász–Huxley packages are kernel-checked and audited.
