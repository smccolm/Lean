...
- Shitlist #15's interior classical density analysis remains open. The selected route is a finite zero-density-from-large-values proof: a zero-specialized Euler–Maclaurin truncation of zeta, targeted van der Corput B/A processes with the `(1/2,1/2)` and `(1/6,2/3)` exponent pairs, the Weyl critical-line zeta bound, the full Montgomery–Halász–Huxley estimate including `T * N^4 * V⁻⁶`, and an exact-`σ` finite transfer to the project's multiplicity-weighted count. Weighted Littlewood, Gabriel interpolation, a standalone zeta fourth moment, and the remaining contour-edge moments are retained as an optional alternative route rather than mandatory #15 blockers. The `σ = 3/4` Huxley boundary will be inherited from Ingham because both target exponents equal `3/5` there.
...
1. Complete #15 through four bounded packages: prove the zero-specialized truncated-zeta relation; selectively port the required `expdb` logarithmic-phase machinery and prove the targeted van der Corput/Weyl estimates; prove the full classical Montgomery–Halász–Huxley large-values theorem; then prove the finite exact-`σ` density transfer and assemble the concrete Ingham and Huxley propositions.
