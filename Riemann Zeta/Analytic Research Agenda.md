...
- Shitlist #15's interior classical density analysis remains open, but its zero-specialized Euler–Maclaurin and targeted van der Corput/Weyl packages are complete. The latter includes the `(1/2,1/2)` B-process, corrected A-process, floor-selected `(1/6,2/3)` exponent bound with uniform prefixes, and exact weighted critical-line block theorem. The remaining selected route is the full Montgomery–Halász–Huxley estimate including `T * N^4 * V⁻⁶`, followed by an exact-`σ` finite transfer to the project's multiplicity-weighted count. Weighted Littlewood, Gabriel interpolation, a standalone zeta fourth moment, and the remaining contour-edge moments are retained as an optional alternative route rather than mandatory #15 blockers. The `σ = 3/4` Huxley boundary will be inherited from Ingham because both target exponents equal `3/5` there.
...
1. Complete #15 through its two remaining bounded packages: prove the full classical Montgomery–Halász–Huxley large-values theorem; then prove the finite exact-`σ` density transfer and assemble the concrete Ingham and Huxley propositions. The truncated-zeta and van der Corput/Weyl packages are already kernel-checked and audited.
