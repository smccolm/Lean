...
theorem probe_integrable_complex_laplace
...
noncomputable def probeComplexLaplace (s a : ℂ) : ℂ :=
  ∫ x : ℝ in Set.Ioi 0, (x : ℂ) ^ (s - 1) * cexp (-(a * x))

theorem probe_hasDerivAt_complexLaplace
    {s a : ℂ} (hs : 0 < s.re) (ha : 0 < a.re) :
    HasDerivAt (probeComplexLaplace s)
      (∫ x : ℝ in Set.Ioi 0,
  ...
  have hFInt : Integrable (F a) (volume.restrict (Set.Ioi 0)) := by
    simpa only [F] using probe_integrable_complex_laplace hs ha
  ...
  have hBound : ∀ᵐ x ∂(volume.restrict (Set.Ioi 0)),
    ...
    simp only [sub_re, one_re, neg_re, mul_re, ofReal_re, ofReal_im,
      mul_zero, sub_zero]
    have hexp : Real.exp (-(b.re * x)) ≤ Real.exp (-(r * x)) := by
  ...
  simpa only [probeComplexLaplace, F, F'] using h.2

theorem probe_analyticOnNhd_complexLaplace {s : ℂ} (hs : 0 < s.re) :
    AnalyticOnNhd ℂ (probeComplexLaplace s) {a : ℂ | 0 < a.re} := by
  have hopen : IsOpen {a : ℂ | 0 < a.re} :=
    isOpen_lt continuous_const continuous_re
  apply (show DifferentiableOn ℂ (probeComplexLaplace s) {a : ℂ | 0 < a.re} by
    intro a ha
    exact (probe_hasDerivAt_complexLaplace hs ha).differentiableAt.differentiableWithinAt
    ).analyticOnNhd hopen

theorem probe_complexLaplace_ofReal
    {s : ℂ} (hs : 0 < s.re) {r : ℝ} (hr : 0 < r) :
    probeComplexLaplace s (r : ℂ) = (r : ℂ) ^ (-s) * Gamma s := by
  have h := integral_cpow_mul_exp_neg_mul_Ioi_eq hs hr
  unfold probeComplexLaplace
...
theorem probe_complexLaplace_eq
    {s a : ℂ} (hs : 0 < s.re) (ha : 0 < a.re) :
    probeComplexLaplace s a = a ^ (-s) * Gamma s := by
  let U : Set ℂ := {b : ℂ | 0 < b.re}
  let F : ℂ → ℂ := probeComplexLaplace s
  let G : ℂ → ℂ := fun b => b ^ (-s) * Gamma s
  have hF : AnalyticOnNhd ℂ F U := by
    simpa only [F, U] using probe_analyticOnNhd_complexLaplace hs
  ...
  have hReal : ∃ᶠ (x : ℝ) in 𝓝[≠] (1 : ℝ), F x = G x := by
    have hEventually : ∀ᶠ x : ℝ in 𝓝 (1 : ℝ), F x = G x := by
      ...
      exact probe_complexLaplace_ofReal hs hx
