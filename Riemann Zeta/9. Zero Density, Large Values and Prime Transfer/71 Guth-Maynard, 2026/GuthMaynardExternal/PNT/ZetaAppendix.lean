/- Adapted from PrimeNumberTheoremAnd/IEANTN/ZetaAppendix.lean at upstream
revision 4ecb950126c4290293c5662dfe0e884123171df5 (Apache-2.0). The unrelated
Mertens import is omitted from this local proof closure. -/
...
import GuthMaynardExternal.PNT.ZetaBoundsUpstream
...
private theorem integral_power_phase_ibp (σ : ℝ) (φ : ℝ → ℝ) (a b : ℝ) (hab : a < b) (ha_pos : 0 < a)
...
private theorem cpow_split_re_im (t : ℝ) (s : ℂ) (ht : 0 < t) :
...
/-- Public phase decomposition of a complex Dirichlet monomial multiplied by
a Fourier exponential. -/
theorem cpow_mul_fourier_exp_eq_log_phase
    (t : ℝ) (s : ℂ) (ν : ℝ) (ht : 0 < t) :
    (t : ℂ) ^ (-s) *
        Complex.exp (2 * Real.pi * I * ((ν * t : ℝ) : ℂ)) =
      ((t ^ (-s.re) : ℝ) : ℂ) *
        Complex.exp (2 * Real.pi * I *
          ((ν * t - (s.im / (2 * Real.pi)) * Real.log t : ℝ) : ℂ)) := by
  simpa only [e] using phase_rewrite t s ν ht
...
-/)
  (latexEnv := "lemma")
  (discussion := 546)]
private theorem lemma_aachIBP (s : ℂ) (ν : ℝ) (hν : ν ≠ 0) (a b : ℝ)
...
-/)
  (latexEnv := "lemma")
  (discussion := 547)]
private theorem lemma_aachra {a b : ℝ} (ha : a < b) (g : ℝ → ℝ)
...
-/)
  (latexEnv := "lemma")
  (discussion := 549)]
private theorem lemma_aachdecre (σ : ℝ) (hσ : 0 ≤ σ) (τ : ℝ) (ν : ℝ) (hν : ν ≠ 0) (a b : ℝ)
...
-/)
  (latexEnv := "lemma")
  (discussion := 550)]
private lemma deriv_e {φ : ℝ → ℝ} {t : ℝ} (hφ : DifferentiableAt ℝ φ t) :
...
set_option backward.isDefEq.respectTransparency false in
private theorem lemma_aachfour (s : ℂ) (hsigma : 0 ≤ s.re) (ν : ℝ) (hν : ν ≠ 0) (a b : ℝ)
    (ha : a > |s.im| / (2 * π * |ν|)) (hb : b > a) :
    let φ : ℝ → ℝ := fun t ↦ ν * t - (s.im / (2 * π)) * Real.log t
...
/-- Public first-order expansion for a non-stationary Fourier mode of a
complex power.  The hypotheses place the stationary point strictly to the
left of the integration interval. -/
theorem nonstationary_power_phase_expansion
    (s : ℂ) (hsigma : 0 ≤ s.re) (ν : ℝ) (hν : ν ≠ 0) (a b : ℝ)
    (ha : a > |s.im| / (2 * Real.pi * |ν|)) (hb : b > a) :
    let φ : ℝ → ℝ := fun t ↦ ν * t - (s.im / (2 * Real.pi)) * Real.log t
    let Φ : ℝ → ℂ := fun t ↦ (t ^ (-s.re) : ℝ) *
      Complex.exp (2 * Real.pi * I * (φ t : ℂ)) /
        (2 * Real.pi * I * (deriv φ t))
    let ϑ : ℝ := s.im / (2 * Real.pi * a)
    ∃ E, ∫ t in Set.Icc a b,
        t ^ (-s) * Complex.exp (2 * Real.pi * I * ((ν * t : ℝ) : ℂ)) =
          Φ b - Φ a + ((a ^ (-s.re - 1) : ℝ) / (2 * Real.pi ^ 2)) * E ∧
      ‖E‖ ≤ s.re / (|ν - ϑ| ^ 2) + |ϑ| / (|ν - ϑ| ^ 3) := by
...
private lemma _root_.Real.IsHalfInteger.floor_add_three_halfs (M : ℝ) : (↑⌊M⌋ + 3 / 2 : ℝ).IsHalfInteger :=
  ⟨⌊M⌋ + 1, by push_cast; ring⟩

private lemma _root_.Real.floor_add_three_halfs_gt (M : ℝ) : M < ↑⌊M⌋ + 3 / 2 := by
  linarith [Int.lt_floor_add_one M]


/-- At half-integers, `(Φ n t + Φ (-n) t) / 2 = Ψ t` where `Φ` and `Ψ` are as in `lemma_aachcanc`. -/
private lemma lemma_aachcanc_pointwise (s : ℂ) {n : ℤ} (hn : n ≠ 0)
...
-/)
  (latexEnv := "lemma")
  (discussion := 551)]
private theorem lemma_aachcanc (s : ℂ) {n : ℤ} (hn : 0 < n) {a b : ℝ}
...
-/)
  ...
  (discussion := 552)]
private theorem proposition_applem (s : ℂ) (hsigma : 0 ≤ s.re) {a b : ℝ} (ha : a > |s.im| / (2 * π))
...
-/)
  (latexEnv := "lemma")
  (discussion := 566)]
private theorem lemma_abadeulmac' {b : ℕ} (hb : 0 < b) {s : ℂ}
...
-/)
  (latexEnv := "lemma")
  (discussion := 566)]
private theorem lemma_abadeulmac {b : ℝ} (hb : 0 < b) (hb' : b.IsHalfInteger) {s : ℂ}
    (hs1 : s ≠ 1) (hsigma : 0 < s.re) :
    ∑ n ∈ Icc 1 ⌊b⌋₊, (n : ℂ) ^ (-s) =
...
-/)
  (latexEnv := "lemma")
  (discussion := 567)]
private theorem lemma_abadtoabsum {a b : ℝ} (ha : 0 < a) (hb' : b.IsHalfInteger) (hab : b > a) {s : ℂ}
    (hs1 : s ≠ 1) (hsigma : 0 < s.re) :
...
private lemma lemma_abadsumas_integrable_explog {s : ℂ} {a b : ℝ} (ha : 0 < a) (hab : a < b) (k : ℤ) :
    IntervalIntegrable
...
set_option backward.isDefEq.respectTransparency false in
private lemma lemma_abadsumas_sum_fourier (s : ℂ) {a b : ℝ} (ha : 0 < a)
    (hab : a < b) :
    let f : ℝ → ℂ := fun y ↦
...
/-- Each paired Fourier coefficient in `poisson_partial_zeta_sum` is the
corresponding cosine oscillatory integral. -/
theorem partial_zeta_fourier_pair_eq_cosine_integral
    (s : ℂ) {a b : ℝ} (ha : 0 < a) (hab : a < b) (n : ℕ) :
    let f : ℝ → ℂ := fun y ↦
      if a ≤ y ∧ y ≤ b then
        (y ^ (-s.re) : ℝ) *
          Complex.exp (2 * π * I *
            ((-(s.im / (2 * π)) * Real.log y : ℝ) : ℂ))
      else 0
    FourierTransform.fourier f (n + 1) +
        FourierTransform.fourier f (-(n + 1 : ℤ)) =
      2 * ∫ y in a..b,
        (y : ℂ) ^ (-s) * Real.cos (2 * π * (n + 1) * y) := by
  simpa only [e] using lemma_abadsumas_sum_fourier s ha hab n
...
private lemma exp_geometric_partial_sums_bounded {x : ℝ} (hx : ¬ ∃ k : ℤ, x = k) :
    ∃ C : ℝ, 0 ≤ C ∧
      ∀ N : ℕ,
...
private lemma cauchySeq_exp_over_nat_of_not_int {x : ℝ} (hx : ¬ ∃ k : ℤ, x = k) :
    CauchySeq fun N : ℕ ↦
      ∑ n ∈ range N,
        ((1 : ℝ) / (n + 1 : ℝ)) •
...
private lemma exists_tendsto_exp_over_nat_of_not_int {x : ℝ} (hx : ¬ ∃ k : ℤ, x = k) :
    ∃ L : ℂ, Tendsto
      (fun N : ℕ ↦
        ∑ n ∈ range N,
          ((1 : ℝ) / (n + 1 : ℝ)) •
...
private lemma tendsto_exp_over_nat_eq_neg_log_of_not_int {x : ℝ} (hx : ¬ ∃ k : ℤ, x = k) :
    Tendsto
      (fun N : ℕ ↦
        ∑ n ∈ range N,
          ((1 : ℝ) / (n + 1 : ℝ)) •
...
private lemma tendsto_sine_series_unit_interval {x : ℝ} (hx0 : 0 < x) (hx1 : x < 1) :
    Tendsto
      (fun N : ℕ ↦
...
private lemma tendsto_sine_series_of_not_int {x : ℝ} (hx : ¬ ∃ k : ℤ, x = k) :
    Tendsto
      (fun N : ℕ ↦
        ∑ n ∈ range N, Real.sin (2 * Real.pi * (n + 1) * x) / (n + 1 : ℝ))
...
private lemma tendsto_sine_over_pi_series_of_not_int {x : ℝ} (hx : ¬ ∃ k : ℤ, x = k) :
    Tendsto
      (fun N : ℕ ↦
...
private lemma B1_eq_fract_sub_half {x : ℝ} (hx0 : 0 ≤ x) :
...
private lemma tendsto_sine_over_pi_series_neg_B1 {x : ℝ} (hx0 : 0 ≤ x)
    (hx : ¬ ∃ k : ℤ, x = k) :
    Tendsto
      (fun N : ℕ ↦
        ∑ n ∈ range N, Real.sin (2 * Real.pi * (n + 1) * x) /
...
private lemma tendsto_boundary_sine_terms {a b : ℝ} (ha0 : 0 ≤ a) (hb0 : 0 ≤ b)
...
private lemma hasDerivAt_sine_antideriv (n : ℕ) (x : ℝ) :
    HasDerivAt
      (fun y : ℝ ↦
...
private lemma cosine_integral_by_parts_cpow {a b : ℝ} (ha : 0 < a) (hab : a < b)
    (s : ℂ) (n : ℕ) :
    2 * ∫ y in a..b, (y : ℂ) ^ (-s) *
...
private lemma hasSum_cos_over_pi_sq_unit_raw {x : ℝ} (hx : x ∈ Set.Icc (0 : ℝ) 1) :
    HasSum
      (fun n : ℕ ↦
        ((n + 1 : ℝ) ^ (2 * 1))⁻¹ *
...
private lemma hasSum_cos_over_pi_sq_fract (x : ℝ) :
    HasSum
      (fun n : ℕ ↦
        ((n + 1 : ℝ) ^ (2 * 1))⁻¹ *
          Real.cos (2 * Real.pi * (n + 1 : ℝ) * x) / Real.pi ^ 2)
...
private lemma summable_cos_over_pi_sq_fract (x : ℝ) :
    Summable
      (fun n : ℕ ↦
        ((n + 1 : ℝ) ^ (2 * 1))⁻¹ *
          Real.cos (2 * Real.pi * (n + 1 : ℝ) * x) / Real.pi ^ 2) :=
...
private lemma hasSum_cos_antideriv_real (x : ℝ) :
    HasSum
      (fun n : ℕ ↦
...
private lemma hasSum_cos_antideriv_complex (x : ℝ) :
    HasSum
      (fun n : ℕ ↦
        ((-Real.cos (2 * Real.pi * (n + 1 : ℝ) * x) /
...
private lemma tendsto_cos_antideriv_partial (x : ℝ) :
    Tendsto
      (fun N : ℕ ↦
        ∑ n ∈ range N,
          ((-Real.cos (2 * Real.pi * (n + 1 : ℝ) * x) /
...
private lemma hasDerivAt_deriv_ofReal_cpow_neg (s : ℂ) {x : ℝ} (hx : 0 < x) :
    HasDerivAt
...
private lemma continuousOn_second_deriv_ofReal_cpow_neg (s : ℂ) {a b : ℝ} (ha : 0 < a) :
    ContinuousOn
...
private lemma hasDerivAt_cos_antideriv (n : ℕ) (x : ℝ) :
    HasDerivAt
      (fun y : ℝ ↦
        ((-Real.cos (2 * Real.pi * (n + 1 : ℝ) * y) /
          (2 * Real.pi ^ 2 * (n + 1 : ℝ) ^ 2)) : ℂ))
...
private lemma deriv_kernel_integral_by_parts_cpow {a b : ℝ} (ha : 0 < a) (hab : a < b)
    (s : ℂ) (n : ℕ) :
    ∫ y in a..b,
        deriv (fun t : ℝ ↦ (t : ℂ) ^ (-s)) y *
          ((Real.sin (2 * Real.pi * (n + 1 : ℝ) * y) /
...
private lemma deriv_kernel_partial_sum_second_ibp {a b : ℝ} (ha : 0 < a) (hab : a < b)
    (s : ℂ) (N : ℕ) :
    ∑ n ∈ range N,
      ∫ y in a..b,
        deriv (fun t : ℝ ↦ (t : ℂ) ^ (-s)) y *
...
private lemma cos_antideriv_norm_bound (n : ℕ) (x : ℝ) :
    ‖((-Real.cos (2 * Real.pi * (n + 1 : ℝ) * x) /
      (2 * Real.pi ^ 2 * (n + 1 : ℝ) ^ 2)) : ℂ)‖ ≤
...
private lemma summable_one_div_two_pi_sq_nat_add_one_sq :
    Summable fun n : ℕ ↦
...
private lemma hasSum_second_ibp_integral_series {a b : ℝ} (ha : 0 < a) (hab : a < b)
    (s : ℂ) :
    HasSum
      (fun n : ℕ ↦
        ∫ y in a..b,
...
private lemma tendsto_deriv_kernel_second_ibp_limit {a b : ℝ} (ha : 0 < a) (hab : a < b)
    (s : ℂ) :
    Tendsto
      (fun N : ℕ ↦
        ∑ n ∈ range N,
...
private lemma fract_eq_sub_nat_of_mem_Ioo (m : ℕ) {x : ℝ}
...
private lemma B1_eq_sub_nat_sub_half_of_mem_Ioo (m : ℕ) {x : ℝ}
    (hx : x ∈ Set.Ioo (m : ℝ) (m + 1 : ℝ)) :
    B1 x = x - m - 1 / 2 := by
  unfold B1
...
private lemma hasDerivAt_bernoulli2_primitive_of_mem_Ioo (m : ℕ) {x : ℝ}
    (hx : x ∈ Set.Ioo (m : ℝ) (m + 1 : ℝ)) :
    HasDerivAt
      (fun y : ℝ ↦
        (((-(Int.fract y ^ 2 - Int.fract y + 1 / 6) / 2) : ℝ) : ℂ))
...
private lemma continuous_bernoulli2_primitive :
    Continuous
      (fun y : ℝ ↦
        (((-(Int.fract y ^ 2 - Int.fract y + 1 / 6) / 2) : ℝ) : ℂ)) := by
...
private lemma intervalIntegrable_B1_complex {a b : ℝ} (ha : 0 ≤ a) (hab : a ≤ b) :
...
private lemma second_ibp_limit_eq_neg_B1_integral_on_unit {u v : ℝ}
...
private lemma intervalIntegrable_second_ibp_integrand {u v : ℝ} (hu_pos : 0 < u)
    (huv : u ≤ v) (s : ℂ) :
    IntervalIntegrable
      (fun y : ℝ ↦
        (s * (s + 1) * (y : ℂ) ^ (-s - 2)) *
...
private lemma intervalIntegrable_deriv_cpow_mul_B1 {u v : ℝ} (hu_pos : 0 < u)
    (huv : u ≤ v) (s : ℂ) :
    IntervalIntegrable
...
private lemma second_ibp_expr_add_adjacent {a c b : ℝ} (hac : a ≤ c) (hcb : c ≤ b)
    (ha_pos : 0 < a) (s : ℂ) :
...
private lemma neg_B1_integral_add_adjacent {a c b : ℝ} (hac : a ≤ c) (hcb : c ≤ b)
    (ha_pos : 0 < a) (s : ℂ) :
...
private noncomputable def bernoulli2Primitive (y : ℝ) : ℂ :=
  (((-(Int.fract y ^ 2 - Int.fract y + 1 / 6) / 2) : ℝ) : ℂ)

private noncomputable def secondIBPExpr (s : ℂ) (u v : ℝ) : ℂ :=
...
private noncomputable def negB1IntegralExpr (s : ℂ) (u v : ℝ) : ℂ :=
  -(∫ y in u..v,
    deriv (fun t : ℝ ↦ (t : ℂ) ^ (-s)) y * ((B1 y : ℝ) : ℂ))

private lemma secondIBPExpr_eq_negB1_on_unit {u v : ℝ}
    (huv : u ≤ v) (hu_pos : 0 < u) (m : ℕ)
    (hmu : (m : ℝ) ≤ u) (hvm : v ≤ (m + 1 : ℝ)) (s : ℂ) :
...
private lemma secondIBPExpr_add_adjacent {a c b : ℝ} (hac : a ≤ c) (hcb : c ≤ b)
    (ha_pos : 0 < a) (s : ℂ) :
...
private lemma negB1IntegralExpr_add_adjacent {a c b : ℝ} (hac : a ≤ c) (hcb : c ≤ b)
    (ha_pos : 0 < a) (s : ℂ) :
...
private lemma secondIBPExpr_eq_negB1IntegralExpr {a b : ℝ} (ha : 0 < a) (hab : a ≤ b)
    (s : ℂ) :
...
private lemma finite_euler_cpow_neg {a b : ℝ} (ha : 0 < a) (hab : a < b) (s : ℂ) :
    ∑ k ∈ Ioc ⌊a⌋₊ ⌊b⌋₊, (k : ℂ) ^ (-s) =
...
private lemma sum_Icc_one_eq_sum_range_succ {α : Type*} [AddCommMonoid α] (F : ℕ → α) :
...
private lemma fourier_partial_sum_eq_range_cos (s : ℂ) {a b : ℝ} (ha : 0 < a)
    (hab : a < b) (N : ℕ) :
    let f : ℝ → ℂ := fun y ↦
      if a ≤ y ∧ y ≤ b then (y ^ (-s.re) : ℝ) * e (-(s.im / (2 * π)) * Real.log y) else 0
...
private lemma fourier_partial_sum_eq_boundary_sub_deriv_kernel (s : ℂ) {a b : ℝ}
    (ha : 0 < a) (hab : a < b) (N : ℕ) :
    let f : ℝ → ℂ := fun y ↦
      if a ≤ y ∧ y ≤ b then (y ^ (-s.re) : ℝ) * e (-(s.im / (2 * π)) * Real.log y) else 0
    ∑ n ∈ Icc 1 N, (FourierTransform.fourier f n + FourierTransform.fourier f (-n)) =
...
private lemma integral_cpow_neg_eq_main {a b : ℝ} (ha : 0 < a) (hab : a < b)
    {s : ℂ} (hs1 : s ≠ 1) :
    (∫ x in a..b, (x : ℂ) ^ (-s)) =
...
private lemma tendsto_deriv_kernel_to_neg_B1 {a b : ℝ} (ha : 0 < a) (hab : a < b)
    (s : ℂ) :
    Tendsto
      (fun N : ℕ ↦
        ∑ n ∈ range N,
...
private lemma lemma_abadusepoisson_from_deriv_kernel {a b : ℝ}
...
-/)
  (latexEnv := "lemma")
  (discussion := 568)]
private theorem lemma_abadusepoisson {a b : ℝ} (ha : ¬∃ n : ℤ, a = n) (hb : ¬∃ n : ℤ, b = n)
    (hab : b > a) (ha' : 0 < a) {s : ℂ} (hs1 : s ≠ 1) :
    let f : ℝ → ℂ := fun y ↦
      if a ≤ y ∧ y ≤ b then (y ^ (-s.re) : ℝ) * e (-(s.im / (2 * π)) * Real.log y) else 0
    ∃ L : ℂ, Filter.atTop.Tendsto
...
/-- Public Poisson-summation identity for a finite partial-zeta block.  This
exposes the proved PNT+ transformation at the project boundary needed by the
medium-length zero-density reflection; all analytic work remains in the
kernel-checked theorem above. -/
theorem poisson_partial_zeta_sum {a b : ℝ}
    (ha : ¬∃ n : ℤ, a = n) (hb : ¬∃ n : ℤ, b = n)
    (hab : b > a) (ha' : 0 < a) {s : ℂ} (hs1 : s ≠ 1) :
    let f : ℝ → ℂ := fun y ↦
      if a ≤ y ∧ y ≤ b then
    ...
    ∃ L : ℂ, Filter.atTop.Tendsto
      ...
      ∑ n ∈ Ioc ⌊a⌋₊ ⌊b⌋₊, (n : ℂ) ^ (-s) =
        ((b ^ (1 - s) : ℂ) - (a ^ (1 - s) : ℂ)) / (1 - s) + L := by
  simpa only [e] using lemma_abadusepoisson ha hb hab ha' hs1

private lemma trig (z : ℂ) : tan z = - cot (z + π / 2) := by
  simp [Complex.tan, Complex.cot, Complex.cos_add_pi_div_two, neg_div', Complex.sin_add_pi_div_two]

private lemma sin_ne_zero {z : ℂ} (hz : ¬∃ (n : ℤ), n * π / 2 = z) : sin z ≠ 0 :=
  Complex.sin_ne_zero_iff.2 fun k h ↦ hz ⟨2 * k, by grind⟩

private lemma cos_ne_zero {z : ℂ} (hz : ¬∃ (n : ℤ), n * π / 2 = z) : cos z ≠ 0 :=
  Complex.cos_ne_zero_iff.2 fun k h ↦ hz ⟨2 * k + 1, by grind⟩

private lemma trig' {z : ℂ} (hz : ¬∃ (n : ℤ), n * π / 2 = z) : cot z + tan z = 2 / sin (2 * z) := by
...
private lemma trig'' {z : ℂ} (hz : ¬∃ (n : ℤ), n * π / 2 = z) :
    cot z - cot (z + π / 2) = 2 / sin (2 * z) := by
  simp [sub_eq_neg_add, ← trig, ← trig' hz, add_comm]

private lemma hsummable {z : ℂ} (hz : z ∈ integerComplement) :
...
private lemma asummable {z : ℂ} (hz : z ∈ integerComplement) :
...
private lemma neg_one_pow (n : ℕ+) : (-1 : ℂ) ^ (2 * n - 1 : ℕ) = -1 := (neg_one_pow_eq_neg_one_iff_odd
...
private lemma telescoping_sum (z : ℂ) (n : ℕ) :
...
private theorem tsum_even_add_odd' {M : Type*} [AddCommMonoid M] [TopologicalSpace M]
...
-/)
  (latexEnv := "lemma")
  (discussion := 569)]
private theorem lemma_abadeuleulmit1 {z : ℂ} (hz : z ∈ integerComplement) :
    (π / sin (π * z)) =
...
set_option backward.isDefEq.respectTransparency false in
private lemma lemma_abadeulmit2_integral_tsum_inv_sub_int_sq {z w : ℂ}
...
private lemma summable_inv_sub_inv_aux {z w : ℂ} (hz : z ∈ integerComplement) (hw : w ∈ integerComplement) :
...
private lemma lemma_abadeulmit2_integral_eq_cot_diff {z w : ℂ}
...
private lemma lemma_abadeulmit2_continuousAt_integral_tsum_one_div_sub_int_sq {z : ℂ}
  (hz : z ∈ integerComplement) :
...
set_option backward.isDefEq.respectTransparency false in
private lemma lemma_abadeulmit2_tsum_one_div_sub_int_sq {z : ℂ} (hz : z ∈ integerComplement) :
  ∑' (n : ℤ), 1 / (z - n) ^ 2 =
...
private lemma lemma_abadeulmit2_deriv_neg_pi_mul_cot_pi_mul {z : ℂ} (hz : z ∈ integerComplement) :
...
-/)
  (latexEnv := "lemma")
  (discussion := 570)]
private theorem lemma_abadeulmit2 {z : ℂ} (hz : z ∈ integerComplement) :
...
-/)
  (latexEnv := "lemma")
  (discussion := 571)]
private lemma lemma_abadimpseri (ϑ : ℝ) (hϑ : |ϑ| < 1) :
...
private lemma lemma_abadsumas_summable_alternating_theta (θ : ℝ) (hθ : |θ| < 1) :
...
private lemma lemma_abadsumas_quad {s : ℂ} {a : ℝ} (ha : 0 < a)
    (haτ : a > |s.im| / (2 * π)) :
    let ϑ : ℝ := s.im / (2 * π * a)
...
-/)
  (latexEnv := "lemma")
  (discussion := 572)]
private theorem lemma_abadsumas {s : ℂ} (hsigma : 0 ≤ s.re) {a b : ℝ} (ha : 0 < a)
...
private noncomputable def dadaro_g (t : ℝ) : ℂ :=
  if t ≠ 0 then (1 / Complex.sin (π * t) - 1 / (π * t)) / (2 * I) else 0

private lemma proposition_dadaro_b_tendsto_zero_atTop {s : ℂ} (hsigma : 0 < s.re) : Tendsto
...
private lemma proposition_dadaro_zero_lt {s : ℂ} (hs1 : s ≠ 1) (hsigma : 0 < s.re) {a : ℝ} (ha : 0 < a)
...
private lemma proposition_dadaro_zero_eq {s : ℂ} (hs1 : s ≠ 1) (hsigma : 0 = s.re) {a : ℝ} (ha : 0 < a)
    (ha' : a.IsHalfInteger) (haτ : a > |s.im| / (2 * π)) :
    let ϑ : ℝ := s.im / (2 * π * a)
    let C : ℝ :=
      if ϑ ≠ 0 then
