...
- [ ] **EPZAE-15 -- Zeta growth bridge.** The generic source `exp-pair-mu`
  ...
  `mu(7/10) <= 3/40` remains open: its specific analytic pair has not been
  proved. The conditional regression fixture does not close this item.
...
- [ ] **EPZAE-21 -- Zeta LV inputs.** Reflection, moment, and
  ...
  **Current:** printed `add-bound(ii)` is complete on the full closed
  ...
  value floor and amplitude-cardinality bound. Transformed-coordinate
  limits and the exact reflection supremum remain open, so EPZAE-21
  is not crossed out.
