...
- [ ] **EPZAE-21 -- Zeta LV inputs.** Reflection, moment, and
  ...
  **Current:** printed `add-bound(ii)` is complete on the full closed
  ...
  Analytic common-shift extraction and genuine target-pattern
  normalization are now proved. Literal three-by-three frequency and
  height selection preserves the active integer interval and exact
  separation, with explicit scale bounds and amplitude-cardinality loss.
  All logarithmic and rounded band-count losses are now uniformly
  absorbed into an arbitrary positive source power, in both the actual
  value floor and amplitude-cardinality bound. Actual counterexample
  families are selected after their accuracy-dependent thresholds;
  transformed-coordinate limits, compact amplitude extraction and
  sequential zeta-bound consumption are proved. Actual mass comparison,
  the genuine affine envelope and both supremum inequalities now prove
  the exact printed reflection identity, including all EReal bottom
  cases, in `zetaLargeValueExponent_reflection_supremum`.
  EPZAE-21 remains unchecked: replacing the separately disproved
  unrestricted growth clause has not been authorized, and the sharper
  Atkinson source-form branch remains open as recorded below.
...
- [x] **EPZAE-24 -- LV-to-density transfer.** `zero-from-large` and all four
  `zero-large-cor*` corollaries consume actual patterns and multiplicity-weighted
  zeta zeros. `zeroDensityExponent_le_sup_limsup_endpoint_two`, the exact
  bounded/two-thirds supremum theorems and the three-over-cutoff/subdivision
...
- [x] **EPZAE-27 -- Improved Bourgain density.** The exact max formula and both
  ...
  `zeroDensityExponent_le_bourgain_improved_upper` in `BourgainImprovedDensity`.
  Actual Bourgain/Jutila consumers, uniform LV, the genuine twelfth moment
  and the endpoint-two zero transfer discharge every analytic input.
  Jutila k=4 repairs an insufficient k=3 comparison in the printed proof;
  the advertised theorem and its intervals are unchanged.
- [x] **EPZAE-28 -- Bourgain pair-to-density theorem.** All printed hypotheses
  and both alternative side conditions are retained in
  `ExponentPair.bourgain_zero_density`.
  Actual Atkinson packets, occupancy-preserving peak counts and the full
  restricted sixth moment feed a symmetric continuous excess moment.
  The fixed smooth Gram kernel is exactly one on the original closed
  dyadic support; its Mellin residue, local integral and tail are proved.
  Exact real-window overlap, finite Holder, cardinality absorption and a
  chosen smoothing order give the genuine general-coefficient bound
  `LV(sigma,tau)<=max(2-2*sigma,tau+9-12*sigma)` for positive tau and
  `(11/72)*tau<2*sigma-3/2`, without a full sixth-moment premise.
  This closes the complementary short-cutoff branch. The long branch,
  zero first coordinate and sigma=1 endpoint are all handled explicitly.
  The original restricted zeta transfer remains separately proved.
...
- [x] **EPZAE-33 -- Energy-density transfer.** `zeroe-from-large` and its
  corollary preserve bounded perturbations and zero multiplicity.
  The unchanged source inequality is proved by
  `zeroDensityEnergyExponent_le_sup_limsup` on its exact endpoint-one domain.
  `zeroDensityEnergyExponent_le_endpointTwo_bounded_suprema` now proves
  the exact `zeroe-large-cor-0` conclusion, including zeta `[2,τ₀)`,
  general `[τ₀,2τ₀]`, and the empty-supremum convention.
  Its proof uses the corrected energy witness, actual bottom/reflected/direct
  global source blocks, one common labeling, full multiplicity-copy energy,
  Type-II source windows, all finite losses and symmetric-rectangle assembly.
  The new endpoint-two energy sup/limsup theorem is a proved strengthening,
  not a change to the printed endpoint-one lemma.
