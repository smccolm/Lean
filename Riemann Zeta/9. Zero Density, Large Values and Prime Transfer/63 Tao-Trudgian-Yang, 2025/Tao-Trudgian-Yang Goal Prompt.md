...
## Closure-first analytic priority
...
A finite rational search is discovery only; literature records and Python
hypotheses are not analytic provenance. If a route stalls, attempt a materially
different route and record its precise residual inequality.

Keep this short remaining-obligation DAG current (the Robert–Sargos alternative
...
sums of actual source-cell sixth moments with radial weight at scale 20R.
It retains arbitrary complex coefficients, finite multiplicities, empty
cells and closed square-root-frequency endpoints. The constant depends
only on positive epsilon and positive cross-family root separation nu.

The exact smaller-grid recurrence is now derived, not assumed. It consumes
the real grouped arrays, derives coarse center separation from witnesses,
and uses two independent averaged shifts and exact coefficient cancellation.
The choice m=(2n+2)/3+2 is proved strictly smaller for n>=9, and all
physical-scale conditions are derived. Strong induction uses the genuine
N^22 polynomial bound for the finite prefix and absorbs fixed constants
with epsilon/2. Regressions include the complete public signature, exact
...
optional and skipped; it must never block this mathematical work.
Prefer finite direct consumers when they prove the exact source contract.
Any continuous-extension or general-scale passage is a separate obligation.
Do not count finite parabola decoupling as the four-coordinate first-spacing
theorem, the Bourgain pair, or the generic D-process.
Preserve all counterexamples, audits and regressions. Keep
`run_tao_trudgian_yang_build.bat` and its production/audit coverage synchronized;
...
Continue from the proved actual normalized reflected pattern.
Bound its actual band count and logarithmic loss by arbitrarily small
powers, prove the transformed physical-coordinate limits, then derive
the exact printed affine supremum equality with all EReal bottom
cases. Preserve the amplitude-cardinality tradeoff and the literal
coefficient-one interval. Do not substitute the heuristic pointwise
reflection identity or silently repair the separate false unrestricted
growth-to-large-values statement.
...
Continue from the actual reflected pattern with arbitrary source-power
loss. In the counterexample-sequence construction, choose the radius
and threshold for each positive error before choosing that source
pattern. Prove that the reflected scale tends to infinity and its
height exponent tends to `τ/(τ−1)`; extract a compact amplitude
subsequence while retaining the value-cardinality product. Derive
the exact printed affine supremum equality and handle all EReal
bottom cases. Do not substitute the heuristic pointwise identity
or silently repair the separate false unrestricted growth statement.
...
Continue with the actual mass comparison on the proved reflected
counterexample family and its compact amplitude subsequence. Apply
uniform target zeta bounds to the actual nonempty target sequences,
retain the affine value loss, and prove the pointwise-to-supremum
comparison. Then use the exact reflection involution and handle
transformed coordinates above one and all EReal bottom cases to
derive the printed supremum equality. Do not assume cardinality
convergence or replace the theorem by its heuristic pointwise form.
...
The exact reflection identity is complete. Continue with independent
open exponent-pair and density obligations: the analytic Watt input and
its A/B/convexity consumer for the old pair, the Sargos D-process and
remaining beta rows, and the Bourgain pair-to-density theorem. Preserve
the explicit distinction between a proved downstream deduction and
an unproved analytic input. Do not re-request or wait for optional
recovery maintenance or the unresolved source-contract correction
while independent mathematical work is available.
...
Continue the complementary short-cutoff branch of EPZAE-28. The
restricted-sixth-moment route requires a genuine exponent-pair estimate
for the actual Atkinson phase differences, followed by the actual
zeta-value counting and transfer consumers. Do not supply that analytic
estimate as a conclusion-shaped hypothesis or infer it from the
already-proved twelfth moment. The current blueprint's Lemma 8.11 and
Ivic's primary exposition, Theorem 2, identify this route; the low-height
exceptions must still be respected. Independent Watt, D-process and
remaining beta-table obligations also remain in scope.

Maintain `run_tao_trudgian_yang_build.bat` and its PowerShell production
inventory as modules are added. Run it and foundation
`run_lake_build.bat --no-pause` after proof changes; both must finish
with exit 0 and no errors, warnings, tactic suggestions or linter
failures. No recovery-record edit, approval request or waiting is
permitted. Continue available mathematical work without waiting for
optional files or the separate unresolved source-contract correction.
...
Continue EPZAE-28 by summing the proved general-pair Gram bounds over
the actual dyadic cutoff, then consuming the physical stationary and
local-mean packets in actual zeta-value counting. The pair
`(2/9,11/18)` targets the restricted-sixth threshold `11/72`;
do not label its rational coordinate check a moment theorem or insert
a conclusion-shaped analytic hypothesis. The existing above-fourth-root
source-width range is compatible with this route; the separate sharper
EPZAE-21 auxiliary source-form branch remains open.
...
Continue EPZAE-28 from the proved actual local-mean counting theorem.
Adapt the existing occupancy-preserving point-cluster consumer to the
`H^(15/4)/(G*Y^11)` term, then choose the actual width
`G=V^2/(K*log(3H)^2)`. The target peak terms are
`H*V^(-6)` and `H^(15/4)*V^(-24)`, up to arbitrary power loss.
Only after those actual peaks are counted should the measure/layer-cake
consumer establish the restricted-sixth threshold `11/72` and feed
Bourgain's complementary short-cutoff density branch. Do not assume
any of these analytic conclusions as hypotheses.

Maintain `run_tao_trudgian_yang_build.bat` and its PowerShell production
inventory as modules are added. Run it and foundation
`run_lake_build.bat --no-pause` after proof changes; both must finish
with exit 0 and no errors, warnings, tactic suggestions or linter
failures. Never create, edit, update or replace a recovery-record file.
Continue mathematical work without waiting for optional files or the
separate unresolved source-contract correction.
...
Continue EPZAE-28 with a genuine restricted-moment transfer to actual
zeta large-value patterns. A continuous positive-part truncation of the
actual critical-line norm can separate its small-value contribution
from the proved restricted sixth moment on the literal Perron source
window. Derive the necessary physical amplitude gap; do not insert a
full sixth-moment assumption. The intended bound
`LV_zeta(sigma,tau)<=tau-6*(sigma-1/2)` requires the restricted range
`(11/72)*tau<sigma-1/2` and the actual Perron height conditions.
Then consume it in the complementary Bourgain density branch.
The false low-height source contract remains independently preserved.
...
Continue EPZAE-28 with the actual general-coefficient Gram consumer
of the restricted sixth moment. First derive a symmetric global
truncated-moment bound, retaining the compact initial interval uniformly
in the terminal height. Then split the genuine smooth Gram/Mellin kernel
at the restricted threshold, keep the pole and tails, and use finite
sixth-power Holder and actual separated-window overlap. Derive
`LV(sigma,tau)<=max(2-2*sigma,tau+9-12*sigma)` only when
`(11/72)*tau<2*sigma-3/2`. Consume this in the complementary
Bourgain short-cutoff density branch. Do not confuse the completed
coefficient-one zeta transfer with this remaining general-coefficient
analytic obligation, or replace it with a full sixth-moment premise.
...
EPZAE-28 is complete; continue EPZAE-29. Prove the strict-boundary
closure needed by the first listed pair `(11/85,59/85)`: it has
`k=11/85` and `15*l+20*k=13`, so direct invocation of the strict
source theorem is invalid. Use genuine convex combinations and a
proved limiting argument, preserving the actual analytic exponent-pair
predicate. Recover and verify the provenance of all eight listed pairs
from the pinned paper-time source. A rational triangle certificate is
not an analytic exponent-pair proof. Then prove the eight rational
density formulas, admissibility ranges, crossovers and interval coverage.
...
Continue the analytic provenance work for EPZAE-29 and its EPZAE-11/13/14
dependencies. The strict-boundary closure is now proved; do not repeat it.
Recover exact A/B/C/convexity or beta-duality certificates from actual
upstream analytic inputs, preserving source ranges and all secondary terms.
Do not turn the eight hypotheses of `optimizedBourgain_bound_of_pairs`
into axioms or promote that conditional assembly to a completed milestone.
The frozen Python placeholders and rational triangle certificates are
not analytic proofs. Continue independent mathematical obligations while
a specific source or the separate correction authorization is unavailable.

Maintain `run_tao_trudgian_yang_build.bat` and its PowerShell production
inventory when modules are added. Run it and foundation
`run_lake_build.bat --no-pause` after proof changes; both gates must
finish with exit 0 and no errors, warnings, tactic suggestions or linter
failures. Never create, edit, update or replace a recovery-record file.

An independent EPZAE-15/26 route may use the Robert--Sargos fourth-derivative
pair `(1/13,10/13)` and convexity with the classical `(1/14,11/14)` pair
to obtain `(3/40,31/40)`. The fourth-derivative source theorem is not yet
proved. Start with its actual two-coordinate counting system, retain the
Taylor-phase error terms, and prove the spacing/counting and finite-sum
estimates before asserting any resulting pair or density bound.
...
Continue the Robert--Sargos 2002 route from the proved actual primitive
count and signed fixed-coefficient consumer. Prove the complementary
fixed-displacement geometry, all parameter-case counts, the zero-displacement
and gcd decomposition of the seven-variable source count, and the finite-sum
fourth-derivative estimate with its complete error and uniform constants.
Only then derive the analytic pair `(1/13,10/13)`, its exact convex
combination with `(1/14,11/14)`, `mu(7/10)<=3/40`, and the full
EPZAE-26 density formula. Do not equate the 2002 and 2003 counting systems.
The actual Taylor-phase remainder is retained, not erased.

Continue independent open obligations while source access or the separate
false-contract correction authorization is unavailable. Preserve the
printed Lemma 62 counterexample and corrected two-witness powering
permanently; do not reinstate the false `s'/s` scaling.
Do not promote the eight conditional optimized-pair consumers to an
unconditional EPZAE-29 theorem without analytic proofs of their inputs.

Maintain `run_tao_trudgian_yang_build.bat` and its PowerShell production
inventory when modules are added. Run it and foundation
`run_lake_build.bat --no-pause` after proof changes. Both must finish
with exit 0 and no errors, warnings, tactic suggestions or linter failures.
Never create, edit, update or replace a recovery-record file; its maintenance
is permanently optional and skipped. A rejected nonessential write is not
a reason to stop mathematical work.
...
Continue from the actual `RobertSargosPrimitiveSystem` and its coordinate
keys. Sum the proved coefficient/displacement fiber counts over actual
source projections. Complete the `H<=Q` branch, both complementary
`Q<=H` regimes, dyadic displacement ranges and the `delta>=1` case.
Then prove the zero-displacement count and the seven-variable gcd
reduction. Preserve multiplicities and every scale relation in these
consumers; the projected image cardinality is not the original solution count.

The complementary geometry and all three uniform epsilon-loss fiber
estimates are now proved: do not redo them or replace their source
conditions with a result-equivalent premise. The complete fourth-derivative
finite-sum argument, analytic pair `(1/13,10/13)`, exact old-pair convex
deduction, growth theorem and full EPZAE-26 density formula remain open.
Continue independent obligations while a source or the separate false-contract
correction authorization is unavailable.

Maintain `run_tao_trudgian_yang_build.bat` and its PowerShell production
inventory as modules change. Run it and foundation
`run_lake_build.bat --no-pause` after proof changes. Both must finish
with exit 0 and no errors, warnings, tactic suggestions or linter failures.
Preserve the printed Lemma 62 counterexample and corrected two-witness
powering permanently; never reinstate the false `s'/s` scaling.
Never create, edit, update or replace a recovery-record file. Recovery-record
maintenance is permanently optional and skipped; a rejected nonessential
write is not a reason to stop mathematical work.
...
Continue from `exists_robertSargos_primitive_count`, which now proves
the actual primitive six-variable target for every nonnegative tolerance.
Do not redo its coefficient, conic, linear or product-fiber branches.
The direct signed harmonic-weight summation replaces the paper's dyadic
summation faithfully; no missing dyadic step remains in Lemma 7.

Next prove the zero-displacement count, exact two-gcd normalization,
the exceptional large coordinate-gcd range, and the seven-variable
source count. Transport actual support, signs, near-equation tolerance,
integer divisibility, finite cardinality and the n2 multiplicity.
Then complete the literal finite-difference, shift, Taylor and
double-large-sieve assembly of the fourth-derivative theorem.
The analytic pair `(1/13,10/13)`, exact old-pair convex deduction,
growth theorem and full EPZAE-26 density formula remain open.
Continue independent obligations while a source or the separate
false-contract correction authorization is unavailable.
...
Continue from `exists_robertSargos_source_count`, the complete finite-set
form of Robert--Sargos (2002), Theorem 2. Do not redo the primitive count,
two-gcd normalization, harmonic summation, zero-displacement count or
seven-variable multiplicity bridge. The proved normalized frequency
scale `max(1,Q/k)` covers the large-gcd range; no separate omitted
exceptional-range counting obligation remains.

The next obligation is the literal fourth-derivative finite-sum argument:
the source A and A-times-A differencing, shifts and support,
coefficient bounds and multiple partial summation, actual Taylor phase
with all retained correction terms, double-large-sieve input and
parameter assembly. Use the new source count as an actual consumer input;
do not replace those analytic steps with a desired-bound premise.
Then derive `(1/13,10/13)`, complete the complementary parameter range
with the available classical pair, and prove the exact convex deduction
of `(3/40,31/40)`. The EPZAE-15 growth theorem and full EPZAE-26 density
formula remain open. Continue independent obligations while a source
or the separate false-contract correction authorization is unavailable.
...
Continue the actual Robert--Sargos 2002 fourth-derivative argument.
Do not redo the complete seven-variable counting theorem, its Taylor-coordinate
consumer, the finite C4 remainder estimates or the exact rectangular A-times-A
inequality. The proved Taylor count keeps the sign reversal and the full
`E+12*R*H^2` correction budget. The proved finite A-times-A inequality keeps
both signed triangular weights, not a maximum correlation.

Next connect the actual symmetric phase array to that finite rectangle,
derive its precise intersection support, complete the initial conjugated
A-process and dyadic selection, remove the zero-shift terms, and prove the
averaged shift with its boundary error. Complete mixed partial derivatives
of the actual exponential remainder and the multiple-partial-summation
consumer, with coefficients independent of the outer sample variable.
Then prove the double-large-sieve, its actual sample-spacing count and
parameter assembly. Use the existing actual Taylor count as an input,
not a desired-bound hypothesis. Finish Theorem 1, the complementary range
and the analytic pair `(1/13,10/13)`, followed by the exact convex deduction
of `(3/40,31/40)`. EPZAE-15 and the full EPZAE-26 formula remain open.
Continue independent work while the separate false-contract correction
authorization or a source is unavailable.

Maintain `run_tao_trudgian_yang_build.bat` and its PowerShell production
inventory as modules change. Run it and foundation
`run_lake_build.bat --no-pause` after proof changes. Both must exit 0
without errors, warnings, tactic suggestions or linter failures.
Preserve the printed Lemma 62 counterexample and corrected two-witness
powering permanently; never reinstate the false `s'/s` scaling.
Never create, edit, update or replace any recovery-record file.
Recovery-record maintenance is permanently optional and skipped.
A denied nonessential write is not a reason to stop mathematical work.
...
Continue the actual Robert--Sargos 2002 fourth-derivative argument.
Do not redo Theorem 2, its actual Taylor-coordinate consumer, the finite C4
estimates, or the now-proved literal source A-times-A entry. Preserve both
signed supports and the explicit `16*M*H^2*R` endpoint error. The trimmed
correlation retains the exact h overlap and mixed phase; it is not an
unrestricted rectangle.

Next prove the initial conjugated A-process and dyadic selection, the
zero-shift estimates, and the averaged translation with its common-interval
boundary errors. Prove the mixed derivatives of the actual exponential
remainder and multiple partial summation with a common prefix and
coefficients independent of the outer sample variable. Then complete
double-large-sieve, actual sample spacing and parameter assembly.
Consume the proved Taylor count with its full correction budget.
Finish Theorem 1, its complementary range and the analytic pair
`(1/13,10/13)`, followed by the exact convex deduction of `(3/40,31/40)`.
EPZAE-15 and the full EPZAE-26 formula remain open. Continue independent
work while the separate false-contract correction authorization or a
...
Continue the actual Robert--Sargos 2002 fourth-derivative argument.
Do not redo Theorem 2, its Taylor-coordinate count, the finite C4 estimates,
source A-times-A entry, charged endpoint replacement, or the now-proved
initial conjugated A-process and full doubled-block selection.
Consume `exists_robertSargos_initial_dyadic_log` with its actual k witness;
the selected sum is the existing source-array object.

Next prove the small-k third-derivative branch and zero-shift estimates.
Continue the averaged translation with a genuinely common m interval
and explicit endpoint errors. Prove every claimed interval containment;
if an auxiliary endpoint must be adjusted, retain the charged error and
document that adjustment. Then prove mixed derivatives of the actual
exponential remainder and multiple partial summation with a common
prefix and coefficients independent of the outer sample variable.
Complete double-large-sieve, actual sample spacing and scale assembly.
Use the actual Taylor count with its full correction budget.

Finish Theorem 1 including its short/long-length reductions and the
complementary range, then the analytic pair `(1/13,10/13)` and exact
convex deduction of `(3/40,31/40)`. EPZAE-15 and the full EPZAE-26
formula remain open. Continue independent work while the separate
false-contract correction authorization or a source is unavailable.
...
Continue the actual Robert--Sargos 2002 fourth-derivative argument.
Do not redo the counting theorem, finite C4 estimates, initial conjugated
A-process, full doubled-block selection, source A-times-A or charged
shifted averaging. Consume `robertSargos_shifted_triple_a_times_a`
with its actual common interval and explicit errors. Preserve the
auxiliary endpoint regressions; never reuse the disproved containments.

Next prove the small-block third-derivative and zero-shift estimates.
Investigate the ordinary third-derivative estimate as a sufficient weaker
auxiliary route, with every parameter budget proved; do not label it as
the sharper printed Lemma 3. Continue independently with mixed
derivatives of the actual exponential remainder and multiple partial
summation with a common prefix and coefficients independent of m.
Complete double-large-sieve, actual sample spacing and scale assembly.
Use the actual Taylor count with its full correction budget.

Finish Theorem 1 including short/long-length reductions and the
complementary range, then the analytic pair `(1/13,10/13)` and exact
convex deduction of `(3/40,31/40)`. EPZAE-15 and the full EPZAE-26
formula remain open. Continue independent work while the separate
false-contract correction authorization or a source is unavailable.
...
Continue the actual Robert--Sargos 2002 fourth-derivative argument.
Do not redo the counting theorem, finite C4 jets, initial A-process,
dyadic selection, source A-times-A, corrected shifted averaging or
the now-proved ordinary third-derivative estimate.

Next consume `continuous_third_derivative_bound` or its finite-C3
version on the actual symmetric source phase. Derive its third
curvature from the actual fourth derivative. Preserve the literal
closed interval: charge an endpoint if needed, rather than assuming
smoothness beyond the source domain. Prove the small-block parameter
budget. For q=0, derive the true linear phase and remainder derivative,
subdivide at the physical square-root scale and use finite Abel
summation; prove the weaker ordinary estimate suffices for the final
budget. For r=0, derive and apply the actual second-derivative bound.
Do not call this the sharper printed Lemma 3.

Continue mixed exponential derivatives and multiple partial summation
with a common prefix and coefficients independent of m. Complete
double-large-sieve, sample spacing and scale assembly with the full
Taylor-count correction budget. Preserve the corrected common core,
explicit boundary charges and auxiliary endpoint counterexamples.
...
Continue the actual Robert--Sargos 2002 fourth-derivative argument.
Do not redo the counting theorem, finite C4 jets, initial dyadic selection,
ordinary third-derivative test, actual centered/doubled-block consumers,
small-block parameter budgets, physical floor choice or the now-proved
`robertSargos_physical_initial_reduction`.

Consume its actual large-block witness. Next remove the zero shifts in
the source A-times-A sum. Derive the literal r=q=0 diagonal count.
For r=0 and q nonzero, derive the actual second curvature from f''''
and apply the continuous second-derivative estimate on the true interval.
For q=0 and r nonzero, derive `g=-2*r*f'` and the true remainder
derivative, subdivide at the square-root curvature scale, use finite
Abel summation, and prove the ordinary third-derivative bound suffices
for the final O(M^2) budget. Do not assume or relabel the sharper
printed Lemma 3. Preserve every endpoint charge.

Continue mixed exponential derivatives and multiple partial summation
with a common prefix and coefficients independent of m. Complete
double-large-sieve, sample spacing and remaining scale assembly with
the full Taylor-count correction budget. Preserve the corrected common
core, explicit boundary charges and auxiliary endpoint counterexamples.

Finish Theorem 1 including Step 0 short/long-length reductions and the
complementary range, then the analytic pair `(1/13,10/13)` and exact
convex deduction of `(3/40,31/40)`. EPZAE-15 and the full EPZAE-26
formula remain open. Continue independent work while the separate
false-contract correction authorization or a source is unavailable.
...
Continue the actual Robert--Sargos 2002 fourth-derivative argument.
Do not redo the counting theorem, finite C4 jets, ordinary derivative
tests, initial physical small-block reduction or the now-proved
`robertSargos_zero_r_physical_a_times_a`.

Consume the actual large-block witness. Next derive the literal
zero-zero diagonal count and its floor-scale budget. For q=0 and
r nonzero, use the true h+r support: derive `g=-2*r*f'` and the
actual remainder derivative, subdivide at the square-root curvature
scale and use finite Abel summation. Prove the ordinary third-derivative
estimate suffices for the final O(M^2) budget; do not assume the sharper
printed Lemma 3. Preserve closed endpoints and every boundary charge.
Combine the zero-r row, zero-q column and diagonal in the exact
weighted A-times-A decomposition.

Continue common-prefix multiple partial summation with coefficients
independent of m, double-large-sieve, sample spacing and scale assembly.
Preserve the full Taylor-count correction, corrected common core and
auxiliary endpoint counterexamples. Finish Theorem 1 with Step 0
short/long-length reductions and the complementary range, then deduce
the analytic pair `(1/13,10/13)` and `(3/40,31/40)`.
EPZAE-15 and the full EPZAE-26 formula remain open. Continue independent
work while the separate false-contract correction authorization or
...
Continue the actual Robert--Sargos 2002 fourth-derivative argument.
Do not redo the counting theorem, C4 jets, ordinary derivative tests,
initial small-block reduction, either zero-shift axis, diagonal,
Abel/square-root block argument or the now-proved
`robertSargos_physical_nonzero_a_times_a`.

Consume its actual zero-excluded weighted source. Carry q nonzero and
r nonzero through the corrected common shifted interval and triple-sum
reordering. Retain the true h-mask, take norms only after the inner
q,h,n sum, and absorb the explicit shift boundary error at the physical
N floor scale. Do not revert to an all-shifts source after removing zeros.

Continue mixed exponential derivatives and multiple partial summation
with a common prefix and coefficients independent of m. Complete
double-large-sieve, sample spacing and scale assembly, preserving the
full Taylor-count correction, corrected common core and auxiliary
endpoint counterexamples. Finish Theorem 1 including Step 0 short/long
length reductions and the complementary range, then deduce
`(1/13,10/13)` and the exact convex pair `(3/40,31/40)`.
EPZAE-15 and the full EPZAE-26 formula remain open. Continue independent
work while the separate false-contract correction authorization or
a source is unavailable.
