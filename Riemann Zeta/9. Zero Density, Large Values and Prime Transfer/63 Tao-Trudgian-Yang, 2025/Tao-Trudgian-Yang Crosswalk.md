...
Current kernel-checked infrastructure comprises EPZAE-00--05, EPZAE-07--10,
EPZAE-12, EPZAE-16--17, EPZAE-20, EPZAE-22--23, EPZAE-25, EPZAE-31--32, and
...
| `hb-density2` | `A(sigma) <= 3/(10 sigma-7)` for `7/10 < sigma <= 1` | `HeathBrownDensityRange.lean`: unconditional through `19/22` and at `1`; full-range old-pair adapter is conditional | EPZAE-26 OPEN |
...
| `mu-bound` | `mu(7/10)<=3/40`: specific pair remains unproved; regression is explicitly conditional | EPZAE-15 still open |
...
| `hb-opt` | exact full-strip Heath–Brown maximum | kernel-checked: `heathBrown_largeValueBound`, `largeValueExponent_le_heathBrown`; EPZAE-19 sub-obligation complete |
| `jutila-lvt` | all positive-integer Jutila bounds | kernel-checked: `jutila_largeValueBound`; EPZAE-19 sub-obligation complete |
...
| `zero-from-large` | Type I/II transfer | Proved by `zeroDensityExponent_le_sup_limsup_endpoint_two`, with zeta supremum over `τ ≥ 2`, genuine general limsup and actual multiplicity-weighted zeros; EPZAE-24 complete |
| `zero-large-cor*` | optimized transfer corollaries | All four source corollaries proved: exact bounded and two-thirds suprema, `3/τ₀` estimate and Montgomery/subdivision variant; empty short-zeta supremum is `-∞` also at `τ₀=2`; EPZAE-24 complete |
...
| `zeroe-large-cor-0` | bounded-range zero-energy transfer | Proved by `zeroDensityEnergyExponent_le_endpointTwo_bounded_suprema`: exact zeta `[2,τ₀)` and general `[τ₀,2τ₀]` suprema, corrected energy powering, one global source labeling and all original multiplicity copies; EPZAE-33 complete |
