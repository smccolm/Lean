...
/-! Occupancy-preserving actual zeta-peak counting with the restricted-sixth pair. -/
...
theorem exists_ivicSixth_pointCluster_superlevel_count
        ...
        ((pointClusterSuperlevel H G W m).card:ℝ) ≤
          2*D*H^ν*
            (H/(G*((m:ℝ)*(V^2/(16*P*Real.log (3*H))))^2) +
              H^(15/4:ℝ)/(G*((m:ℝ)*(V^2/(16*P*Real.log (3*H))))^11)) := by
  obtain ⟨P,hP,hentry⟩ := exists_pointCluster_superlevel_entry
  obtain ⟨C,hC,D,hD,H₀,hH₀,hcount⟩ :=
    exists_ivicSixth_localMeanExcess_card_le hδ hκ hν
  ...
  have hcolor : ∀ e : ℕ,
      ((S.filter (fun n => n%2 = e)).card:ℝ) ≤
        D*H^ν*(H/(G*Y^2)+H^(15/4:ℝ)/(G*Y^11)) := by
    intro e
  ...
  change (S.card:ℝ) ≤ 2*D*H^ν*(H/(G*Y^2)+H^(15/4:ℝ)/(G*Y^11))
  rw [hcards]
  have h0 := hcolor 0
  have h1 := hcolor 1
  linarith

theorem ivicSixth_occupancy_inverse_power_budget {H G A m : ℝ}
    (hH : 0 ≤ H) (hG : 0 < G) (hA : 0 < A) (hm : 1 ≤ m) :
    H/(G*(m*A)^2)+H^(15/4:ℝ)/(G*(m*A)^11) ≤
      (H/(G*A^2)+H^(15/4:ℝ)/(G*A^11))/m^2 := by
  have hm0 : 0 < m := by linarith
  have hp : m^2 ≤ m^11 := pow_le_pow_right₀ hm (by omega)
  rw [mul_pow,mul_pow]
  calc
    _ ≤ H/(G*(m^2*A^2))+H^(15/4:ℝ)/(G*(m^2*A^11)) := by
      exact add_le_add le_rfl
        (div_le_div_of_nonneg_left (Real.rpow_nonneg hH _) (by positivity)
          (mul_le_mul_of_nonneg_left
            (mul_le_mul_of_nonneg_right hp (by positivity)) hG.le))
    _ = _ := by field_simp

theorem exists_ivicSixth_pointValue_card_le_with_width
    {δ κ ν : ℝ} (hδ : 0 < δ) (hκ : 0 < κ) (hν : 0 < ν) :
    ∃ P C D H₀ : ℝ, 0 < P ∧ 0 < C ∧ 0 < D ∧ 40000 ≤ H₀ ∧
      ∀ (H G V : ℝ) (W : Finset ℝ),
        H₀ ≤ H → 0 < G → G ≤ H → 0 < V →
  ...
  have hsuper : ∀ m : ℕ, 0 < m →
    ...
    have hp := ivicSixth_occupancy_inverse_power_budget hHpos.le hG hA hm1
    calc
      _ ≤ 2*D*H^ν*(H/(G*((m:ℝ)*A)^2)+H^(15/4:ℝ)/(G*((m:ℝ)*A)^11)) := hc
      _ ≤ 2*D*H^ν*((H/(G*A^2)+H^(15/4:ℝ)/(G*A^11))/(m:ℝ)^2) :=
  ...
  calc
    (W.card:ℝ) ≤ 2*B := hsum
    _ = (4*D)*H^ν*(H/(G*A^2)+H^(15/4:ℝ)/(G*A^11)) := by dsimp only [B]; ring
