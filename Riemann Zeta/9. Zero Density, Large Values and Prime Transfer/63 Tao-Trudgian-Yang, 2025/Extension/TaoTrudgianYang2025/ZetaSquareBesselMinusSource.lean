...
/-!
# Removing the proved negligible modified-Bessel branch

The genuine physical zeta source is now the full logarithmic main term
plus the oscillatory Neumann-kernel divisor sum. The discarded K0
...
theorem exists_zetaSquarePhysicalGaussian_main_minus_approximation {δ : ℝ} (hδ : 0 < δ) :
    ∃ C : ℝ, 0 < C ∧ ∃ T₀ : ℝ, 16 ≤ T₀ ∧ ∀ T G : ℝ,
      T₀ ≤ T → T ^ δ ≤ G → G ≤ T ^ (1 / 2 - δ) →
      |(∫ t : ℝ, zetaGaussianWeight T G t * zetaMomentCriticalNorm t ^ 2) -
        2 * (zetaDivisorVoronoiMain T G (Real.log T) +
          zetaDivisorBesselMinus T G (Real.log T)).re| ≤ C * G * Real.log T := by
  obtain ⟨C, hC, B, hB, hsource⟩ := exists_zetaSquarePhysicalGaussian_bessel_approximation hδ
  obtain ⟨B₁, hB₁, hplus⟩ := exists_zetaDivisorBesselPlus_powerSaving hδ 0
  have hev : ∀ᶠ T : ℝ in atTop, ∀ G : ℝ, T ^ δ ≤ G → G ≤ T ^ (1 / 2 - δ) →
      |(∫ t : ℝ, zetaGaussianWeight T G t * zetaMomentCriticalNorm t ^ 2) -
        2 * (zetaDivisorVoronoiMain T G (Real.log T) +
          zetaDivisorBesselMinus T G (Real.log T)).re| ≤ (C + 2) * G * Real.log T := by
    ...
    have hkr := (Complex.abs_re_le_norm (zetaDivisorBesselPlus T G (Real.log T))).trans hk
...
theorem exists_zetaSquareLocalMean_le_main_minus {δ : ℝ} (hδ : 0 < δ) :
    ∃ C : ℝ, 0 < C ∧ ∃ T₀ : ℝ, 16 ≤ T₀ ∧ ∀ T G : ℝ,
      T₀ ≤ T → T ^ δ ≤ G → G ≤ T ^ (1 / 2 - δ) →
      (∫ t in T - G..T + G, zetaMomentCriticalNorm t ^ 2) ≤
        2 * Real.exp 1 * (zetaDivisorVoronoiMain T G (Real.log T) +
          zetaDivisorBesselMinus T G (Real.log T)).re + C * G * Real.log T := by
  obtain ⟨C, hC, B, hB, hsource⟩ := exists_zetaSquareLocalMean_le_bessel hδ
  obtain ⟨B₁, hB₁, hplus⟩ := exists_zetaDivisorBesselPlus_powerSaving hδ 0
  have hev : ∀ᶠ T : ℝ in atTop, ∀ G : ℝ, T ^ δ ≤ G → G ≤ T ^ (1 / 2 - δ) →
      (∫ t in T - G..T + G, zetaMomentCriticalNorm t ^ 2) ≤
        2 * Real.exp 1 * (zetaDivisorVoronoiMain T G (Real.log T) +
          zetaDivisorBesselMinus T G (Real.log T)).re +
    ...
    have hk := hplus T G hTB₁ hlower hupper
    simp only [neg_zero, Real.rpow_zero, mul_one] at hk
    have hkr := (Complex.re_le_norm (zetaDivisorBesselPlus T G (Real.log T))).trans hk
