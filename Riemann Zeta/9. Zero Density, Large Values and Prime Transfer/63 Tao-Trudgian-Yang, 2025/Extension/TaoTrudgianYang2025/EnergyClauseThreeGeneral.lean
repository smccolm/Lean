...
/-!
# General-pattern energy bound for Add-est (iii)
...
theorem energyClauseThree_tall_bound {σ t r : ℝ}
    (hlo : 173/229 ≤ σ) (hhi : σ ≤ 443/586)
    (htlo : 6/5 ≤ t) (hthi : t ≤ 3/2)
    (hg : r ≤ max (18/5-4*σ) (12/5-4*σ+t)) (hcap : r ≤ 3-3*σ) :
    max (r+4-4*σ) ((3-4*σ+5*r)/2) ≤ energyClauseThreeRate σ*t := by
  have hC := mul_le_mul_of_nonneg_right (energyClauseThreeThirdRate_le σ)
    (by linarith : 0 ≤ t)
  rw [max_eq_right (by linarith : 18/5-4*σ ≤ 12/5-4*σ+t)] at hg
  apply max_le
  · by_cases ht : t ≤ σ+3/5
    · have hc := energyClauseThree_tall_0_0 hlo hhi htlo ht
      linarith
    · have hc := energyClauseThree_tall_0_1 hlo hhi (le_of_not_ge ht) hthi
      linarith
  · by_cases ht : t ≤ σ+3/5
    · have hc := energyClauseThree_tall_1_0 hlo hhi htlo ht
      linarith
    · have hc := energyClauseThree_tall_1_1 hlo hhi (le_of_not_ge ht) hthi
      linarith

theorem InCardinalityEnergyRegion.energyClauseThree_general
  ...
  obtain ⟨htone,htthree,hj,hg,hcap⟩ :=
    h.energyClauseThree_cardinality_caps hlo hhi q hq htq hqt
  have hscaled : e/q ≤ energyClauseThreeRate σ*(τ/q) := by
    by_cases ht : 6/5 ≤ τ/q
    · exact (h.heathBrown_two_branches_powered q hqNat htthree
        (by linarith)).trans (energyClauseThree_tall_bound hlo hhi ht htthree hg hcap)
    · have htlow : τ/q ≤ 6/5 := le_of_lt (lt_of_not_ge ht)
      ...
      have hahi : ((q-1 : ℕ) : ℝ)/q ≤ (2 : ℝ)/3 := by
        rcases hq with rfl|rfl <;> norm_num
      by_cases hs : σ ≤ 241/319
      · rw [jutila_thirteen_low_sigma hs] at hj
        exact energyClauseThree_low_short_branch hlo hs htone htlow halo hahi
          (by simpa only [add_sub_assoc] using hj) hg i hi
      · have hs' : 241/319 ≤ σ := le_of_not_ge hs
        rw [jutila_thirteen_high_sigma hs'] at hj
        exact energyClauseThree_high_short_branch hs' hhi htone htlow halo hahi
          (by simpa only [add_sub_assoc] using hj) hg i hi
  exact (div_le_div_iff_of_pos_right hqpos).1
    (show e/q ≤ (energyClauseThreeRate σ*τ)/q by
      simpa only [mul_div_assoc] using hscaled)

theorem energyClauseThree_general_bound {σ τ : ℝ}
    (hlo : 173/229 ≤ σ) (hhi : σ ≤ 443/586) (htlo : 2 ≤ τ) (hthi : τ ≤ 4) :
    IsLargeValueEnergyBound σ τ (energyClauseThreeRate σ*τ) := by
  by_contra hnot
  ...
  exact (not_lt_of_ge ((show InCardinalityEnergyRegion σ τ ρ e from
    ⟨s,hregion⟩).energyClauseThree_general hlo hhi htlo hthi)) hlarge
