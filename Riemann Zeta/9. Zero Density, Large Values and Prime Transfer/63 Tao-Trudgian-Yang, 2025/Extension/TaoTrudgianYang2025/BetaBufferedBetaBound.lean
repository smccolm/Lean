...
/-!
# Source beta consumer for the actual buffered stationary main term

The canonical slope chart is constructed from the original model.
The original source cutoff is not replaced by one: width-independent
variation transfers the unweighted upstream beta bound to that weight.
-/
...
theorem bufferedStationaryMain_bound_of_exponentSumBound
    {α : ℝ≥0} {β σ l r : ℝ}
  ...
  intro T N F a L p q η hη hT hN hF hTC hlow hhigh hwindow
  ...
  have hmain := norm_modelPhaseBufferedStationaryBlock_le_prefixMax hσ hdamp hF₂
    hA hlpos hT hN hη p q a L
  ...
  simpa only [mul_assoc] using mul_le_mul_of_nonneg_left hmax
    (by positivity : 0 ≤ 8*((N/Real.sqrt T)*(Real.sqrt (modelPhaseCurvatureLower σ))⁻¹))
