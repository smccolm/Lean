...
/-! Layer-cake integration of the actual restricted sixth-power distribution. -/
...
theorem integrableOn_ivicSixth_pointValueSuperlevel (H V : ℝ) :
    IntegrableOn (fun t => zetaMomentCriticalNorm t^6) (pointValueSuperlevel H V) :=
  ((continuous_zetaMomentCriticalNorm.pow 6).continuousOn.integrableOn_Icc).mono_set
    (pointValueSuperlevel_subset_Icc H V)

theorem ivicSixth_power_tail_measure_le {H V s C : ℝ}
    (hV : 0 < V) (hs : 0 < s) (hC : 0 ≤ C)
    (hcount : ∀ U : ℝ, V ≤ U →
      volume (pointValueSuperlevel H U) ≤ ENNReal.ofReal (C/U^6)) :
    (volume.restrict (pointValueSuperlevel H V)).real
      {t | s ≤ zetaMomentCriticalNorm t^6} ≤ C/max (V^6) s := by
  have hevent : MeasurableSet {t | s ≤ zetaMomentCriticalNorm t^6} :=
    (isClosed_le continuous_const (continuous_zetaMomentCriticalNorm.pow 6)).measurableSet
  have hmeasure :
      (volume.restrict (pointValueSuperlevel H V))
        {t | s ≤ zetaMomentCriticalNorm t^6} ≤ ENNReal.ofReal (C/max (V^6) s) := by
    rw [Measure.restrict_apply hevent]
    by_cases hsmall : s ≤ V^6
    · rw [max_eq_left hsmall]
      exact (measure_mono inter_subset_right).trans (hcount V le_rfl)
    · have hVs : V^6 ≤ s := (lt_of_not_ge hsmall).le
      let U : ℝ := s^((6:ℝ)⁻¹)
      have hU : 0 < U := by dsimp only [U]; positivity
      have hUpow : U^6 = s := Real.rpow_inv_natCast_pow hs.le (by norm_num)
      have hVU : V ≤ U := le_of_pow_le_pow_left₀ (by norm_num : (6:ℕ) ≠ 0)
        hU.le (by simpa only [hUpow] using hVs)
      have hsub : {t | s ≤ zetaMomentCriticalNorm t^6} ∩
          pointValueSuperlevel H V ⊆ pointValueSuperlevel H U := by
        intro t ht
        refine ⟨ht.2.1,ht.2.2.1,?_⟩
        apply le_of_pow_le_pow_left₀ (by norm_num : (6:ℕ) ≠ 0)
  ...
  rw [ENNReal.toReal_ofReal (div_nonneg hC ((pow_nonneg hV.le 6).trans (le_max_left _ _)))] at ht
  exact ht

theorem ivicSixth_high_integral_le_log {H V M C : ℝ}
  ...
  · filter_upwards [self_mem_ae_restrict (measurableSet_pointValueSuperlevel H V)] with t ht
    exact hGrowth t ht
  · intro s hs _
    exact ivicSixth_power_tail_measure_le hV hs hC hcount

theorem exists_ivicSixth_power_le_height_cube :
    ∃ H₀ : ℝ, 40000 ≤ H₀ ∧ ∀ H t : ℝ,
      H₀ ≤ H → H ≤ t → t ≤ 2*H →
      zetaMomentCriticalNorm t^6 ≤ H^3 := by
  obtain ⟨H₀,hH₀,hGrowth⟩ :=
  ...
  have hp := pow_le_pow_left₀ (norm_nonneg _) (hGrowth H t hH htlo hthi).le 6
  have he : (H^((1/6:ℝ)+1/48))^6 = H^(9/8:ℝ) := by
    rw [← Real.rpow_mul_natCast (by linarith : 0 ≤ H)]
    norm_num
  rw [he] at hp
  calc
    zetaMomentCriticalNorm t^6 ≤ H^(9/8:ℝ) := hp
