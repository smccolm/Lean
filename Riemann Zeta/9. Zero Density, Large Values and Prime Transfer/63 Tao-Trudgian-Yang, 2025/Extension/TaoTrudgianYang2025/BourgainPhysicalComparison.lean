...
/-!
# The level-free comparison at the original pattern height

The enclosing height is derived, not an extra premise. All native threshold
and constant-factor height losses are absorbed before choosing the pattern.
The faithful subdivision range N <= L <= P.T remains explicit.
-/
...
/-- Source-family comparison with the original physical height in both budgets. -/
theorem bourgain_subdivided_physical_comparison {σ τ η θ : ℝ}
  ...
  obtain ⟨B, C, δ, M, N₀, D, E₀, hB, hC, hδ, hδ₁, hM, hN₀, hD, hE₀, hp⟩ :=
    bourgain_subdivided_level_free_comparison (τ := τ) hσ hη hθ hε
  refine ⟨B, C, δ, M, max N₀ E₀, 3*D*(8 : ℝ)^θ,
    hB, hC, hδ, hδ₁, hM, hN₀.trans (le_max_left _ _), by positivity, ?_⟩
  intro P L hL hN hNN₀ hNL hLT hTu hVl
  have hN₀N := (le_max_left N₀ E₀).trans hNN₀
  have hE₀N := (le_max_right N₀ E₀).trans hNN₀
  obtain ⟨W, hlocal, hfamily⟩ := hp P L hL hN hN₀N hNL hTu hVl
  refine ⟨W, hlocal, ?_⟩
  intro α
  ...
  rcases hbranch with hsmall | ⟨hS, u, hu, hZ, hcomp⟩
  · exact Or.inl hsmall
  · right
    refine ⟨hS, u, hu, hZ, ?_⟩
    have hNpos : 0 < P.N := zero_lt_one.trans P.one_lt_N
    have hTE : P.T ≤ 8*P.T := by linarith [P.T_pos]
    have hE : E₀ ≤ 8*P.T := hE₀N.trans (hNL.trans (hLT.trans hTE))
    have hbound := hcomp (8*P.T) hE hTE
      (bourgain_slice_height_le_original P hNL hLT hε₈)
    have hr : 0 ≤ 1+2*Real.pi*P.N^η := by positivity
    have hbudget := bourgain_mixed_budget_eight_height
      (θ := θ) hNpos.le P.T_pos.le
      (Nat.cast_nonneg (A.biUnion (fun i => (P.localized L hL i).retainedOriginal (W i))).card)
      (Nat.cast_nonneg (bourgainIntegerSlice (P.N^(ε/8)) (L+P.N^(ε/8)+1)
        (P.N^(-bourgainSharedFloorExponent α τ ε)*(2 : ℝ)^q) u).card) hD.le hr
    exact hbound.trans_le
      (mul_le_mul_of_nonneg_left hbudget (mul_nonneg hM.le (Real.rpow_nonneg hNpos.le η)))
