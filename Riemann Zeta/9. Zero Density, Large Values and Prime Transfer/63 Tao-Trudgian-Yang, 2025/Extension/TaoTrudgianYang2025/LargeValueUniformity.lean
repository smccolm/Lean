...
/-!
# Uniform zeta cardinality estimates over compact height-exponent ranges
...
bridge between pointwise cardinality bounds and the zero-cardinality transfer.
-/
...
/-- Pointwise zeta cardinality bounds with exponent `B*τ` yield one uniform
...
theorem zetaLargeValueBound_uniform_near_logScale_interval
  ...
  calc
    (P.ordinates.card : ℝ) ≤ C₀ τ * P.N ^ (B * τ.1 + ε / 2) :=
      hcard
    _ ≤ C * P.N ^ (B * α + ε) :=
      mul_le_mul hCτ
...
theorem zetaLargeValueBound_uniform_on_logScale_interval
    (σ B l u : ℝ) (hB : 0 ≤ B) (hlu : l ≤ u)
    (hLV : ∀ τ ∈ Set.Icc l u, IsZetaLargeValueBound σ τ (B * τ)) :
    ∀ ε : ℝ, 0 < ε →
      ∃ C : ℝ, 1 ≤ C ∧ ∃ δ : ℝ, 0 < δ ∧
  ...
  obtain ⟨C, hC, δ, hδ, hbound⟩ :=
    zetaLargeValueBound_uniform_near_logScale_interval σ B l u hB hlu hLV ε hε
  refine ⟨C, hC, δ, hδ, ?_⟩
  intro P hCN hscale hV
...
theorem largeValueBound_uniform_near_logScale_interval
    (σ B l u : ℝ) (hB : 0 ≤ B) (hlu : l ≤ u)
