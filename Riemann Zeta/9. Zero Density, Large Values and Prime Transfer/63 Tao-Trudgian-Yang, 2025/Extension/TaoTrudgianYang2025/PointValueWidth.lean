...
/-!
# A physical width chosen from the actual peak size

The width G = V^2/(K log(3H)^2) is constructed, not supplied. The stated
amplitude interval derives every Atkinson width condition, the logarithmic
window fit, and absorption of the real local-source error. The result
counts the original unit-separated zeta peaks with no lost occupancy.
-/
...
theorem exists_pointValue_card_le_source_range
  ...
  obtain ⟨P,C,D,B,hP,hC,hD,hB,hcount⟩ :=
    exists_pointValue_card_le_with_width hδ hκ hν
  let K : ℝ := 16*P*C+1
