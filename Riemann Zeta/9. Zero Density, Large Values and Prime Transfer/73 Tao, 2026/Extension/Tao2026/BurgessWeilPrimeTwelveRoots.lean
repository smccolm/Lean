...
/-!
# Twelve active roots in the prime Burgess sum

This file isolates the next source-shaped endpoint obtained by sending one
of twelve degree-balanced active roots to infinity. The eleven finite local
...
/-- The source-shaped eleven-point endpoint at one prime. -/
def TaoPrimePowerElevenPointHypergeometricWeilBoundAt
    (p : ℕ) [NeZero p] [Fact p.Prime] : Prop :=
  ∀ (χ : MulChar (ZMod p) ℂ) (m n k l s o q r j uExp vExp : ℕ)
    (lam mu nu xi rho sigma tau upsilon phi psi : ZMod p),
    χ ^ m ≠ 1 → χ ^ n ≠ 1 → χ ^ k ≠ 1 → χ ^ l ≠ 1 →
    χ ^ s ≠ 1 → χ ^ o ≠ 1 → χ ^ q ≠ 1 → χ ^ r ≠ 1 → χ ^ j ≠ 1 →
    χ ^ uExp ≠ 1 → χ ^ vExp ≠ 1 →
    χ ^ (m + n + k + l + s + o + q + r + j + uExp + vExp) ≠ 1 →
    lam ≠ 0 → mu ≠ 0 → nu ≠ 0 → xi ≠ 0 → rho ≠ 0 → sigma ≠ 0 → tau ≠ 0 →
    upsilon ≠ 0 → phi ≠ 0 → psi ≠ 0 →
    lam ≠ mu → lam ≠ nu → lam ≠ xi → lam ≠ rho → lam ≠ sigma → lam ≠ tau →
    lam ≠ upsilon → lam ≠ phi → lam ≠ psi →
    ...
    rho ≠ sigma → rho ≠ tau → rho ≠ upsilon → rho ≠ phi → rho ≠ psi →
    sigma ≠ tau → sigma ≠ upsilon → sigma ≠ phi → sigma ≠ psi →
    tau ≠ upsilon → tau ≠ phi → tau ≠ psi →
    upsilon ≠ phi → upsilon ≠ psi → phi ≠ psi →
    ‖∑ y : ZMod p,
      (χ ^ m) y * (χ ^ n) (y - lam) * (χ ^ k) (y - mu) *
        (χ ^ l) (y - nu) * (χ ^ s) (y - xi) * (χ ^ o) (y - rho) *
          (χ ^ q) (y - sigma) * (χ ^ r) (y - tau) *
            (χ ^ j) (y - upsilon) * (χ ^ uExp) (y - phi) *
              (χ ^ vExp) (y - psi)‖ ≤ 10 * Real.sqrt p

/-- The large-characteristic range needed by the exact Burgess branch. -/
def TaoPrimePowerElevenPointHypergeometricWeilBoundAboveSixtyFour : Prop :=
  ∀ (p : ℕ) [NeZero p] [Fact p.Prime],
    64 < p → TaoPrimePowerElevenPointHypergeometricWeilBoundAt p

/-- Finite exponent-range form of the source-shaped eleven-point endpoint. -/
def TaoPrimeReducedPowerElevenPointHypergeometricWeilBoundAboveSixtyFour : Prop :=
...
/-- Scaling by the first nonzero point puts an eleven-point sum into the
nine-parameter Legendre form `0`, `1`, `t`, `u`, `v`, `w`, `z`, `r₀`, `s₀`,
`a₀`, `b₀`. -/
theorem elevenPointMulCharSum_eq_legendreForm
    (p : ℕ) [NeZero p] [Fact p.Prime]
    (α β γ δ ε ζ η θ ι κ lambdaChar : MulChar (ZMod p) ℂ)
    (lam mu nu xi rho sigma tau upsilon phi psi : ZMod p) (hlam : lam ≠ 0) :
    (∑ y : ZMod p,
  ...
  calc
    ...
    _ = (α lam * β lam * γ lam * δ lam * ε lam * ζ lam * η lam * θ lam * ι lam * κ lam *
      ...
      rw [hsubOne, hsubMu, hsubNu, hsubXi, hsubRho, hsubSigma, hsubTau, hsubUpsilon,
        hsubPhi, hsubPsi, map_mul, map_mul, map_mul, map_mul, map_mul, map_mul, map_mul,
        map_mul, map_mul, map_mul, map_mul]
      ring

/-- The final nine-parameter analytic boundary for twelve active roots. -/
def TaoPrimeReducedPowerElevenPointLegendreWeilBoundAboveSixtyFour : Prop :=
  ∀ (p : ℕ) [NeZero p] [Fact p.Prime], 64 < p →
    ∀ (χ : MulChar (ZMod p) ℂ) (m n k l s o q r j uExp vExp : ℕ)
      (t u v w z r₀ s₀ a₀ b₀ : ZMod p),
    m < orderOf χ → n < orderOf χ → k < orderOf χ →
    l < orderOf χ → s < orderOf χ → o < orderOf χ → q < orderOf χ →
    r < orderOf χ → j < orderOf χ → uExp < orderOf χ → vExp < orderOf χ →
    χ ^ m ≠ 1 → χ ^ n ≠ 1 → χ ^ k ≠ 1 → χ ^ l ≠ 1 →
    ...
    u ≠ v → u ≠ w → u ≠ z → u ≠ r₀ → u ≠ s₀ → u ≠ a₀ → u ≠ b₀ →
    v ≠ w → v ≠ z → v ≠ r₀ → v ≠ s₀ → v ≠ a₀ → v ≠ b₀ →
    w ≠ z → w ≠ r₀ → w ≠ s₀ → w ≠ a₀ → w ≠ b₀ →
    z ≠ r₀ → z ≠ s₀ → z ≠ a₀ → z ≠ b₀ →
    r₀ ≠ s₀ → r₀ ≠ a₀ → r₀ ≠ b₀ → s₀ ≠ a₀ → s₀ ≠ b₀ → a₀ ≠ b₀ →
    ‖∑ y : ZMod p,
...
theorem TaoPrimeReducedPowerElevenPointLegendreWeilBoundAboveSixtyFour.toElevenPoint
    (hweil : TaoPrimeReducedPowerElevenPointLegendreWeilBoundAboveSixtyFour) :
    TaoPrimeReducedPowerElevenPointHypergeometricWeilBoundAboveSixtyFour := by
  intro p _ _ hp χ m n k l s o q r j uExp vExp
    lam mu nu xi rho sigma tau upsilon phi psi
    hm hn hk hl hs ho hq hr hj huExp hvExp
    hpowM hpowN hpowK hpowL hpowS hpowO hpowQ hpowR hpowJ hpowU hpowV hpowSum
  ...
  have hbound := hweil p hp χ m n k l s o q r j uExp vExp t u v w z r₀ s₀ a₀ b₀
    hm hn hk hl hs ho hq hr hj huExp hvExp
    hpowM hpowN hpowK hpowL hpowS hpowO hpowQ hpowR hpowJ hpowU hpowV hpowSum
    ht0 hu0 hv0 hw0 hz0 hr₀0 hs₀0 ha₀0 hb₀0
    ht1 hu1 hv1 hw1 hz1 hr₀1 hs₀1 ha₀1 hb₀1
    htu htv htw htz htr₀ hts₀ hta₀ htb₀
    huv huw huz hur₀ hus₀ hua₀ hub₀
    hvw hvz hvr₀ hvs₀ hva₀ hvb₀
    hwz hwr₀ hws₀ hwa₀ hwb₀
    hzr₀ hzs₀ hza₀ hzb₀ hr₀s₀ hr₀a₀ hr₀b₀ hs₀a₀ hs₀b₀ ha₀b₀
  rw [elevenPointMulCharSum_eq_legendreForm
    p (χ ^ m) (χ ^ n) (χ ^ k) (χ ^ l) (χ ^ s) (χ ^ o) (χ ^ q) (χ ^ r) (χ ^ j)
      (χ ^ uExp) (χ ^ vExp) lam mu nu xi rho sigma tau upsilon phi psi hlam]
  simp only [norm_mul]
  have hC :
      ‖(χ ^ m) lam‖ * ‖(χ ^ n) lam‖ * ‖(χ ^ k) lam‖ * ‖(χ ^ l) lam‖ *
          ‖(χ ^ s) lam‖ * ‖(χ ^ o) lam‖ * ‖(χ ^ q) lam‖ * ‖(χ ^ r) lam‖ *
            ‖(χ ^ j) lam‖ * ‖(χ ^ uExp) lam‖ * ‖(χ ^ vExp) lam‖ ≤ 1 := by
    calc
      ‖(χ ^ m) lam‖ * ‖(χ ^ n) lam‖ * ‖(χ ^ k) lam‖ * ‖(χ ^ l) lam‖ *
            ‖(χ ^ s) lam‖ * ‖(χ ^ o) lam‖ * ‖(χ ^ q) lam‖ * ‖(χ ^ r) lam‖ *
      ...
      1 * ‖∑ y : ZMod p,
        (χ ^ m) y * (χ ^ n) (y - 1) * (χ ^ k) (y - mu / lam) *
          (χ ^ l) (y - nu / lam) * (χ ^ s) (y - xi / lam) *
            (χ ^ o) (y - rho / lam) * (χ ^ q) (y - sigma / lam) *
              (χ ^ r) (y - tau / lam) * (χ ^ j) (y - upsilon / lam) *
                (χ ^ uExp) (y - phi / lam) * (χ ^ vExp) (y - psi / lam)‖ := by gcongr
    _ ≤ 10 * Real.sqrt p := by simpa [t, u, v, w, z, r₀, s₀, a₀, b₀] using hbound

theorem TaoPrimeReducedPowerElevenPointHypergeometricWeilBoundAboveSixtyFour.toPower
    (hweil : TaoPrimeReducedPowerElevenPointHypergeometricWeilBoundAboveSixtyFour) :
    TaoPrimePowerElevenPointHypergeometricWeilBoundAboveSixtyFour := by
  intro p _ _ hp χ m n k l s o q r j uExp vExp
    lam mu nu xi rho sigma tau upsilon phi psi
    hpowM hpowN hpowK hpowL hpowS hpowO hpowQ hpowR hpowJ hpowU hpowV hpowSum
    hlam hmu hnu hxi hrho hsigma htau hupsilon hphi hpsi
  ...
  have hsumEq :
      χ ^ (m' + n' + k' + l' + s' + o' + q' + r' + j' + uExp' + vExp') =
        χ ^ (m + n + k + l + s + o + q + r + j + uExp + vExp) := by
    simp only [pow_add, hmEq, hnEq, hkEq, hlEq, hsEq, hoEq, hqEq, hrEq, hjEq,
      huExpEq, hvExpEq]
  have hpowSum' :
      χ ^ (m' + n' + k' + l' + s' + o' + q' + r' + j' + uExp' + vExp') ≠ 1 := by
    simpa only [hsumEq] using hpowSum
  have hbound := hweil p hp χ m' n' k' l' s' o' q' r' j' uExp' vExp'
    lam mu nu xi rho sigma tau upsilon phi psi
    hmLt hnLt hkLt hlLt hsLt hoLt hqLt hrLt hjLt huExpLt hvExpLt
    hpowM' hpowN' hpowK' hpowL' hpowS' hpowO' hpowQ' hpowR' hpowJ' hpowU' hpowV'
      hpowSum'
    hlam hmu hnu hxi hrho hsigma htau hupsilon hphi hpsi
    hlammu hlamnu hlamxi hlamrho hlamsigma hlamtau hlamupsilon hlamphi hlampsi
    hmunu hmuxi hmurho hmusigma hmutau hmuupsilon hmuphi hmupsi
...
/-- Twelve scalar denominators whose product is one cancel without expanding
...
/-- Pointwise twelve-root Möbius reduction away from the deleted projective
point. Triviality of the twelve-character product cancels the denominator and
leaves eleven finite character factors. -/
theorem twelveRootMobius_term
    (p : ℕ) [NeZero p] [Fact p.Prime]
...
/-- A degree-balanced twelve-root sum is an eleven-point sum with one deleted
projective value, up to a character-valued constant. -/
theorem twelveRootMulCharSum_eq_elevenPointSum_sub
    (p : ℕ) [NeZero p] [Fact p.Prime]
    (α β γ δ ε ζ η θ ι κ lambdaChar muChar : MulChar (ZMod p) ℂ)
    (hprod : α * β * γ * δ * ε * ζ * η * θ * ι * κ * lambdaChar * muChar = 1)
    (a b c d e f g h i j k₀ l₀ : ZMod p)
  ...
  have hterm (y : ZMod p) (hy : y ∈ Finset.univ.erase 1) :
    ...
    simpa [F, G, C, mob] using
      twelveRootMobius_term p α β γ δ ε ζ η θ ι κ lambdaChar muChar hprod
...
/-- Once the canonical eleven-point trace is bounded, the twelve-root identity
costs only the single deleted projective value. -/
theorem norm_twelveRootMulCharSum_le_ten_mul_sqrt_add_one_of_elevenPoint
    (p : ℕ) [NeZero p] [Fact p.Prime]
    (α β γ δ ε ζ η θ ι κ lambdaChar muChar : MulChar (ZMod p) ℂ)
    (hprod : α * β * γ * δ * ε * ζ * η * θ * ι * κ * lambdaChar * muChar = 1)
    (a b c d e f g h i j k₀ l₀ : ZMod p)
    ...
    ‖∑ x : ZMod p,
      α (x - a) * β (x - b) * γ (x - c) * δ (x - d) *
        ε (x - e) * ζ (x - f) * η (x - g) * θ (x - h) * ι (x - i) * κ (x - j) *
          lambdaChar (x - k₀) * muChar (x - l₀)‖ ≤
      10 * Real.sqrt p + 1 := by
  rw [twelveRootMulCharSum_eq_elevenPointSum_sub
  ...
  calc
    ...
    _ ≤ 10 * Real.sqrt p + 1 := by
      exact add_le_add (by simpa [T] using height) (by simpa [D] using hdeleted)

/-- The eleven-point power endpoint gives the projective estimate for twelve
distinct roots whose twelve local powers multiply to one. -/
theorem norm_twelveRootMulCharPowerSum_le_ten_mul_sqrt_add_one
    (p : ℕ) [NeZero p] [Fact p.Prime]
    (hweil : TaoPrimePowerElevenPointHypergeometricWeilBoundAt p)
  ...
  have height :
    ...
    hweil χ m n l s o q z rExp tExp uExp vExp
      ...
      hsigmatau hsigmaupsilon hsigmaphi hsigmapsi
      htauupsilon htauphi htaupsi hupsilonphi hupsilonpsi hphipsi
  exact norm_twelveRootMulCharSum_le_ten_mul_sqrt_add_one_of_elevenPoint
...
/-- Exactly twelve active roots of character-order-divisible degree reduce to
the source-shaped eleven-point endpoint. -/
theorem norm_primeActiveRootCharacterSum_le_ten_mul_sqrt_add_one_of_card_eq_twelve_degree_power
    (p : ℕ) [NeZero p] [Fact p.Prime]
    (hweil : TaoPrimePowerElevenPointHypergeometricWeilBoundAt p)
    (χ : MulChar (ZMod p) ℂ) (P : Polynomial (ZMod p))
    (hP : P.Splits) (hdegree : orderOf χ ∣ P.natDegree)
    (hcard : (primeActiveRoots p χ P).card = 12) :
    ‖primeActiveRootCharacterSum p χ P‖ ≤ 10 * Real.sqrt p + 1 := by
  ...
  have htwelve := norm_twelveRootMulCharPowerSum_le_ten_mul_sqrt_add_one
...
/-- The eleven-point power endpoint discharges the split-polynomial target when
exactly twelve roots are active and degree is character-order divisible. -/
theorem primeSplitPolynomialWeilBound_of_card_activeRoots_eq_twelve_degree_power
    (p : ℕ) [NeZero p] [Fact p.Prime]
    (hweil : TaoPrimePowerElevenPointHypergeometricWeilBoundAt p)
    (χ : MulChar (ZMod p) ℂ) (P : Polynomial (ZMod p))
  ...
  have hactive :=
    norm_primeActiveRootCharacterSum_le_ten_mul_sqrt_add_one_of_card_eq_twelve_degree_power
      p hweil χ P hP hdegree hcard
  have hrestricted :=
    norm_filter_primeActiveRootProduct_le_of_activeRootCharacterSum
      p χ P (10 * Real.sqrt p + 1) hactive
  ...
  have hroom :
      10 * Real.sqrt p + 1 + (primeInactiveRoots p χ P).card ≤
        ((2 * P.roots.toFinset.card : ℕ) : ℝ) * Real.sqrt p := by
    have hInactiveCard' :
  ...
  calc
    ...
    _ ≤ 10 * Real.sqrt p + 1 + (primeInactiveRoots p χ P).card := by
