...
/-- High-frequency prime reciprocal-phase estimate on the pure-linear
coordinate axis.  The same lower scale at `4P` controls every initial prefix
used by reverse Abel summation. -/
theorem eventually_norm_primeReciprocalPhaseSum_le_sourceRange_zero_quadratic
    ...
    ∀ᶠ P : ℕ in atTop, ∀ (a b : ℕ) (N : ℝ),
      ...
      ‖primeReciprocalPhaseSum a b N 0 2‖ ≤
        vaughanPrimeQuadraticDecayConstant * (P : ℝ) *
          (Real.log P) ^ (-S) := by
  have hSone : 0 ≤ S + 1 := by linarith
  have hMangoldt :=
    eventually_norm_mangoldtReciprocalPhaseSum_le_sourceRange_zero_quadratic
  ...
  filter_upwards [htail, hlogLarge, eventually_ge_atTop K] with
      P htailP hlogP hPK
  intro a b N hPa hab hbP hN hlower hNupper
  have hPpos : 0 < P := by
    by_contra h
  ...
  have h64 : (64 : ℝ) ≤ reciprocalPhaseScale N 0 2 (4 * (P : ℝ)) :=
    h64log.trans hlower
  have hpartial : ∀ k, a < k → k ≤ b →
      ‖primeLogReciprocalPhaseSum (Finset.Ico a k) N 0 2‖ ≤
        vaughanPrimeQuadraticDecayConstant * (P : ℝ) *
          (Real.log P) ^ (-(S + 1)) := by
    intro k hak hkb
    ...
    have hlowerk : (Real.log k) ^
          vaughanTypeIILogSavingPhaseExponent (S + 1) ≤
        reciprocalPhaseScale N 0 2 (4 * (P : ℝ)) := hpowkb.trans hlower
    have hNupperk : |N| ≤
    ...
    have hMangoldtk := hK k hkK P a N hPpos hPa hak.le hkP hN h64
      hlowerk hNupperk
    have hMangoldtUniform :
        ‖mangoldtReciprocalPhaseSum N 0 2 a k‖ ≤
        ...
        _ ≤ vaughanMangoldtQuadraticDecayConstant * (P : ℝ) *
          ...
          exact mul_le_mul_of_nonneg_left
            ...
            (mul_nonneg (le_of_lt vaughanMangoldtQuadraticDecayConstant_pos)
              (by positivity))
    have htailk := htailP a k N 0 2 hPa hak.le hkP
    have hprimeLog := norm_primeLogReciprocalPhaseSum_Ico_le_mangoldt_add_tail
      a k N 0 2
    calc
      ‖primeLogReciprocalPhaseSum (Finset.Ico a k) N 0 2‖ ≤
          ‖mangoldtReciprocalPhaseSum N 0 2 a k‖ +
            ‖primePowerTailReciprocalPhaseSum (Finset.Ico a k) N 0 2‖ :=
        hprimeLog
      _ ≤ vaughanMangoldtQuadraticDecayConstant * (P : ℝ) *
            (Real.log P) ^ (-(S + 1)) +
          (P : ℝ) * (Real.log P) ^ (-(S + 1)) :=
