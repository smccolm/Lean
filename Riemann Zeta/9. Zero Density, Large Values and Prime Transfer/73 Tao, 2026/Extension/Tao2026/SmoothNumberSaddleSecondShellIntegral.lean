...
/-!
# Vanishing of the normalized second outer Perron shell

This module converts the square-root-alphabet loss into the explicit scale
`c * u^(2/5) / log u`.  That scale tends to infinity in every critical
regime, and its exponential absorbs the square-root saddle normalization.
The exact symmetric physical contribution from `3*pi/(2*log y)` through
`3*pi/log y` therefore tends to zero.
-/
...
theorem rankinRatio_rpow_two_fifths_le_secondPrimeScale_rpow
    {X y : ℕ} (hX : 2 ≤ X) (hy : 4 ≤ y)
    ...
    (smoothRankinRatio X y / (15 * Real.log 4)) ^ (2 / 5 : ℝ) ≤
      (smoothSaddleSecondPrimeScale y : ℝ) ^
  ...
  have hN : 2 ≤ N := by
    dsimp only [N]
    exact two_le_smoothSaddleSecondPrimeScale hy
  have hNPos : (0 : ℝ) < N := by
  ...
  have hraise : (u / (15 * Real.log 4)) ^ (2 / 5 : ℝ) ≤
      ((y : ℝ) ^ e) ^ (2 / 5 : ℝ) :=
    Real.rpow_le_rpow (div_nonneg huPos.le hCPos.le) hbase (by norm_num)
  have hlogN : (2 / 5 : ℝ) * Real.log (y : ℝ) ≤ Real.log (N : ℝ) := by
    simpa only [N] using
      two_fifths_log_le_log_smoothSaddleSecondPrimeScale hy hlogY
  have hexponent : (2 / 5 : ℝ) * e * Real.log (y : ℝ) ≤
      e * Real.log (N : ℝ) := by
    have := mul_le_mul_of_nonneg_left hlogN he
    nlinarith
  calc
    (u / (15 * Real.log 4)) ^ (2 / 5 : ℝ) ≤
        ((y : ℝ) ^ e) ^ (2 / 5 : ℝ) := hraise
    _ = (y : ℝ) ^ (e * (2 / 5 : ℝ)) := by
...
theorem secondOuterMultiShell_cosineLoss_lower_at_saddle
    {B X y : ℕ} {t : ℝ} (hB : 4 ≤ B) (hX : 2 ≤ X) (hy : 4 ≤ y)
    (hlogY : 10 * Real.log 2 ≤ Real.log (y : ℝ))
    (hlogYOne : 1 ≤ Real.log (y : ℝ))
    (hlogU : 12 ≤ Real.log (smoothRankinRatio X y))
    (hBu : (B : ℝ) ≤ smoothRankinRatio X y)
    (huN : smoothRankinRatio X y ≤
      Real.sqrt (smoothSaddleSecondPrimeScale y))
    (hpnt : ∀ n : ℕ, B ≤ n →
  ...
  have hlogNUpper := log_smoothSaddleSecondPrimeScale_le_half_log hy
  have hlogyNonneg : 0 ≤ Real.log (y : ℝ) := hlogyPos.le
  have hlogNLeY : Real.log (N : ℝ) ≤ Real.log (y : ℝ) := by
    calc
      Real.log (N : ℝ) ≤ (1 / 2 : ℝ) * Real.log (y : ℝ) := by
  ...
  have hrankin := rankinRatio_rpow_two_fifths_le_secondPrimeScale_rpow
  ...
  have hmulti := secondOuterMultiShell_cosineLoss_lower
    hB hy hlogY hlogU hBu huN hpnt hsigmaOne htLower htUpper
  have hleft : Real.exp (-16) *
      (u / (15 * Real.log 4)) ^ (2 / 5 : ℝ) ≤
      (w : ℝ) ^ (1 - sigma) := by
    calc
      Real.exp (-16) * (u / (15 * Real.log 4)) ^ (2 / 5 : ℝ) ≤
...
noncomputable def smoothSaddleSecondOuterLossScale (X y : ℕ) : ℝ :=
  (Real.exp (-16) *
      (smoothRankinRatio X y / (15 * Real.log 4)) ^ (2 / 5 : ℝ)) /
    (16 * Real.log 2 * Real.log (smoothRankinRatio X y))

theorem tendsto_rpow_two_fifths_div_log_atTop
    {u : ℕ → ℝ} (hu : Tendsto u atTop atTop) :
    Tendsto (fun n => (u n) ^ (2 / 5 : ℝ) / Real.log (u n))
      atTop atTop := by
  have hpow : Tendsto (fun n => (u n) ^ (1 / 5 : ℝ)) atTop atTop :=
    (tendsto_rpow_atTop (by norm_num : (0 : ℝ) < 1 / 5)).comp hu
  have hlogSmall := (isLittleO_log_rpow_atTop
    (by norm_num : (0 : ℝ) < 1 / 5)).def (by norm_num : (0 : ℝ) < 1)
...
theorem tendsto_sqrt_mul_exp_neg_rpow_two_fifths_div_log_zero
    {u : ℕ → ℝ} (hu : Tendsto u atTop atTop) {a : ℝ} (ha : 0 < a) :
    Tendsto (fun n => Real.sqrt (u n) *
      Real.exp (-a * ((u n) ^ (2 / 5 : ℝ) / Real.log (u n))))
      atTop (𝓝 0) := by
  have hroot : Tendsto (fun n => (u n) ^ (1 / 5 : ℝ)) atTop atTop :=
    (tendsto_rpow_atTop (by norm_num : (0 : ℝ) < 1 / 5)).comp hu
  have hupper : Tendsto (fun n =>
      ((u n) ^ (1 / 5 : ℝ)) ^ (5 / 2 : ℝ) *
        Real.exp (-a * (u n) ^ (1 / 5 : ℝ))) atTop (𝓝 0) :=
    (tendsto_rpow_mul_exp_neg_mul_atTop_nhds_zero (5 / 2) a ha).comp hroot
  have hlogSmall := (isLittleO_log_rpow_atTop
    (by norm_num : (0 : ℝ) < 1 / 5)).def (by norm_num : (0 : ℝ) < 1)
  ...
  have huPos : 0 < u n := lt_trans zero_lt_one huOne
  have hlogPos : 0 < Real.log (u n) := Real.log_pos huOne
  have hrootPos : 0 < (u n) ^ (1 / 5 : ℝ) :=
    Real.rpow_pos_of_pos huPos _
  have hlogLe : Real.log (u n) ≤ (u n) ^ (1 / 5 : ℝ) := by
    simpa only [Real.norm_eq_abs, abs_of_pos hlogPos,
      abs_of_pos hrootPos, one_mul] using hlog
  have hratio : (u n) ^ (1 / 5 : ℝ) ≤
      (u n) ^ (2 / 5 : ℝ) / Real.log (u n) := by
    rw [le_div_iff₀ hlogPos]
    calc
      (u n) ^ (1 / 5 : ℝ) * Real.log (u n) ≤
          (u n) ^ (1 / 5 : ℝ) * (u n) ^ (1 / 5 : ℝ) :=
      ...
      _ = (u n) ^ (2 / 5 : ℝ) := by
        ...
        congr 1
        ring
  have hsqrt : ((u n) ^ (1 / 5 : ℝ)) ^ (5 / 2 : ℝ) =
      Real.sqrt (u n) := by
    rw [← Real.rpow_mul huPos.le]
    norm_num
    rw [← Real.sqrt_eq_rpow]
  have hexp : Real.exp (-a *
      ((u n) ^ (2 / 5 : ℝ) / Real.log (u n))) ≤
      Real.exp (-a * (u n) ^ (1 / 5 : ℝ)) := by
    apply Real.exp_le_exp.mpr
    nlinarith
  rw [hsqrt]
  exact mul_le_mul_of_nonneg_left hexp (Real.sqrt_nonneg _)

theorem IsTaoCriticalSmoothRegime.tendsto_smoothSaddleSecondOuterLossScale_atTop
    {X y : ℕ → ℕ} {α : ℝ}
    (hregime : IsTaoCriticalSmoothRegime X y α) (hα : 0 < α) :
    Tendsto (fun n => smoothSaddleSecondOuterLossScale (X n) (y n))
      atTop atTop := by
  let C : ℝ := 15 * Real.log 4
  let K : ℝ := Real.exp (-16) /
    (C ^ (2 / 5 : ℝ) * (16 * Real.log 2))
  have hC : 0 < C := by dsimp only [C]; positivity
  have hK : 0 < K := by dsimp only [K]; positivity
  have hbase := tendsto_rpow_two_fifths_div_log_atTop
  ...
  have hlogu : Real.log (smoothRankinRatio (X n) (y n)) ≠ 0 :=
    (Real.log_pos hu).ne'
  unfold smoothSaddleSecondOuterLossScale
  dsimp only [K, C]
  rw [Real.div_rpow huPos.le hC.le]
  field_simp [hlogu, hC.ne',
    (Real.rpow_pos_of_pos hC (2 / 5 : ℝ)).ne',
    (Real.log_pos (by norm_num : (1 : ℝ) < 2)).ne']
  ring

theorem IsTaoCriticalSmoothRegime.tendsto_sqrt_rankinRatio_mul_secondOuterEnvelope_zero
    {X y : ℕ → ℕ} {α : ℝ}
    (hregime : IsTaoCriticalSmoothRegime X y α) (hα : 0 < α) :
    Tendsto (fun n =>
      Real.sqrt (smoothRankinRatio (X n) (y n)) *
        Real.exp (-(smoothSaddleSecondOuterLossScale (X n) (y n) / 96)))
      atTop (𝓝 0) := by
  let C : ℝ := 15 * Real.log 4
  let a : ℝ := Real.exp (-16) /
    (C ^ (2 / 5 : ℝ) * (16 * Real.log 2) * 96)
  have hC : 0 < C := by dsimp only [C]; positivity
  have ha : 0 < a := by dsimp only [a]; positivity
  have h := tendsto_sqrt_mul_exp_neg_rpow_two_fifths_div_log_zero
...
theorem IsTaoCriticalSmoothRegime.eventually_smoothSaddleSecondOuterLossScale_le_cosineLoss
    {X y : ℕ → ℕ} {α : ℝ}
    (hregime : IsTaoCriticalSmoothRegime X y α) (hα : 0 < α) :
    ∀ᶠ n in atTop, ∀ t : ℝ,
      3 * Real.pi / (2 * Real.log (y n : ℝ)) ≤ t →
      t ≤ 3 * Real.pi / Real.log (y n : ℝ) →
      smoothSaddleSecondOuterLossScale (X n) (y n) ≤
  ...
  filter_upwards [hregime.eventually_two_le_X,
    ...
    hregime.eventually_one_sub_saddle_lt_eight_log_ratio hα] with
      n hX hy hlogY hlogYOne hlogU hBu huN hsigmaHalf hsigmaOne hdisplacement
  intro t htLower htUpper
  unfold smoothSaddleSecondOuterLossScale
  exact secondOuterMultiShell_cosineLoss_lower_at_saddle
    hB hX hy
    hlogY hlogYOne hlogU hBu huN hpnt hsigmaHalf.le hsigmaOne.le
    hdisplacement.le htLower htUpper

noncomputable def smoothSaddleSecondOuterUpperHeight (y : ℕ) : ℝ :=
  3 * Real.pi / Real.log (y : ℝ)

noncomputable def smoothSaddleSecondOuterPerronContribution
    (X y : ℕ) : ℂ :=
...
theorem IsTaoCriticalSmoothRegime.eventually_norm_smoothSaddlePerronLineIntegrand_le_secondOuterEnvelope_abs
    {X y : ℕ → ℕ} {α : ℝ}
    (hregime : IsTaoCriticalSmoothRegime X y α) (hα : 0 < α) :
    ∀ᶠ n in atTop, ∀ t : ℝ,
      smoothSaddleExtendedOuterUpperHeight (y n) ≤ |t| →
      |t| ≤ smoothSaddleSecondOuterUpperHeight (y n) →
      ‖smoothSaddlePerronLineIntegrand (X n) (y n) t‖ ≤
        Real.exp (-(smoothSaddleSecondOuterLossScale (X n) (y n) / 96)) := by
  filter_upwards [hregime.eventually_two_le_X,
    hregime.eventually_two_le_y hα,
    (hregime.tendsto_smoothSaddlePoint_one hα).eventually
      (Ioi_mem_nhds (by norm_num : (1 / 2 : ℝ) < 1)),
    hregime.eventually_smoothSaddleSecondOuterLossScale_le_cosineLoss hα] with
      n hX hy hsigma hloss
  intro t htLower htUpper
  have hlossT : smoothSaddleSecondOuterLossScale (X n) (y n) ≤
      smoothSaddleCosineLoss (y n) (smoothSaddlePoint (X n) (y n)) t := by
    by_cases ht : 0 ≤ t
    · have habs : |t| = t := abs_of_nonneg ht
      exact hloss t
        (by simpa [smoothSaddleExtendedOuterUpperHeight, habs] using htLower)
        (by simpa [smoothSaddleSecondOuterUpperHeight, habs] using htUpper)
  ...
  calc
    ...
    _ ≤ Real.exp (-(smoothSaddleSecondOuterLossScale (X n) (y n) / 96)) := by
      apply Real.exp_le_exp.mpr
      nlinarith

theorem IsTaoCriticalSmoothRegime.tendsto_smoothSaddleSecondOuterPerronContribution_zero
    {X y : ℕ → ℕ} {α : ℝ}
    (hregime : IsTaoCriticalSmoothRegime X y α) (hα : 0 < α) :
    Tendsto (fun n =>
      smoothSaddleSecondOuterPerronContribution (X n) (y n))
      atTop (𝓝 0) := by
  let C : ℝ := 3 * Real.pi * Real.sqrt 7 / Real.sqrt (2 * Real.pi)
  have hC : 0 ≤ C := by dsimp only [C]; positivity
  have hdecay :=
    hregime.tendsto_sqrt_rankinRatio_mul_secondOuterEnvelope_zero hα
  ...
  filter_upwards [hregime.eventually_two_le_X,
    ...
    (hregime.tendsto_smoothSaddlePoint_one hα).eventually
      (Ioi_mem_nhds (by norm_num : (1 / 2 : ℝ) < 1)),
    hregime.eventually_norm_smoothSaddlePerronLineIntegrand_le_secondOuterEnvelope_abs hα]
    with n hX hy hsigma hpoint
  let E := Real.exp (-(smoothSaddleSecondOuterLossScale (X n) (y n) / 96))
  let L := smoothSaddleExtendedOuterUpperHeight (y n)
  let U := smoothSaddleSecondOuterUpperHeight (y n)
  have hlogy : 0 < Real.log (y n : ℝ) :=
    Real.log_pos (by exact_mod_cast (show 1 < y n by omega))
  have hL : 0 ≤ L := by
    dsimp only [L, smoothSaddleExtendedOuterUpperHeight]
    positivity
  have hLU : L ≤ U := by
    dsimp only [L, U, smoothSaddleExtendedOuterUpperHeight,
      smoothSaddleSecondOuterUpperHeight]
    apply (div_le_div_iff₀ (mul_pos (by norm_num) hlogy) hlogy).mpr
    nlinarith [Real.pi_pos]
  have hnorm := norm_smoothSaddleSymmetricPerronShellContribution_le
    hX hy hL hLU (E := E) hpoint
  have hwidth : U - L = 3 * Real.pi / (2 * Real.log (y n : ℝ)) := by
    dsimp only [U, L, smoothSaddleSecondOuterUpperHeight,
  ...
  calc
    ‖smoothSaddleSymmetricPerronShellContribution (X n) (y n) L U‖ ≤
        (smoothSaddleStandardDeviation (X n) (y n) /
          Real.sqrt (2 * Real.pi)) *
          (2 * E * (3 * Real.pi / (2 * Real.log (y n : ℝ)))) := hnorm
    _ = (3 * Real.pi / Real.sqrt (2 * Real.pi)) *
        (smoothSaddleStandardDeviation (X n) (y n) /
          Real.log (y n : ℝ) * E) := by
