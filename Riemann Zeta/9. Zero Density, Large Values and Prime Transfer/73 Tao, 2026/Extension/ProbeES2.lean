...
theorem probe_gcd_prod_dvd_prod_gcd {ι : Type*} [DecidableEq ι]
...
theorem probe_dvd_prod_gcd_of_dvd_prod {ι : Type*} [DecidableEq ι]
    {q : ℕ} {s : Finset ι} {f : ι → ℕ}
    (h : q ∣ ∏ i ∈ s, f i) :
  ...
  calc
    ...
    _ ∣ ∏ i ∈ s, Nat.gcd q (f i) :=
      probe_gcd_prod_dvd_prod_gcd q s f

theorem probe_gcd_lt_length_of_interval
...
theorem probe_consecutive_subproduct_injective_of_pow_lt_start
  ...
  have hmDvdGcd : m ∣ ∏ b ∈ T, Nat.gcd m b :=
    probe_dvd_prod_gcd_of_dvd_prod hmDvdT
  have hgcdLe : ∏ b ∈ T, Nat.gcd m b ≤ H ^ T.card := by
    apply Finset.prod_le_pow_card
    intro b hb
    exact (probe_gcd_lt_length_of_interval hmInterval (hT hb)
...
theorem probe_pow_add_length_lt_add_one_pow {H l : ℕ}
...
theorem probe_pow_lt_start_of_failure_of_sylvesterSchur
  ...
  obtain ⟨m, hm, hpm⟩ := (hp.prime.dvd_finset_prod_iff id).mp hpProduct'
  have hmPos : 0 < m := by
  ...
  have hsum : H ^ l + H < N + H :=
    (probe_pow_add_length_lt_add_one_pow (by omega) hl).trans_le
      (hHpPow.trans (hpPowLeM.trans hmUpper))
  omega

theorem probe_add_pow_succ_le {N H s : ℕ} (hHN : H ≤ N) :
    (N + H) ^ (s + 1) ≤
...
theorem probe_add_pow_sub_pow_le (N H r : ℕ) :
  ...
  rw [abs_of_nonneg hsub, abs_of_nonneg hdiff,
    abs_of_nonneg (by positivity : (0 : ℝ) ≤
      ((N + H : ℕ) : ℝ)), abs_of_nonneg (by positivity : (0 : ℝ) ≤ N),
...
theorem probe_succ_mul_two_pow_lt_three_pow_succ (n : ℕ) :
    (n + 1) * 2 ^ n < 3 ^ (n + 1) := by
  induction n with
  | zero => norm_num
  | succ n ih =>
...
theorem probe_r_mul_two_pow_sub_one_lt_pow
    {H r : ℕ} (hH : 3 ≤ H) (hr : 1 ≤ r) :
    r * (2 ^ (r - 1) - 1) < H ^ r := by
  have hmain : r * 2 ^ (r - 1) < 3 ^ r := by
    have h := probe_succ_mul_two_pow_lt_three_pow_succ (r - 1)
...
theorem probe_interval_power_gap_upper
  ...
  · simp
    omega
  have hHN : H ≤ N := by
    have hHpow : H ≤ H ^ (r + 1) := by
      exact Nat.le_pow (by omega : 0 < r + 1)
    omega
  have hmean := probe_add_pow_sub_pow_le N H r
  have happrox : (N + H) ^ (r - 1) ≤
      N ^ (r - 1) + (2 ^ (r - 1) - 1) * H * N ^ (r - 2) := by
    have h := probe_add_pow_succ_le (s := r - 2) hHN
    have hexp : r - 2 + 1 = r - 1 := by omega
    rwa [hexp] at h
  have hcoef := probe_r_mul_two_pow_sub_one_lt_pow hH hr
...
theorem probe_interval_power_gap_pow_upper
    {N H l r : ℕ} (hH : 3 ≤ H) (hl : 2 ≤ l)
    (hr : 1 ≤ r) (hrl : r < l) (hscale : H ^ l < N) :
    ((N + H) ^ r - N ^ r) ^ l < l ^ l * N ^ (r * (l - 1)) := by
  have hrSuccLe : r + 1 ≤ l := by omega
  have hscaleR : H ^ (r + 1) < N := by
    exact (Nat.pow_le_pow_right (by omega : 0 < H) hrSuccLe).trans_lt hscale
  have hgap := probe_interval_power_gap_upper hH hr hscaleR
...
theorem probe_rationalPowerRatio_gap_pow_lower
  ...
  have hyPowDvdQ : y ^ l ∣ Q := by
    ...
    exact ⟨P, by ring⟩
  let A : ℕ := Q / y ^ l
  have hQeq : Q = A * y ^ l := by
    exact (Nat.div_mul_cancel hyPowDvdQ).symm
  have hApos : 0 < A := by
    exact Nat.div_pos (Nat.le_of_dvd hQ hyPowDvdQ) (pow_pos hy l)
  ...
  calc
    l ^ l * Q ^ (l - 1) ≤ A * (l ^ l * Q ^ (l - 1)) := by
      simpa using
        Nat.mul_le_mul_right (l ^ l * Q ^ (l - 1)) hApos
    _ = (A * (l * y ^ (l - 1))) ^ l := hfactorization.symm
    _ ≤ (P - Q) ^ l := hgapPow

theorem probe_start_pow_lt_subproduct
...
theorem probe_subproduct_le_endpoint_pow
    {N H r : ℕ} {S : Finset ℕ}
    (hS : S ⊆ consecutiveInterval N H) (hScard : S.card = r) :
    (∏ m ∈ S, m) ≤ (N + H) ^ r := by
  calc
...
theorem probe_consecutiveSubproduct_not_rationalPowerRatio
  ...
  let Q := ∏ m ∈ T, m
  have hPpos : 0 < P := by
    exact Finset.prod_pos fun m hm => by
      have := (Finset.mem_Ioc.mp (hS hm)).1
      omega
  have hQpos : 0 < Q := by
    exact Finset.prod_pos fun m hm => by
  ...
  have hPneQ : P ≠ Q := by
    intro hEq
    exact hST (probe_consecutive_subproduct_injective_of_pow_lt_start
      (by omega) hscale hrl hS hT hScard hTcard hEq)
  have hcrossPQ : P * v ^ l = Q * u ^ l := hcross
  have hPupper : P ≤ (N + H) ^ r :=
    probe_subproduct_le_endpoint_pow hS hScard
  have hQupper : Q ≤ (N + H) ^ r :=
    probe_subproduct_le_endpoint_pow hT hTcard
  have hPlower : N ^ r < P := probe_start_pow_lt_subproduct hr hS hScard
  have hQlower : N ^ r < Q := probe_start_pow_lt_subproduct hr hT hTcard
  have hscalar := probe_interval_power_gap_pow_upper hH hl hr hrl hscale
  rcases lt_or_gt_of_ne hPneQ with hPQ | hQP
  · have hlower := probe_rationalPowerRatio_gap_pow_lower (by omega) hPpos hv hu
        hcop.symm hcrossPQ.symm hPQ
    ...
    omega
  · have hlower := probe_rationalPowerRatio_gap_pow_lower (by omega) hQpos hu hv
        hcop hcrossPQ hQP
...
theorem probe_consecutiveSubproduct_not_rationalPowerRatio'
  ...
  exact probe_consecutiveSubproduct_not_rationalPowerRatio hH hl hr hrl hscale
    hS hT hScard hTcard hST hu₀ hv₀ hcop hcross₀

theorem probe_powerFreePart_subproduct_not_rationalPowerRatio
    {N H l r u v : ℕ} (hH : 3 ≤ H) (hl : 2 ≤ l)
    (hr : 1 ≤ r) (hrl : r < l) (hscale : H ^ l < N)
    {S T : Finset ℕ} (hS : S ⊆ consecutiveInterval N H)
    (hT : T ⊆ consecutiveInterval N H) (hScard : S.card = r)
