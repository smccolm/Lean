...
try {
    ...
    Write-Host 'MILESTONE: the full corrected Lemma 3.1 contract is closed conditional on Theorem 2.5: (N,H)=(0,1) is the unique exception to H<N, unconditional eventual quadratic-window scale and H/400 measure lower bound, the complete fixed-constant logarithmic contradiction, and every fixed-slack subexponential bound are verified.'

    Write-Host 'MILESTONE: full Theorem 1.8 is closed conditional only on Theorem 2.5: the finite nontrivial-value cover, injective Lemma 3.2 certificate/length/offset encoding, subpolynomial natural length and coefficient budgets, uniform Corollary 2.11 family count, x^(2/5+o(1)) bound, exponent-gap little-o argument, and final zeta-ratio asymptotic are verified.'
    ...
    Write-Host 'MILESTONE: the Burgess boundary for Lemma 5.1 is source-faithful and scale-complete. The exact sieve-prefix predicate only requests H=floor((2Z-1)/d); a proposition-valued explicit cubefree Burgess target uses saving 0.0163 and range q<=H^3.1. For canonical R=floor(Z^0.0001), complementary floor scales have product at most Z, every prefix eventually exceeds any fixed Burgess cutoff, Z^3.09 is eventually at most H^3.1 uniformly over supported divisors, and Bertrand discharges prime-band nonemptiness. Thus explicit Burgess itself feeds the finite normalized family estimate with no remaining scale hypotheses.'
