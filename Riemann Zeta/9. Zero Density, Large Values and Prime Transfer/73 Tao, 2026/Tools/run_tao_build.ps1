...
$taoTranscriptActive = $false

try {
    $nodeRoot = Split-Path -Parent $PSScriptRoot
    $extensionRoot = Join-Path $nodeRoot 'Extension'
    $taoLogDirectory = Join-Path $nodeRoot 'logs'
    if (-not (Test-Path -LiteralPath $taoLogDirectory -PathType Container)) {
        New-Item -ItemType Directory -Path $taoLogDirectory -ErrorAction Stop | Out-Null
    }
    $taoLogStamp = Get-Date -Format 'yyyyMMdd-HHmmss'
    $taoLogId = [guid]::NewGuid().ToString('N').Substring(0, 8)
    $taoVerificationLogPath = Join-Path $taoLogDirectory "tao-build-$taoLogStamp-$taoLogId.log"
    Start-Transcript -LiteralPath $taoVerificationLogPath -NoClobber -ErrorAction Stop | Out-Null
    $taoTranscriptActive = $true
    ...
    $requiredFiles = @(
        ...
        'Extension\Tao2026\BurgessWeilPrimeKummerFrobeniusSubfields.lean',
        'Extension\Tao2026\FinitePermutationWeightedOrbits.lean',
        'Extension\Tao2026\WeightedOrbitNewtonIntegrality.lean',
        'Extension\Tao2026\BurgessWeilPrimeKummerNewtonAllDegreeIntegrality.lean',
        'Extension\Tao2026\BurgessWeilPrimeKummerMonicPolynomialSums.lean',
        'Extension\Tao2026\BurgessWeilPrimeKummerOrbitPolynomials.lean',
        'Extension\Tao2026\PolynomialEulerCoefficients.lean',
        'Extension\Tao2026\BurgessWeilPrimeKummerNewtonPolynomiality.lean',
        'Extension\Tao2026\BurgessWeilPrimeKummerLiteralWeilOnly.lean',
        'Extension\Tao2026\BurgessWeilPrimeKummerWeilAmplification.lean',
        'Extension\Tao2026\PolynomialStepanovMultiplicity.lean',
        'Extension\Tao2026\PolynomialKummerIrreducibility.lean',
        'Extension\Tao2026\PolynomialTwistedHasse.lean',
        'Extension\Tao2026\PolynomialStepanovAuxiliary.lean',
        'Extension\Tao2026\PolynomialStepanovSimpleRoot.lean',
        'Extension\Tao2026\PolynomialStepanovSqrtParameters.lean',
        'Extension\Tao2026\FiniteCharacterFiberCancellation.lean',
        'Extension\Tao2026\FiniteFieldNormStepanov.lean',
        'Extension\Tao2026\BurgessWeilPrimeSimpleRoot.lean',
        'Extension\Tao2026\BurgessWeilPrimeComplete.lean',
        'Extension\Tao2026\Theorem17Complete.lean',
        'Extension\Tao2026\PrimeFreeLogShortness.lean',
        'Extension\Tao2026\Theorem19And110Complete.lean',
        'Extension\Tao2026\PublicTheorems.lean',
        'Extension\Tao2026\SemanticRegression.lean',
        ...
        'Sources\cochrane-pinner-stepanov-author.pdf',
    ...
    if ($hashLines.Count -ne 12) {
        throw 'SHA256SUMS.txt must contain exactly the Tao PDF and TeX archive, Baker-Harman-Pintz PDF, Erdos-Selfridge PDF, Granville and Canfield-Erdos-Pomerance smooth-number PDFs, Burgess 1962 PDF, explicit cubefree Burgess PDF and TeX archive, Singmaster PDF and TeX archive, and Cochrane-Pinner author PDF.'
    }
    ...
    $forbiddenPattern = '(?m)(^\s*(axiom|constant)\s+|\bsorry\b|\bsorryAx\b|\badmit\b|\bnative_decide\b|\bimplemented_by\b|\bunsafe\b)'
    ...
    foreach ($source in $allExtensionLeanFiles) {
        $matches = Select-String -LiteralPath $source.FullName -Pattern $forbiddenPattern
        if ($matches) {
            $first = $matches | Select-Object -First 1
    ...
    if ($buildOutput | Where-Object {
        ...
        $_ -match '(?i)(Try this:|The .+ tactic failed to close the goal|declaration uses.+sorry)'
    }) {
        throw 'The canonical Tao2026 build emitted a warning or tactic diagnostic.'
    }

    Write-Host 'Running the complete transitive axiom audit explicitly...'
    Push-Location $extensionRoot
    try {
        $auditOutput = @(& $lakeExecutable env lean Tao2026\Audit.lean 2>&1)
        $taoAuditExit = $LASTEXITCODE
    }
    finally {
        Pop-Location
    }
    $auditOutput | ForEach-Object { Write-Host $_ }
    if ($taoAuditExit -ne 0) {
        throw "Tao2026/Audit.lean failed with exit code $taoAuditExit"
    }
    if ($auditOutput | Where-Object {
        $_ -match '(?i)(^|:\d+:\d+:\s*)warning:' -or
        $_ -match '(?i)(Try this:|The .+ tactic failed to close the goal|declaration uses.+sorry)'
    }) {
        throw 'The explicit axiom audit emitted a warning or tactic diagnostic.'
    }
    $taoBuildText = $auditOutput -join "`n"
    $taoAxiomLists = [regex]::Matches($taoBuildText, 'depends on axioms:\s*\[([^\]]*)\]')
    $taoAllowedAxioms = @('propext', 'Classical.choice', 'Quot.sound')
    foreach ($taoAxiomList in $taoAxiomLists) {
        foreach ($taoAxiomName in ($taoAxiomList.Groups[1].Value -split ',')) {
            if ($taoAxiomName.Trim() -notin $taoAllowedAxioms) {
                throw "The production axiom audit found an unapproved axiom: $($taoAxiomName.Trim())"
            }
        }
    }
    if ($taoBuildText -notmatch 'Tao2026\.primeRootMultisetNewtonElementary_integral.+depends on axioms:') {
        throw 'The release-5.06 all-degree Newton axiom audit was not present in the build output.'
    }
    if ($taoBuildText -notmatch 'Tao2026\.primeRootMultisetNewtonPowerSum_characteristic_recurrence.+depends on axioms:' -or
        $taoBuildText -notmatch 'Tao2026\.TaoPrimeLiteralWeilConditions.toTheorem17.+depends on axioms:') {
        throw 'The release-5.07 recurrence and Weil-only endpoint audits were not present in the build output.'
    }
    if ($taoBuildText -notmatch 'Tao2026\.taoPrimeLinearQuotientWeilBound_unconditional.+depends on axioms:' -or
        $taoBuildText -notmatch 'Tao2026\.taoTheorem17_unconditional.+depends on axioms:') {
        throw 'The release-5.08 unconditional prime Weil and Theorem 1.7 audits were not present in the build output.'
    }
    $taoRequiredFinalAudits = @(
        'taoTheorem17_unconditional',
        'taoTheorem18_unconditional',
        'taoTheorem19_unconditional',
        'taoTheorem110_unconditional',
        'taoMainTheorems_unconditional',
        'taoLemma42_unconditional',
        'eventually_four_factorialPrimeScale_le_unconditional',
        'eventually_two_sqrt_mul_log_le_div_log_pow',
        'classicalChebyshevThetaLogSaving_native',
        'eventually_abs_theta_sub_le_self_div_sixteen_log_sq',
        'chebyshevTheta_nat_add_eq_of_primeFree',
        'eventually_four_mul_primeFree_length_log_sq_le',
        'regression_interval_endpoints',
        'regression_interval_fixture',
        'regression_count_partitions',
        'regression_squarefree_zero_one',
        'regression_squarefree_parity',
        'regression_factorial_fiber_multiplicity',
        'regression_asymptotic_epsilon_quantifier',
        'regression_exact_public_contracts',
        'regression_power_scale_both_directions',
        'regression_quotient_scale_signs'
    )
    foreach ($taoRequiredFinalAudit in $taoRequiredFinalAudits) {
        $taoRequiredAuditPattern = 'Tao2026\.' + [regex]::Escape($taoRequiredFinalAudit) + "'.+depends on axioms:"
        if ($taoBuildText -notmatch $taoRequiredAuditPattern) {
            throw "Required public or semantic-regression axiom audit is missing: $taoRequiredFinalAudit"
        }
    }
    ...
    Write-Host 'HISTORICAL MILESTONE: the complete fourth outer Perron shell is integrated. Its eighth-root-alphabet loss is at least a positive constant times u^(1/9)/log(u), this exponential absorbs the saddle normalization, and the exact symmetric contribution through 12*pi/log(y) tends to zero. Later shells and the infinite outer line remain.'
    Write-Host 'HISTORICAL MILESTONE: the repeated outer-shell construction now has indexed infrastructure. Arbitrary iterated natural square-root alphabets have closed-form lower and upper logarithmic envelopes, recover the certified concrete scales, and carry physical shell endpoints that double exactly. Uniform indexed phase loss and infinite aggregation remain.'
    Write-Host 'HISTORICAL MILESTONE: uniform indexed phase loss is compiled. At every k>=2, one four-fifths lower bound for the ideal iterated-root logarithmic scale gives the retained CEP support, exact nonpositive-cosine phase window between adjacent indexed heights, and the full accumulated cosine-loss bound. Growing-range verification and shell summation remain.'
    Write-Host 'HISTORICAL MILESTONE: the indexed scale range is explicit. Iterated roots are antitone, terminal noncollapse controls every preceding scale, and 10*(2^k-1)*log(2)<=log(y) implies the four-fifths logarithmic retention required by the uniform phase theorem. A growing terminal-index choice and aggregate tail estimate remain.'
