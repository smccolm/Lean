...
`erdosSelfridgeNextPrime_le_endpoint_of_lt_start`,
`erdosSelfridgeSquareConclusion_of_primeMultiplicity`, and
`taoTheorem110_of_source_inputs_and_primeMultiplicity`. Thus every
representation and endpoint condition between Theorem 2 and the public
Theorem 1.10 consumer is proved; only the prime-multiplicity theorem itself
remains an input.

The next source line, equation (3) on journal page 293, is represented by
`powerFreePart_mul_powerRootPart_pow`,
`factorization_powerFreePart_lt`,
