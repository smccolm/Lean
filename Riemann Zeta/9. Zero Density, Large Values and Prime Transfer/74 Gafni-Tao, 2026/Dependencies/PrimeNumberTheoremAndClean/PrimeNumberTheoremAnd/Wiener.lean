...
/-!
The pinned upstream file next contains the declarations `prelim_decay_2`,
`AbsolutelyContinuous`, `prelim_decay_3`, and `decay_alt`.  The two preliminary
theorems are admitted upstream, and this entire four-declaration block is
unreachable from `WeakPNT` and `Consequences`.  This minimal release dependency
therefore omits that block.  The provenance manifest records the upstream and
patched hashes, and the isolated runner verifies both the omission and the
unchanged hashes of every other retained source file.
-/
