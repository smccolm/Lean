import GafniTao.HeathBrownExponentialOverlap
...
theorem sum_intervalIntegral_exp_kernel_zeta_sq_le_four
    ...
    ∑ t ∈ W,
      ...
      4 * ∫ u in a..b, heathBrownCriticalZetaNorm u ^ (2 : ℕ) := by
  exact sum_intervalIntegral_exp_neg_abs_sub_mul_le_four W
    (fun u : ℝ ↦ heathBrownCriticalZetaNorm u ^ (2 : ℕ))
    continuous_heathBrownCriticalZetaNorm_sq (fun u ↦ sq_nonneg _) a b hab hSep
...
theorem sum_truncated_exp_kernel_zeta_sq_le_four
    ...
    ∑ t ∈ W,
        (∫ u in t - L..t + L,
          Real.exp (-|t - u|) * heathBrownCriticalZetaNorm u ^ (2 : ℕ)) ≤
      4 * ∫ u in lo - L..hi + L,
        heathBrownCriticalZetaNorm u ^ (2 : ℕ) := by
  calc
    ∑ t ∈ W,
        (∫ u in t - L..t + L,
          Real.exp (-|t - u|) * heathBrownCriticalZetaNorm u ^ (2 : ℕ)) ≤
        ∑ t ∈ W,
          (∫ u in lo - L..hi + L,
            Real.exp (-|t - u|) * heathBrownCriticalZetaNorm u ^ (2 : ℕ)) := by
      ...
      · exact intervalIntegrable_exp_neg_abs_sub_mul
          (fun u : ℝ ↦ heathBrownCriticalZetaNorm u ^ (2 : ℕ))
          continuous_heathBrownCriticalZetaNorm_sq t (lo - L) (hi + L)
    _ ≤ 4 * ∫ u in lo - L..hi + L,
        heathBrownCriticalZetaNorm u ^ (2 : ℕ) := by
