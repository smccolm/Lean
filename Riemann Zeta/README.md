...
- **Selected #15 path**: Prove a zero-specialized Euler–Maclaurin zeta truncation; targeted van der Corput B/A and Weyl estimates; the full classical Montgomery–Halász–Huxley large-values theorem; and a finite exact-`σ` zero-density transfer. Then assemble the concrete Ingham and Huxley propositions, deriving the Huxley `σ = 3/4` boundary from Ingham because both exponents equal `3/5` there.
