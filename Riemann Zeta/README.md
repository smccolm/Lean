...
- **[`RiemannZeta/Audit.lean`](RiemannZeta/Audit.lean)**: Executable transitive dependency audit covering all 113 exported source-level theorems across every production module.
...
The default root imports every intended production module and builds with zero Lean warnings. The audit list and discovered production theorem set currently both contain 113 declarations. The audit exits nonzero because 17 theorems have project-specific axiom dependencies; no audited theorem depends on `sorryAx`. The detector results, conditional powered-coefficient theorem, mean-value-to-large-values implication, and five generic Type II deductions pass the dependency audit. The former Type II axiom and its misleading Halász–Montgomery wrapper are gone. The remaining failure is the expected honest status until the other proof obligations are discharged.
