...
- **`RiemannZeta/GuthMaynard/LargeValuesDefinitions.lean`, `LargeValuesMatrix.lean`, and `TraceDispersion.lean`**: Source-facing #19 foundation replacing the deleted false decoupling model. These modules prove cutoff existence and derivative bounds, matrix action and exact Gram kernel, positive spectrum and trace identities, and the sixth-moment dispersion reduction.
...
- **[`RiemannZeta/Audit.lean`](RiemannZeta/Audit.lean)**: Executable transitive dependency audit covering all 923 exported source-level theorems across every production module, including the globally named public declarations in vendored PNT+ and the new #19 matrix/dispersion layer.
...
The runner performs five warning-failing Lean stages: the default production build, redundant explicit production-module coverage, both retained examples, and the transitive axiom/output audit. Its integrity scans use `--no-ignore`. The latest run, `logs/overall_proof_20260810_041446.log`, passes all builds, all 923 existing dependency audits, and every integrity scan with zero warnings. It then reports nine missing research outputs—two for #15, four for #18, and three for #19—and returns the honest overall result `FAIL`/`1`.
